...
{
	var gifName = form;
	document.frmsearch.action="icicipllms_view.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <? $current_date=date('Y-m-d');?>
  <table width="712" border="0">
   <tr><td align="right"><a href="commonlms_plreport.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a> &nbsp; | &nbsp; <a href="http://www.deal4loans.com/icicipl_complist.pdf" target="_blank">ICICI Company List</a>&nbsp; | &nbsp; <a href="http://www.deal4loans.com/ICICISept_offer.pdf" target="_blank">ICICI Special List</a>&nbsp; | &nbsp;<a href="icicipl_eligibility.jpg" target="_blank">Product Details</a></td></tr>   
 <tr><td align="center" width="100%">
 <div align="center">
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <form name="frmsearch" action="icicipllms_view.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
	...
	<option value="TU Reject" <? if($varCmbFeedback == "TU Reject") { echo "selected"; } ?>>TU Reject</option>
	...
	<option value="Already Applied with ICICI Bank" <?if($varCmbFeedback == "Already Applied with ICICI Bank") { echo "selected"; }?>>Already Applied with ICICI Bank</option>
	</select>
	 </td>
   </tr>  
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$resultst = ExecQuery("select iciciallocateid from icicipllms_allocation where icicirequestID=$RequestID and BidderID=".$_SESSION['BidderID']." AND product=1");		
		$num_rows = mysql_num_rows($resultst);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($resultst);
			$strSQLst="Update icicipllms_allocation Set icici_feedback='".$Feedback."' ";
			$strSQLst=$strSQLst."Where iciciallocateid=".$row["iciciallocateid"];
		}
		else
		{
			$strSQLst="Insert into icicipllms_allocation(icicirequestID, BidderID, 	product , icici_feedback) Values (";
			$strSQLst=$strSQLst.$RequestID.",".$_SESSION['BidderID'].",'1','".$Feedback."')";
		}
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (icicipllms_allocation.icici_feedback IS NULL OR icicipllms_allocation.icici_feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND icicipllms_allocation.icici_feedback='".$varCmbFeedback."' ";
		}
	?>
 <p class="bodyarial11"><?=$Msg?></p>
 <table width="758" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <?
		$qry="Select RequestID From ICICI_Allocated_Leads  LEFT OUTER JOIN icicipllms_allocation ON ICICI_Allocated_Leads.icicirequestID= icicipllms_allocation.icicirequestID  Where (((ICICI_Allocated_Leads.Allocation_Date Between '".$min_date."' and '".$max_date."') or (icicipllms_allocation.icici_followupdate Between '".$min_date."' and '".$max_date."')) and ICICI_Allocated_Leads.TelecallerID='".$_SESSION['BidderID']."')";
			$qry=$qry.$FeedbackClause;
			$qry=$qry."group by ICICI_Allocated_Leads.RequestID";				
   ...
   <tr>   
	...
	<?
		...
		else
		{
		 ...
		 $qry="		 
Select *,ICICI_Allocated_Leads.icicirequestID AS icicireq From ICICI_Allocated_Leads LEFT OUTER JOIN icicipllms_allocation ON ICICI_Allocated_Leads.icicirequestID= icicipllms_allocation.icicirequestID Where (((ICICI_Allocated_Leads.Allocation_Date Between '".$min_date."' and '".$max_date."') or (icicipllms_allocation.icici_followupdate Between '".$min_date."' and '".$max_date."')) and ICICI_Allocated_Leads.TelecallerID='".$_SESSION['BidderID']."')";
$qry=$qry.$FeedbackClause;
		$qry=$qry."group by ICICI_Allocated_Leads.RequestID";
...
//echo $qry;
		...
		while($row=mysql_fetch_array($result))
		{
			...
			$FinalDay = $exptodayformat[0];	
				$tudetails="Select  plcibilid from icici_pl_cibili_check Where icicirequestID=".$row["icicirequestID"]; 
	$tudetailsQuery = ExecQuery($tudetails);
	$turow=mysql_fetch_array($tudetailsQuery);
	//echo $row['icicirequestID'];
	?>
<tr>
<td class="bodyarial11"><a href="icici_pleditlead.php?id=<? echo urlencode($row['icicireq']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
<td class="bodyarial11"><? echo $row["Mobile_Number"]; ?><!--<img src="gButt.php?text=<? //echo $row["Mobile_Number"]; ?>" />--></td>
...
<?
	   ...
	   ?>
	<td><form method="post" name="bidder_array" action=""><? if($turow["plcibilid"]>0) {echo "TU DONE";}	
		?></td>

    <td class="bodyarial11"><? echo getJumpMenu("icicipllms_view.php",$row["icicireq"],"1",$row["icici_feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
	<?php if(($Followup_Date>$FinalMinDate || $Followup_Date<$FinalMaxDate) && ($Followup_Date !='0000-00-00 00:00:00') &&  ($TodayFormat==$FinalDay) )  { ?><td bgcolor="#FF0000"> <?php } else { ?>	
	 ...
	 <input type="Text"  name="FollowupDate-<?php echo $row["icicireq"]; ?>" maxlength="25" size="15" <?php if($Followup_Date !='0000-00-00 00:00:00') { ?>value="<?php  echo $Followup_Date; ?>" <?php } ?>>
	 <a href="javascript:NewCal('FollowupDate-<?php echo $row["icicirequestID"]; ?>','yyyymmdd',true,24)">
...
?>
	<select name="type" id="type" onChange="MM_jumpMenu('parent',this,0)">
		...
		<option value="<? echo $strURL.'&Feedback=Already Applied with ICICI Bank'?>" <? if($varFeedback == "Already Applied with ICICI Bank") { echo "selected"; } ?>>Already Applied with ICICI Bank</option>
