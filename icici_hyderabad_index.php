...
{
	var gifName = form;
	document.frmsearch.action="icici_hyderabad_index.php?search=y"+gifName;
...
<div align="center">
 ...
 <form name="frmsearch" action="icici_hyderabad_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//		echo $strSQL;
 ...
 <?
		$search_qry="select *,Req_Loan_Home.DOB as dob,Req_Loan_Home.LandLine as Land,Req_Loan_Home.Std_Code as Code from Req_Loan_Home, wUsers LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Home.RequestID AND Req_Feedback.BidderID=".$_SESSION['BidderID']." where   Req_Loan_Home.UserID=wUsers.UserID and Req_Loan_Home.City Like '%Hyderabad%' and Req_Loan_Home.Net_Salary>=150000 and Req_Loan_Home.Loan_Amount >=500000 ".$search_date." and (wUsers.IsPublic=1 or Req_Loan_Home.IsPublic=1) and ((wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= wUsers.DOB) or (Req_Loan_Home.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= Req_Loan_Home.DOB)) group by Req_Loan_Home.Mobile_Number";
		$search_qry=$search_qry.$FeedbackClause;

		$qry="select *,Req_Loan_Home.DOB as dob,Req_Loan_Home.LandLine as Land,Req_Loan_Home.Std_Code as Code from Req_Loan_Home, wUsers LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Home.RequestID AND Req_Feedback.BidderID=".$_SESSION['BidderID']." where   Req_Loan_Home.UserID=wUsers.UserID and Req_Loan_Home.City Like '%Hyderabad%' and Req_Loan_Home.Net_Salary>=150000 and Req_Loan_Home.Loan_Amount >=500000 ".$search_date." and (wUsers.IsPublic=1 or Req_Loan_Home.IsPublic=1) and ((wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= wUsers.DOB) or (Req_Loan_Home.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= Req_Loan_Home.DOB)) group by Req_Loan_Home.Mobile_Number ";
 ...
 ?>
   ...
   <tr>
    ...
    <td class="bodyarial11"><? echo $row[$i]["Company_Name"]; ?></td>
     ...
     <td class="bodyarial11"><? echo getJumpMenu("icici_hyderabad_index.php",$row[$i]["RequestID"],"1",$row[$i]["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback) ?></td>
   </tr>
	<?	
					$i=$i+1;
		}
 ...
 <form name="frmdownload" action="icici_hyderabad_download.php" method="post">
   <tr>
     <td align="center">
