...
$resultsecure_chk = d4l_ExecQuery($secure_chk);
$recordcount = d4l_mysql_num_rows($resultsecure_chk);
$securerow = d4l_mysql_fetch_array($resultsecure_chk);
...
if($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.161.193.191" || $IP=="122.160.74.241" || $IP=="122.160.74.235"  || $IP=="182.71.109.218" || $IP=="1.23.114.53" || "185.93.231.12") { ?> <tr><td style="float:right" bgcolor="#45B2D8"> <a href="/bidders_consolidate_appointment.php" style="color:#FFFFFF;" target="_blank"><strong>Appointment</strong></a> | <a href="/bidders_update_feedback.php" style="color:#FFFFFF;"><strong>Update Feedbacks</strong></a></td></tr><? } ?>
 ...
 <form name="frmsearch" action="fullerton_majorcty_view.php?search=y" method="post" onSubmit="return chkform();">   
  <tr><td colspan="3">&nbsp;</td></tr>  
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
	...
	<tr>
		...
		<option value="2" <?php if($city==2) { echo "selected";} ?>>Delhi</option>
		<option value="3" <?  if($city==3) { echo "selected";} ?>>Hyderabad</option>
		<option value="4" <? if($city==4) { echo "selected";} ?>>Mumbai</option>
		<option value="5" <? if($city==5) { echo "selected";} ?>>Chennai </option>
		<option value="6" <? if($city==6) { echo "selected";} ?>>Bangalore</option>
		<option value="7" <? if($city==7) { echo "selected";} ?>>Pune</option>
		</select>
		</td>
  ...
  <tr><td colspan="3">&nbsp;</td></tr>   <tr>    
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = d4l_ExecQuery("select FeedbackID from Req_Feedback where AllRequestID=$RequestID and BidderID in (".$allocated_bidder.")");		
...
<td align="center" bgcolor="#DFF6FF" class="style3" color="#FF0000"><? if(strlen($row["Barclays_Eligibility"])>0) { echo $row["Barclays_Eligibility"]; } else { echo "Not Eligibile";}?></td>
 ...
 $ggQuery = d4l_ExecQuery($ggSQl);
 $ggMobile = d4l_mysql_result($ggQuery,0,'Mobile');
  $ggBName = d4l_mysql_result($ggQuery,0,'BName'); 
  echo $ggBName;
  echo "<br>";
  echo $ggMobile;
 ?>
...
//		echo $row["RequestID"]; echo $row['sentbidder'];
 ...
 <br>
 <form name="xlsdownload" action="bidder_download_fullerton.php" method="post">
