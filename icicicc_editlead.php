...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	 ...
	 if(strlen($ccfeedback)>0)
	{
		$strSQL="";
		$Msg="";
		$result = ExecQuery("select FeedbackID from Req_Feedback_CC where (AllRequestID=".$post." and BidderID=".$Bidder_Id." AND Reply_Type=4)");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			
			$strSQL="Update Req_Feedback_CC Set Feedback='".$ccfeedback."',comment_section='".$ccadd_comment."',Followup_Date='".$FollowupDate."'";
			$strSQL=$strSQL."Where FeedbackID=".$row["FeedbackID"];
		}
		else
		{
			$strSQL="Insert into Req_Feedback_CC(AllRequestID, BidderID, Reply_Type , Feedback, Followup_Date,comment_section) Values (";
			$strSQL=$strSQL.$post.",".$Bidder_Id.",4,'".$ccfeedback."','".$FollowupDate."','".$ccadd_comment."')";
	...
	if($ccfeedback=="Appointment fixed" && (strlen($appt_address)>1 || strlen($appdate)>1))
	{
		$strSQLappt="";
		$Msgapt="";
		$resultappt = ExecQuery("select ApptID from ICICI_CCAppt_Details where AllRequestID=".$post." and BidderID=".$Bidder_Id." AND Reply_Type=4");		
		...
		{
			$strSQLappt="Insert into ICICI_CCAppt_Details(AllRequestID, BidderID, Reply_Type , Appt_Address, Appt_Date, Appt_Time, Appt_card_name, Appt_Dated) Values (";
			$strSQLappt=$strSQLappt.$post.",".$Bidder_Id.",4,'".$appt_address."','".$appdate."','".$app_time."','".$which_card."',Now())";
...
<?php 
$viewqry="select Name,Mobile_Number,Updated_Date,Followup_Date,Feedback,Followup_Date,comment_section from Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID=Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID= '".$bidid."' where Req_Credit_Card.RequestID=".$post." "; 
...
$apptqry="select * From ICICI_CCAppt_Details where (AllRequestID=".$post." and BidderID=".$bidid.")";
//echo $apptqry."<br>";
