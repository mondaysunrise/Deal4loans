...
$checkDupSql = "select * from hdfc_pl_calc_leads where (mobile_number = '".$mobile_number."' and mobile_number not in (9971396361,9891118553,9811215138) and (Dated between '".$days30ago."' and '".$todayDate."'))";
list($checkDupNum,$checkDupQuery)=MainselectfuncNew($checkDupSql,$array = array());
if($checkDupNum>0)
{
//HDFC OUtStanding
	header("Location: http://www.deal4loans.com/hdfc-personal-loan-new-thank.php");
	exit();	
}
else
{
	...
	$dataArray = array('Name'=>$Name, 'Email'=>$Email, 'City'=>$City, 'City_Other'=>$City_Other, 'Mobile_Number'=>$Mobile_Number, 'Net_Salary'=>$Net_Salary, 'Dated'=>$Dated, 'source'=>$source, 'Updated_Date'=>$Dated, 'IP_Address'=>$IP, 'Reference_Code'=>$Reference_Code, 'Direct_Allocation'=>$Direct_Allocation, 'Employment_Status'=>'1');
	
	$last_inserted_id = Maininsertfunc ('Req_Loan_Personal', $data);
  ...
  <tr>
    <td><table width="275"  border="0" align="right" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td height="30" align="left" class="blutxt"  >Rate of Interest ranges between<br>
        13.99-22.25 %</td>
        </tr>
      <tr>
...
<td valign="top">
			<form name="hdfc_calc" method="POST" action="hdfc-personal-loannewthanks.php" onSubmit="return chkpersonalloan(document.hdfc_calc);">
