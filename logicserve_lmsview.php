...
{
	var gifName = form;
	document.frmsearch.action="logicserve_lmsview.php?search=y"+gifName;
...
function chkform()
{
	...
	<?if($_SESSION['Date']>$mindefineDate)
		{?>
	if(document.frmsearch.min_date.value<"2013-10-23")
	{
		alert("Sorry!!!! Your minimum date is 2013-10-23.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="logicserve_lmsview.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?  if($_SESSION["Comp_Source"]=="logicserve") {$cmpdate="2013-10-23"; }?> 
...
<td width="141" align="center" bgcolor="#FFFFFF" class="style2">Feedback</td>
  ...
  <?  
	 $qry="SELECT komli_uniqueid,Feedback,komli_date from  komli_plcompaign LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= komli_plcompaign.komli_uniqueid AND Req_Feedback_PL.BidderID in (846,847,854,9,10) WHERE ((komli_date Between '".$min_date."' and '".$max_date."') and  komli_city in ('Ahmedabad','Kolkata','Delhi','Noida','Gurgaon','Mumbai','Thane','Bangalore','Chennai','Pune','Gaziabad') and komli_netsalary  >=360000  and Req_Feedback_PL.Feedback not in ('Send Now','Other Product','Not Eligible') and komli_date >='2013-10-23 00:00:00')";
...
if($recordcount>0)
		...
		{ ?>
 <tr>
 <td width="141" align="center" bgcolor="#DFF6FF" class="style3"><? echo $row2[$k]["komli_uniqueid"]; ?> </td>
 <td width="141"align="center" bgcolor="#DFF6FF" class="style3">
 <? if($row2[$k]["komli_uniqueid"]>0)
	{ 
		$plsql= "Select Name from Req_Loan_Personal Where (RequestID=".$row2[$k]["komli_uniqueid"].")"; 
		list($plsqlcount,$row)=MainselectfuncNew($plsql,$array = array());
		echo $row[0]["Name"];  
	}
?>
...
<td width="141" align="center" bgcolor="#DFF6FF" class="style3"><? echo $row2[$k]["Feedback"]; ?></td>
  </tr>
   <? $i=$i+1; }  
		}?>
    </table>
