...
$strcity = trim($_SESSION['city']);
$arrcity = explode(",",$strcity);

$checkcity="";
	if(isset($_REQUEST['city']))
	{
		$checkcity=$_REQUEST['city'];
	}
...
{
	var gifName = form;
	document.frmsearch.action="hdfcpl_citywise_view.php?search=y"+gifName;
...
function chkform()
{
	...
	<?if($_SESSION['BidderID']==1727)
		{?>
	if(document.frmsearch.min_date.value<"2015-01-01")
	{
		alert("Sorry!!!! Your minimum date is 2015-01-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="hdfcpl_citywise_view.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td align="left" valign="middle" class="style1" width="24%" >  <input name="max_date" type="text" id="max_date" size="15" value="<? echo $max_date; ?>"></td>
	   ...
	   <? if(count($arrcity)>0)
	   { ?>
	   <tr><td><strong>Cities:</strong></td><td><select name="city" id="city">
		<option value="All">All</option>
	   <? for($i=0;$i<count($arrcity);$i++) 
		   { ?>
				<option value="<? echo $arrcity[$i]; ?>" <? if($checkcity==$arrcity[$i]) { echo "selected";} ?> ><? echo $arrcity[$i]; ?></option>
		 <? } ?>
     			</select></td></tr>
				<? } ?>
     <tr>
		...
		if($mob_num>0)
{
 ...
 <? 
	...
	if($checkcity=="All")
		...
		{
		$search_qry="SELECT * FROM ".$feedback_tble.",`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=".$val.".RequestID AND Req_Feedback.BidderID  = ".$_SESSION['BidderID']." WHERE ".$feedback_tble.".AllRequestID=`".$val."`.RequestID and ".$feedback_tble.".BidderID = ".$_SESSION['BidderID']." and ".$feedback_tble.".Reply_Type=".$pro_code." and (".$val.".City like '%".$checkcity."%' or ".$val.".City_Other like '%".$checkcity."%') and (".$feedback_tble.".Allocation_Date  Between '".($min_date)."' and '".($max_date)."' ) ";	
		//$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by ".$val.".Mobile_Number";
		$search_qry=$search_qry." order by ".$val.".Dated DESC";
 ...
 ?>
   ...
   <tr>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3"><? if($_SESSION['BidderID']==2997)
			{
