<?php
	...
	//error_reporting(E_ALL);
		...
		$SQL_CC = "select * from Articles where  Art_Main_Title like '%Other Articles%' order by  Art_DOE desc";
...
if ($NumRows_CC != 0) {
 ...
 $file= fopen("results-other-articles.xml", "w");
...
$_xml .= "<description>".$Title."</description>";
...
while($cntr<count($getrow))
        {
			$Art_Sub_Title = htmlentities($getrow[$cntr]['Art_Sub_Title']);
			$Art_Url = "http://www.deal4loans.com/".$getrow[$cntr]['Art_Url'];
		$Art_Content = htmlentities($getrow[$cntr]['Art_Content']);
...
fwrite($file, $_xml);
 ...
 echo "XML has been written.  <a href=\"results-other-articles.xml\">View the XMLz	.</a>";
