...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	//	echo $Final_Bidder." RequestID - ".$plrequestid." Remarks - ".$special_remarks." Time - ".$appointment_time." Date - ".$appointment_date;
		 $GetBank_Sql = "select leadlogid from zexternal_leadallocation_log where (BidderID = ".$Final_Bidder ." and RequestID=".$plrequestid." and ProductID=6)";
		...
		if($Final_Bidder>0 && $plrequestid>0 && ((strlen($special_remarks)>1) || strlen($appointment_time)>1 || $appointment_date!=''))
		{
			...
			$appdetailsqry="INSERT INTO zexternal_appointment_details (`AllRequestID`, `caller_id`, `BidderID`, `appt_date`, `appt_time`, `special_remarks`,`Flag`, `stat_dated`, ProductID, Bidder_Number, IDProof, AddressProof, PanCard, SalSlip, BankStmnt, PassSizePhoto,  leadlogid) VALUES ('".$plrequestid."', '".$callerid."', '".$Final_Bidder."', '".$appointment_date."', '".$appointment_time."', '".$special_remarks."', '1', Now(),'6' ,'".$Bidder_Number."', '".$pl_IDProof."', '".$pl_AddressProof."', '".$pl_PanCard."', '".$pl_SalSlip."', '".$pl_BankStmnt."', '".$pl_PassSizePhoto."', '".$leadlogid."')";
...
if($plcity=='Others'){
			...
			$appDocsqry="INSERT INTO zexternal_appointment_docs (`RequestID`, `caller_id`, CityName, Reply_Type,  IDProof, AddressProof, PanCard, SalSlip, BankStmnt, PassSizePhoto,  leadlogid, `appt_date`, `appt_time`, `special_remarks`, dated, updated_date,rescheduled, Address,viewstatus) VALUES ('".$plrequestid."', '".$callerid."', ".$CityVal."', '6', '".$pl_IDProof."', '".$pl_AddressProof."', '".$pl_PanCard."', '".$pl_SalSlip."', '".$pl_BankStmnt."', '".$pl_PassSizePhoto."', '".$leadlogid."', '".$appointment_date."', '".$appointment_time."', '".$special_remarks."', Now(), Now(), '".$rescheduled."', '".$Address."', '1')";
	...
	header("Location: callinglms/editbllead.php?id=".$plrequestid."&Bid=".$callerid);
	exit();
}
?>