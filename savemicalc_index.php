...
{
	var gifName = form;
	document.frmsearch.action="savemicalc_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2014-06-01")
	{
		alert("Sorry!!!! Your minimum date is 2014-06-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<div align="center">
 ...
 <form name="frmsearch" action="savemicalc_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//	echo "<br>".$getendtime=date("H:i:s");
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(saveemicalc_tbl.dated)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(saveemicalc_tbl.dated)<='".strtotime($max_date)."'";
		}
		else if($min_date!="" and $max_date=="")
		{
			$min_date=$min_date." 00:00:00";
			$search_date=$search_date." and UNIX_TIMESTAMP(saveemicalc_tbl.dated)>='".strtotime($min_date)."'";
		}
		else if($min_date=="" and $max_date!="")
		{
			$max_date=$max_date." 23:59:59";
			$search_date=$search_date." and UNIX_TIMESTAMP(saveemicalc_tbl.dated)<='".strtotime($max_date)."'";
		}
		else
		{
			$search_date=$search_date." and saveemicalc_tbl.dated!=''";
		}
	?>
 <table width="758" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <?
		$search_qry="select * from saveemicalc_tbl where (1=1 ".$search_date.") ";
			
		 $qry="select saveemiid from saveemicalc_tbl where 1=1 ".$search_date."  order by saveemicalc_tbl.dated desc";
 ...
 ?>
   ...
   <tr>
	 ...
	 <td class="head1">PL Running</td>
	 <td class="head1">HL Running</td>
	  <td class="head1">View Quote</td>
		</tr>
	<?
		...
		$qry="select * from saveemicalc_tbl where (1=1 ".$search_date.")  order by saveemicalc_tbl.dated desc LIMIT $startrow, $pagesize";
		$result=ExecQuery($qry);
   ...
   <tr>
     ...
     <td class="bodyarial11"><? if($row["Employment_Status"]==0) { echo "Self Employed"; } else { echo "Salaried"; }?></td>
	  <td class="bodyarial11"><? if($row["flag_plbt"]==1) { echo "Yes";} else { echo "No";} ?></td>
	   <td class="bodyarial11"><? if($row["flag_hlbt"]==1) { echo "Yes";} else { echo "No";} ?></td>
	    <td class="bodyarial11"><? if($row["saveemiid"]>=54) { ?><a href="saveemi_showquotes.php?savemiid=<? echo $row["saveemiid"]; ?>" target="_blank">View</a><? } ?></td>
	</tr>
	<?	
					$i=$i+1;
		}
