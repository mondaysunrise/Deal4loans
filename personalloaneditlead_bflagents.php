...
$ExtraFieldsRows = mysql_num_rows($ExtraFieldSelect);
	 ...
	 if(strlen($plfeedback)>0)
	{
		...
		if($plfeedback=="Not Contactable" || $plfeedback=="Ringing" || $plfeedback=="Wrong Number" || $plfeedback=="Not Eligible")
			...
			$strSQL="Update Req_Feedback_PL Set Feedback='".$plfeedback."',not_contactable_counter='".$updatedcounter."'";
                        
                        if(($plfeedback=='OTP Done - OK' || $plfeedback=='OTP Done - Not Ok') && ($FollowupDate=='' || $FollowupDate=='0000-00-00 00:00:00'))
                        {
                        $strSQL.= ", Followup_Date='".date("Y-m-d h:i:s")."'";
                        }else{
                          $strSQL.= ", Followup_Date='".$FollowupDate."'";  
                        }
                        $strSQL.= ", Caller_Name='".$_SESSION['Caller_Name']."', comment_section='".$BankRmrks."', axis_executive_name='".$BankFdbck."', post_login_stat='".$LanNum."'";
...
$VerifierFollowupDate = mysql_result($viewlead,0,'VerifierFollowupDate');
...
font-weight:bold;color:666699;font-Family:Verdana;text-transform: capitalize;'></span></td></tr>
	...
	<td>Company Type</td><td><select name="plCompany_Type" id="plCompany_Type" style="width: 190px;">
		  <option value="0" <? if($Company_Type==0) {echo "selected";} ?>>Please Select</option>
		  	<option value="1" <? if($Company_Type==1) {echo "selected";} ?>>Listed With Bajaj</option>
...
<tr>
	...
	<td class="fontstyle"><?php 
        ...
        <td class="fontstyle"><textarea rows="2" cols="20" name="BnkRmark" id="BnkRmark" readonly="readonly"><?php echo $BankRemarks;?></textarea></td>
	</tr>
        <tr>
            <td class="fontstyle"><b>Verifier Feedback</b></td>
        <td class="fontstyle"><?php echo $BankFeedabck;?></td>
            <td class="fontstyle"><b>Verifier Follow Up Date</b></td>
        <td class="fontstyle"><?php echo $VerifierFollowupDate;?></td>
	</tr>
 <tr>
