...
function breadcrumbs_plus_get_items( $args ) {
	...
	/* Link to front page. */
	if ( !is_front_page() )
		$item[] = '<span ><a  href="'. home_url( '/' ) .'" class="home"><span >' . $args['home'] . '</span></a></span>';
	...
	elseif ( is_home() ) {
		...
		$item['last'] = '<span >'.get_the_title( $home_page->ID ).'</span>';
	}
	...
	elseif ( is_singular() ) {
		...
		if ( 'post' === $wp_query->post->post_type && $args['show_blog'] ) {
			$item[] = '<span ><a  href="' . get_permalink( get_option( 'page_for_posts' ) ) . '"><span >' . get_the_title( get_option( 'page_for_posts' ) ) . '</span></a></span>';
		}
		...
		if ( 'page' !== $wp_query->post->post_type ) {
			...
			if ( function_exists( 'get_post_type_archive_link' ) && !empty( $post_type_object->has_archive ) )
				$item[] = '<span ><a  href="' . get_post_type_archive_link( $post_type ) . '" title="' . esc_attr( $post_type_object->labels->name ) . '"><span >' . $post_type_object->labels->name . '</span></a></span>';
		...
		$item['last'] = '<span >'.get_the_title().'</span>';
	}
...
function breadcrumbs_plus_get_bbpress_items( $args = array() ) {
	...
	$page5 = isset($bp->achievements->root_slug); // slug for the Achievements page. The BuddyPress default is 'achievements'.
		...
		$item[] = '<span ><a  href="' . get_post_type_archive_link( bbp_get_forum_post_type() ) . '"><span >' . bbp_get_forum_archive_title() . '</span></a></span>';
				}
	}
	...
	if ( bbp_is_forum_archive()) {
		...
		$item[] = '<span ><a  href="'. get_post_type_archive_link( bbp_get_forum_post_type() ) .'" class="home"><span >' . bbp_get_forum_archive_title() . '</span></a></span>';
	...
	elseif ( bbp_is_single_topic() ) {
		...
		if ( bbp_is_topic_split() || bbp_is_topic_merge() || bbp_is_topic_edit() )
			$item[] = '<span ><a  href="' . bbp_get_topic_permalink( $topic_id ) . '"><span >' . bbp_get_topic_title( $topic_id ) . '</span></a></span>';
		else
	...
	elseif ( bbp_is_single_reply() ) {
		...
		if ( !bbp_is_reply_edit() ) {
			...
			$item[] = '<span ><a  href="' . bbp_get_reply_url( $reply_id ) . '"><span >' . bbp_get_reply_title( $reply_id ) . '</span></a></span>';
			$item[] = __( 'Edit', 'theme' );
		}
	...
	elseif ( bbp_is_single_user() || bbp_is_single_user_edit() ) {
		...
		if ( bbp_is_single_user_edit() ) {
			$item[] = '<span ><a  href="' . bbp_get_user_profile_url() . '"><span >' . bbp_get_displayed_user_field( 'display_name' ) . '</span></a></span>';
			$item[] = __( 'Edit' );
		} else {
...
function breadcrumbs_plus_get_parents( $post_id = '', $separator = '/' ) {
	...
	while ( $post_id ) {
		...
		$parents[]  = '<span ><a  href="' . get_permalink( $post_id ) . '" title="' . esc_attr( get_the_title( $post_id ) ) . '"><span >' . get_the_title( $post_id ) . '</span></a></span>';
...
function breadcrumbs_plus_get_term_parents( $parent_id = '', $taxonomy = '', $separator = '/' ) {
	...
	while ( $parent_id ) {
		...
		$parents[] = '<span ><a  href="' . get_term_link( $parent, $taxonomy ) . '" title="' . esc_attr( $parent->name ) . '"><span >' . $parent->name . '</span></a></span>';
