...
/** 
	//Still in Development 
		function mom_get_first_image($id, $size = 'thumbnail') {
		  ...
		  ob_start();
		  ob_end_clean();
		  $output = preg_match_all('/<img.+class=[\'"]([^\'"]+)[\'"].*>/i', $queried_post->post_content, $matches);

		  $first_img = '';

			if (isset($matches[1][0])) {$first_img = preg_match('/wp-image-\d+/i', $matches[1][0], $matches);}

			if (isset($matches[0])) { $first_img = $matches[0];}
...
if($categories){
	...
	$author = mom_option('post_meta-author') == 1 ? '<span >'.__('Posted By:', 'theme').' <span class="fn"><a href="'.$author_link.'">'.get_the_author().'</a></span></span>': '';
	...
	if (is_single()) {
		...
		$output .= '<a href="javascript:window.print()" rel="nofollow" class="print"><i class="fa-icon-print"> </i>'.__('Print', 'theme').'</a>';
