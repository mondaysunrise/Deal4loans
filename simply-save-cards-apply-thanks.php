    ...
    if($age <= 21){
		...
		if($Employment_Status==1 || $Employment_Status==0){
			...
			if($flag){
				...
				header('Location: http://www.deal4loans.com/simply-save-cards-apply-continue.php?RequestID='.$RequestID.'&cc_bankid='.$cc_bankid.'&cc_bankname='.$cc_bankname.'&sbiccid='.$sbiccid_encoded.'&errArray='.urlencode(serialize($errArray)));
			}
			else{
				$dataArr = array();
