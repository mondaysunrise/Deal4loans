<?php
	...
	function formHandler(){
		foreach($_POST as $a=>$b)
			$$a=$b;
		//echo "select * from Req_Partner where (Partner_Username='$Email' and Partner_Password='$PWD' and Is_Verified>1)";
		$result = "select * from Req_Partner where (Partner_Username='".$Email."' and Partner_Password='".$PWD."' and Is_Verified=1)";
		...
		if($num_rows > 0){
			 ...
			 $_SESSION['BidderID'] = $row["Partner_ID"];
	$_SESSION['UName'] = $row["Partner_Name"];
	$_SESSION['Partner_Company '] = $row["Partner_Company "];
	$_SESSION['City'] = $row["Partner_City"];
	$_SESSION['Email'] = $row["Partner_Email"];
	$_SESSION['Contact_Num'] = $row["Partner_Mobile"];
	$_SESSION['IsVerified'] = $row["Is_Verified"];
	$_SESSION['UserType'] = "bidder";
	$_SESSION['Partner_Date']=$row["Partner_Date"];
			...
			mysql_free_result($result);
			 ...
			 /* Redirect browser */
			 $strDir = dir_name();
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."partners_report.php");
			exit;
		}
		else{
			global $Msg;
...
.style1 {
 ...
 <table width="107%" border="0" align="center" cellpadding="0" cellspacing="0" style="border-collapse: collapse" valign="top">
	 <tr bgcolor="#FFFFFF">
