...
{
	var gifName = form;
	document.frmsearch.action="incomplete_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2008-07-29")
	{
		alert("Sorry!!!! Your minimum date is 2008-07-29.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<div align="center">
 ...
 <form name="frmsearch" action="incomplete_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if($search=="y")
	{
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Incomplete_Lead.Dated)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(Req_Incomplete_Lead.Dated)<='".strtotime($max_date)."'";
		}
		else if($min_date!="" and $max_date=="")
		{
			$min_date=$min_date." 00:00:00";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Incomplete_Lead.Dated)>='".strtotime($min_date)."'";
		}
		else if($min_date=="" and $max_date!="")
		{
			$max_date=$max_date." 23:59:59";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Incomplete_Lead.Dated)<='".strtotime($max_date)."'";
		}
		else
		{
			$search_date=$search_date." and Req_Incomplete_Lead.Dated!=''";
		}
	?>
 <table width="758" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <?
		
		$search_qry="select * from Req_Incomplete_Lead where 1=1 ".$search_date." ";
		$qry="select * from Req_Incomplete_Lead where 1=1 ".$search_date."  order by Dated desc";
 ...
 ?>
   ...
   <tr>
	...
	<?
		...
		$qry="select * from Req_Incomplete_Lead where 1=1 ".$search_date." order by Dated desc LIMIT $startrow, $pagesize";
		$result=ExecQuery($qry);
		...
		while($row=mysql_fetch_array($result))
		{
			...
			elseif($row["Product_Type"]==4)
			{
				$product="Credit Card"; 
			}
			elseif($row["Product_Type"]==5)
			{
				 $product="Loan Against Property"; 
			}
			elseif($row["Product_Type"]==6)
			{
   ...
   <tr>
    ...
    <td class="bodyarial11"><? echo $row["Email"]; ?></td>
     ...
     <td class="bodyarial11"><? echo $product;?></td>
   </tr>
	<?	
					$i=$i+1;
		}
...
<table width="500" border="0" cellspacing="1" cellpadding="4">
 <form name="frmdownload" action="incomplete_download.php" method="post">
   <tr>
     <td align="center">
