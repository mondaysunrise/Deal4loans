...
$post=$_REQUEST['id'];
//$getBidqry = "select AllRequestID,BidderID from lead_allocate where AllRequestID=$post AND   AND Reply_Type=1";
//$resultBid = ExecQuery($getBidqry);
//$rowBid = mysql_fetch_array($resultBid);

$min_date =$_REQUEST['to'];
$max_date=$_REQUEST['from'];

$bidid =$_SESSION['BidderID'];
...
//echo "select FeedbackID,not_contactable_counter from Req_Feedback_PL where AllRequestID=".$post." and BidderID=".$_REQUEST['bid']." AND Reply_Type=1"; 
...
<tr>
	...
	<td class="fontstyle">
		...
		<option value="" <? if($Feedback == "") { echo "selected"; }?>>Fresh lead</option>
    ...
    <option value="Post Login Rejected" <? if($Feedback == "Post Login Rejected") { echo "selected"; }?>>Post Login Rejected</option>
...
<tr>
	...
	<td class="fontstyle">
		...
		<option value="Call Later" <? if($BankFeedabck == "Call Later") { echo "selected"; }?>>Call Later</option>
		...
		<option value="Not Interested" <? if($BankFeedabck == "Not Interested") { echo "selected"; }?>>Not Interested</option>
		<option value="Login" <? if($BankFeedabck == "Login") { echo "selected"; }?>>Login</option>
		<option value="Approved" <? if($BankFeedabck == "Approved") { echo "selected"; }?>>Approved</option>
    <option value="Disbursed" <? if($BankFeedabck == "Disbursed") { echo "selected"; }?>>Disbursed</option>
    <option value="Ringing" <? if($BankFeedabck == "Ringing") { echo "selected"; }?>>Ringing</option>
    <option value="Not Connected" <? if($BankFeedabck == "Not Connected") { echo "selected"; }?>>Not Connected</option>
    <option value="Wrong Number" <? if($BankFeedabck == "Wrong Number") { echo "selected"; }?>>Wrong Number</option>
    <option value="Pre Login Reject" <? if($BankFeedabck == "Pre Login Reject") { echo "selected"; }?>>Pre Login Reject</option>
