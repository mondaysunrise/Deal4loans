...
{
	var gifName = form;
	document.frmsearch.action="LicHousingFinance_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2007-07-11")
	{
		alert("Sorry!!!! Your minimum date is 2007-07-11.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<div align="center">
 ...
 <form name="frmsearch" action="LicHousingFinance_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">From 
       <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2007-07-11"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
...
//		echo $strSQL;
 ...
 <?
		$search_qry="select * from Req_Loan_Home, wUsers LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Loan_Home.RequestID AND Req_Feedback.BidderID=".$_SESSION['BidderID']." where Req_Loan_Home.UserID=wUsers.UserID and (Req_Loan_Home.City Like '%Delhi%' or Req_Loan_Home.City Like '%Noida%' or Req_Loan_Home.City Like '%Gurgaon%' or Req_Loan_Home.City Like '%Gaziabad%' or Req_Loan_Home.City Like '%Faridabad%' or Req_Loan_Home.City_Other Like '%Greater noida%') and Req_Loan_Home.Net_Salary>=400000 and Req_Loan_Home.Loan_Amount>=1500000 ".$search_date." and (wUsers.IsPublic=1 or Req_Loan_Home.IsPublic=1) and((wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 20 YEAR) >= wUsers.DOB) or (Req_Loan_Home.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 20 YEAR) >= Req_Loan_Home.DOB))";
		$search_qry=$search_qry.$FeedbackClause;

		$qry="select * from Req_Loan_Home, wUsers LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Loan_Home.RequestID AND Req_Feedback.BidderID=".$_SESSION['BidderID']." where Req_Loan_Home.UserID=wUsers.UserID and (Req_Loan_Home.City Like '%Delhi%' or Req_Loan_Home.City Like '%Noida%' or Req_Loan_Home.City Like '%Gurgaon%' or Req_Loan_Home.City Like '%Gaziabad%' or Req_Loan_Home.City Like '%Faridabad%' or Req_Loan_Home.City_Other Like '%Greater noida%') and Req_Loan_Home.Net_Salary>=400000 and Req_Loan_Home.Loan_Amount>=1500000 ".$search_date." and (wUsers.IsPublic=1 or Req_Loan_Home.IsPublic=1) and((wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 20 YEAR) >= wUsers.DOB) or (Req_Loan_Home.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 20 YEAR) >= Req_Loan_Home.DOB)) ";
 ...
 ?>
   ...
   <tr>
	...
	<?
		...
		$qry="select * from Req_Loan_Home, wUsers LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Loan_Home.RequestID AND Req_Feedback.BidderID=".$_SESSION['BidderID']." where Req_Loan_Home.UserID=wUsers.UserID and (Req_Loan_Home.City Like '%Delhi%' or Req_Loan_Home.City Like '%Noida%' or Req_Loan_Home.City Like '%Gaziabad%' or Req_Loan_Home.City Like '%Gurgaon' or Req_Loan_Home.City_Other Like '%Greater noida%' or Req_Loan_Home.City Like '%Faridabad%') and Req_Loan_Home.Net_Salary>=400000 and Req_Loan_Home.Loan_Amount>=1500000 ".$search_date." and (wUsers.IsPublic=1 or Req_Loan_Home.IsPublic=1) and ((wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 20 YEAR) >= wUsers.DOB) or (Req_Loan_Home.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 20 YEAR) >= Req_Loan_Home.DOB)) ";
		$qry=$qry.$FeedbackClause;
   ...
   <tr>
    ...
    <td class="bodyarial11"><? echo $row["Company_Name"]; ?></td>
     ...
     <td class="bodyarial11"><? echo getJumpMenu("LicHousingFinance_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback) ?></td>
   </tr>
	<?	
					$i=$i+1;
		}
 ...
 <form name="frmdownload" action="LicHousingFinance_download.php" method="post">
   <tr>
     <td align="center">
