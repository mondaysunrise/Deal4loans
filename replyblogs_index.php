<?php
	...
	$FeedbackClause="";
	$OrderBy=" order by req_reply_message.Dated desc";
...
{
	var gifName = form;
	document.frmsearch.action="replyblogs_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2007-06-16")
	{
		alert("Sorry!!!! Your minimum date is 2007-06-16.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function MM_jumpMenu(targ,selObj,restore){ //v3.0
 ...
 <form name="frmsearch" action="replyblogs_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">From 
       <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2007-06-16"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
...
//		echo $strSQL;
	...
	if($search=="y")
	{
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(req_reply_message.Dated)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(req_reply_message.Dated)<='".strtotime($max_date)."'";
		}
		else if($min_date!="" and $max_date=="")
		{
			$min_date=$min_date." 00:00:00";
			$search_date=$search_date." and UNIX_TIMESTAMP(req_reply_message.Dated)>='".strtotime($min_date)."'";
		}
		else if($min_date=="" and $max_date!="")
		{
			$max_date=$max_date." 23:59:59";
			$search_date=$search_date." and UNIX_TIMESTAMP(req_reply_message.Dated)<='".strtotime($max_date)."'";
		}
		else
		{
			$search_date=$search_date." and req_reply_message.Dated!=''";
		}
 ...
 <?
		$search_qry="Select * from req_reply_message where Is_Verified=0   ".$search_date ;
		$search_qry=$search_qry.$FeedbackClause;
		

		$qry="Select * from req_reply_message where Is_Verified=0  ".$search_date  ;
...
//echo $qry;
   ...
   <tr>
    ...
    <td class="bodyarial11"><? echo $row["Subject"]; ?></td>
     <td class="bodyarial11"><? echo $row["Message"]; ?></td>
      <td class="bodyarial11"><a href="replyblog.php?id=<?php echo $row["PostID"]; ?>&replyid=<?php echo $row["Reply_ID"]; ?>" target="_blank">View</a></font></td>
   </tr>
	<?	
					$i=$i+1;
		}
 ...
 <form name="frmdownload" action="centurian_bankdownload.php" method="post">
   <tr>
     <td align="center">
