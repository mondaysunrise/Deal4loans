...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if($submit_p=="Submit")
	{
		...
		{
			$sqlData = "update Req_Loan_Personal_Extra_Fields set Salary='".$Salary."', unsecured_emi='".$unsecured_emi."', secured_emi='".$secured_emi."', card_outstanding='".$card_outstanding."' where RequestID='".$plrequestid."'";
		}
		else
		{
			$sqlData = "INSERT INTO Req_Loan_Personal_Extra_Fields (`RequestID`, `Salary`, `unsecured_emi`, `secured_emi`, `card_outstanding`) VALUES ('".$plrequestid."', '".$Salary."', '".$unsecured_emi."', '".$secured_emi."', '".$card_outstanding."')";
		}
//		echo $sqlData;
		ExecQuery($sqlData);
	}
	...
	$sqlExtraFields = "select * from Req_Loan_Personal_Extra_Fields where RequestID='".$plrequestid."'";
	$queryExtraFields = ExecQuery($sqlExtraFields);
	$numRowsExtraFields = mysql_num_rows($queryExtraFields);
	if($numRowsExtraFields>0)
	{
...
<table width="100%" cellspacing="2" cellpadding="5" align="center" border="0">
  ...
  <tr>
   ...
   <tr>
        ...
        <td valign="top" bgcolor="#F0F0F0" colspan="2" align="middle"><input type="submit" name="submit_p" value="Submit" style="margin-bottom: 3px;    padding: 5px 10px;    background-color: #289dcc;    border: 0px;    transition: background-color 0.3s linear 0s;    -webkit-border-radius: 3px;    -moz-border-radius: 3px;    border-radius: 3px;    font-size: 16px;    -moz-appearance: none !important;    -webkit-appearance: none !important;    appearance: none !important;    cursor: pointer;    display: inline-block;    color: #FFFFFF;" /></td>
  </tr>  
 <?php
 if($averageSalary>0)
 {
  ...
  ?>
      ...
      <td colspan="4">&nbsp;</td>
        ...
        <tr>  <td bgcolor="#FFFFFF" colspan="2"><textarea rows="2" cols="55" name="special_remarks" id="special_remarks_3" onchange="NosplcharComment(this);"></textarea></td>
          <td bgcolor="#FFFFFF" valign="top"><input type="Text" name="appointment_date" id="appointment_date" maxlength="25" value="" size="15" ><a href="javascript:NewCal('appointment_date','yyyymmdd',true,24)"><img src="/images/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td>
