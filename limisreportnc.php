...
Life Insurance [Not Called]</td></tr>
  <tr>
    <td align="center">
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" style="border:#0033CC 1px solid;">
 ...
 <form name="frmsearch" action="limisreportnc.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="85%">
     ...
     <option value="28" <? if($lms_details == "28") { echo "selected"; }?>>LI LMS 1</option>
     <option value="80" <? if($lms_details == "80") { echo "selected"; }?>>LI LMS 2</option>
     <option value="232" <? if($lms_details == "232") { echo "selected"; }?>>LI LMS 3</option>
     <option value="1453" <? if($lms_details == "1453") { echo "selected"; }?>>LI LMS 4</option>
     <option value="5" <? if($lms_details == "5") { echo "selected"; }?>>LI LMS 5</option>                    
     </select>
		</td>
...
if($lms_details=="All")
{
	$biddIDS = array(28,80,232,1453,5);
	$seq = array(0,1,2,3,4);
}
else 
{
	$biddIDS = array($lms_details);
		if($lms_details==28)	{ $seq = array(0);} else if($lms_details==80) { $seq = array(1);} else if($lms_details==232) { $seq = array(2);} else if($lms_details==1453) { $seq = array(1);} else if($lms_details==854) { $seq = array(2);}
}
//$biddIDS = array(28,80);
	for($j=0;$j<count($biddIDS);$j++)
	{	
	
	 $sql_PL ="SELECT Req_Feedback.Feedback as Feedback, count(*) as CountLeads from Req_Life_Insurance LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Life_Insurance.RequestID AND Req_Feedback.BidderID= '".$biddIDS[$j]."' and Req_Feedback.Reply_Type=1 WHERE ((MOD(Req_Life_Insurance.RequestID,5)='".$seq[$j]."' ) and Req_Life_Insurance.Source!='SMS' and Req_Life_Insurance.Source!='test' and ((Req_Life_Insurance.Updated_Date Between '".$min_date."' and '".$max_date."') or (Req_Feedback.Followup_Date Between '".$min_date."' and '".$max_date."'))) group by Req_Feedback.Feedback";
	
	
	//$sql_PL = "select Req_Feedback.Feedback as Feedback, count(*) as CountLeads from Req_Feedback left join Req_Life_Insurance on Req_Life_Insurance.RequestID = Req_Feedback.AllRequestID where (Req_Life_Insurance.Dated between '".$min_date."' and '".$max_date."') and  Req_Feedback.BidderID='".$biddIDS[$j]."' and Req_Feedback.Reply_Type=1 group by Req_Feedback.Feedback";
 ...
 <table width="950" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" >
  <tr>
	 ...
	 else if($biddIDS[$j]==80)
	 {
	 	echo "LI LMS 2 [80]";
	 }
	 else if($biddIDS[$j]==232)
	 {
	 	echo "LI LMS 3 [232]";
	 }
	 else if($biddIDS[$j]==1453)
	 {
	 	echo "LI LMS 4 [1453]";
	 }
	 else if($biddIDS[$j]==5)
