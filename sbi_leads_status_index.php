...
<div align="center">
 ...
 <form name="frmsearch" action="sbi_leads_status_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <table width="758" border="0" cellpadding="4" cellspacing="1" class="blueborder">
   ...
   <tr>
     <td class="head1">Total Leads</td>
      <td class="head1"><?php
      $totalSql = "SELECT count(*) as countTotal FROM `Req_Credit_Card` as rcc LEFT JOIN `sbi_credit_card_5633`as scc ON(scc.RequestID = rcc.RequestID AND scc.process_type = 'direct') WHERE (rcc.`source` = 'sbi_cards_apply') AND (DATE(rcc.`Dated`) BETWEEN '".$min_date."' AND '".$max_date."')  ORDER BY `rcc`.`RequestID`  DESC";
