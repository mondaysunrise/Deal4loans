...
{
	var gifName = form;
	document.frmsearch.action="indusind_cardsmlrlmsview.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2015-01-15")
	{
		alert("Sorry!!!! Your minimum date is 2015-01-15.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function insertData(id)
		{			
		var get_comment_section = document.getElementById('indusind_comment_'+ id).value;
		...
		var queryString = "?comment_section=" + get_comment_section + "&get_FollowupDate=" + get_FollowupDate + "&get_requestid=" + get_requestid;				
				alert(queryString); 
				ajaxRequest.open("GET", "insert_comment_indusind_ccmlrlms.php" + queryString, true);
...
<tr>
 ...
 <form name="frmsearch" action="indusind_cardsmlrlmsview.php?search=y" method="post" onSubmit="return chkform();">
...
<td width="25%"><select name="cmbfeedback" id="cmbfeedback" style="width:120px;">
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$DataArray = array("indusind_feedback"=>$Feedback);
		$wherecondition ="(indusindccid = '".$RequestID."')";
		Mainupdatefunc ('indusind_credit_card', $DataArray, $wherecondition);
...
if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (indusind_credit_card.indusind_feedback IS NULL OR indusind_credit_card.indusind_feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND indusind_credit_card.indusind_feedback='".$varCmbFeedback."' ";
		}
...
<? 
$search_qry="SELECT * FROM indusind_credit_card Where (((indusind_credit_card.indusind_emp_status=1 and indusind_credit_card.indusind_income>=480000) or (indusind_credit_card.indusind_emp_status=0 and indusind_credit_card.indusind_income>=780000)) and (indusind_credit_card.indusind_dob!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= indusind_credit_card.indusind_dob) and indusind_city in ('Ahmedabad','Bangalore','Chandigarh','Chennai','Coimbatore','Delhi','Hyderabad','Indore','Jaipur','Kanpur','Kochi','Kolkata','Lucknow','Ludhiana','Mumbai','Pune') and indusind_eligible_flag=1 and ((indusind_dated between '".($min_date)."' and '".($max_date)."') or (indusind_followup_date between '".($min_date)."' and '".($max_date)."'))) group by indusind_mobileno";

$search_qry=$search_qry.$FeedbackClause;
$search_qry=$search_qry." order by indusind_credit_card.indusind_dated DESC";

	$qry="SELECT * FROM indusind_credit_card Where (((indusind_credit_card.indusind_emp_status=1 and indusind_credit_card.indusind_income>=480000) or (indusind_credit_card.indusind_emp_status=0 and indusind_credit_card.indusind_income>=780000)) and (indusind_credit_card.indusind_dob!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= indusind_credit_card.indusind_dob) and indusind_city in ('Ahmedabad','Bangalore','Chandigarh','Chennai','Coimbatore','Delhi','Hyderabad','Indore','Jaipur','Kanpur','Kochi','Kolkata','Lucknow','Ludhiana','Mumbai','Pune') and indusind_eligible_flag=1 and ((indusind_dated between '".($min_date)."' and '".($max_date)."') or (indusind_followup_date between '".($min_date)."' and '".($max_date)."'))) group by indusind_mobileno";
...
<?
   ...
   $qry="SELECT * FROM indusind_credit_card Where (((indusind_credit_card.indusind_emp_status=1 and indusind_credit_card.indusind_income>=480000) or (indusind_credit_card.indusind_emp_status=0 and indusind_credit_card.indusind_income>=780000)) and (indusind_credit_card.indusind_dob!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= indusind_credit_card.indusind_dob) and indusind_city in ('Ahmedabad','Bangalore','Chandigarh','Chennai','Coimbatore','Delhi','Hyderabad','Indore','Jaipur','Kanpur','Kochi','Kolkata','Lucknow','Ludhiana','Mumbai','Pune') and indusind_eligible_flag=1 and ((indusind_dated between '".($min_date)."' and '".($max_date)."') or (indusind_followup_date between '".($min_date)."' and '".($max_date)."'))) group by indusind_mobileno";
		$qry=$qry.$FeedbackClause;
		$qry=$qry." order by indusind_credit_card.indusind_dated DESC";
		...
		if($recordcount>0)
		{
		for($jj=0;$jj<$recordcount1;$jj++)
		{																					$followup_date=$row[$jj]['indusind_followup_date'];
		?>
		<input type="hidden" name="requestid_<? echo $i;?>" id="requestid_<? echo $i;?>" value="<? echo $row[$jj]["indusindccid"];?>">	
  <tr>  
     <td align="center" bgcolor="#DFF6FF" class="style3" ><? echo $row[$jj]["indusind_name"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$jj]["indusind_city"]; ?></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$jj]["indusind_mobileno"]; ?></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$jj]["indusind_income"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? if($row[$jj]["indusind_emp_status"]==1) { echo "Salaried";} else { echo "Self Employed";} ?></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? 
	echo getJumpMenu("indusind_cardsmlrlmsview.php",$row[$jj]["indusindccid"],"4",$row[$jj]["indusind_feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback); ?>
		</td>   
<td align="center" bgcolor="#DFF6FF" class="bodyarial11" >
<textarea  name="indusind_comment_<? echo $i;?>" id="indusind_comment_<? echo $i;?>" cols="15" rows="2"><? echo $row[$jj]["indusind_comment"]; ?></textarea>
</td>
...
<? 
 ...
 <form name="frmdownload" action="indusindcc_mlr_download.php" method="post">
   <tr>
     <td align="center">
