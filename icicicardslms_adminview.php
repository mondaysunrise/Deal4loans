...
{
	var gifName = form;
	document.frmsearch.action="icicicardslms_adminview.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="icicicardslms_adminview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">
		<select name="cmbfeedback" id="cmbfeedback">
			...
			<option value="All" <? if($varCmbFeedback == "") { echo "selected"; } ?>>No Feedback</option>	
   ...
   <tr>
    <td width="20%"><strong>Lead Type</strong></td>
     <td width="80%">
			<select name="leadtype" id="leadtype">
				<option value="1" <? if($leadtype==1) {echo "Selected";} ?>>Type 1</option>
				<option value="2" <? if($leadtype==2) {echo "Selected";} ?>>Type 2</option>
			</select>
		</td>
   </tr>
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select iciciallocateid from icicilms_allocation where cc_requestid=$RequestID  AND product=4");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update icicilms_allocation Set verifier_feedback='".$Feedback."' ";
	...
	if($search=="y")
	{
		...
		else
		{
			if(strlen(trim($varCmbFeedback))==0)
			{
				$FeedbackClause=" AND (Req_Feedback_CC.icici_feedback IS NULL OR Req_Feedback_CC.Feedback='') ";
			}
			else if($varCmbFeedback=="All")
			{
				$FeedbackClause=" ";
 ...
 <?
		if($leadtype==1)
		{
		$search_qry="Select * From icicilms_allocation LEFT OUTER JOIN Req_Credit_Card ON Req_Credit_Card.RequestID= icicilms_allocation.cc_requestid Where ((icicilms_allocation.allocation_date Between '".$min_date."' and '".$max_date."')) ";
		$search_qry=$search_qry.$FeedbackClause;
			$search_qry=$search_qry."group by icicilms_allocation.cc_requestid";
		}
		else
		{
			$search_qry="Select * From icici_cards_calling LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= icici_cards_calling.RequestID Where ((icici_cards_calling.Dated Between '".$min_date."' and '".$max_date."') and icici_cards_calling.Privacy =1)";
			$search_qry=$search_qry.$FeedbackClause;
			$search_qry=$search_qry."group by icici_cards_calling.RequestID";
		}
		
	//echo $search_qry."<br>";
		$changedresult=ExecQuery($search_qry);
 ...
 ?>
   ...
   <td class="head1">Eligible Bidders</td>
     <td class="head1">Telecaller Feedback</td>
	  <td class="head1">Verifier Feedback</td>
	...
	<?
		...
		if($leadtype==1)
		{
		 $qry="Select * From icicilms_allocation LEFT OUTER JOIN Req_Credit_Card ON Req_Credit_Card.RequestID= icicilms_allocation.cc_requestid Where ((icicilms_allocation.allocation_date Between '".$min_date."' and '".$max_date."'))";
		 $qry=$qry.$FeedbackClause;
			$qry=$qry."group by icicilms_allocation.cc_requestid";
		}
		else
		{
		 $qry="Select RequestID, Followup_Date, Mobile_Number, Net_Salary, City, City_Other, Name, Feedback From icici_cards_calling LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= icici_cards_calling.RequestID Where ((icici_cards_calling.Dated Between '".$min_date."' and '".$max_date."') and icici_cards_calling.Privacy =1)";
		 $qry=$qry.$FeedbackClause;
		 $qry=$qry."group by icici_cards_calling.RequestID";
		}
...
<? if($leadtype==1) { ?><a href="icici_cardadminlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><?  echo $row["Name"]; ?></a> <? } else {  echo $row["Name"]; }?></td>
...
<? list($FinalBidder,$finalBidderName)= getBiddersList("Req_Credit_Card",$row['RequestID'],$City);
  for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo $row["Feedback"];?>
	</td>
	<td class="bodyarial11">
<? $getverifyfedbk='select verifier_feedback from icicilms_allocation where (	cc_requestid="'.$row['RequestID'].'")';
// echo $getcompany;
$getverifyfedbkresult = ExecQuery($getverifyfedbk);
$grow=mysql_fetch_array($getverifyfedbkresult);
echo $grow["verifier_feedback"];
	?>
</td>
	<td><? echo $row["Followup_Date"];?></td>
</tr>
	<?						$i=$i+1;
		}		
		}
		//icici_ccleads_download.php
	?>
 </table>
 <br>
 <? if($leadtype==1)
 ...
 <form name="frmdownload" action="icici_ccleads_download.php" method="post">
   <tr>
     <td align="center">
...
<? } 
		...
		{ ?>
		<table width="500" border="0" cellspacing="1" cellpadding="4">
 <form name="frmdownload" action="icici_cccallingleads_download.php" method="post">
   <tr>
     <td align="center">
	   <input type="hidden" name="qry1" value="<? echo $search_qry; ?>">
	   <input type="hidden" name="qry2" value="<? echo $val; ?>">
