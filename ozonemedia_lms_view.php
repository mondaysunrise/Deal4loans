...
{
	var gifName = form;
	document.frmsearch.action="ozonemedia_lms_view.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2010-06-04")
	{
		alert("Sorry!!!! Your minimum date is 2010-06-04.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="ozonemedia_lms_view.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?$current_date=date('Y-m-d');?> 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2010-06-04"<? } else { ?>value="<? echo $min_date; ?>" <? }?>></td>
	   <td>
...
<option value="Req_Loan_Home"  <? if($product=="Req_Loan_Home") { echo "selected";} ?>>Home Loan</option>
 ...
 <? 
		$qry="SELECT RequestID,Source from  ".$product." WHERE ((".$product.".Updated_Date Between '".$min_date."' and '".$max_date."') and source='ozonemedia' )";
...
//echo $qry;
 ...
 ?>
   <!--<tr>
     <td colspan="2" style="border-bottom:1px solid #45B2D8;"><strong><? //echo $startrow+1; ?> to <? //echo min($startrow+$pagesize,$recordcount); ?> Out of <? //echo $recordcount; ?> Records </strong></td>
     </tr>-->
   <tr>
...
<td width="141" align="center" bgcolor="#FFFFFF" class="style2">Count </td>
    ...
    <tr>
       <td align="center" bgcolor="#DFF6FF" class="style3">GIF Banners</td>
