...
$getBiddersSql = "select BidderID from Bidders where leadidentifier='diallerleadcc1'";
...
$query1="select RequestID, Dated from Req_Compaign Where (Compaign_ID=6621 and Reply_Type=4)";
...
if($RequestID>0)
{
	$qrysql="SELECT sbicc_name as Name,sbiccoffersid as RequestID,sbicc_mobile as Mobile_Number FROM sbi_ccoffers_directonsite where  (sbicc_dated Between '".($Dated)."' and '".$todaydate." 23:59:59') and sbiccoffersid >'".$RequestID."' ";
	
}
else
{
	$qrysql="SELECT sbicc_name as Name,sbiccoffersid as RequestID,sbicc_mobile as Mobile_Number FROM sbi_ccoffers_directonsite where  (sbicc_dated Between '".($Dated)."' and '".$todaydate." 23:59:59') ";
}
...
while($plrow = mysql_fetch_array($qrysqlresult))
{
	...
	$InsertSQl = "INSERT INTO Req_Dialer_Records_PL (Reply_Type, RequestID, Name, Mobile_Number, Feedback, Dated,dialer_camp_id) VALUES ('1', '".$param["unique_id"]."', '".$param["c_name"]."', '".$param["p_number"]."', '', '".$dated."', '1502')";
	...
	//echo $InsertSQl;
	$updateSql = "update Req_Compaign set RequestID='".$ReqID."', Dated=Now() where (Compaign_ID=6621 and Reply_Type=4)";
	ExecQuery($updateSql);
	echo "<br>";
	//  print_r($content); 
	curl_close ($ch);
