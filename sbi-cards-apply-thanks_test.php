...
if($getRecordsCount){
	...
	if($Employment_Status==1 || $Employment_Status==0){
		...
		if(empty($FirstName)){
			$errArray['field'] = 'FirstName';
			$errArray['value'] = $FirstName;
			$errArray['errmsg'] = 'FirstName is empty';
		...
		if(strlen($FirstName) > 12){
			$errArray['field'] = 'FirstName';
			$errArray['value'] = $FirstName;
			$errArray['errmsg'] = 'FirstName: Max length is 12 chars';
			
			$flag = 1;
		}

		if(empty($LastName)){
			$errArray['field'] = 'LastName';
			$errArray['value'] = $LastName;
			$errArray['errmsg'] = 'LastName is empty';
		...
		if(strlen($LastName) > 16){
			$errArray['field'] = 'LastName';
			$errArray['value'] = $LastName;
			$errArray['errmsg'] = 'LastName: Max length is 16 chars';

			$flag = 1;
		}

		if(empty($CompanyName)){
			$errArray['field'] = 'CompanyName';
			$errArray['value'] = $CompanyName;
			$errArray['errmsg'] = 'CompanyName is empty';

			$flag = 1;
		}elseif(strlen($CompanyName) > 30){
			$errArray['field'] = 'CompanyName';
			$errArray['value'] = $CompanyName;
			$errArray['errmsg'] = 'CompanyName: Max length is 30 chars';
		...
		if(empty($PAN)){
			$errArray['field'] = 'PAN';
			$errArray['value'] = $PAN;
			$errArray['errmsg'] = 'PAN is empty';

			$flag = 1;
		}elseif(strlen($PAN) != 10){
			$errArray['field'] = 'PAN';
			$errArray['value'] = $PAN;
			$errArray['errmsg'] = 'PAN should be of 10 numbers';

			$flag = 1;
		}elseif(!preg_match("/^([a-zA-Z]){5}([0-9]){4}([a-zA-Z]){1}?$/", $PAN)) {
			$errArray['field'] = 'PAN';
			$errArray['value'] = $PAN;
			$errArray['errmsg'] = 'PAN is invalid';

			$flag = 1;
		}
		
		$panValidationResponse = checkPancardValidation($RequestID,$PAN,'CC','UAT');
		if($panValidationResponse != 'MATCHED'){
			$errArray['field'] = 'PAN';
			$errArray['value'] = $PAN;
			$errArray['errmsg'] = $panValidationResponse;

			$flag = 1;
		}

		if(empty($ResiAddress1)){
			$errArray['field'] = 'ResiAddress1';
			$errArray['value'] = $ResiAddress1;
			$errArray['errmsg'] = 'ResiAddress1 is empty';

			$flag = 1;
		}elseif(strlen($ResiAddress1) > 40){
			$errArray['field'] = 'ResiAddress1';
			$errArray['value'] = $ResiAddress1;
			$errArray['errmsg'] = 'ResiAddress1: Max length is 40 chars';

			$flag = 1;
		}

		if(empty($ResiAddress2)){
			$errArray['field'] = 'ResiAddress2';
			$errArray['value'] = $ResiAddress2;
			$errArray['errmsg'] = 'ResiAddress2 is empty';

			$flag = 1;
		}elseif(strlen($ResiAddress2) > 40){
			$errArray['field'] = 'ResiAddress2';
			$errArray['value'] = $ResiAddress2;
			$errArray['errmsg'] = 'ResiAddress2: Max length is 40 chars';

			$flag = 1;
		}

		if(empty($OfficeAddress1)){
			$errArray['field'] = 'OfficeAddress1';
			$errArray['value'] = $OfficeAddress1;
			$errArray['errmsg'] = 'OfficeAddress1 is empty';

			$flag = 1;
		}elseif(strlen($OfficeAddress1) > 40){
			$errArray['field'] = 'OfficeAddress1';
			$errArray['value'] = $OfficeAddress1;
			$errArray['errmsg'] = 'OfficeAddress1: Max length is 40 chars';

			$flag = 1;
		}
		
		if(!empty($OfficeAddress2) && (strlen($OfficeAddress2) > 40)){
			$errArray['field'] = 'OfficeAddress2';
			$errArray['value'] = $OfficeAddress2;
			$errArray['errmsg'] = 'OfficeAddress2: Max length is 40 chars';
			$flag = 1;
		}

		if($flag){
			header('Location: http://www.deal4loans.com/sbi-cards-apply-continue.php?RequestID='.$RequestID.'&cc_bankid='.$cc_bankid.'&cc_bankname='.$cc_bankname.'&errArray'.urlencode(serialize($errArray)));
		}
		else{
