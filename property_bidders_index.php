...
{
	var gifName = form;
	document.frmsearch.action="property_bidders_index.php?search=y"+gifName;
...
function insertData(id)
		...
		var queryString = "?comment_section=" + get_comment_section + "&get_requestid=" + get_requestid + "&get_product=" + get_product + "&get_bidderid=" + get_bidderid;
				...
				//alert(queryString); 
				ajaxRequest.open("GET", "insert_comment_propertylms.php" + queryString, true);
...
<b>2.</b> Tell the loan seeker how much tax savings will this home loan get him/her.
 ...
 <form name="frmsearch" action="property_bidders_index.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select FeedbackID from  Req_Feedback_Property where AllRequestID=$RequestID and BidderID=".$_SESSION['BidderID']);		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update  Req_Feedback_Property Set Feedback='".$Feedback."'";
			$strSQL=$strSQL."Where FeedbackID=".$row["FeedbackID"];
		}
		else
		{
			
			$strSQL="Insert into  Req_Feedback_Property(AllRequestID, BidderID, Reply_Type , Feedback) Values (";
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND ( Req_Feedback_Property.Feedback IS NULL OR  Req_Feedback_Property.Feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND  Req_Feedback_Property.Feedback='".$varCmbFeedback."' ";
		}
 ...
 <? 	
	$qry="SELECT RequestID FROM  Req_Property_Bidder,`".$val."` LEFT OUTER JOIN  Req_Feedback_Property ON  Req_Feedback_Property.AllRequestID=".$val.".RequestID AND  Req_Feedback_Property.BidderID= '".$_SESSION['BidderID']."' WHERE  Req_Property_Bidder.AllRequestID=`".$val."`.RequestID and  Req_Property_Bidder.BidderID = '".$_SESSION['BidderID']."' and  Req_Property_Bidder.Reply_Type=".$pro_code." and ( Req_Property_Bidder.Allocation_Date Between '".($min_date)."' and '".($max_date)."' or  Req_Feedback_Property.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
...
<tr>
	...
	$qry="SELECT  	Dated,PropertyID, RequestID,Name,City,Mobile_Number,Net_Salary,Company_Name,Employment_Status,comment_section,Feedback,Followup_Date  FROM  Req_Property_Bidder,`".$val."` LEFT OUTER JOIN  Req_Feedback_Property ON  Req_Feedback_Property.AllRequestID=".$val.".RequestID AND  Req_Feedback_Property.BidderID= '".$_SESSION['BidderID']."' WHERE  Req_Property_Bidder.AllRequestID=`".$val."`.RequestID and  Req_Property_Bidder.BidderID = '".$_SESSION['BidderID']."' and  Req_Property_Bidder.Reply_Type=".$pro_code." and ( Req_Property_Bidder.Allocation_Date Between '".($min_date)."' and '".($max_date)."' or  Req_Feedback_Property.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
...
//echo $qry;
   ...
   <tr>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3" ><a href="/propertylead-details.php?postid=<? echo $row["RequestID"]; ?>&biddt=<? echo $_SESSION['BidderID']; ?>&proptyid=<? echo $row["PropertyID"]; ?>" target="_blank">
	 <? echo $row["Name"]; ?></a></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["City"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["Mobile_Number"]; ?></td>
   	    ...
   	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("property_bidders_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
		</td>
