...
{
	var gifName = form;
	document.frmsearch.action="hdfc_index_n.php?search=y"+gifName;
...
<b>2.</b> Tell the loan seeker how much tax savings will this home loan get him/her.
 ...
 <form name="frmsearch" action="hdfc_index_n.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass">
	     <!--<input name="min_date" type="text" id="min_date" size="15" >--></td>
 ...
 <? 
  /*	$cityListSql = "select Location from hdfc_bidders where status=1";
	$cityListQuery = ExecQuery($cityListSql);
	$cityListNumRows = mysql_num_rows($cityListQuery);
	$cityList = "";
	 for($cL=0;$cL<$cityListNumRows;$cL++)
	 {
	 	$cityList[] = mysql_result($cityListQuery,$cL,'Location');
	 }
 	$city_List = implode("','",$cityList);
 */
// $city_List = "Ahmedabad','Delhi','Gurgaon','Pune','Noida','Gaziabad','Faridabad','Sahibabad";
 // $city_List = "Delhi','Gurgaon','Pune','Noida','Gaziabad','Faridabad','Sahibabad','Chandigarh','Mohali','Kharar','Panchkula','Ziragpur','Jaipur";//01-08-13 Upendra
  $city_List = "Delhi','Gurgaon','Noida','Gaziabad','Faridabad','Sahibabad";
 
 	$search_qry="SELECT RequestID,name,mobile_number,email_id,DOB,Employment_Status,company_name,city,net_salary,primary_acc,eligible_loanAmt,eligible_interestRate,eligible_emi,eligible_term,AppID,Dated,Feedback,comment_section FROM hdfc_pl_calc_leads LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=hdfc_pl_calc_leads.hdfcplid AND Req_Feedback.BidderID  =".$_SESSION['BidderID']." WHERE city in ('".$city_List."') and hdfc_pl_calc_leads.eligible='Yes' and (hdfc_pl_calc_leads.Dated  Between '".($min_date)."' and '".($max_date)."' ) ";	
		...
		$qry="SELECT * FROM hdfc_pl_calc_leads LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=hdfc_pl_calc_leads.hdfcplid AND Req_Feedback.BidderID  =".$_SESSION['BidderID']." WHERE city in ('".$city_List."') and hdfc_pl_calc_leads.eligible='Yes' and (hdfc_pl_calc_leads.Dated  Between '".($min_date)."' and '".($max_date)."' )";
 ...
 <td width="141" align="center" bgcolor="#FFFFFF" class="style2">Feedback </td>
   ...
   <tr>
     <td align="center" bgcolor="#DFF6FF" class="style3" >
		 <? echo $row["name"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? 
	if($row["city"]=="Delhi" || $row["city"]=="Gurgaon" || $row["city"]=="Gaziabad" || $row["city"]=="Noida" || $row["city"]=="Faridabad")
			{
		echo "*****"; 
			}
			else
	...
	<td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("hdfc_index_n.php",$row["hdfcplid"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
