...
$filename = $RequestID."_".$filename;
  ...
  if(move_uploaded_file($tmpfile, "upload/".$filename)){
	...
	if($numRows>0)
	{
		$DataArray = array("Loan_Track"=>$filename);
  ...
  echo returnStatus($status);
	...
	function returnStatus($status){
		return "<html><body><script type='text/javascript'>function init(){if(top.uploadComplete_13) top.uploadComplete_13('".$status."');}window.onload=init;
	</script></body></html>";
	}
