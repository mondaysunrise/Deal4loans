...
{   
	...
	{
	if($strsource=="wfappointment")
	{
		$qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and BankID!=66 and BankID!=80 and lms_block_flag!=2) order by Bidder_Name ASC";
	...
	else
	{
		if($strreferral==1)
		{
		 $qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and Referral_Flag=1 and BankID!=66 and BankID!=80 and lms_block_flag!=1)";
		}
		else if($strreferral==2)
		{
		
		 $qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and  bidder_postpaid=1 and lms_block_flag!=1)";
		}
		else if($strreferral==3)
		{
			$getIncludeBiddersSql = "select BidderID from Bidders where Global_Access_ID =6383";
			$getIncludeBiddersQry = d4l_ExecQuery($getIncludeBiddersSql);
			$recordCountIncludeBidders = d4l_mysql_num_rows($getIncludeBiddersQry);
			for($ii=0;$ii<$recordCountIncludeBidders;$ii++)
			{
				$BidderIDInclude = d4l_mysql_result($getIncludeBiddersQry,$ii,'BidderID');
				$BidderIDIncludeArr[$ii] = $BidderIDInclude;
			}
			$BidderIDIncludeStr = implode(',', $BidderIDIncludeArr);
			 $qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and  BidderID in (".$BidderIDIncludeStr.") and lms_block_flag!=1)";
		}
		else
		{
			$qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and BankID!=66 and BankID!=80 and lms_block_flag!=1) order by Bidder_Name ASC";
		}
	}
	...
	$Total4LifeTime = $row_result_LT[0]['SumLifeTime'];
		...
		if($strsource=="SMS_Lead_Appointment_Model")
		{
			$ValidBidder_Day=1;
			$ValidBidder_Week=1;
			$ValidBidder_Month=1;
