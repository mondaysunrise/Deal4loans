...
function getReqValue1($pKey){
 ...
 if($strbidder>0)
 {
	 $finalbidderid=$strbidder;
 }
 else
 {
	$finalbidderid=$strCity;
 }

if($strCity=="3662" || $strbidder=="3662")
{
	$citylist="Mumbai','Thane','Navi Mumbai";
}
else if($strCity=="3778" || $strbidder=="3778")
{
		$citylist="Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Greater Noida";
}
else
{	
	if($strCity>0)
	{
	$citylist = getReqValue1($strCity);
	}
	else
	{
		$citylist = getReqValue1($strbidder);
	}
}
...
{
	var gifName = form;
	document.frmsearch.action="icici_cards2lmsview.php?search=y"+gifName;
...
function insertData(id)
		...
		var get_comment_section = document.getElementById('icici_comment_'+ id).value;
		...
		var queryString = "?comment_section=" + get_comment_section + "&get_FollowupDate=" + get_FollowupDate + "&get_requestid=" + get_requestid;
				
				alert(queryString); 
				ajaxRequest.open("GET", "insert_comment_icici_ccmlrlms.php" + queryString, true);
...
<tr>
 ...
 <form name="frmsearch" action="icici_cards2mlrlmsview.php?search=y" method="post" onSubmit="return chkform();">
...
<option value="Wrong Number" <? if($varCmbFeedback == "Wrong Number") { echo "selected"; } ?>>Wrong Number</option>
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$DataArray = array("icici_feedback"=>$Feedback);
		$wherecondition ="(iciciccid=".$RequestID.")";
...
if($varCmbFeedback!="select")
		...
		{
			$FeedbackClause=" AND (icici_credit_card.icici_feedback IS NULL OR icici_credit_card.icici_feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND icici_credit_card.icici_feedback='".$varCmbFeedback."' ";
		}
		}
		else
		{
			if(isset($lead_stat) && ($lead_stat==1 || $lead_stat==2))
			{
				if($lead_stat==1)
			{
				$FeedbackClause=" AND (icici_credit_card.icici_feedback='FollowUp' OR icici_credit_card.icici_feedback IS NULL OR icici_credit_card.icici_feedback='' OR icici_credit_card.icici_feedback='Logged In' OR icici_credit_card.icici_feedback='No Feedback') ";
			}
			else if ($lead_stat==2)
			{
				$FeedbackClause=" AND (icici_credit_card.icici_feedback!='FollowUp' and icici_credit_card.icici_feedback!='') ";
			}
		}
		}
		if($mob_num>0)
{
	$mob_num_clause = " AND icici_credit_card.icici_mobileno = '".$mob_num."' ";
}
?>
 <p class="bodyarial11"><?=$Msg?></p>
 <table width="1000" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" >
<? 
			$strbidderlist=$citylist;
		
if($_SESSION['BidderID']==3667 && $strCity=="")
	{
	
$search_qry="SELECT * FROM icici_credit_card Where (icici_eligible_flag=1 and ((icici_dated between '".($min_date)."' and '".($max_date)."') or (icici_followup_date between '".($min_date)."' and '".($max_date)."')))";

$search_qry=$search_qry.$FeedbackClause;
$search_qry=$search_qry." order by icici_credit_card.icici_dated DESC";

	$qry="SELECT * FROM icici_credit_card Where ( icici_eligible_flag=1 and ((icici_dated between '".($min_date)."' and '".($max_date)."') or (icici_followup_date between '".($min_date)."' and '".($max_date)."')))";
	...
	{
		$strbidderlist=$citylist;
		$search_qry="SELECT * FROM icici_credit_card Where (icici_city in ('".$strbidderlist."') and icici_eligible_flag=1 and ((icici_dated between '".($min_date)."' and '".($max_date)."') or (icici_followup_date between '".($min_date)."' and '".($max_date)."')))";
		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry.$mob_num_clause;
	
	$search_qry=$search_qry." order by icici_credit_card.Dated DESC";

	$qry="SELECT * FROM icici_credit_card Where (icici_city in ('".$strbidderlist."') and icici_eligible_flag=1 and ((icici_dated between '".($min_date)."' and '".($max_date)."') or (icici_followup_date between '".($min_date)."' and '".($max_date)."')))";
	$qry=$qry.$FeedbackClause;
...
$qry=$qry.$FeedbackClause;
		...
		$qry=$qry." order by icici_credit_card.icici_dated DESC";
	...
	list($recordcount1,$row)=MainselectfuncNew($qry,$array = array());
		...
		{																					$followup_date=$row[$ii]['icici_followup_date'];
		?>
		<input type="hidden" name="requestid_<? echo $i;?>" id="requestid_<? echo $i;?>" value="<? echo $row[$ii]["iciciccid"];?>">
	
  <tr>
  
     <td align="center" bgcolor="#DFF6FF" class="style3" ><a href="iciciccmlr_editlead.php?id=<? echo urlencode($row[$ii]['iciciccid']); ?>&Bid=<? echo $finalbidderid;?>" target="_blank"><? echo $row[$ii]["icici_name"]; ?></a></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$ii]["icici_city"]; ?></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$ii]["icici_mobileno"]; ?></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$ii]["icici_income"]; ?></td>

	  <td align="center" bgcolor="#DFF6FF" class="style3"><? if($row[$ii]["icici_emp_status"]==1) { echo "Salaried";} else { echo "Self Employed";} ?></td>
      	  <td align="center" bgcolor="#DFF6FF" class="style3" width="107">ICICI Bank Platinum Chip Credit Card</td>
<td align="center" bgcolor="#DFF6FF" class="style3">
 <? if($row[$ii]["icici_feedback"]=="" || $row[$ii]["icici_feedback"]=="FollowUp" || $row[$ii]["icici_feedback"]=="Logged In" ||  $row[$ii]["icici_feedback"]=="Appointment fixed" || $row[$ii]["icici_feedback"]=="Appointment postponed") { echo "Open"; }?>
<? if($row[$ii]["icici_feedback"]!="" && $row[$ii]["icici_feedback"]!="FollowUp" && $row[$ii]["icici_feedback"]!="Logged In" && $row[$ii]["icici_feedback"]!="Appointment fixed" && $row[$ii]["icici_feedback"]!="Appointment postponed") { echo "Closed"; }?>

</td>
    <td align="center" bgcolor="#DFF6FF" class="style3"><? 
	if($_SESSION['BidderID']==3667)
		{
echo $row[$ii]["icici_feedback"];
	}
			else
			{
	echo getJumpMenu("icici_cards2mlrlmsview.php",$row[$ii]["iciciccid"],"4",$row[$ii]["icici_feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback);
			} ?>
	</td>
...
<? if($_SESSION['BidderID']==3667)
		{
			echo $row[$ii]["icici_comment"]; 	
		}
		else
			{ ?>
<textarea  name="icici_comment_<? echo $i;?>" id="icici_comment_<? echo $i;?>" cols="15" rows="2"><? echo $row[$ii]["icici_comment"]; ?></textarea>
<? } ?>
</td>
 <? if($_SESSION['BidderID']==3667)
		{
...
<? 
 ...
 <form name="frmdownload" action="icicicc_mlr_download.php" method="post">
   <tr>
     <td align="center">
