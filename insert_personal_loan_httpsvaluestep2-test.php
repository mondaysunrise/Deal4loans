	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	$getpldetails=ExecQuery("select Employment_Status,CC_Holder,Email,City_Other,City,Company_Name,Name,Net_Salary,DOB,Reference_Code From Req_Loan_Personal Where (RequestID='".$leadid."')");	
	...
	$Reference_Code = $plrow[$myrowcontr]['Reference_Code'];
	$CC_Holder = $plrow[$myrowcontr]['CC_Holder'];
...
list($plshowRows,$plquote)=MainselectfuncNew($plshowquotes,$array = array());
	...
	for($i=0;$i<$plshowRows;$i++)
	 {
	$shwlogo="select logo from Bank_Master Where Bank_Name like '%".$plquote[$plshowRows]["pl_bankname"]."%'";
	...
	?><tr><?
	if($plquote[$plshowRows]["pl_bankrate"]=="100.00")
			 { if($plquote[$plshowRows]["pl_bankname"]=="Citibank")
				 { ?>
				 <td class="banks" style="color:#000000;">&nbsp;&nbsp;<? if(strlen($imagebank)>5) {echo  $imagebank;}else { echo $plquote[$plshowRows]["pl_bankname"]; } ?></td>
...
<li>24X7 online account management</li>
	...
	<td class="info">
	 <form action="/get-citibankquote-submit.php" method="GET" target="_blank" >
		  ...
		  <input type="hidden" name="max_loan_amount" value="<?php echo $plquote[$plshowRows]["pl_loanamount"]; ?>" />
	   <input type="hidden" name="calc_emi" value="<?php echo $plquote[$plshowRows]["pl_bankemi"]; ?>" />
	    <input type="hidden" name="loan_tenure" value="<?php echo $plquote[$plshowRows]["pl_banktenure"]; ?>" />
            <input type="hidden" name="pl_bank_name" id="pl_bank_name" value="<? echo $plquote[$plshowRows]["pl_bankname"]; ?>">
			<input type="submit" style="width: 49px; height: 20px; border: 0px none ; cursor:pointer; background-image: url(/new-images/apl-yelo-nw.jpg); margin-bottom: 0px; margin-top:10px; background-repeat:no-repeat; background-color:#FFFFFF;" value=""  />
				 ...
				 <? }
				 elseif($plquote[$plshowRows]["pl_bankname"]=="Bajaj Finserv" && $Employment_Status==1)
				 { ?>
		...
		<input type="submit" style="width: 49px; height: 20px; border: 0px none ; cursor:pointer; background-image: url(/new-images/apl-yelo-nw.jpg); margin-bottom: 0px; margin-top:10px; background-repeat:no-repeat; background-color:#FFFFFF;" value=""  />
				...
				<? }
				 ...
				 {	
					 ?>
		<td class="banks" style="color:#000000;">&nbsp;&nbsp;<? if(strlen($imagebank)>5) {echo  $imagebank;}else { echo $plquote[$plshowRows]["pl_bankname"]; } ?></td>
		<td class="i-rate"><? if($plquote[$plshowRows]["pl_bankname"]=="Fullerton") {echo "21% - 32%";} else {echo $plquote[$plshowRows]["pl_bankrate"]."%";} ?></td>
		<td class="emi">Rs.<? echo $plquote[$plshowRows]["pl_bankemi"]; ?></td>
		<td class="tenure"><? echo $plquote[$plshowRows]["pl_banktenure"]; ?> yrs.</td>
		<td class="loan">Rs. <? echo substr(trim($plquote[$plshowRows]["pl_loanamount"]), 0, strlen(trim($plquote[$plshowRows]["pl_loanamount"]))-3);  ?></td>
		<td class="i-pfrate"> <? echo $plquote[$plshowRows]["pl_bankpf"]; ?></td>
		<td class="prepay"> <? echo $plquote[$plshowRows]["pl_bankppc"]; ?></td>
		<td class="info"><form action="/apply_pl_consent.php" method="GET" target="_blank" >
			 <input type="hidden" name="pl_requestid" value="<? echo $leadid; ?>" id="pl_requestid">
		    <input type="hidden" name="pl_bank_name" id="pl_bank_name" value="<? echo $plquote[$plshowRows]["pl_bankname"]; ?>">
			<input type="submit" style="width: 49px; height: 20px; border: 0px none ; cursor:pointer; background-image: url(/new-images/apl-yelo-nw.jpg); margin-bottom: 0px; margin-top:10px; background-repeat:no-repeat; background-color:#FFFFFF;" value=""  />
