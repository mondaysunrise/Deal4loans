...
if(($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="182.71.109.218"))
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		$PWD = $_POST["PWD"];
		$pdo = db_connect_PDO();
		$stmt = $pdo->prepare('SELECT * FROM Bidders WHERE Email = :email and  PWD = :pwd and BidderID=94');
		...
		if($num_rows>0)
			...
			ExecQuery($sqlTrackBidder);
				...
				$strDir = dir_name();
					...
					if($BidderID==94)
					{
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."bajajfinserv_unallocateview.php");
						exit;					
					}
					else
					{
