...
if($IP_Remote=='185.93.228.12') { $IP= $_SERVER['HTTP_X_REAL_IP']; }
else { $IP=$IP_Remote;	}
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		$PWD = $_POST["PWD"];
		$pdo = db_connect_PDO();
		$stmt = $pdo->prepare('SELECT * FROM Bidders WHERE Email = :email and  PWD = :pwd and BidderID in (2679, 7340)');
		...
		if($num_rows>0)
			...
			setSessionBidder($Email, $row);
					...
					if($BidderID==2679 && ($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.161.193.191" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="182.73.4.58" || $IP=="182.73.4.59" || $IP=="182.73.4.60" || $IP=="182.73.4.61" || $IP=="182.73.4.62" || $IP=="182.71.109.218" || $IP=="1.23.114.53" || "185.93.231.12") || $BidderID==7340)
					{
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."fullerton_index.php");
						exit;					
					}
					else
					{
