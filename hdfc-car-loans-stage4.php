...
$sqlcnSql = "Select hdfc_car_name from hdfc_car_list_category Where hdfc_clid='".$car_name."'";
list($Rows,$sqlcn)=MainselectfuncNew($sqlcnSql,$array = array());
$car_name =  $sqlcn[0]['hdfc_car_name'];

//$car_name = "Tata Safari Dicor 2.2 Lx 4X2";
$getCustSql = "select * from hdfc_car_loan_leads where RequestID = '".$last_inserted_id."'";
list($Rows,$getCustQuery)=MainselectfuncNew($getCustSql,$array = array());
...
list($Rows,$rowcn)=Mainselectfunc($sqlcn,$array = array());
...
list($numRows,$rowcmp)=Mainselectfunc($getcompdetails,$array = array());
...
for($i=0; $i<count($listofcars); $i++)
{
	...
	list($numRows,$row)=Mainselectfunc($getcardetails,$array = array());
...
list($numRows,$getCarQuery)=MainselectfuncNew($getCarSql,$array = array());
...
for($j=0; $j<=count($listofcars); $j++)
{
	...
	list($numRows,$resultvedio)=MainselectfuncNew($getvediodetails,$array = array());
	...
	>
                ...
                <tr>
                  <td  style="background:url(images/newimages/<?php echo $model; ?>specification.jpg) repeat-x;"><table width="100%" border="0" cellpadding="0" cellspacing="0" height="190">
                      <tr>
                        <td colspan="2" class="text_car_body" align="center" >
                        <?php if(strlen($car_videocode)>10)
						{
						?>
                        <iframe width="224" height="190" src="<?php echo $car_videocode; ?>" frameborder="0" allowfullscreen></iframe>	<?php } else { ?> <div style="width:224px; height:190px;">&nbsp;</div> <?php } ?>
...
<table width="960" border="0" cellpadding="0" cellspacing="0">
	...
	$getGiftsSql = "SELECT * FROM hdfc_car_loan_gifts WHERE ".$lnAmt[0]." >= min_range AND ".$lnAmt[0]." < max_range AND status=1 ORDER BY RAND()";	
		list($numGiftsQuery,$getGiftsQuery)=Mainselectfunc($getGiftsQuery,$array = array());
		if($numGiftsQuery>6)
		{
			$numGiftsQuery = 6;
		}
...
<div style="width:100%; height:28px; background:url(images/newimages/<?php echo $model; ?>specification-bg.jpg) repeat-x; margin: auto; margin-top:5px; text-align:right;"><a href="hdfc-terms-and-conditions.php" style="color:#000000; font-size:11px; font-weight:bold; text-decoration:none; color:#FFFFFF;" target="_blank">Terms & Conditions</a>&nbsp;</div></div>
