...
<title>Home Loan MIS Report</title>
<style type="text/css">
...
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" style="border:#0033CC 1px solid;">
 ...
 <form name="frmsearch" action="hlmisreportnc.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="85%">
     ...
     <option value="207" <? if($lms_details == "207") { echo "selected"; }?>>HL LMS 1</option>
     <option value="732" <? if($lms_details == "732") { echo "selected"; }?>>HL LMS 2</option>
     <option value="812" <? if($lms_details == "812") { echo "selected"; }?>>HL LMS 3</option>
     <option value="460" <? if($lms_details == "460") { echo "selected"; }?>>HL LMS 4</option>
	 <option value="64" <? if($lms_details == "64") { echo "selected"; }?>>HL LMS 5</option>
     </select>
		</td>
...
if($lms_details=="All")
{
	$biddIDS = array(732,812,460,207,64);
	$seq = array(0,1,2,3);
}
else 
{
	$biddIDS = array($lms_details);
	if($lms_details==460)	{ $seq = array(0);} else if($lms_details==732) { $seq = array(1);} else if($lms_details==812) { $seq = array(2);} else if($lms_details==207) { $seq = array(3);}
}

	for($j=0;$j<count($biddIDS);$j++)
	{	
	 $sql_PL = "SELECT count(RequestID) as countLeads, Feedback from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (".$biddIDS[$j].") WHERE (((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."' ) and MOD(Req_Loan_Home.RequestID,4)='".$seq[$j]."' ) or ((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and Req_Feedback_HL.BidderID in (".$biddIDS[$j]."))) group by Req_Feedback_HL.Feedback";
 ...
 <table width="950" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" >
  <tr>
	 ...
	 else if($biddIDS[$j]==732)
	 {
	 	echo "HL LMS 1 [732]";
	 }
	 else if($biddIDS[$j]==812)
	 {
	 	echo "HL LMS 3 [812]";
	 }
	 else if($biddIDS[$j]==460)
	 {
	 	echo "HL LMS 4 [460]";
	 }
	 ?>
