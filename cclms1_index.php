...
{
	var gifName = form;
	document.frmsearch.action="cclms1_index.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="cclms1_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//		echo $strSQL;
 ...
 <?
		$qry="SELECT * from Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID='935' WHERE 
((Req_Credit_Card.Allocated=0 and ((Req_Credit_Card.Dated Between '".$min_date."' and '".$max_date."') or 
(Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."' )  and  Req_Feedback_CC.BidderID='935') and Req_Credit_Card.source in ('BnnrCmpgn_CC','komlicc') and Req_Credit_Card.Is_Valid=0 ) and Req_Credit_Card.source not in ('sbitest','SMS_Lead'))";
...
<!--///////////////////////-->
<tr>
<td class="bodyarial11"><a href="cceditlead_1.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
	   ...
	   ?>
	<td><form method="post" name="bidder_array" action=""><? list($FinalBidder,$finalBidderName)= getBiddersList("Req_Credit_Card",$row["RequestID"],$City);
   for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo getJumpMenu("cclms_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
