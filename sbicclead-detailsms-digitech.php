<?php
//Return Page from Digitech Predective Call
...
$getBidderIDQry = "SELECT BidderID FROM Bidders WHERE Profile = '".$BidID."' AND leadidentifier = 'sbicallerdigilms_cc'";
...
if($getBidderID > 0){
	...
	$updateqry= "Update lead_allocate set BidderID='".$BidID."' Where AllRequestID = '".$ReqID."' and BidderID=7297";
	$updateqryresult = d4l_ExecQuery($updateqry);
}else{
	echo "This BidderID is not Valid.";
	exit;
}
...
{
?>
<form name="loan_form" method="post" action="<? echo $_SERVER['PHP_SELF'] ?>?postid=<?echo $requestid;?>&biddt=<? echo $_REQUEST["biddt"];?>&process="<?php echo $process; ?>"" >
<?php
}
else {
?>
<form name="loan_form" method="post" action="<? echo $_SERVER['PHP_SELF'] ?>?postid=<?echo $requestid;?>&biddt=<? echo $_REQUEST["biddt"];?>" >
<?php } ?>
