...
<tr>
	...
	<tr><td align="right" style="padding-right:50px;"><a href="/icici_cards2mlrlmsview.php" target="_blank"><b>Mailer leads</b></a></td></tr>
 <tr><td align="center">
 ...
 <form name="frmsearch" action="icici_cards2lmsview.php?search=y" method="post" onSubmit="return chkform();">
...
$getfields="applied_card_name,Existing_Relationship,Account_No,RequestID,Name,Mobile_Number,Allocation_Date,Net_Salary,Company_Name,Loan_Amount,Employment_Status,Feedback,comment_section,City,Followup_Date";
if($_SESSION['BidderID']==3667 && $strCity=="")
	{
	
$search_qry="SELECT * FROM Req_Feedback_Bidder_CC,`".$val."` LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID=".$val.".RequestID AND Req_Feedback_CC.BidderID in (3662,3663,3664,3665,3666,3778,3820,3821,3822,3823) WHERE Req_Feedback_Bidder_CC.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder_CC.BidderID in (3662,3663,3664,3665,3666,3778,3820,3821,3822,3823,4499,4500,4501,4502,4503) and Req_Feedback_Bidder_CC.Reply_Type=".$pro_code." and (Req_Feedback_Bidder_CC.Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CC.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
...
$search_qry=$search_qry.$FeedbackClause;
	...
	$qry="SELECT RequestID FROM Req_Feedback_Bidder_CC,`".$val."` LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID=".$val.".RequestID AND Req_Feedback_CC.BidderID in (3662,3663,3664,3665,3666,3778,3820,3821,3822,3823) WHERE Req_Feedback_Bidder_CC.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder_CC.BidderID in (3662,3663,3664,3665,3666,3778,3820,3821,3822,3823,4499,4500,4501,4502,4503) and Req_Feedback_Bidder_CC.Reply_Type=".$pro_code." and (Req_Feedback_Bidder_CC.Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CC.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
...
if($_SESSION['BidderID']==3667 && $strCity=="")
		{
   $qry="SELECT ".$getfields." FROM Req_Feedback_Bidder_CC,`".$val."` LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID=".$val.".RequestID AND Req_Feedback_CC.BidderID in (3662,3663,3664,3665,3666,3778,3820,3821,3822,3823) WHERE Req_Feedback_Bidder_CC.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder_CC.BidderID in (3662,3663,3664,3665,3666,3778,3820,3821,3822,3823) and Req_Feedback_Bidder_CC.Reply_Type=".$pro_code." and (Req_Feedback_Bidder_CC.Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CC.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
		$qry=$qry.$FeedbackClause;
		}
		else
		{
...
$qry=$qry.$FeedbackClause;
  ...
  <tr>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3" ><a href="icicicc_editlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>" target="_blank"><? echo $row["Name"]; ?></a></td>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["Net_Salary"]; ?></td>
	  ...
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? if($row["Employment_Status"]==1) { echo "Salaried";} else { echo "Self Employed";} ?></td>
		  ...
		  $cardsarr=explode(",",$row["applied_card_name"]);
$cardsarrunq="";
...
<td align="center" bgcolor="#DFF6FF" class="style3">
 <? if($row["Feedback"]=="" || $row["Feedback"]=="FollowUp" || $row["Feedback"]=="Logged In" ||  $row["Feedback"]=="Appointment fixed" || $row["Feedback"]=="Appointment postponed") { echo "Open"; }?>
...
echo $row["Feedback"];
			...
			{
	echo getJumpMenu("icici_cards2lmsview.php",$row["RequestID"],"3",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback);
			} ?>
	</td>
