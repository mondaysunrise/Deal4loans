...
{
	var gifName = form;
	document.frmsearch.action="billingindexprepaid.php?search=y"+gifName;
...
<div align="center">
 <center>
 <?php  include '~TopBillingPrepaid.php'; ?>
 <br>
 <br>
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <form name="frmsearch" action="billingindexprepaid.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
<?
	...
	$search_qry="select * from Bidders_List where  1 = 1  and  Multiplier>0 and Reply_type = '".$Repy_TypeProduct."' ORDER BY `BidderID` DESC ";
	$qry="select * from Bidders_List where 1 = 1  and  Multiplier>0 and Reply_type = '".$Repy_TypeProduct."' ORDER BY `BidderID` DESC ";
	}
 ...
 ?>
		...
		<!--<td width="85" class="head1">GeneratedBy</td> -->
          ...
          <td width="66" class="head1">Cost/Lead</td>
		   <td width="66" class="head1">Total Amount</td>
          </tr>
        <?
	...
	*/
	if($Repy_TypeProduct==-1)
		$qry="select * from Bidders_List where  1 = 1 ORDER BY `BidderID` DESC ";
	else 
		$qry="select * from Bidders_List where  1 = 1 and Reply_type = '".$Repy_TypeProduct."' ORDER BY `BidderID` DESC ";
	
	
	 list($num_rows,$Arrrow)=MainselectfuncNew($qry,$array = array());
		$ctr=0;
	...
	//	while($row=mysql_fetch_array($result))
		while($ctr<count($Arrrow))
        {
        
			//From Bidders_List
			$Bidder_Name = $Arrrow[$ctr]['Bidder_Name'];
			$Reply_Type = $Arrrow[$ctr]['Reply_Type'];
			$BidderID = $Arrrow[$ctr]['BidderID'];
			$Multiplier = $Arrrow[$ctr]['Multiplier'];
			$City = $Arrrow[$ctr]['City'];
			$Restrict_Bidder = $Arrrow[$ctr]['Restrict_Bidder'];	
			
			$CapLead_Count = $Arrrow[$ctr]['CapLead_Count'];	
			$CapLead_CountArray = explode(',', $CapLead_Count);
			$totalLeadsCount[] = $CapLead_CountArray[3];
			$search_date="";
			...
			//echo $Row_Bidder['Define_PrePost'];
...
//		$Sql = "select * from Bill_Record where Bill_Sent>=1 and BidderID=$BidderID and  Bill_Period='".$finalminval."' ";
		...
		list($Validate,$Myrows)=MainselectfuncNew($Sql,$array = array());
			$k=0;
		...
		$StoredLeadCount = $Myrows[$k]['Lead_Volume'];
		$StoredCost_Lead = $Myrows[$k]['Cost_Lead'];
		$StoredSub_Total = $Myrows[$k]['Sub_Total'];
		$Invoice_Number = $Myrows[$k]['Invoice_Number'];
		$Generated_By  = $Myrows[$k]['Generated_By'];	
	...
	if($Row_Bidder[$i]['Define_PrePost']=='PrePaid')
	{
	...
	?> 
		...
		<!-- <td  class="bodyarial11"><select name="GeneratedBy_<?php //echo $z; ?>" >
			...
			<td class="bodyarial11"><?php echo $Row_Bidder[$i]['BD_Name']; ?></td>
          
		  <td class="bodyarial11"><?php echo $Row_Bidder[$i]['Bidder_Name']; ?> (<?php echo $BidderID; ?>)</td>
		  <td class="bodyarial11"><?php echo $Row_Bidder[$i]['Associated_Bank']; ?></td>
		   
		  		  <td class="bodyarial11"><?php echo $Row_Bidder[$i]['Join_Date']; ?></td>
	...
	//echo"hello".$qry23."<br>";
		 ...
		 list($numberofrecords,$getrow12)=MainselectfuncNew($qry23,$array = array());
		$cntr12=0;
	...
	if($Validate<1)
	{
		...
		$finalAllocated = $numberofrecords;
	}
	else
	{
		echo $StoredLeadCount;
		$finalAllocated = $StoredLeadCount;
	}
	
//	$getLeadsCountSql = "select sum(BookLeadCount) as county from Bidders_Book_Keeping where BidderID='".$BidderID."' and  BookProduct='".$Reply_Type."'";
	//$getLeadsCountQuery = ExecQuery($getLeadsCountSql);
	//echo $county = mysql_result($getLeadsCountQuery,0,'county');
	
	
	$totalLeadsAllocated[] = $finalAllocated;
		
		?></td><td class="bodyarial11" ><?php
	$mul = $Multiplier;
...
//	Upendra 
	$TotAmt = $finalAllocated * $Multiplier;
	echo $TotAmt;
	$totAmt_arr[] = $TotAmt;
	/*
	...
	*/
	?></td>
		...
		?>
		  <tr>
		<td width="55" class="head1">&nbsp;</td>  
	<!--	<td width="85" class="head1">&nbsp;</td> -->
          <td width="51" class="head1">&nbsp;</td>
		  <td width="52" class="head1">&nbsp;</td>
		  <td width="67" class="head1">&nbsp;</td>
		  <td width="67" class="head1">&nbsp;</td>
		  <td width="82" class="head1">&nbsp;</td>
          <td width="98" class="head1">&nbsp;</td>
          <td width="102" class="head1">
		  <?php 
		  echo array_sum($totalLeadsCount);
		  
		  ?>		  </td>
          <td width="92" class="head1"><?php echo array_sum($totalLeadsAllocated); ?></td>
          <td width="66" class="head1">&nbsp;</td>
		   <td width="66" class="head1"><?php echo array_sum($totAmt_arr); ?></td>
          </tr>
      </table>
