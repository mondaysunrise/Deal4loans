...
$BidID = $_REQUEST['BidderID'];
	$getDocSql = "select * from citi_appointments where RequestID = '".$RequestID."'";
	...
	$getCustomerSql = "select * FROM Req_Loan_Personal where  RequestID = '".$RequestID."' ";
	 ...
	 list($recordcount,$Arrrow)=MainselectfuncNew($getCustomerSql,$array = array());
		$k=0;
	
	//$getCustomerQuery = ExecQuery($getCustomerSql);
	$Name =$Arrrow[$k]['Name'];
	$Mobile_Number =$Arrrow[$k]['Mobile_Number'];
	$City =$Arrrow[$k]['City'];
	if($City=="Others")
	{
		$City =$Arrrow[$k]['City_Other'];
	}
	...
	$BidderName = $Myrow[$j]['Bidder_Name'];
	$Bid_Email = $Myrow[$j]['BidderEmailID'];
	
	
	$getSmsSql = "SELECT * FROM `Req_Compaign` where BidderID = '".$BidID."'";	
	//$getSmsQuery = ExecQuery($getSmsSql);
	
	 list($recordcount,$getMyrow)=MainselectfuncNew($getSmsSql,$array = array());
		$m=0;
	
	
	$BidMobile_no = $getMyrow[$m]['Mobile_no'];
	$Sms_Flag = $getMyrow[$m]['Sms_Flag'];
	if($Sms_Flag==1)
	{
		$smsValid = "Send SMS";
	}
...
<form action="submit_editcitiAppointment.php" method="post" name="appt">
