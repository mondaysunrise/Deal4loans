...
if(!empty($RequestIDStr)){
	if($tableIdentifier == 'product'){
		$sql = "SELECT scc.RequestID, rcc.Name, rcc.Mobile_Number, rcc.IP_Address, scc.first_dated As punch_date FROM `Req_Credit_Card` AS rcc JOIN `sbi_credit_card_5633` AS scc ON (rcc.RequestID = scc.RequestID) WHERE rcc.`RequestID` IN ('".$RequestIDStr."') AND productflag != 10 AND table_name != 'Req_Credit_Card_Sms' GROUP BY scc.RequestID";
	}
	else{
		$sql = "SELECT scc.RequestID, scd.sbicc_name as Name, scd.sbicc_mobile As Mobile_Number, scd.IP_Address as IP_Address, scc.first_dated As punch_date FROM `sbi_ccoffers_directonsite` AS scd JOIN `sbi_credit_card_5633` AS scc ON (scd.sbiccoffersid = scc.RequestID) WHERE scd.`sbiccoffersid` IN ('".$RequestIDStr."') AND productflag = 10 GROUP BY scc.RequestID";
	}
}
//echo $sql.'<br>';
...
<tr>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#45B2D8">
		<tr>
			<td>
				<form name="frmsearch" action="getUserFullDetailsSBI_NoResponse.php" method="post">
