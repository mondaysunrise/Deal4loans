...
{
	var gifName = form;
	document.frmsearch.action="personalloan_sourcewiseindex.php?search=y"+gifName;
...
<div align="center">
 ...
 <form name="frmsearch" action="personalloan_sourcewiseindex.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//	echo "<br>".$getendtime=date("H:i:s");
 ...
 <?
		$qry="SELECT COUNT(DISTINCT source) AS countrecord FROM Req_Loan_Personal where (1=1 ".$search_date.") ";

	$result=ExecQuery($qry);
	$row1=mysql_fetch_array($result);
		$recordcount = $row1["countrecord"];
 ?>
   <tr>
   ...
   <tr>
	...
	<?
		...
		$qry="select count(RequestID) AS count,sum(Bidder_Count) AS totalbidders,source from Req_Loan_Personal where (1=1 ".$search_date.") group by source order by count(RequestID) DESC LIMIT $startrow, $pagesize";	
		$result=ExecQuery($qry);
