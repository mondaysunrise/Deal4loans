...
return false;
 ...
 <div id="dvColumn3"> 
  <!--<form name="loan_form" method="post" action="<? echo $_SERVER['PHP_SELF'] ?>" onSubmit="return chkform();">--> 
 	<form name="loan_form" method="post" action="/Right_test.php" onSubmit="return chkform();">
...
<!--
	...
	<? if((strlen(strpos($_SERVER['REQUEST_URI'], "rate-your-banks")) > 0))
     ...
     <?  if((strlen(strpos($_SERVER['REQUEST_URI'], "viewnewsletter")) > 0) || (strlen(strpos($_SERVER['REQUEST_URI'], "newsletter-archives-main")) > 0))
	 {
		 ...
		 {
			$News_Content = mysql_result($newsresult,$h,'News_Content');
			$News_Subject = mysql_result($newsresult,$h,'News_Subject');
			$News_Date = mysql_result($newsresult,$h,'News_Month');
