...
//Get Manager Id's
$qryCheck = "SELECT * FROM Bidders where (leadidentifier in ('hlallocatelms') AND Selection_Category = 2)";
...
if($_POST['method'] == 'FollowUpCount'){
	...
	$bidder = $_POST['bidder'];
	$getFollowUpCountSql = "SELECT COUNT(*) as FollowUpCount FROM `Req_Feedback_LAP` WHERE `Reply_Type` = 5 AND BidderID = '".$bidder."' AND DATE(`Followup_Date`) = '".$selectedFollowUpDate."'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	$resultfb = ExecQuery("select feedbkid from feedback_bookkeeping where (AllRequestID=".$post." and BidderID=".$Bidder_Id." AND Reply_Type=5 and leadidentifier='CallerLAP' and Feedback='".$plfeedback."' and Comments='".$pladd_comment."' and Followup_Date='".$FollowupDate."')");	
	$fbnum_rows = mysql_num_rows($resultfb);
	if($fbnum_rows > 0)
	{
		//$fbrow = mysql_fetch_array($resultfb);
		//$strfbSQL="Update feedback_bookkeeping Set Feedback='".$plfeedback."' ,Comments='".$pladd_comment."', Followup_Date='".$FollowupDate."', Dated=Now()";
	...
	else
	{
		...
		$strfbSQL=$strfbSQL.$post.",".$Bidder_Id.",5,'".$plfeedback."','".$FollowupDate."','".$pladd_comment."','CallerLAP', Now())";
	}
	//echo $strfbSQL;
	...
	if($managerid>0)
	{
		...
		$Msg="";
		$leadid = "";
		$checkmanagerqry="Select leadid, BidderID from lead_allocate WHERE AllRequestID='".$post."' AND BidderID in (".$hlmgridstr.") AND Reply_Type = 5";
		//echo $fbqry;
		$checkmanagerresult = ExecQuery($checkmanagerqry);	
			
		$checkmanagerrows = mysql_num_rows($checkmanagerresult);
		if($checkmanagerrows > 0)
		{
			$row = mysql_fetch_array($checkmanagerresult);
			if($managerid != $row["BidderID"])
			{
				$strSQL="UPDATE lead_allocate SET BidderID='".$managerid."', Allocation_Date=NOW(), Add_Date=NOW()";
		...
		else
		{
			...
			$strSQL="INSERT INTO lead_allocate(AllRequestID, BidderID, Reply_Type, Allocation_Date, Add_Date) Values (";
			$strSQL=$strSQL.$post.",'".$managerid."',5, Now(), Now())";
			$resultfb = ExecQuery($strSQL);
			$leadid = mysql_insert_id();
			$lyfcycle="insert";
		}

		//Insert entry in feedback_bookkeeping table
		if ($leadid > 0)
		{
			if($lyfcycle=="insert")
			{
			...
			// lead lifecycle
			$fbqry="select feedbkid,BidderID from feedback_bookkeeping where (AllRequestID=$post and BidderID=$bidid and Feedback='".$lyfcycleclause."' and Reply_Type='5' and Comments='".$managerid."' AND leadidentifier = 'CallerLAP')";
			$resultfb = ExecQuery($fbqry);

			$num_rowsfb = mysql_fetch_array($resultfb);
			if($num_rowsfb>0) {
				$strfbSQL="Update feedback_bookkeeping Set Dated=Now()";
				$strfbSQL=$strfbSQL." Where feedbkid=".$num_rowsfb["feedbkid"];
				echo $strfbSQL;
				$resultfb = ExecQuery($strfbSQL);
			}
			else
			{
				$strfbSQL="INSERT INTO feedback_bookkeeping(AllRequestID, BidderID, Reply_Type, Feedback, Comments, Dated, leadidentifier) Values (";
				$strfbSQL=$strfbSQL.$post.",'".$bidid."', 5, '".$lyfcycleclause."','".$managerid."', Now(), 'CallerLAP')";
				$resultfb = ExecQuery($strfbSQL);
			}
		}
		else
...
$Annual_Turnover = mysql_result($viewleadsbled,0,'Annual_Turnover');

//Check if allocated to any manager
$managerid = 'ef';
$checkmanageridqry="SELECT BidderID FROM lead_allocate WHERE AllRequestID='".$post."' AND BidderID in (".$hlmgridstr.") AND Reply_Type = 5";
$checkmanageridresult = ExecQuery($checkmanageridqry);
$row = mysql_fetch_array($checkmanageridresult);
$managerid = $row["BidderID"];
//echo $managerid;exit;
...
font-weight:bold;color:666699;font-Family:Verdana;text-transform: capitalize;'></span></td>
    ...
    <tr>
		<td valign="top" bgcolor="#F0F0F0" ><strong>Select Manager</strong></td>
		<td valign="top" bgcolor="#F0F0F0" >
			<select name="managerid" style="height:28px; width:250px;">
				<option value="">Select</option>
				<?php
				foreach($hlmgrid as $key=>$val){
				?>
					<option value="<?php echo $val; ?>" <?php if($managerid==$val) { echo "selected"; } ?>><?php echo $hlmgrname[$key]?></option>
				<?php
				}
				?>
			</select>
...
<tr>  <td bgcolor="#003399" colspan="2" align="left"><strong style="color:#FFF;"><?php if($j==0) { echo "Appointment - "; } else { echo "Re-scheduled on ".$rowApptDetails->updated_date ; } ?>
    ...
    <tr>
		<td colspan="4" align="center" bgcolor="#F0F0F0" class="heading"><b>Lead Lifecycle</b></td>
	</tr>
	<tr>
		<td height="30" colspan="4">
			<?php $sqllifecycle="Select BidderID,Dated,Feedback,Comments,Followup_Date, smstext, emailtext From feedback_bookkeeping Where (AllRequestID='".$post."' and Reply_Type=5 and leadidentifier='CallerLAP') ORDER BY Dated ASC";
			$sqllifecycleresult = ExecQuery($sqllifecycle); 
			$lcrecordcount = mysql_num_rows($sqllifecycleresult);
			if($lcrecordcount>0)
			{
				echo "<table align='center' border=1><tr><td>Agentid</td><td>Feedback</td><td>Followup Date</td><td>Comments</td><td>Date</td></tr>";
				while($lcrow=mysql_fetch_array($sqllifecycleresult)){
					...
					$sqllifecyclebid=ExecQuery("Select Bidder_Name From Bidders Where (BidderID='".$lcrow["BidderID"]."' and leadidentifier='CallerLAP')");
					$bidrow=mysql_fetch_array($sqllifecyclebid);
					$Bidder_Name = isset($bidrow["Bidder_Name"]) ? $bidrow["Bidder_Name"] : $lcrow["BidderID"];
					echo "<tr><td>".$Bidder_Name."</td><td>".$lcrow["Feedback"]."</td><td>".$lcrow["Followup_Date"]."</td><td>".$lcrow["Comments"]."</td><td>".$lcrow["Dated"]."</td></tr>";
				}
				echo "</table>";
			}
			?>
