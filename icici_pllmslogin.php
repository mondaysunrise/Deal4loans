	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		if($num_rows>0)
			...
			setSessionBidder($Email, $row);
					...
					if(($row["Global_Access_ID"]==4886))
						{
							header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."icicipllms_view.php");
							exit;
						}
					elseif($row["BidderID"]==4886)
						{
							header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."icicipllms_verifyview.php");
							exit;
						}
					elseif($row["BidderID"]==4884)
						{
							header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."icicipllms_adminview.php");
							exit;
						}
					elseif($row["BidderID"]==5134)
						{
							header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."icici_customer_details.php");
							exit;
						}
					else
						{
