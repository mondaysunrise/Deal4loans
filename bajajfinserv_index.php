...
require 'scripts/db_init.php';
	...
	//print_r($_SESSION);
	//$city_str = $_SESSION['city'];
	//$sqlCity = "select ";
	//2490,2496,2497,2498,2499,2500,2813

//	$city = str_replace(",","','", $city_str);
 // echo "bye".$val;
//echo $_REQUEST['cityS'];
...
{
	var gifName = form;
	document.frmsearch.action="bajajfinserv_index.php?search=y"+gifName;
...
<tr>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#45B2D8">
		<tr><td style="padding-top:5px; font-size:13px;">&nbsp;&nbsp;</td>
	  </tr>
      <tr>
...
<tr><td>&nbsp;</td></tr>
 ...
 <form name="frmsearch" action="bajajfinserv_index.php?search=y" method="post" onSubmit="return chkform();">
 ...
 <? 
		$search_qry="SELECT * FROM Req_Bajaj_HomenBT where (Allocated=1 and (Dated  Between '".($min_date)."' and '".($max_date)."'))";

		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by Req_Bajaj_HomenBT.Mobile_Number";
		$search_qry=$search_qry." order by Req_Bajaj_HomenBT.Dated DESC";

		$qry="SELECT RequestID FROM Req_Bajaj_HomenBT where  (Allocated=1 and (Dated  Between '".($min_date)."' and '".($max_date)."'))";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by Req_Bajaj_HomenBT.Mobile_Number";
...
//	echo"hello -  ".$qry."<br>";
 ...
 <td width="71" align="center" bgcolor="#FFFFFF" class="style2">ROI</td>
	 <td width="180" align="center" bgcolor="#FFFFFF" class="style2">Existing Bank</td>
   </tr>
	<?
		...
		$qry="SELECT * FROM Req_Bajaj_HomenBT where (  Allocated=1 and (Dated  Between '".($min_date)."' and '".($max_date)."'))";
				
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by Req_Bajaj_HomenBT.Mobile_Number";
		$qry=$qry." order by Req_Bajaj_HomenBT.Dated DESC";
...
//echo $qry;
   ...
   <tr>
	...
	<?php
 	echo '<b style="font:Verdana, Arial, Helvetica, sans-serif; color:#FF0000; font-size:9px;"> [Exclusive Lead] </b>';
 ?>
<?php if(strlen($row["Existing_ROI"])>0) { echo ' <b style="font:Verdana, Arial, Helvetica, sans-serif; color:#FF0000; font-size:9px;">BT </b>'; } ?>
<br>
	 <? echo $row["Name"]; ?></td>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["Net_Salary"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><?  if($row["City"]=="Others") echo $row["City_Other"]; else echo $row["City"] ?></td>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3"><? if($row["Employment_Status"]==0) { echo "Self Employed"; } else { echo "Salaried"; }?></td> 
   	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["Existing_ROI"]; ?>		</td>
	    <td align="center" bgcolor="#DFF6FF" class="bodyarial11" ><? echo $row["Existing_Bank"]; ?></td>
   </tr>
	<?
 ...
 <br>
<form name="frmdownload" action="downloadbajaj.php" method="post">
