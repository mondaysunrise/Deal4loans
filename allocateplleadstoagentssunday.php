...
function main()
{
	...
	if($day=="Sun")
	{
		allocateplleadstoagentssunday();
	}
		
}

function allocateplleadstoagentssunday()
{
	$sourceDefined = 'plmainlmsS';
	$globalBidderID = 6272;
	...
	$getExcludeBiddersSql = "select BidderID from Bidders where leadidentifier in ('plmainlms79','plmainlms')";
	...
	//	$plSql="select RequestID, Updated_Date, Net_Salary, Feedback, Followup_Date from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID Where (".$excludeSourceC." and Req_Loan_Personal.Employment_Status=1 and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Direct_Allocation=0 and RequestID>'".$total_lead_count."' and Updated_Date  between '".$min_date."' and '".$max_date."') and Req_Feedback_PL.Feedback in ('FollowUp','No Feedback', 'Ringing', 'Callback Later') order by RequestID asc";
...
$sundayDate = date("Y-m-d", $sunday_Date);
			$mindate =  $sundayDate." 00:00:00";
			$maxdate = $sundayDate." 23:59:59";
			$plSql="select RequestID, Updated_Date, Net_Salary, Feedback, Followup_Date from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID in (".$BidderIDExcludeStr.") Where ((".$excludeSourceC." and Req_Loan_Personal.Employment_Status=1 and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Direct_Allocation=0 and RequestID>'".$total_lead_count."' and Net_Salary>=60000 and Updated_Date  between '".$mindate."' and '".$maxdate."') and (Req_Feedback_PL.Feedback in ('FollowUp','No Feedback', 'Ringing', 'Callback Later', '') or Req_Feedback_PL.Feedback IS NULL)) order by RequestID asc";*/
		echo $plSql;
		echo "<br>";
