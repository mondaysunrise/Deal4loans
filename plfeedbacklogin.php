<?php
	...
	function formHandler(){
		...
		$result = ("select * from Bidders where Email='$Email' and PWD='$PWD' and BidderID in (2632,2633,2634,2635)");
		 list($num_rows,$row)=Mainselectfunc($result,$array = array());
		
		//$_SESSION['ReplyType']=" ";

		$IP = getenv("REMOTE_ADDR");
		if($IP=="122.176.100.28" || $IP=="122.176.100.27" || $IP=="122.176.100.29" || $IP=="122.176.122.134" || $IP=="123.238.18.54" || $IP=="180.215.190.235")
		{
			if($num_rows > 0){
				/* Get Resultset */
				...
				$strDir = dir_name();
				header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."plfeedback_index.php");
				exit;
			}
			else
			{
