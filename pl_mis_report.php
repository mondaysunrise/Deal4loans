...
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" style="border:#0033CC 1px solid;">
 ...
 <form name="frmsearch" action="pl_mis_report.php?search=y" method="post" >
...
$BidderID = array(9,10,846,847,854);
...
for($j=0;$j<count($BidderID);$j++)
{
	...
	$totalLeadsSql = "SELECT RequestID,BidderID from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='".$BidderID[$j]."'  WHERE (((MOD(Req_Loan_Personal.RequestID,5)='".$seq[$j]."') or (MOD(Req_Loan_Personal.RequestID,5)='".$seq[$j]."' and (DATE_SUB( NOW() , INTERVAL '00:07' HOUR_MINUTE ) >= Req_Loan_Personal.Updated_Date) and Req_Loan_Personal.Is_Valid=0 and Req_Loan_Personal.Direct_Allocation=1)) and ((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) or ((Req_Feedback_PL.Followup_Date Between '".$min_date."' and '".$max_date."' ) and Req_Feedback_PL.BidderID='".$BidderID[$j]."')) and (Req_Loan_Personal.source!='hdfc_plmlr' and Req_Loan_Personal.source!='PlUploadibibo' and Req_Loan_Personal.source!='postpaid-mailer' and Req_Loan_Personal.ex_source!='below60k' and Req_Loan_Personal.source!='HDFC Bank PL NEW' and Req_Loan_Personal.source!='HDFC Bank PL' and Req_Loan_Personal.source!='bajajfinserv_testing'))";
