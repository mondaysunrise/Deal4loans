...
$getBidderIDQry = "SELECT BidderID FROM Bidders WHERE BidderID = '".$BidID."' AND leadidentifier = 'CallerVerifierYesBank'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if($yesapirun==1)
	{
		...
		$getYesBankBidderSql = "SELECT BidderID, City, Query FROM Bidders_List WHERE Bidder_Name LIKE '%Yes Bank%' AND Reply_Type=4 AND City Like '%".$City."%'";
		list($alreadyExistRow,$getYesBankBidder)=Mainselectfunc($getYesBankBidderSql,$array = array());
		if($alreadyExistRow>0)
		{
			$CityYes = $getYesBankBidder['City'];
			$CityListYes = str_replace(",", "', '", $CityYes);
			$sqlYes =  $getYesBankBidder['Query']." AND Req_Credit_Card.RequestID='".$RequestID."' AND Req_Credit_Card.City IN ('".$CityListYes."')";
			list($numRowsYes,$getYesRow)=Mainselectfunc($sqlYes,$array = array());
			if($numRowsYes>0)
			{
				$insertFeedbackArr = array("AllRequestID"=>$RequestID, "BidderID"=>$getYesBankBidder['BidderID'], "Reply_Type"=>4, "Allocation_Date"=>$Dated);
...
//Sub Verifier Comments
$subverifierCommentsSql = "SELECT * FROM `Req_Feedback_CC` WHERE AllRequestID = '".$requestid."' AND BidderID In (SELECT BidderID FROM `Bidders` WHERE `leadidentifier` LIKE '%CallerDigitechVerifierYesBank%')";
$subverifierCommentsResult = d4l_ExecQuery($subverifierCommentsSql);
$subverifierCommentsResponse = d4l_mysql_fetch_assoc($subverifierCommentsResult);
$subverifier_feedback = $subverifierCommentsResponse["Feedback"];
$subverifier_comment = $subverifierCommentsResponse["comment_section"];
$subverifier_followup_date = ($subverifierCommentsResponse["Followup_Date"] != '0000-00-00 00:00:00') ? $subverifierCommentsResponse["Followup_Date"]: '0000-00-00 00:00:00';
//echo '<pre>';print_r($callerCommentsResponse);exit;
...
<tr>
	...
	<td style="width: 294px"><input type="text"  name="AccountNumber" id="AccountNumber" maxlength="20" value="<? echo $AccountNumber; ?>"/></td>
...
<tr>
	<td colspan="4" align="center" style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold; height:25px;">Sub Verifier Feedback details</td></tr>
<tr>
	<td style="width: 261px"><span class="style2">LMS Comments: </span></td>
	<td style="width: 294px"><span class="style21"><textarea rows="2" cols="15" name="subverifier_comment" readonly ><? echo $subverifier_comment; ?></textarea></span></td>   
	<td style="width: 387px"><span class="style2">LMS feedback </span></td>
	<td><span class="style21"><input type="text" name="subverifier_feedback" id="subverifier_feedback" value="<? echo $subverifier_feedback; ?>" readonly /></span>
	</td>
</tr>	
<tr>
	<td class="fontstyle" style="width: 261px"><b>Follow Up Date</b></td>
	<td class="fontstyle" style="width: 294px">
		<input type="Text" name="subverifier_followup_date" id="subverifier_followup_date" maxlength="25" size="15" value="<?php echo $subverifier_followup_date; ?>" readonly />
	</td>
	<td style="width: 387px"><span class="style2"></span></td>
	<td width="392"><span class="style21"></span></td>
</tr>
...
<tr>
	<td colspan="4">
	<?php
	$checkBidderQry = "SELECT * FROM `Req_Feedback_Bidder_CC` WHERE BidderID IN (SELECT BidderID FROM Bidders_List WHERE Bidder_Name LIKE '%Yes Bank%' AND Reply_Type=4) AND AllRequestID = '".$requestid."'";
	$checkBidderResult = d4l_ExecQuery($checkBidderQry);
	$checkBidderNumRows = d4l_mysql_num_rows($checkBidderResult);
