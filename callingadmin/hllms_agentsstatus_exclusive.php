...
<div id="payment_container-dash_page">
  ...
  <div class="content_right">
    <h1>List of Agents active @ HL LMS(Exclusive)</h1>
    <br />
    <?php 
	if($_REQUEST['msg']!="")
	{?>
	...
	<thead>
	<form id="form1" method="post" action="hllms_agentsstatus_exclusive_continue.php">
	<tr>
		<td  align="left" valign="middle" colspan="4">Agents 1 : </td>
	</tr>
	<tr>
		<td align="left">
		...
		<?php
	    $w = 1;
        $Query = "SELECT `BidderID`, `Email`, `PWD`, `Bidder_Name`, `Associated_Bank`, `Profile`, `leadidentifier`, `Status` FROM Bidders WHERE (leadidentifier='hluncalledlms' and Selection_Category=0)";
		...
		<tr bgcolor="<?php echo $colorvar;?>">
			<td><?php echo $w;?></td>
			<td>
				<input type="checkbox" value="<?php echo $rowsCon->BidderID; ?>" name="toActivate[]" <?php if($status==1) {echo "checked";} ?> />
			</td>
		...
		<tr>
			<td colspan="4" align="right">       
				<input type="submit" name="button" id="button"  class="button small2" value="Submit" />
			</td>
		</tr>
        </table>
        </td>
