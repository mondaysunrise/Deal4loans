...
<div id="payment_container-dash_page">
  ...
  <div class="content_right">
    <h1>List of Bidders in Evening LMS</h1>
    <br />
    <?php 
	if($_REQUEST['msg']!="")
			{
    ...
    <table width="633" cellspacing="0" cellpadding="0" border="0" style="border:solid 1px #ccc; margin-bottom:10px;" class="table-prolist">
      ...
      <form id="form1" method="post" action="edit_main_lms__number_continue.php">
       ...
       <?php
	    $w = 1;
        $Query = "SELECT `BidderID`, `Email`, `PWD`, `Bidder_Name`, `Associated_Bank`, `Profile`, `leadidentifier`, `CallStatus` FROM Bidders WHERE leadidentifier='plmainlms79'";
        ...
        <tr bgcolor="<?php echo $colorvar;?>">
          <td><?php echo $w;?></td>
          <td>   
<input type="hidden" name="leadidentifier" id="leadidentifier" value="plmainlms79" />           
       <input type="checkbox" value="<?php echo $rowsCon->BidderID; ?>" name="toActivate[]" <?php if($status==1) {echo "checked";} ?> /></td>
          ...
          <td><?php echo "********"; //echo $rowsCon->PWD; ?></td>
