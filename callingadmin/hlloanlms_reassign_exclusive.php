...
<h1>HL LMS Re- Assign(Exclusive Leads)</h1>
<?php
define("TABLE_REQ_LOAN_HOME", "Req_Loan_Home");
$time = date("G");
$hlallocatelms = 'hluncalledlms';
$qryCheck = "SELECT * FROM Bidders where (leadidentifier in ('hluncalledlms'))";
...
<script> 
	...
	$(document).ready(function() {
		$('#save_value').click(function(){
			...
			var strURL = "hlloanlms_reassign_exclusive.php?search=y&selectedRequestID="+selectedRequestID+"&type=2&page="+<?php echo $page; ?>+"&min_date="+min_date+"&max_date="+max_date+"&allocatebidder="+allocatebidder+"&BidderIDstatic=6717&cmbfeedback="+varCmbFeedback+"&reassignbiddertoselected="+reassignbiddertoselected;
			//console.log(strURL);
      ...
      <tr>
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="hlloanlms_reassign_exclusive.php" method="get" onSubmit="return chkform();">
			...
			if($reassignbidder>0)
			{
				...
				if ($result == 1)
				{
					...
					{
					$strfbSQL="Insert into feedback_bookkeeping(AllRequestID, BidderID, Reply_Type , Feedback, Comments, leadidentifier, Dated) Values (";
					$strfbSQL=$strfbSQL.$RequestID.",'6717', 2, 'Re-assign','".$reassignbidder."','hluncalledlms', Now())";
				...
				<tr bgcolor="<?php echo $colorvar;?>">	
					...
					<?php if($reallocation==1) { echo "Re Assigned"; } ?>
						<select name="reassignbidder" onChange="MM_jumpMenu('parent',this,0)">
							...
							$strURL="hlloanlms_reassign_exclusive.php?search=y&RequestID=".$AllRequestID."&type=2&page=".$page."&min_date=".urlencode($varmin_date)."&max_date=".urlencode($varmax_date)."&allocatebidder=".$allocatebidder."&BidderIDstatic=6717&cmbfeedback=".$varCmbFeedback;
