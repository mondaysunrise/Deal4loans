    ...
    <table width="98%" border="0">
		...
		<tr>
            ...
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
				<form name="frmsearch" action="hlloancallinglms_index.php" method="get" onSubmit="return chkform();">
			...
			if($BidderIDstatic!="")
			{
				...
				{
					$dateclause="((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."'))";
				}
				else
				{
					$dateclause="((".$feedback_tble.".DOE Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."'))";
				}
				
				$qry="SELECT *,hlcallinglms_allocation.BidderID AS allocatedBidid FROM ".$feedback_tble.",".TABLE_REQ_LOAN_HOME." LEFT OUTER JOIN Req_Feedback_HL ON ((Req_Feedback_HL.AllRequestID=".TABLE_REQ_LOAN_HOME.".RequestID) AND (Req_Feedback_HL.BidderID in (".$hlbidderidstr.")) AND (Req_Feedback_HL.last_update_dated = (SELECT last_update_dated FROM Req_Feedback_HL WHERE AllRequestID = ".TABLE_REQ_LOAN_HOME.".RequestID AND (Req_Feedback_HL.BidderID in (".$hlbidderidstr.")) ORDER BY last_update_dated DESC LIMIT 1))) WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LOAN_HOME.".RequestID and ".$feedback_tble.".BidderID in (".$hlbidderidstr.") and ".$dateclause;
