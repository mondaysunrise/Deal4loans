...
$time = date("G");
$hlallocatelms = 'hlallocatelmsCitywise';
...
<script> 
	...
	$(document).ready(function() {
		$('#save_value').click(function(){
			...
			var strURL = "hlloancallinglms_reassign.php?search=y&selectedRequestID="+selectedRequestID+"&type=2&page="+<?php echo $page; ?>+"&min_date="+min_date+"&max_date="+max_date+"&allocatebidder="+allocatebidder+"&BidderIDstatic=7348&cmbfeedback="+varCmbFeedback+"&reassignbiddertoselected="+reassignbiddertoselected;
			//console.log(strURL);
      ...
      <tr>
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="hlloancallinglms_reassign.php" method="get" onSubmit="return chkform();">
            ...
            if($reassignbiddertoselected>0)
			{
				...
				foreach($selectedRequestIDArr as $key=>$value){
					...
					{
						// lead lifecycle
						$fbqry="select hlallocateid,BidderID from feedback_bookkeeping where (AllRequestID=$value and BidderID='".$BidderID."' and Feedback='Re-assign' and Comments='".$reassignbiddertoselected."' and leadidentifier='".$hlallocatelms."')";
						...
						else
						{
							...
							$strfbSQL=$strfbSQL.$value.",'".$BidderID."', 2, 'Re-assign','".$reassignbiddertoselected."','hlallocatelmsCitywise', Now())";
			...
			if($reassignbidder>0)
			{
				...
				{
					// lead lifecycle
					$fbqry="select hlallocateid,BidderID from feedback_bookkeeping where (AllRequestID=$RequestID and BidderID='".".$BidderID."."' and Feedback='Re-assign' and Comments='".$reassignbidder."' and leadidentifier='".$hlallocatelms."')";
					$resultfb = $obj->fun_db_query($fbqry);	
					...
					{
					$strfbSQL="Insert into feedback_bookkeeping(AllRequestID, BidderID, Reply_Type , Feedback, Comments, leadidentifier, Dated) Values (";
					$strfbSQL=$strfbSQL.$RequestID.",'".".$BidderID."."', 2, 'Re-assign','".$reassignbidder."','hlallocatelmsCitywise', Now())";
				...
				<tr bgcolor="<?php echo $colorvar;?>">	
					...
					<?php if($reallocation==1) { echo "Re Assigned"; } ?>
						<select name="reassignbidder" onChange="MM_jumpMenu('parent',this,0)">
							...
							$strURL="hlloancallinglms_reassign.php?search=y&RequestID=".$AllRequestID."&type=2&page=".$page."&min_date=".urlencode($varmin_date)."&max_date=".urlencode($varmax_date)."&allocatebidder=".$allocatebidder."&BidderIDstatic=".$BidderID."&cmbfeedback=".$varCmbFeedback;
