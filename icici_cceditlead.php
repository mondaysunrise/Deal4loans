...
if($appointment_date!='0000-00-00' && $appointment_date!='' && strlen($appointment_time)>0 && strlen($appointment_place)>1)
	{
		...
		$dataArray = array('appointment_date'=>$appointment_date, 'appointment_time'=>$appointment_time, 'appointment_place'=>$appointment_place, 'appointment_address'=>$appointment_address, 'documents'=>$completedocumentsstr, 'other_documents'=>$other_docs);
		$wherecondition ="(RequestID=".$post.")";
		Mainupdatefunc ('credit_card_cibil_check', $dataArray, $wherecondition);
	}
...
// echo $getcompany;
	...
	{
		$hdfc_cccategory = $grow[$growcontr]["company_category"];
	}
...
if(strlen($Final_Bid)>0)
	...
	if(strlen($Final_Bid)>0)
	{
		...
		$dataUpdate = array('Residence_Address'=>$Residence_Address, 'Credit_Limit'=>$Credit_Limit, 'Card_Vintage'=>$Card_Vintage, 'applied_card_name'=>$final_cardselect, 'Salary_Account'=>$salary_account, 'Name'=>$ccname, 'Company_Name'=>$cccompany_name, 'DOB'=>$ccdob, 'Email'=>$ccemail, 'City'=>$cccity, 'City_Other'=>$cccity_other, 'Mobile_Number'=>$ccmobile, 'Std_Code'=>$ccstd_code, 'Landline'=>$cclandline, 'Std_Code_O'=>$ccstd_code_o, 'Landline_O'=>$cclandline_o, 'Net_Salary'=>$ccnet_salary, 'Pincode'=>$ccpincode, 'Employment_Status'=>$ccemployment_status, 'CC_Holder'=>$cccc_holder, 'Add_Comment'=>$ccadd_comment, 'No_of_Banks'=>$No_of_Banks, 'Pancard'=>$ccpancard, 'Updated_Date'=>$Dated, 'Allocated'=>$Allocated, 'Pancard_No'=>$ccpancard_no, 'Bidderid_Details'=>$Final_Bid, 'Company_ICICI_Cat'=>$Company_HDFC_Cat, 'Company_HDFC_Cat'=>$Company_HDFC_Cat, 'Is_Valid'=>'1','Salary_Account'=>$Salary_Account);
		$wherecondition ="(RequestID=".$post.")";

		$upccquery ="Update Req_Credit_Card set lead_cost=1 Where (RequestID=".$post.")";
		$dataUpdateCC = array( 'lead_cost'=>'1')
		$where = "(RequestID=".$post.")";
		Mainupdatefunc ('Req_Credit_Card', $dataUpdateCC, $where);
	}
	else
	{
		...
		$dataUpdate = array('Residence_Address'=>$Residence_Address, 'Credit_Limit'=>$Credit_Limit, 'Card_Vintage'=>$Card_Vintage, 'applied_card_name'=>$final_cardselect, 'Salary_Account'=>$salary_account, 'Name'=>$ccname, 'Company_Name'=>$cccompany_name, 'DOB'=>$ccdob, 'Email'=>$ccemail, 'City'=>$cccity, 'City_Other'=>$cccity_other, 'Mobile_Number'=>$ccmobile, 'Std_Code'=>$ccstd_code, 'Landline'=>$cclandline, 'Std_Code_O'=>$ccstd_code_o, 'Landline_O'=>$cclandline_o, 'Net_Salary'=>$ccnet_salary, 'Pincode'=>$ccpincode, 'Employment_Status'=>$ccemployment_status, 'CC_Holder'=>$cccc_holder, 'Add_Comment'=>$ccadd_comment, 'No_of_Banks'=>$No_of_Banks, 'Pancard'=>$ccpancard, 'Pancard_No'=>$ccpancard_no, 'Company_ICICI_Cat'=>$Company_HDFC_Cat, 'Company_HDFC_Cat'=>$Company_HDFC_Cat, 'Salary_Account'=>$Salary_Account);
		$wherecondition ="(RequestID=".$post.")";
	}
	 ...
	 if(strlen($ccfeedback)>0)
	{
		...
		$result = "select FeedbackID,not_contactable_counter from Req_Feedback_CC where AllRequestID=".$post." and BidderID=".$Bidder_Id." AND Reply_Type=4";		
		...
		if($num_rows > 0)
		{
			$notcontactableCounter = $row[$rowcontr]["not_contactable_counter"];
			...
			$whereClause = "(FeedbackID=".$row["FeedbackID"].")";
			Mainupdatefunc ('Req_Feedback_CC', $dataRows, $whereClause);
		...
		else
		{
			...
			$insert = Maininsertfunc ('Req_Feedback_CC', $data);
...
$followup_date = $viewlead[$viewleadcontr]'Followup_Date'];
$Feedback = $viewlead[$viewleadcontr]'Feedback'];
...
list($getapptqry_count,$appt_dt)=MainselectfuncNew($getapptqry,$array = array());
...
$ApplicationID = $appt_dt[$appt_dtcontr]["ApplicationID"];
