<?php
	...
	require 'eligiblebidderfuncPLPP.php';
...
{
	var gifName = form;
	document.frmsearch.action="plpplms_view.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2012-04-10")
	{
		alert("Sorry!!!! Your minimum date is 2012-04-10.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function submitform()
{
 ...
 <form name="frmsearch" action="plpplms_view.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//		echo $strSQL;
 ...
 <? 
		...
		if($_SESSION['BidderID']==11)
		{
			$qry="SELECT RequestID from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='11'  WHERE (((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) or (Req_Feedback_PL.Followup_Date Between '".$min_date."' and '".$max_date."' )) and Req_Loan_Personal.source='postpaid-mailer' and Req_Loan_Personal.Allocated=0)";
...
if($_SESSION['BidderID']==11)
{
	 $qry="SELECT RequestID,  	  source,Referral_Flag,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID,Net_Salary,  	  updated_flag from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='11'  WHERE (((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) or (Req_Feedback_PL.Followup_Date Between '".$min_date."' and '".$max_date."' )) and Req_Loan_Personal.source='postpaid-mailer' and Req_Loan_Personal.Allocated=0)";
}
		else
		{
			echo "not authoried to view the details";
...
<!--///////////////////////-->
<tr>
<td class="bodyarial11"><a href="plppeditlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
   ...
   for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo getJumpMenu("plpplms_view.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
