...
{
	var gifName = form;
	document.frmsearch.action="icici_mlrindex.php?search=y"+gifName;
...
function chkform()
{
	...
	<? } 
		...
		if(document.frmsearch.min_date.value<"2014-04-17")
	{
		alert("Sorry!!!! Your minimum date is 2014-04-17.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#45B2D8">
		<tr><td style="padding-top:5px; font-size:13px;">&nbsp;&nbsp; <a  href="/bidders_index.php" style="color:#FFFFFF;"><b>Main</b></a>
 </td></tr>
      <tr>
...
<tr><td>&nbsp;</td></tr>
 ...
 <form name="frmsearch" action="icici_mlrindex.php?search=y" method="post" onSubmit="return chkform();">
...
<tr>    
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select iciciappid from icici_exclusive_app where iciciappid=$RequestID");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update icici_exclusive_app Set Feedback='".$Feedback."'";
			$strSQL=$strSQL."Where iciciappid=".$row["iciciappid"];
		}
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (icici_exclusive_app.Feedback IS NULL OR icici_exclusive_app.Feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND icici_exclusive_app.Feedback='".$varCmbFeedback."' ";
		}
 ...
 <? 
	$search_qry="SELECT * FROM icici_exclusive_app where (age>=25 and Is_Valid=1 and status=1 and (iciciapp_salary>=300000 or (iciciapp_salary>=300000>=240000 and iciciapp_relation in ('SAVINGS_ACCOUNT','SALARY_ACCOUNT','HOME_LOAN','PERSONAL_LOAN','CAR_LOAN','CREDIT_CARD'))) and iciciapp_occupation=1 and iciciapp_city in ('".$citystr."') and (iciciapp_dated between '".$min_date."' and '".$max_date."'))";

		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by icici_exclusive_app.iciciapp_mobile_number";
		$search_qry=$search_qry." order by icici_exclusive_app.iciciapp_dated DESC";

		$qry="SELECT iciciappid FROM icici_exclusive_app where (age>=25 and Is_Valid=1 and status=1 and (iciciapp_salary>=300000 or (iciciapp_salary>=300000>=240000 and iciciapp_relation in ('SAVINGS_ACCOUNT','SALARY_ACCOUNT','HOME_LOAN','PERSONAL_LOAN','CAR_LOAN','CREDIT_CARD'))) and iciciapp_occupation=1 and iciciapp_city in ('".$citystr."') and (iciciapp_dated between '".$min_date."' and '".$max_date."'))";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by icici_exclusive_app.iciciapp_mobile_number";
...
//echo"hello -  ".$search_qry."<br><br>";
 ...
 ?>
  <tr>
   ...
   <tr>   
	 ...
	 <td width="90" align="center" bgcolor="#FFFFFF" class="style2">Req. Loan Amount </td>
 <td width="71" align="center" bgcolor="#FFFFFF" class="style2">Feedback</td>	 
	...
	<?
		...
		$qry="SELECT * FROM icici_exclusive_app where (age>=25 and Is_Valid=1 and status=1 and (iciciapp_salary>=300000 or (iciciapp_salary>=300000>=240000 and iciciapp_relation in ('SAVINGS_ACCOUNT','SALARY_ACCOUNT','HOME_LOAN','PERSONAL_LOAN','CAR_LOAN','CREDIT_CARD'))) and iciciapp_occupation=1 and iciciapp_city in ('".$citystr."') and (iciciapp_dated between '".$min_date."' and '".$max_date."'))";				
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by icici_exclusive_app.iciciapp_mobile_number";
		$qry=$qry." order by icici_exclusive_app.iciciapp_dated DESC";
		...
		{																												
	?>
	<input type="hidden" name="requestid_<? echo $i;?>" id="requestid_<? echo $i;?>" value="<? echo $row["ingvyasyaplid"];?>">
 ...
 ?>
	 <? echo $row["iciciapp_name"]; ?></td>
      <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["iciciapp_mobile_number"]; ?></td>
	  	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["iciciapp_city"]; ?></td> 
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["iciciapp_salary"]; ?></td>	
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["iciciapp_company_name"]; ?></td>
	   <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["customer_loan_amt"]; ?></td>	
     
   	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("icici_mlrindex.php",$row["iciciappid"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>		</td>
	   </tr><?
 ...
 <br>
<form name="frmdownload" action="download_iciciexclusive.php" method="post">
