...
<title>Health Insurance MIS Report</title>
<style type="text/css">
...
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" style="border:#0033CC 1px solid;">
 ...
 <form name="frmsearch" action="health_mis_report.php?search=y" method="post" >
...
for($j=0;$j<count($BidderID);$j++)
{
	...
	$totalLeadsSql = "SELECT * from  Req_Health_Insurance LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Health_Insurance.RequestID AND Req_Feedback.BidderID= ".$BidderID[$j]." and Req_Feedback.Reply_Type=2 WHERE ((Req_Health_Insurance.Updated_Date Between '".$min_date."' and '".$max_date."' or Req_Feedback.Followup_Date Between '".$min_date."' and '".$max_date."') ".$FeedbackClause.") group by Req_Health_Insurance.Phone";
...
/*		$table .= "<tr><td colspan='2' class='data_text_c'><b>";
		$sqlPL = "select * from Req_Feedback left join Req_Health_Insurance on Req_Health_Insurance.RequestID = Req_Feedback.AllRequestID where (Req_Health_Insurance.Dated between '".$start_time."' and '".$end_time."') and  Req_Feedback_PL.BidderID='".$BidderID[$j]."'  and Req_Feedback.Reply_Type=2";
		$sqlQuery = ExecQuery_bima($sqlPL);
		$countLeads = mysql_num_rows($sqlQuery);
		//echo $sqlPL."<br>" ;
		$table .= "Total Leads - ".$countLeads;
		$table .= "</b></td></tr>";
*/		
		$sql_PL = "select Req_Feedback.Feedback as Feedback, count(*) as CountLeads from Req_Feedback left join Req_Health_Insurance on Req_Health_Insurance.RequestID = Req_Feedback.AllRequestID where (Req_Health_Insurance.Dated between '".$start_time."' and '".$end_time."') and  Req_Feedback.BidderID='".$BidderID[$j]."' and Req_Feedback.Reply_Type=2 group by Req_Feedback.Feedback";
