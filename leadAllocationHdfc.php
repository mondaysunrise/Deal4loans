...
//$LeadID = 405904;
//1889,1948,1951,1952,1953,1954,1956
$arrBidderID = array(1887,1888,1890,1891,1950,1957,1958,1959,1960,4647,4648,4649,4650,5157,5315,5317,5316,5314,5313,5408);
$today= date('Y-m-d');
...
for($arr=0;$arr<count($arrBidderID);$arr++)
{
	...
	if($req["RequestID"]>0)
	{
		$qry="SELECT * FROM Req_Feedback_Bidder_PL,Req_Loan_Personal  WHERE Req_Feedback_Bidder_PL.AllRequestID=Req_Loan_Personal.RequestID and Req_Feedback_Bidder_PL.BidderID = '".$arrBidderID[$arr]."' and Req_Feedback_Bidder_PL.Reply_Type=1 and Req_Feedback_Bidder_PL.Feedback_ID>".$req["RequestID"]."  and (Req_Feedback_Bidder_PL.Allocation_Date Between '".($min_date)."' and '".($max_date)."' ) ";
		$qry=$qry."group by Req_Loan_Personal.Mobile_Number";
	}
	else
	{
		$qry="SELECT * FROM Req_Feedback_Bidder_PL,Req_Loan_Personal  WHERE Req_Feedback_Bidder_PL.AllRequestID=Req_Loan_Personal.RequestID and Req_Feedback_Bidder_PL.BidderID = '".$arrBidderID[$arr]."' and Req_Feedback_Bidder_PL.Reply_Type=1 and (Req_Feedback_Bidder_PL.Allocation_Date Between '".($min_date)."' and '".($max_date)."' ) ";
		$qry=$qry."group by Req_Loan_Personal.Mobile_Number";
	}
		echo $qry."<br>";
...
function getEligibleLead($LeadID,$BidderID)
{
	...
	$Existing_Bank = mysql_result($query,0,'Existing_Bank');
	$Existing_Loan = mysql_result($query,0,'Existing_Loan');
	$Existing_ROI = mysql_result($query,0,'Existing_ROI');
	$PL_EMI_Amt = mysql_result($query,0,'PL_EMI_Amt');
	...
	$company_cate_type = 3;
	$company_type =  mysql_result($query,0,'Company_Type');
	...
	else
	{
		...
		$hdfcpl="INSERT INTO hdfc_pl_calc_leads (name,age,mobile_number,email_id,company_name,city,company_category,company_type,net_salary,primary_acc,no_of_loan_running,clubbed_emi,availed_loan_amt,pl_emi_amt,pl_tenure,pl_no_of_emi,Dated,AppID,DOB,Employment_Status,RequestID,Existing_Bank,Existing_Loan,Existing_ROI, mainBidderID) VALUES ('".$full_name."','".$age."','".$mobile_number."','".$email_id."','".$Company_Name."','".$city."','".$company_cate_type."','".$company_type."','".$salary."','".$primary_acc."','".$no_of_loans."','".$clubbed_emi."','".$availed_loan_amt."','".$PL_EMI_Amt."','".$hdfc_loan_tenure."','".$no_emi_paid."', NOW(),'".$app_code."', '".$DOB."','".$Employment_Status."','".$RequestID."','".$Existing_Bank."','".$Existing_Loan."','".$Existing_ROI."','".$BidderID."')";
