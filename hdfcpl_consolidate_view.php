...
{
	var gifName = form;
	document.frmsearch.action="hdfcpl_consolidate_view.php?search=y"+gifName;
...
function chkform()
{
	...
	<? } 
		...
		if(document.frmsearch.min_date.value<"2010-12-14")
	{
		alert("Sorry!!!! Your minimum date is 2010-02-14.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<b>2.</b> Tell the loan seeker how much tax savings will this home loan get him/her.
 ...
 <form name="frmsearch" action="hdfcpl_consolidate_view.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?$current_date=date('Y-m-d');?> 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2010-12-14"<? } else { ?>value="<? echo $min_date; ?>" <? }?>></td>
	   <td>
  ...
  <tr><td colspan="3" align="center"><table border="0" width="90%" cellpadding="0" cellspacing="0">
  <tr><td valign="middle" class="style1">City</td><td valign="middle" class="style1">
	  <select name="city" id="city">
	  <!--<option value="-1">Please select</option>-->
 <option value="1" <?if($city==1) { echo "selected";} ?>>All</option>
	<option value="2" <?php if($city==2) { echo "selected";} ?>>Mumbai</option>
<option value="3" <?  if($city==3) { echo "selected";} ?>>Delhi</option>
<option value="4" <? if($city==4) { echo "selected";} ?>>Hyderabad</option>
<option value="5" <? if($city==5) { echo "selected";} ?>>Bangalore </option>
<option value="6" <? if($city==6) { echo "selected";} ?>>Chennai</option>
<option value="7" <? if($city==7) { echo "selected";} ?>>Baroda</option>
<option value="8" <? if($city==8) { echo "selected";} ?>>Jaipur</option>
     </select>
</td>  <td valign="middle" class="style1">Feedback:</td><td width="50%" align="left" valign="middle" class="bidderclass">
 ...
 <? 
	if($city>0)
		...
		$search_qry="SELECT * FROM Req_Feedback_Bidder1,`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Loan_Personal.RequestID AND Req_Feedback.BidderID='".$_SESSION['BidderID']."'  WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID in ".$biddervalue." and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date  Between '".($min_date)."' and '".($max_date)."' ) ";	
		...
		$qry="SELECT RequestID,Name,City,Mobile_Number,Net_Salary,Loan_Amount,Employment_Status,Feedback FROM Req_Feedback_Bidder1,`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Loan_Personal.RequestID AND Req_Feedback.BidderID ='".$_SESSION['BidderID']."' WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID in ".$biddervalue."  and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date Between '".($min_date)."' and '".($max_date)."' ) ";
...
//echo $qry;
   ...
   <tr>
	 ...
	 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("hdfcpl_consolidate_view.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val,$_REQUEST['city']) ?>
	</td>
