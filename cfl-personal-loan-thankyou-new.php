...
if(strlen($middle_name)>0 && $middle_name!="Middle Name" && $last_name=="")
	...
	$upDataArray = array("Gender" =>$Gender, "Residence_Address" =>$Residence_Address, "State" =>$State, "Pancard" =>$panno, "Pincode" =>$pincode);
	echo $wherecondition ="(RequestID=".$requestID.")";
	//print_r($upDataArray);
	Mainupdatefunc('Req_Credit_Card', $upDataArray, $wherecondition);

 $InsertProductSql= array("RequestID" => $requestID, "OfficeAddress1" => $OfficeAddress1 , "OfficeAddress2" => $OfficeAddress2 ,"OfficeAddress3" => $OfficeAddress3 , "OfficePin" => $OfficePin, "TypeOfLandline" => $Land_linenumber, "OfficeCity" => $OfficeCity, "LandlineNo" => $Phone_Number, "OfficeState"=> $OfficeState, "Qualification"=> $Qualification, "CardName" => $card_name, "Designation" => $Designation, "Dated" =>$Dated);
		...
		if($offstd=="")
		  {
			...
			{
				$offislct="select std,state from sbi_cc_city_state_list Where (city like'%".strtoupper($OfficeCity)."%') group by city Limit 0,1";
			}
...
if($card_id==52) { $card_type="SSU2"; } elseif($card_id==53){ $card_type="VPTL"; } elseif($card_id==54){ $card_type="VISC"; } elseif($card_id==59){ $card_type="SCU2"; } elseif($card_id==60){ $card_type="AIPU"; } elseif($card_id==61){ $card_type="AISU"; } elseif($card_id==62){ $card_type="SCU2"; }
//echo $strcity;
$SourceCode = GetSourceCode($strcity);
$CardType = $card_type;
...
$OfficeAddress1 = trim($offiadd[0]);//mandatory
$OfficeAddress2 = trim($offiadd[1]);//mandatory
$OfficeAddress3 = trim($offiadd[2]);
$EmployeePFNumber="";
...
$message = $xml->message;
 ...
 $DataArray = array("request_xml" =>$xmlstr, "response_xml" =>$output, "LeadRefNumber" =>$LeadRefNumber, "ApplicationNumber" =>$ApplicationNumber, "StatusCode" =>$StatusCode, "ProcessingStatus" =>$ProcessingStatus, "CreditLimit" =>$CreditLimit, "Messages" =>$Message, "CompanyName"=> $CompanyName);
