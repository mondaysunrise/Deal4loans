...
{
	var gifName = form;
	document.frmsearch.action="icici_plaprvdview.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="icici_plaprvdview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <?
		$search_qry="SELECT * FROM `ICICI_Allocated_Leads` LEFT OUTER JOIN icicipllms_allocation ON ICICI_Allocated_Leads.icicirequestID= icicipllms_allocation.icicirequestID WHERE (ICICI_Allocated_Leads.approved=1 and icicipllms_allocation.icici_sent=1 and icici_sendnow_date between '".$min_date."' and '".$max_date."')";;
		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by  ICICI_Allocated_Leads.icicirequestID";
	//echo $search_qry."<br>";
   ...
   <tr>   
	...
	<?
		...
		else
		{
		 ...
		 $qry="SELECT * FROM `ICICI_Allocated_Leads` LEFT OUTER JOIN icicipllms_allocation ON ICICI_Allocated_Leads.icicirequestID= icicipllms_allocation.icicirequestID WHERE (ICICI_Allocated_Leads.approved=1 and icicipllms_allocation.icici_sent=1 and icici_sendnow_date between '".$min_date."' and '".$max_date."')";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by  ICICI_Allocated_Leads.icicirequestID";
...
<td class="bodyarial11"><? echo $row["City"]; ?></td>
	<td class="bodyarial11"><? echo $row["icici_sendnow_date"]; ?></td>
  </tr>
	<?						$i=$i+1;
		}		
		}
 ...
 <form name="frmdownload" action="icici_plleadsappt_dwnld.php" method="post">
   <tr>
     <td align="center">
