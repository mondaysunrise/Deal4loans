...
function allocateplleadstoagents()
{
	...
	$startprocess="Select total_lead_count From lead_allocation_table Where (Citywise='plmainlms' and BidderID=6241)";
	...
	if($total_lead_count>0)
	{
		...
		if(($day=="Mon" && $hour==6 && $minute<10))
		{
			$getExcludeBiddersSql = "select BidderID from Bidders where leadidentifier in ('plmainlmsS')";
			...
			$sunday_Date = mktime(0, 0, 0, date("m")  , date("d")-1, date("Y"));
			$sundayDate = date("Y-m-d", $sunday_Date);
			$mindate =  $sundayDate." 00:00:00";
			$maxdate = $sundayDate." 23:59:59";
//			$plSql="select RequestID, Updated_Date, Net_Salary, Feedback, Followup_Date from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID in (".$BidderIDExcludeStr.") Where ((".$excludeSourceC." and Req_Loan_Personal.Employment_Status=1 and Req_Loan_Personal.City!='Coimbatore' and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Direct_Allocation=0 and RequestID>'".$total_lead_count."' and Updated_Date  between '".$mindate."' and '".$maxdate."') and (Req_Feedback_PL.Feedback in ('FollowUp','No Feedback', 'Ringing', 'Callback Later', '') or Req_Feedback_PL.Feedback IS NULL)) order by RequestID asc";
		...
		else if(($hour==21 && $minute<4))
		{
			$getExcludeBiddersSql = "select BidderID from Bidders where leadidentifier in ('plmainlms79')";
			$getExcludeBiddersQry = ExecQuery($getExcludeBiddersSql);
			$recordCountExcludeBidders = mysql_num_rows($getExcludeBiddersQry);
			$BidderIDExcludeArr = '';
			$counterExcludeVal = 1;
			...
			$mindate =  $currentdate." 18:00:00";
			$maxdate = $currentdate." 21:00:00";
			$plSql="select RequestID, Updated_Date, Net_Salary,Feedback, Followup_Date from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID in (".$BidderIDExcludeStr.") Where ((".$excludeSourceC." and Req_Loan_Personal.Employment_Status=1 and Req_Loan_Personal.City!='Coimbatore' and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Direct_Allocation=0 and RequestID>'".$total_lead_count."' and Updated_Date  between '".$mindate."' and '".$maxdate."') and (Req_Feedback_PL.Feedback in ('FollowUp','No Feedback', 'Ringing', 'Callback Later', '') or Req_Feedback_PL.Feedback IS NULL)) order by RequestID asc";
		}
		else
		{
			$plSql="select RequestID, Updated_Date, Net_Salary from Req_Loan_Personal where (".$excludeSourceC." and Req_Loan_Personal.Employment_Status=1 and Req_Loan_Personal.City!='Coimbatore' and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Direct_Allocation=0 and RequestID>'".$total_lead_count."') order by RequestID asc";
		}
...
//	exit();
		for($i=0;$i<$recordCount;$i++)	
		{
			...
			if($AllRequestID>0)
			{
				if($Net_Salary>60000)
				{
					$sequenceid=ExecQuery("Select last_allocated_to,total_no_agents From lead_allocation_table Where (Citywise='plmainlms' and BidderID=6241)");
					...
					{				
						/*if($sequence==1){$bidderID="9";}
						elseif($sequence==2){$bidderID="10";}
						elseif($sequence==3){$bidderID="63";}
						elseif($sequence==4){$bidderID="67";}
						elseif($sequence==5){$bidderID="68";}
						elseif($sequence==6){$bidderID="846";}
						elseif($sequence==7){$bidderID="847";}
						//elseif($sequence==8){$bidderID="854";}
						...
						*/
						...
						if($AllRequestID>0 && $bidderID>1)
						{
							//insert allocation
							echo "<br>*******************************************<br>";
							$inserticiciqry="INSERT INTO plcallinglms_allocation (AllRequestID, BidderID, Allocation_Date, DOE) VALUES ('".$AllRequestID."', '".$bidderID."', '".$Dated."', '".$DOE."')";
							$inserticiciqryresult = ExecQuery($inserticiciqry);
							echo "<br>";
							$updateqry= "Update lead_allocation_table set last_allocated_to='".$sequence."' , total_lead_count='".$AllRequestID."' Where (Citywise='plmainlms' and BidderID=6241)";
				...
				else
				{
					...
					$inserticiciqryresult = ExecQuery($inserticiciqry);
					echo "<br>";
					 $updateqry= "Update lead_allocation_table set total_lead_count='".$AllRequestID."' Where (Citywise='plmainlms' and BidderID=6241)";
					$updateqryresult = ExecQuery($updateqry);
					echo "<br>";
