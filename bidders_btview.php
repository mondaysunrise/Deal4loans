...
{
	var gifName = form;
	document.frmsearch.action="bidders_btview.php?search=y"+gifName;
...
<tr><td>&nbsp;</td></tr>
 ...
 <form name="frmsearch" action="bidders_btview.php?search=y" method="post" onSubmit="return chkform();">
 ...
 <input type="hidden" value="<? echo $bidderidplbt; ?>" name="bidplbt" id="bidplbt">
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="85%">
     ...
     <td width="50%"  valign="middle" class="bidderclass" colspan="2">
	 Personal Loan Balance Transfer</td>
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		$strSQL="";
		$Msg="";
		$result = ExecQuery("select FeedbackID from Req_Feedback where AllRequestID=$RequestID and BidderID=".$bidderidplbt);		
		...
		else
		{
			...
			$strSQL=$strSQL.$RequestID.",".$bidderidplbt.",".$pro_code.",'".$Feedback."')";
		}
	//echo $strSQL;
...
$search_qry="SELECT * FROM ".$feedback_tble.",`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=".$val.".RequestID AND Req_Feedback.BidderID= '".$bidderidplbt."' WHERE ".$feedback_tble.".AllRequestID=`".$val."`.RequestID and ".$feedback_tble.".BidderID = '".$bidderidplbt."' and ".$feedback_tble.".Reply_Type=".$pro_code." and (".$feedback_tble.".Allocation_Date  Between '".($min_date)."' and '".($max_date)."' or Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";	
...
//echo $qry;
 ...
 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("bidders_btview.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val, $bidderidplbt) ?>
</td>
</td> <td align="center" bgcolor="#DFF6FF" class="bodyarial11" ><table width="100%"><tr><td><textarea  name="comment_section_<? echo $i;?>" id="comment_section_<? echo $i;?>" cols="15" rows="2"><? echo $row["comment_section"]; ?></textarea></td><td><a onClick="insertData(<? echo $i;?>);" style="cursor:pointer; color:blue;" class="style3">Save</a></td></tr>	</table></td>
