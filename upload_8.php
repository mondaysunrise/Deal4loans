...
$filename = $RequestID."_".$filename;
  ...
  if(move_uploaded_file($tmpfile, "upload/".$filename)){
	...
	if($numRows>0)
	{
		...
		$DataArray = array("Driving_License"=>$filename);
  ...
  echo returnStatus($status);
	...
	function returnStatus($status){
		return "<html><body><script type='text/javascript'>function init(){if(top.uploadComplete_8) top.uploadComplete_8('".$status."');}window.onload=init;
	</script></body></html>";
	}
