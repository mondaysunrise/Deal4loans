...
{
	$selqry="select CL_Bank,Loan_Amount,Name,DOB,Company_Name,Net_Salary,City,Mobile_Number,Email,source from Req_Loan_Car Where RequestID=".$cl_requestid;
	list($Numrows,$clrow)=MainselectfuncNew($selqry,$array = array());
	$countr = count($clrow)-1;
	$cl_banks=$clrow['CL_Bank'];
	$loan_amount = $clrow[$countr]["Loan_Amount"];
	$name = $clrow[$countr]["Name"];
	$DOB = $clrow[$countr]["DOB"];
	$company_name = $clrow[$countr]["Company_Name"];
	$salary = $clrow[$countr]["Net_Salary"]/12;
	$City = $clrow[$countr]["City"];
	$Mobile_Number = $clrow[$countr]["Mobile_Number"];
	$Email = $clrow[$countr]["Email"];
	$source = $clrow[$countr]["source"];

if(strlen($cl_banks)>1)
	{
		$newcl_banks= $cl_banks.",".$cl_bank_name;
		$wherecondition= "(Req_Loan_Car.RequestID=".$cl_requestid.")";
		$dataarray=array("CL_Bank"=>$newcl_banks);	
	}
	else
	{
	...
	$rowcount=Mainupdatefunc("Req_Loan_Car", $dataarray, $wherecondition);
	//echo $plupdate."<br>";
}
?>
...
<div style="clear:both;"></div>
<div class="common-bread-crumb" style="margin:auto; width:74%; margin-top:70px; color:#0a8bd9;"><u><a href="index.php" class="text12" style="color:#0080d6;" >Home</a></u> > <a href="#"  class="text12" style="color:#0080d6;"><u>Car Loan</u></a> <span class="text12" style="color:#4c4c4c;">> Apply for Car Loan</span></div>
<div style="clear:both; height:15px;"></div>
<div style="width:995px;  margin:auto;">
<div id="content"><form name="cibil_form" method="post" action="apply_cl_consent_thank.php"  onSubmit="return ckhcreditcard(document.cibil_form); ">
<input type="hidden" name="requestid" id="requestid" value="<? echo $cl_requestid; ?>"/> 
...
<input type="hidden" name="cl_bank_name" id="cl_bank_name" value="<? echo $cl_bank_name; ?>"/>
  ...
  <td><input type="text" name="salary" id="salary" value="<? echo round($salary);?>" class="bajaj-fin_input"/></td></tr>
    ...
    <div class="widget">
      <table width="100%" align="center" cellpadding="0" cellspacing="0">
        <tr>
          <td width="58%" height="40" class="heading_text1" style="font-size:18px;"><span class="heading_text_b">Why <? echo $cl_bank_name; ?>?</span></td>
