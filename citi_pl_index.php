...
function getReqValue1($pKey){
	$titles = array(
		...
		'8' => '2072',
		'9' => '2073',
		'10' => '1053,1054,1057,1058,1055,1056,1154,2073,2072'
...
{
	var gifName = form;
	document.frmsearch.action="citi_pl_index.php?search=y"+gifName;
...
<b>2.</b> Tell the loan seeker how much tax savings will this home loan get him/her.
 ...
 <form name="frmsearch" action="citi_pl_index.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?$current_date=date('Y-m-d');?> 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2009-03-31"<? } else { ?>value="<? echo $min_date; ?>" <? }?>></td>
	   <td>
  ...
  <tr><td valign="middle" class="style1">City</td><td>
	  ...
	  <option value="10" <?if($city==10) { echo "selected";} ?>>All</option>
		<option value="1" <?if($city==1) { echo "selected";} ?>>Delhi</option>
		<option value="2" <?if($city==2) { echo "selected";} ?>>Bangalore</option>
		<option value="3" <?if($city==3) { echo "selected";} ?>>Chennai</option>
		<option value="4" <?if($city==4) { echo "selected";} ?>>Hyderabad</option>
		<option value="5" <?if($city==5) { echo "selected";} ?>>Mumbai</option>
		<option value="6" <?if($city==6) { echo "selected";} ?>>Pune</option>
		<option value="7" <?if($city==7) { echo "selected";} ?>>Kolkata</option>
		<option value="8" <?if($city==8) { echo "selected";} ?>>Ahmedabad</option>
		<option value="9" <?if($city==9) { echo "selected";} ?>>Chandigarh</option>
      </select>
</td><td></td></tr>
...
//		echo $strSQL;
 ...
 <? 
	if($city>0)
		...
		$search_qry="SELECT Name , DOB , Email , Employment_Status , Company_Name , City , City_Other , Years_In_Company , Total_Experience , Mobile_Number , Std_Code , Landline , Std_Code_O , Landline_O , Net_Salary , Marital_Status , Residential_Status , Vehicles_Owned , Loan_Any , EMI_Paid , CC_Holder , Loan_Amount , Hdfc_Eligibility , Citibank_Eligibility , Barclays_Eligibility , PL_EMI_Amt , Pincode , Allocation_Date , Card_Vintage , Card_Limit , IP_Address ,  identification_proof,Primary_Acc FROM Req_Feedback_Bidder1,`".$val."`  WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID in ".$biddervalue." and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date  Between '".($min_date)."' and '".($max_date)."' ) ";	
		...
		$qry="SELECT RequestID,Name,City,Mobile_Number,Net_Salary,DOB,Loan_Amount,Employment_Status FROM Req_Feedback_Bidder1,`".$val."`  WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID in ".$biddervalue."  and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date Between '".($min_date)."' and '".($max_date)."' ) ";
