...
{
	var gifName = form;
	document.frmsearch.action="insurancelogin_index.php?search=y"+gifName;
...
<div align="center">
 ...
 <form name="frmsearch" action="insurancelogin_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if($search=="y")
	{
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Life_Insurance.Dated)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(Req_Life_Insurance.Dated)<='".strtotime($max_date)."'";
		}
		else if($min_date!="" and $max_date=="")
		{
			$min_date=$min_date." 00:00:00";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Life_Insurance.Dated)>='".strtotime($min_date)."'";
		}
		else if($min_date=="" and $max_date!="")
		{
			$max_date=$max_date." 23:59:59";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Life_Insurance.Dated)<='".strtotime($max_date)."'";
		}
		else
		{
			$search_date=$search_date." and Req_Life_Insurance.Dated!=''";
		}
	?>
 <table width="758" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <?
		$search_qry="select *, Req_Life_Insurance.LandLine as Land, Req_Life_Insurance.Std_Code as Code from Req_Life_Insurance, wUsers where Req_Life_Insurance.UserID=wUsers.UserID ".$search_date." and (wUsers.IsPublic=1 or Req_Life_Insurance.IsPublic=1) and wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 19 YEAR) >= wUsers.DOB";
		$qry="select *,Req_Life_Insurance.LandLine as Land, Req_Life_Insurance.Std_Code as Code from Req_Life_Insurance, wUsers where Req_Life_Insurance.UserID=wUsers.UserID ".$search_date." and (wUsers.IsPublic=1 or Req_Life_Insurance.IsPublic=1) and wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 19 YEAR) >= wUsers.DOB order by Req_Life_Insurance.Dated desc";
 ...
 ?>
   ...
   <tr>
	...
	<?
		...
		$qry="select *,Req_Life_Insurance.LandLine as Land, Req_Life_Insurance.Std_Code as Code from Req_Life_Insurance, wUsers where Req_Life_Insurance.UserID=wUsers.UserID ".$search_date." and  (wUsers.IsPublic=1 or Req_Life_Insurance.IsPublic=1)  and wUsers.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 19 YEAR) >= wUsers.DOB order by Req_Life_Insurance.Dated desc LIMIT $startrow, $pagesize";
		$result=ExecQuery($qry);
   ...
   <tr>
     ...
     <td class="bodyarial11"><? echo date('Y')-substr($row["DOB"],0,4); ?></td>
	 <td class="bodyarial11"><? echo $row["Annual_Income"]; ?></td>
 ...
 <form name="frmdownload" action="insurancelogin_download.php" method="post">
   <tr>
     <td align="center">
