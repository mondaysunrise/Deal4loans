...
{
	var gifName = form;
	document.frmsearch.action="hl_lms615fd.php?search=y"+gifName;
...
<?php include '~Top12.php';?>
 ...
 <br>
  <table width="712" border="0" align="center">
    <tr><td align="center" style="font-size:24px; font-weight:bold;">FollowUp</td></tr>
  <tr><td align="right"><a href="hl_lms615.php">Leads After 6:15 PM</a> | <a href="commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=2" target="_blank">today's Report</a></td></tr>
 <tr><td align="center" width="100%">
 <div align="center">
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <form name="frmsearch" action="hl_lms615fd.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if($search=="y")
	{
		...
		{
			$min_date=$min_date." 08:15:00";
			$max_date=$max_date." 18:14:59";
		}
 ...
 <?
		if($_SESSION['BidderID']==3635)
		{
			$qry="SELECT RequestID from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (460,732,812,207,3635) WHERE ((((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."' ) and  Req_Loan_Home.Allocated=0) or ((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and Req_Feedback_HL.BidderID in (460,732,812,207,3635)))  and Req_Feedback_HL.Feedback not in ('Other Product', 'Not Interested', 'Not Contactable', 'Wrong Number', 'Not Eligible', 'Duplicate', 'Not Applied'))";
 ...
 ?>
   ...
   <tr>
	...
	<?
		...
		if($_SESSION['BidderID']==3635)
		{	
			$qry="SELECT Referral_Flag ,RequestID,Updated_Date,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID,updated_flag from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (460,732,812,207,3635) WHERE ((((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."' ) and  Req_Loan_Home.Allocated=0) or ((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and Req_Feedback_HL.BidderID in (460,732,812,207,3635))) and Req_Feedback_HL.Feedback not in ('Other Product', 'Not Interested', 'Not Contactable', 'Wrong Number', 'Not Eligible', 'Duplicate', 'Not Applied'))";
			$qry=$qry.$FeedbackClause;
...
//echo $qry;
   ...
   <tr>
   <td class="bodyarial11"><a href="editlead615.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
  ...
  <td class="bodyarial11"><?php echo $dated;?></td>
    <td class="bodyarial11"><?
		$getFeedback = '';
	$originalFeedback = '';
	//echo "select FeedbackID, Feedback from Req_Feedback_HL where AllRequestID='".$row["RequestID"]."' and BidderID='3635' AND Reply_Type=2";
	$getFedbackQuery = ExecQuery("select FeedbackID, Feedback from Req_Feedback_HL where AllRequestID='".$row["RequestID"]."' and BidderID='3635' AND Reply_Type=2");
	$num_rows_feedback = mysql_num_rows($getFedbackQuery);
		if($num_rows_feedback > 0)
		{
			$getFeedback = mysql_result($getFedbackQuery,0,'Feedback');
			$originalFeedback = $row["Feedback"];
		}
		else
		{
			$getFeedback = $row["Feedback"];
		}
		echo $originalFeedback."<br>";
		
	 echo getJumpMenu("hl_lms615fd.php",$row["RequestID"],"2",$getFeedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
	<?php
