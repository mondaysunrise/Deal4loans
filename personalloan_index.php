...
{
	var gifName = form;
	document.frmsearch.action="personalloan_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2010-12-01")
	{
		alert("Sorry!!!! Your minimum date is 2010-12-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<div align="center">
 ...
 <form name="frmsearch" action="personalloan_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">From 
       <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2012-01-01"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
...
//	echo "<br>".$getendtime=date("H:i:s");
 ...
 <?
		$search_qry="select 	 	Mobile_Number, Email, Caller_Name,Referrer,RequestID,Employment_Status,Residential_Status,Loan_Any,CC_Holder,EMI_Paid,Card_Vintage,Salary_Drawn,Bidder_Count,Name,DOB,Company_Name,City,City_Other,Years_In_Company,Total_Experience,Net_Salary,Loan_Amount,Pincode,Updated_Date,source,Add_Comment,Annual_Turnover from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=Req_Loan_Personal.RequestID AND Req_Feedback_PL.BidderID in (846,847,854,9,10,63) where (1=1 ".$search_date.") ";
	$qry="select RequestID from Req_Loan_Personal where 1=1 ".$search_date."  order by Req_Loan_Personal.Updated_Date desc";
   ...
   <tr>
	 ...
	 <!-- <td class="head1">Activation code churn</td>-->
      ...
      <td class="head1">Feedback</td>
	   <!--<td class="head1">Shown Banks</td>-->
     <td class="head1">Clicked Banks</td>
	     <td class="head1">bidders</td>
		  <!--<td class="head1">sms status</td>
            <td class="head1">Missed Call </td>
	...
	<?
		...
		{*/
		$qry="select 	Feedback,Referrer,PL_Bank,Company_Name,source,Name,City,Net_Salary,Employment_Status,Bidder_Count,RequestID,DOB as dob,LandLine as Land,Std_Code as Code,Edelweiss_Compaign from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=Req_Loan_Personal.RequestID AND Req_Feedback_PL.BidderID in (846,847,854,9,10,63) where (1=1 ".$search_date.")  order by Req_Loan_Personal.Updated_Date desc LIMIT $startrow, $pagesize";
		//}
		//echo "<br>";
		//echo $qry;
		$result=ExecQuery($qry);
	 ...
	 <!--</td>-->
	  <td class="bodyarial11"><? echo $row["source"]; ?></td>
	   <td class="bodyarial11"><? echo $row["Feedback"]; ?></td>
	    <!--<td class="bodyarial11"><? //echo $row["checked_bidders"]; ?></td>-->
	    <td class="bodyarial11"><? echo $row["PL_Bank"]; ?></td>
