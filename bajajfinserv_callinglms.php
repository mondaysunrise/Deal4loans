...
{
	var gifName = form;
	document.frmsearch.action="bajajfinserv_callinglms.php?search=y"+gifName;
...
<tr>
 ...
 <table width="600" border="0" align="center" cellpadding="0" cellspacing="0">
   ...
   <tr>
     <td align="center" valign="middle" background="images/login-form-login-bg.gif"> <form name="frmsearch" action="bajajfinserv_callinglms.php?search=y" method="post" onSubmit="return chkform();"><table width="95%" border="0"  cellpadding="1" cellspacing="0"> 
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td align="left" valign="middle" class="style1" width="17%"><input name="b122" type="button" class="buttonfordate" onClick="javascript:pedirFecha(max_date,'');" value="&lt; Insert"></td>
       ...
       <tr>
       <td></td><td height="40"><table><tr><td width="15%" valign="middle" class="style1">Feedback:</td>
   ...
   <tr>
	...
	if($search=="y")
	{
		/*$minarry=explode("-",$min_date);
		$mintomorrow  = mktime(0, 0, 0, $minarry[1] , $minarry[2]-4, $minarry[0]);
		$min_datedate=date('Y-m-d',$mintomorrow);
		$min_date=$min_datedate." 00:00:00";
		$maxarry=explode("-",$max_date);
		$maxtomorrow  = mktime(0, 0, 0, $maxarry[1] , $maxarry[2]-4, $maxarry[0]);
		$max_datedate=date('Y-m-d',$maxtomorrow);
		$max_date=$max_datedate." 23:59:59";	*/
...
if(strlen(trim($varCmbFeedback))==0)
 ...
 <? 
		//$search_qry="SELECT * FROM bajaj_cibildetails LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=bajaj_cibildetails.bajajcibilid AND Req_Feedback.BidderID=".$_SESSION['BidderID']." WHERE	((bajaj_dated Between '".($min_date)."' and '".($max_date)."') and bajajf_city not in ('Pune','Bangalore','Mumbai','Thane','Navi Mumbai') and bajajcibilid>3920)";	
		$search_qry="SELECT * FROM bajaj_cibildetails LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=bajaj_cibildetails.bajajcibilid AND Req_Feedback.BidderID=".$_SESSION['BidderID']." WHERE (bajajf_city not in ('Pune','Bangalore','Mumbai','Thane','Navi Mumbai') and bajajcibilid>3920)";	
		$qry=$qry.$FeedbackClause;
		$search_qry=$search_qry."group by bajaj_cibildetails.bajajf_mobile";
		$search_qry=$search_qry." order by bajaj_cibildetails.bajaj_dated DESC";

		$qry="SELECT * FROM bajaj_cibildetails LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=bajaj_cibildetails.bajajcibilid AND Req_Feedback.BidderID=".$_SESSION['BidderID']." WHERE ((bajaj_dated Between '".($min_date)."' and '".($max_date)."') and bajajf_city not in ('Pune','Bangalore','Mumbai','Thane','Navi Mumbai') and bajajcibilid>3920)";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by bajaj_cibildetails.bajajf_mobile";	
...
?>
   ...
   <tr>   
	 ...
	 <td width="88" align="center" bgcolor="#FFFFFF" class="style2">Lead Source</td>
	 <td width="88" align="center" bgcolor="#FFFFFF" class="style2">Feedback</td>
	 <td width="88" align="center" bgcolor="#FFFFFF" class="style2">Followup Date</td>
	 <td width="88" align="center" bgcolor="#FFFFFF" class="style2">Remarks</td>	 
</tr>
<?
		...
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by bajaj_cibildetails.bajajf_mobile";
		$qry=$qry." order by bajaj_cibildetails.bajaj_dated DESC";
...
//echo $qry;
	...
	?>	
   <tr>
       <td align="center" bgcolor="#DFF6FF" class="style3"><a href="bajajleaddetails.php?bjcblid=<? echo $row["bajajcibilid"]; ?>&bidid=<? echo $_SESSION['BidderID']; ?>" target="_Blank"><? echo $row["bajajf_name"]; ?></a></td> 
	   		 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_city"]; ?></td> 
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_salary"]; ?></td>
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_loan_amt"]; ?></td>
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? if($row["bajajf_source"]=="bfsmailer") { echo "Mailer";} else  { echo "Green Channel";} ?></td>
		<td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("bajajfinserv_callinglms.php",$row["bajajcibilid"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
		</td>
		 <td align="center" bgcolor="#DFF6FF" class="style3"><input name="followup_date_<? echo $i;?>" type="text" id="followup_date_<? echo $i;?>" size="10" value="<? echo $row["Followup_Date"];?>">&nbsp;</td>
		 <td align="center" bgcolor="#DFF6FF" class="bodyarial11" ><table width="100%"><tr><td><textarea  name="comment_section_<? echo $i;?>" id="comment_section_<? echo $i;?>" cols="15" rows="2"><? echo $row["comment_section"]; ?></textarea></td></tr>	</table></td>
   </tr>
	<?
					$i=$i+1;
		}
   ...
   <tr>
     <td align="center">
	   ...
	   <input type="hidden" name="qry2" value="<? //echo $val; ?>">
	    <input type="hidden" name="section" value="GreenChannel">	 
...
?>
	<select name="type" id="type" onChange="MM_jumpMenu('parent',this,0)" class="style3" style="width:110px;">
		...
		<option value="<? echo $strURL.'&Feedback=OTP Done'?>" <? if($varFeedback == "OTP Done") { echo "selected"; } ?>>OTP Done</option>
