...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$Telecaller_name = $_POST["Telecaller_name"];
//print_r ($Loan_Any)."<br>";
...
if(strlen($Final_Bid)>0)
	 ...
	 if(strlen($lapfeedback)>0)
	{
		...
		if($num_rows > 0)
		{
			...
			$strSQL="Update Req_Feedback_LAP Set Feedback='".$lapfeedback."',not_contactable_counter='".$updatedcounter."',Followup_Date='".$FollowupDate."', Caller_Name='".$Telecaller_name."'";
		...
		else
		{
			...
			$strSQL=$strSQL.$post.",".$Bidder_Id.",5,'".$lapfeedback."','".$FollowupDate."','".$counter."','".$Telecaller_name."')";
...
if($Bidder_Count!=0)
{
	$strbidderid="";
	$z=0;
$retrieve_query="select BidderID from Req_Feedback_Bidder_LAP where AllRequestID=".$post." and Reply_Type=5";
