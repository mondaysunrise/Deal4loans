...
{
	var gifName = form;
	document.frmsearch.action="pllms-incomingcall-view.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2013-10-01")
	{
		alert("Sorry!!!! Your minimum date is 2013-10-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function submitform()
{
 ...
 <form name="frmsearch" action="pllms-incomingcall-view.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">From 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2012-10-01"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
...
//		echo $strSQL;
 ...
 <? 
			if($_SESSION['BidderID']==65)
		{
			$qry="SELECT RequestID,  	  source,Referral_Flag,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID,Net_Salary,updated_flag from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='65' WHERE (
((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) and Req_Loan_Personal.source='incoming_call' )  and Req_Loan_Personal.Allocated=0 )";
...
<b>	<?php
	...
	<? echo getJumpMenu("pllms-incomingcall-view.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
