...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if(($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.161.193.191" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="182.73.4.58" || $IP=="182.73.4.59" || $IP=="182.73.4.60" || $IP=="182.73.4.61" || $IP=="182.73.4.62" || $IP=="182.71.109.218"  || $IP=="180.151.74.83" || $IP=="185.93.231.12"  || $IP=="115.249.245.30" || $IP=="122.176.54.210" || $IP=="122.176.77.240" || $IP=="122.176.77.79" || $IP=="122.180.253.3" || $IP=="122.176.77.239" || $IP=="203.122.21.163" || $IP=="113.193.239.185" || $IP=="1.22.91.116" || $IP=="122.176.68.155" || ($getIPNum>0)))
	{
		...
		{
			redirectURL(SITE_URL."leadslogin.php?msg=".urlencode("Please enter correct username!"));
		}
		if(!$objAdmin->fun_check_pwd_admin_existance($adminPass))
		{
			redirectURL(SITE_URL."leadslogin.php?msg=".urlencode("Your password does not match with our record. Please enter correct password!"));
		}
...
//				$sqlTrackreult= $obj->fun_db_query($sqlTrackBidder);
                        if($adminInfo['BidderID']=='7151' || $adminInfo['BidderID']=='7158') {
                           header("Location:leadslms_back_calling_index.php"); 
                        }else{
				header("Location:leadslms_index.php");
                        }
			}
		}
		else {
			redirectURL(SITE_URL."leadslogin.php?msg=".urlencode("Incorrect User or Password!"));
			}
	}
	else{
	redirectURL(SITE_URL."leadslogin.php?msg=".urlencode("Not permission for you!"));
	}
}
?>
<html>
