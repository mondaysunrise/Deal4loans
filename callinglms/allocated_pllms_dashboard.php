    ...
    <table width="98%" border="0">
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="allocated_pllms_dashboard.php" method="get" onSubmit="return chkform();">
                ...
                <tr>
                  ...
                  <td width="29%"><select name="cmbfeedback" id="cmbfeedback">
                      ...
                      <option value="Called" <? if($varCmbFeedback == "Called") { echo "selected"; } ?>>Called</option>
	                  <option value="Not Called" <?if($varCmbFeedback == "Not Called") { echo "selected"; }?>>Not Called</option>
                      ...
                      <option value="Docs Picked" <? if($varCmbFeedback == "Docs Picked") { echo "selected"; }?>>Docs Picked</option> 
                        ...
                        <option value="Appointment Made" <?if($varCmbFeedback == "Appointment Made") { echo "selected"; }?>>Appointment Made</option>
					                 </select></td>
	  ...
	  <td width="58%"  valign="middle" class="bidderclass"><input type="text" name="mob_num" id="mob_num" value="<?php echo $mob_num; ?>" >
                ...
                <tr>
                  ...
                  <td><select name="CityName" id="CityName">
					...
					<?php echo $objAdmin->funGetCityOptions($_REQUEST['CityName']);
?>
                  </select></td>
                  <td>&nbsp;</td>
                  <td align="left">&nbsp;</td>
...
//echo $strSQL;
	...
	?>       <p class="bodyarial11">
		...
		if($BidderIDstatic!="")
		{
			$qry="SELECT * FROM ".$feedback_tble.",".TABLE_REQ_LOAN_PERSONAL." LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID AND Req_Feedback_PL.BidderID= '".$BidderIDstatic."' WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID and ".$feedback_tble.".BidderID = '".$BidderIDstatic."'";
			
		if($_REQUEST['CityName'])
		{
		
		$qry .="	AND ".TABLE_REQ_LOAN_PERSONAL.".City='".$_REQUEST['CityName']."'";
		
		}
			 $qry.=" and ".$feedback_tble.".Reply_Type=1 and ( ".$feedback_tble.".Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_PL.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
...
$qry.= " group by ".TABLE_REQ_LOAN_PERSONAL.".Mobile_Number order by Allocation_Date DESC LIMIT $start,$limit ";
...
$result = $obj->fun_db_query($qry);
 ?>
	...
	?>
                ...
                <td class="bodyarial11"><a href="/plallocatedleadlms-details.php?postid=<? echo urlencode($row->RequestID); ?>&biddt=<? echo $BidderIDstatic;?>" target="_blank"><? echo $row->Name; ?></a></td>
	   ...
	   ?>              
		<td class="bodyarial11">
                 <? echo getJumpMenu("allocated_pllms_dashboard.php",$row->RequestID,"4",$row->Feedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
     </td>
 ...
 <form name="frmdownload" action="callinglms-excel-download.php" method="post">
   <tr>
     <td align="center">
...
?>
		...
		<option value="<? echo $strURL.'&Feedback=Called'?>" <? if($varFeedback == "Called") { echo "selected"; } ?> >Called</option>
		...
		<option value="<? echo $strURL.'&Feedback=Appointment Made'?>" <? if($varFeedback == "Appointment Made") { echo "selected"; } ?> >Appointment Made</option>
	</select>	
<?
}
?>
