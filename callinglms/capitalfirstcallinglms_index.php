...
<div style="clear:both; height:15px;"></div>
	  ...
	  <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=4" target="_blank">today's Report</a></td></tr>
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="capitalfirstcallinglms_index.php" method="get" onSubmit="return chkform();">
	...
	if(strlen(trim($RequestID))>0)
	{
		$strSQL="";
		$Msg="";
		$fbqry="select FeedbackID from Req_Feedback_PL where AllRequestID=$RequestID and BidderID=".$_SESSION['BidderID']." AND Reply_Type=4";
...
?>       <p class="bodyarial11">
		...
		if($_SESSION['BidderID']!="")
		{
			...
			//if($_SESSION['BidderID']==5706) { $pllms=0;} elseif($_SESSION['BidderID']==5705){ $pllms=1;}			elseif($_SESSION['BidderID']==5749){ $pllms=2;}
				
		if($_SESSION['BidderID']==5705){ $pllms=0;}	elseif($_SESSION['BidderID']==5749){ $pllms=1;}

$qry="SELECT * FROM ".TABLE_REQ_LOAN_PERSONAL." LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID AND Req_Feedback_PL.BidderID in (5706,5705,5749) WHERE (".TABLE_REQ_LOAN_PERSONAL.".Employment_Status=1 and ".TABLE_REQ_LOAN_PERSONAL.".Net_Salary<=300000 and (".TABLE_REQ_LOAN_PERSONAL.".City in ('Ahmedabad','Bangalore','Baroda','Vadodara','Bhopal','Bhubneshwar','Chandigarh','Panchkula','Mohali','Chennai','Coimbatore','Cuttack','Delhi','Gurgaon','Faridabad','Hyderabad','Indore','Jaipur','Jalandhar','Jodhpur','Kolkata','Lucknow','Ludhiana','Pune','Surat','Udaipur','Vijaywada','Vijayawada','Vishakapatnam','Vizag','Anand','Madurai','Ambala','Rajkot','Cochin','Kochi','Ernakulam','Siliguri','Guwahati','Mysore','Raipur') or ".TABLE_REQ_LOAN_PERSONAL.".City_Other in ('Ahmedabad','Bangalore','Baroda','Vadodara','Bhopal','Bhubneshwar','Chandigarh','Panchkula','Mohali','Chennai','Coimbatore','Cuttack','Delhi','Gurgaon','Faridabad','Hyderabad','Indore','Jaipur','Jalandhar','Jodhpur','Kolkata','Lucknow','Ludhiana','Pune','Surat','Udaipur','Vijaywada','Vijayawada','Vishakapatnam','Vizag','Anand','Madurai','Ambala','Rajkot','Cochin','Kochi','Ernakulam','Siliguri','Guwahati','Mysore','Raipur')) and ( ".TABLE_REQ_LOAN_PERSONAL.".Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_PL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and (".TABLE_REQ_LOAN_PERSONAL.".DOB!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= ".TABLE_REQ_LOAN_PERSONAL.".DOB) and ".TABLE_REQ_LOAN_PERSONAL.".Card_Vintage>=1 and (MOD(Req_Loan_Personal.RequestID,".NoOFLMS.")=".$pllms."))";
...
$result = $obj->fun_db_query($qry);
 ?>
		...
		while($row = $obj->fun_db_fetch_rs_object($result))
		{
			...
			$capitalfirst_requestid = $row->RequestID;
	?>
                <tr  bgcolor="<?php echo $colorvar;?>">			 
                <td class="bodyarial11">
				<? 
	 $qrycapfirst="Select capitalfirst_requestid From apply_pl_capitalfirst where (capitalfirst_requestid=".$capitalfirst_requestid." and direct_flag=1 and capitalfirst_dated Between '".($min_date)."' and '".($max_date)."') group by capitalfirst_requestid"; $result1 = $obj->fun_db_query($qrycapfirst);
	  $myrow = $obj->fun_db_fetch_rs_object($result1);
	  
	if(($myrow->capitalfirst_requestid) >1)
			{  echo $row->Name;  } 
	else
			{ ?>
		<a href="/capitalfirstleadlms-details.php?postid=<? echo urlencode($row->RequestID); ?>&biddt=<? echo $_SESSION['BidderID'];?>" target="_blank"><? echo $row->Name; ?></a>
			<? } ?>	</td>
	   ...
	   ?>              
		<td class="bodyarial11">
          <? echo getJumpMenu("capitalfirstcallinglms_index.php",$row->RequestID,"4",$row->Feedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
     </td>
 <?php if($varCmbFeedback == "Send Now" ) { 
...
?>
	<select name="type" id="type" onChange="MM_jumpMenu('parent',this,0)">
		...
		<option value="<? echo $strURL.'&Feedback=Reject'?>" <? if($varFeedback == "Reject") { echo "selected"; } ?>>Entry Done- Reject</option>
		 <option value="<? echo $strURL.'&Feedback=Direct Sent'?>" <? if($varFeedback == "Direct Sent") { echo "selected"; } ?>>Direct Sent</option>
