<?php
require_once("includes/application-toplap.php");

$qryCheck = "SELECT BidderID FROM Bidders where ((leadidentifier in ('CallerLAP') and BidderID='".$_SESSION["BidderID"]."') OR (leadidentifier in ('hlallocatelms') AND Selection_Category = 2 AND BidderID='".$_SESSION["BidderID"]."'))";
...
<script type="text/javascript">
	...
	{	
	   $.ajax({ type: 'post',  url: '/dialerclick2calllap.php',  data: {  RequestID:RequestID,agent_user:agent_user, },
...
<script type="text/javascript">
	...
	{	
		var funcVal = 'call';
		$.ajax({ type: 'post',  url: '/external_dialer_c2c_lap.php',  data: {  RequestID:RequestID,agent_user:agent_user, functionC:funcVal },
			success: function (response) {
				//alert(response);
				$( '#name_status' ).html(response);
				if(response=="OK") { return true;	 } else { return false;	}
    ...
    {	
		...
		$.ajax({ type: 'post',  url: '/external_dialer_c2c_lap.php',  data: {  RequestID:RequestID,agent_user:agent_user, functionC:funcVal, disPos:dispID },
			success: function (response) {
				//alert(response);
				$( '#name_status' ).html(response);
				if(response=="OK") { return true;	 } else { return false;	}
...
<div style="width:100%; background: #CCC; padding:0px 0px 10px 0px;">
	<div style="background:#F00; width:40px; padding-left:7px; line-height:30px; height:30px; float:right; margin-right:50px; margin-top:7px; border-radius:20%;">
		<a href="logoutbl.php" style="color:#FFF; text-align:center; text-decoration:none; font-size:12px;">Logout</a>
	</div>
	 ...
	 <!-- <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr>-->
        ...
        <tr>
            ...
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
				<form name="frmsearch" action="lms_lap_index.php" method="get" onSubmit="return chkform();">
					...
					<tr>
						...
						<td width="58%"  valign="middle" class="bidderclass">
							<input type="text" name="refernce_no" id="refernce_no" value="<?php echo $refernce_no; ?>" >
						</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
			...
			if(strlen(trim($RequestID))>0)
			{
				$strSQL="";
				$Msg="";
				$fbqry="select FeedbackID from Req_Feedback_LAP where AllRequestID=$RequestID and BidderID=".$_SESSION['BidderID']." AND Reply_Type=1";
			...
			if($search=="y")
			{
				...
				{
					$qry="SELECT * ,".$feedback_tble.".BidderID AS sentbidder FROM ".$feedback_tble.",".TABLE_REQ_LAP." LEFT OUTER JOIN Req_Feedback_LAP ON Req_Feedback_LAP.AllRequestID=".TABLE_REQ_LAP.".RequestID AND Req_Feedback_LAP.BidderID= '".$_SESSION['BidderID']."' WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LAP.".RequestID and ".$feedback_tble.".BidderID = '".$_SESSION['BidderID']."' and ( ".$feedback_tble.".Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_LAP.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
				...
				$qry.= " group by ".$feedback_tble.".AllRequestID order by Allocation_Date DESC LIMIT $start,$limit ";
				//echo $qry;
				...
				?>
					...
					<tr bgcolor="<?php echo $colorvar;?>">	
						<td class="bodyarial11"><?php echo $uniqueid ="LAP".$row->RequestID."S".$row->sentbidder; ?></td>
						<td class="bodyarial11">
							<?php $RequestID= $row->RequestID; ?>
							<a href="/lapeditlead_agents.php?id=<? echo urlencode($row->RequestID); ?>&Bid=<? echo $_SESSION['BidderID'];?>" target="_blank"><? echo $row->Name; ?></a>
						</td>
						...
						<? if($processing_status==1 || $processing_status==7) { } else { 
								echo getJumpMenu("lms_lap_index.php",$row->RequestID,"5",$row->Feedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val); ?>
						</td>
