...
//echo $IP_Remote;
if($IP_Remote=='192.124.249.12' || $IP_Remote=='185.93.228.12' || $IP_Remote=='185.93.231.12' || $IP_Remote=='185.93.230.12') { $IP= $_SERVER['HTTP_X_SUCURI_CLIENTIP']; }
else { $IP= $IP_Remote;	}
//echo $IP_Remote;
...
{
	if(($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.161.193.191" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="122.176.54.194" || $IP=="182.71.109.218" || $IP=="122.176.54.210" || $IP=="180.151.229.113" || $IP=="117.212.67.240" || $IP=="113.193.239.185" || $IP=="122.176.54.194" || $IP=="160.202.186.177" || $IP=="122.176.54.210" || $IP=="1.22.91.57"  || $IP=="122.180.253.3" || $IP=="122.176.77.79" || $IP=="1.22.91.116" || $IP=="122.176.77.240"  || $IP=="122.176.68.155" || $IP=="122.176.77.239" || ($getIPNum>0)))
	{
		...
		{
			redirectURL(SITE_URL."pllmslogin.php?msg=".urlencode("Please enter correct username!"));
		}
		if(!$objAdmin->fun_check_pwd_admin_existance($adminPass))
		{
			redirectURL(SITE_URL."pllmslogin.php?msg=".urlencode("Your password does not match with our record. Please enter correct password!"));
		}
...
//				$sqlTrackreult= $obj->fun_db_query($sqlTrackBidder);
				...
				if($_SESSION['leadidentifier']=="plalloclms")
				{
					header("Location:plallocation_index.php");
				}
				else if($_SESSION['leadidentifier']=="pl_alloclms_admin" || $_SESSION['leadidentifier']=="pl_alloclms_subadmin")
				{
					header("Location:plallocation_consolidated.php");
				}else if($_SESSION['BidderID']=="6978")
				{
					header("Location:plappointment_consolidated.php");
				}                  
				else
				{
					header("Location:ploanlms_index.php");
				}
			}
		}
	}
