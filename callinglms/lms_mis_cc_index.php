...
//mis_cc@d4l.com
//miscc@031116
$qryCheck = "SELECT BidderID FROM Bidders where leadidentifier = 'MISCC' and BidderID='".$_SESSION["BidderID"]."'";
$resCountCheck = $objAdmin->fun_get_num_rows($qryCheck);
$qryCheckResult = $obj->fun_db_query($qryCheck);
//exit();
if($resCountCheck>0 && ($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="182.73.4.60" || $IP=="180.151.74.83" || $IP=="115.249.245.30" || $IP=="182.71.109.218"  || $IP=="185.93.231.12"))
{
	 ...
	 <!-- <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr>-->
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="lms_mis_cc_index.php" method="get" onSubmit="return chkform();">
	  ...
	  <td width="58%"  valign="middle" class="bidderclass"><input type="text" name="refernce_no" id="refernce_no" value="<?php echo $refernce_no; ?>" >
                ...
                -->
                 <tr>
                  ...
                  <?php
	                $qryCheck = "SELECT BidderID, leadidentifier FROM Bidders where leadidentifier in ('diallerleadccsmsnew', 'diallercallerccpredictive') group by leadidentifier";//'CallerAccountCity',
					$resCountCheck = $objAdmin->fun_get_num_rows($qryCheck);
					$qryCheckResult = $obj->fun_db_query($qryCheck);
...
//echo $strSQL;
	...
	{		
		...
		//echo $refernce_no;	
			...
			$refernce_no_clause = " AND Req_Credit_Card_Sms.RequestID = '".$refernce_no_section."' ";
		}
	?>       <p class="bodyarial11">
              <?=$Msg?>
            </p>
...
//			RequestID,Name,Net_Salary,Loan_Amount,City,Employment_Status,City_Other,Feedback, Followup_Date, Dated,Add_Comment
		...
		//	$qry=$qry."group by ".TABLE_REQ_LAP.".Mobile_Number";
			...
			$qry = "SELECT *, lead_allocate.BidderID as AgentID  FROM Req_Credit_Card_Sms LEFT OUTER JOIN lead_allocate ON lead_allocate.AllRequestID=Req_Credit_Card_Sms.RequestID WHERE (( Req_Credit_Card_Sms.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Credit_Card_Sms.Followup_Date Between '".($min_date)."' and '".($max_date)."') and lead_allocate.BidderID in (".$BidderIDstatic.")) ";
                       $qry=$qry.$FeedbackClause;
                       $qry=$qry.$refernce_no_clause;
 ...
 ?>
     <tr>
		...
		while($row = $obj->fun_db_fetch_rs_object($result))
		{
			//print_r($row);
                    ...
                    $RefID = "CC".$row->RequestID."S".$row->AgentID;
	...
	?>
		  ...
		  <tr  bgcolor="<?php echo $colorvar;?>">
                      <td class="bodyarial11"><?php echo $RefID; ?></td>
...
<? echo $row->Feedback; ?>
	 ...
	 <td class="bodyarial11"><textarea readonly="readonly" rows="2" cols="30"><? echo $row->comment_section; ?></textarea></td>
	  <?php 
	 $RequestID= $row->RequestID;
?>	
    </tr>
  ...
  <table  border="0" cellpadding="5" cellspacing="1" align="center">
              <tr>
                ...
                <form name="frmdownload" action="/miscc_download.php" method="post">
