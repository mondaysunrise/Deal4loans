...
.tooltip .tooltiptext {visibility:hidden; width:160px; background-color: black; color: #fff; text-align: center; padding: 5px 0; border-radius:6px; position:absolute; z-index:1; }
				...
				<td align="center" width="100%"><div align="center">
					<table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
						...
						<tr>
							...
							<td width="29%" align="center" valign="middle"><strong>To</strong></td>
							<td width="58%">
		...
		if($search=="y")
		{
			...
			?>
				...
				if($BidderIDstatic!="")
				{
					...
					$qry="SELECT * FROM ".$feedback_tble.",".TABLE_REQ_LOAN_HOME." LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID=".TABLE_REQ_LOAN_HOME.".RequestID AND Req_Feedback_HL.BidderID= '".$BidderIDstatic."' WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LOAN_HOME.".RequestID AND ".$feedback_tble.".BidderID = '".$BidderIDstatic."'";
					...
					$exporttoexcelqry = "SELECT RequestID, Name, Net_Salary, Loan_Amount, City, Feedback, Followup_Date, comment_section FROM ".$feedback_tble.",".TABLE_REQ_LOAN_HOME." LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID=".TABLE_REQ_LOAN_HOME.".RequestID AND Req_Feedback_HL.BidderID= '".$BidderIDstatic."' WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LOAN_HOME.".RequestID AND ".$feedback_tble.".BidderID = '".$BidderIDstatic."'";
					if($varFilter == 'Assigned Date'){
						$exporttoexcelqry .= " AND ((".$feedback_tble.".DOE Between '".($min_date)."' and '".($max_date)."'))";
					}
					elseif($varFilter == 'Followup Date'){
						$exporttoexcelqry .= " AND ((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."'))";
					}
					else{
						$exporttoexcelqry .= " AND ((".$feedback_tble.".DOE Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."'))";
					}
				...
				while($row = $obj->fun_db_fetch_rs_object($result))
				{
					...
					<tr bgcolor="<?php echo $colorvar;?>">			 
						...
						<td class="bodyarial11">
							<a href="/hdfchllmsallocate_editlead.php?id=<? echo urlencode($row->RequestID); ?>&Bid=<? echo $BidderIDstatic;?>" target="_blank"><? echo $row->Name; ?></a>
						</td>
						<td class="bodyarial11">
							<img src="gButt.php?text=<? //echo $row->Mobile_Number; ?>" />
						</td>
						<?
						}
						else
						...
						<td class="bodyarial11">
							<? echo getJumpMenu("hlloanlms_index.php",$row->RequestID,"2",$row->Feedback,$page,$varmin_date,$varmax_date,$varCmbFeedback, $val, $leadidentifier) ?>
						</td>
...
{
//www.deal4loans.com/callinglms/hlloanlms_index.php?search=y&RequestID=2115107&type=1&pageno=&min_date=2016-05-01&max_date=2016-06-01&cmbfeedback=All&product=Req_Loan_Personal&Feedback=Not%20Applied 
	$strURL="";
