...
do { 
		...
		{ 
			...
			{
			$ResourceCode=trim($data[1]);
			$time=date('H:i:s');
			// add code with date logic here 
			$getdlcode="Select resourcing_days FROM `sbicard_decline_code` WHERE (decline_code='".$ResourceCode."') ";
			$getdlcoderesult = $obj->fun_db_query($getdlcode);
			$rowSbidl = $obj->fun_db_fetch_rs_object($getdlcoderesult);
			$resourcing_days = $rowSbidl->resourcing_days;//from DB
			if($resourcing_days>0)
				{ 
					$tomorrow  = mktime(0, 0, 0, date("m")  , date("d")+$resourcing_days, date("Y"));
					$days6mths=date('Y-m-d',$tomorrow);
					$dated=$days6mths." ".$time;
				}
			else
				{
			...
			$AgentID='';
				...
				if($process==44)
				{
					$getRefRequestIDRow=   $obj->fun_db_fetch_rs_object($obj->fun_db_query("SELECT RequestID as RefRequestID FROM  Req_Credit_Card_Sms WHERE UserID='".$RequestID."'"));
				...
				else
				{
					...
					$insert_status="Agent ID Not Found";
				}
	
				$insertleadqry=("INSERT INTO ".$tblname." (bankname, identifier, process, RequestID,RefRequestID, AgentID, dated, modified_date, LeadRefNumber, code, insert_status) VALUES ('".$bankname."', '".$identifier."','".$process."', '".$RequestID."','".$getRefRequestID."', '".$AgentID."', '".$dated."', Now(), '".($data[0])."' , '".($data[1])."', '".$insert_status."')"); 
...
<div align="center"><b>UPLOAD FOR SBI Sales Reject Cases</b></div>
