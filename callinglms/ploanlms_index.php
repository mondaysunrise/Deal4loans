...
$qryCheck = "SELECT * FROM Bidders where leadidentifier in ('plmainlms','plmainlms79','plmainlmsS') and BidderID='".$_SESSION["BidderID"]."'";
...
if($_SESSION['leadidentifier']=='plmainlmsS')
{
	$validatetimestamp = time();
	if(date('D', $validatetimestamp) == 'Sun') 
	{
	}
	else
	{
		echo "Login on Sunday";
		echo '<meta http-equiv="refresh" content="5; URL=http://www.deal4loans.com/callinglms/pllmslogin.php">';
		die(); 
	}
}
	  ...
	  <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr>
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="ploanlms_index.php" method="get" onSubmit="return chkform();">
	...
	*/	
	...
	//		  echo "BidderIDstatic - ".$BidderIDstatic;
		...
		if($BidderIDstatic!="")
		{
			...
			$qry="SELECT * FROM ".$feedback_tble.",".TABLE_REQ_LOAN_PERSONAL." LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID AND Req_Feedback_PL.BidderID= '".$BidderIDstatic."' WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID and ".$feedback_tble.".BidderID = '".$BidderIDstatic."' and ".TABLE_REQ_LOAN_PERSONAL.".Allocated=0 and ((".$feedback_tble.".DOE Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback_PL.Followup_Date Between '".($min_date)."' and '".($max_date)."')) ";
...
$result = $obj->fun_db_query($qry);
 ?>
	...
	?>
                ...
                <a href="/pleditlead.php?id=<? echo urlencode($row->RequestID); ?>&Bid=<? echo $BidderIDstatic;?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row->Name; ?></a></td>
                <td class="bodyarial11">
                 <?php 
                if($CallStatus==1) {
                ?>
...
<span id="clkNum<?php echo $color; ?>" onClick="getNumValue(<?php echo $color; ?>,<?php echo $row->RequestID; ?>,<?php echo $getParameterVal; ?>);" style="cursor:hand;"><? echo $row->Net_Salary; ?></span> <?php } else {  echo $row->Net_Salary; } ?>
   ...
   for($i=0;$i<count($FinalBidder);$i++)
		...
		<td class="bodyarial11">
                 <? echo getJumpMenu("ploanlms_index.php",$row->RequestID,"1",$row->Feedback,$page,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
       <?php
	   $day = date("D");	
		if($time >=18 && $time<21 )
		{
...
{
//www.deal4loans.com/callinglms/ploanlms_index.php?search=y&RequestID=2115107&type=1&pageno=&min_date=2016-05-01&max_date=2016-06-01&cmbfeedback=All&product=Req_Loan_Personal&Feedback=Not%20Applied 
	$strURL="";
