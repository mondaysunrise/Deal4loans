...
//print_r($_SESSION);
$qryCheck = "SELECT BidderID,leadidentifier FROM Bidders where leadidentifier in ('blmainlms', 'agent_othermetros', 'CallerLAP', 'hdfcbusinessloan','CallingHDFCBL') and BidderID='".$_SESSION["BidderID"]."'";
...
//	$val= getTableName($_SESSION['product']);
	...
	if(isset($_REQUEST['refernce_no']))
	{
        ...
        if( $_SESSION['BidderID']==7006 || $_SESSION['BidderID']==7007 || $_SESSION['BidderID']==6389 || $_SESSION['BidderID']==6390 || $_SESSION['BidderID']==6285 || $_SESSION['BidderID']==6454 || $_SESSION['BidderID']==6508 || $_SESSION['BidderID']==7196){
		$inhouseCut = 1;	
	}
...
if (restore) selObj.selectedIndex=0;
    ...
    function getNumberValue(iLoc,id, parameterVal)
	{
		...
		<!--DatePicker End-->
		<?php 
		 if($_SESSION["BidderID"]==6468 || $_SESSION["BidderID"]==7104 || $_SESSION["BidderID"]==7105 || $_SESSION["BidderID"]==7106 || $_SESSION["BidderID"]==7107 || $_SESSION["BidderID"]==7108|| $_SESSION["BidderID"]==7109 || $_SESSION["BidderID"]==7006 || $_SESSION["BidderID"]==7007)
		 {
		 ?>
    ...
    {	
	   $.ajax({ type: 'post',  url: '/dialerclick2callbl.php',  data: {  RequestID:RequestID,agent_user:agent_user, },
...
?>
    ...
    {	
		var funcVal = 'call';
	   $.ajax({ type: 'post',  url: '/external_dialer_c2c_bl.php',  data: {  RequestID:RequestID,agent_user:agent_user, functionC:funcVal },
			   success: function (response) {
			   //alert(response);
			   $( '#name_status' ).html(response);
  		          if(response=="OK") { return true;	 } else { return false;	}
    ...
    {	
	   ...
	   $.ajax({ type: 'post',  url: '/external_dialer_c2c_bl.php',  data: {  RequestID:RequestID,agent_user:agent_user, functionC:funcVal, disPos:dispID },
			   success: function (response) {
			   //alert(response);
			   $( '#name_status' ).html(response);
  		          if(response=="OK") { return true;	 } else { return false;	}
...
<div style="clear:both; height:15px;"></div>
    ...
    <table width="98%" border="0">
	 <!-- <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr>-->
              <tr>
                  <td align="right"><?php if($_SESSION["BidderID"]==6285) {?><a href="http://www.deal4loans.com/callinglms/editbllead_071117yash.php?id=3724978&Bid=6285&leadident=blmainlms" target="_blank">Edit New Form</a> <?php }?></td>
      </tr>
              <tr>
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="lms_index.php" method="get" onSubmit="return chkform();">
...
$qry="SELECT * ,".$feedback_tble.".BidderID AS sentbidder FROM ".$feedback_tble.",".TABLE_REQ_LOAN_PERSONAL." LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID AND Req_Feedback_PL.BidderID= '".$_SESSION['BidderID']."' WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID and ".$feedback_tble.".BidderID = '".$_SESSION['BidderID']."' and ( ".$feedback_tble.".Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_PL.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
		$qry=$qry.$FeedbackClause." ".$ref_num_clause." ".$refernce_no_clause;
...
<td class="bodyarial11"><?php echo $uniqueid ="PL".$row->RequestID."S".$row->sentbidder; ?></td>
         	<td class="bodyarial11"><?php
			 $RequestID= $row->RequestID; ?>
		<a href="editbllead.php?id=<? echo urlencode($row->RequestID); ?>&Bid=<? echo $_SESSION['BidderID'];?>&leadident=<?php echo $leadidentifier; ?>" target="_blank"><? echo $row->Name; ?></a></td>
			<td class="bodyarial11">
                            
                            <?php if($leadidentifier=="hdfcbusinessloan") {echo $row->Mobile_Number;} else { ?><span id="clik4Num_<?php echo $color; ?>">XXXXXXXXXX</span><? } ?>
	 ...
	 if($inhouseCut==1)
	 {
	?><span id="clkNum<?php echo $color; ?>" onClick="return getNumberValue(<?php echo $color; ?>,<?php echo $row->RequestID; ?>,<?php echo $getParameterVal; ?>)" style="cursor:hand;"><? echo $row->Net_Salary; ?></span><?php } else { ?><? echo $row->Net_Salary; ?><?php } ?>
...
<? if($processing_status==1 || $processing_status==7) { } else { 
               echo getJumpMenu("lms_index.php",$row->RequestID,"4",$row->Feedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val); ?>
     </td>
	 ...
	 <td class="bodyarial11"><textarea rows="2" cols="10"><? echo $row->comment_section; ?></textarea></td><? 
	  ...
	  <?php 
	  if($leadidentifier=="hdfcbusinessloan")
			{
			}else
			{
	 $RequestID= $row->RequestID;
?>	<td class="bodyarial11"><input type="button" name="chkCall" value="Call" onClick="checkCall(<?php echo $row->RequestID ?>,<?php echo $_SESSION['BidderID'] ?>)" /></td> <? } ?>
    </tr>
                <?
		$color++;
		}		
