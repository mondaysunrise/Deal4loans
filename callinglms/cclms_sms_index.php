...
if (restore) selObj.selectedIndex=0;
    ...
    {	
	   $.ajax({ type: 'post',  url: '/dialerclick2callccsms.php',  data: {  RequestID:RequestID,agent_user:agent_user, },
	  ...
	  <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=4" target="_blank">today's Report</a></td></tr>
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="cclms_sms_index.php?search=y" method="post" onSubmit="return chkform();">
...
if($_SESSION['BidderID']==5633)
			{
			$qry="SELECT *,Req_Feedback_CC.BidderID as allocateccbid FROM ".TABLE_REQ_CREDIT_CARD." LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID=".TABLE_REQ_CREDIT_CARD.".RequestID AND Req_Feedback_CC.BidderID in (5658,5657) WHERE (( ".TABLE_REQ_CREDIT_CARD.".Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CC.Followup_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CC.last_updated between '".($min_dateonly)."' and '".($max_dateonly)."') and ".TABLE_REQ_CREDIT_CARD.".source='SMS_Lead')";
		$qry=$qry.$FeedbackClause." ".$mob_num_clause;
			}
			else
			{
$qry="SELECT * FROM ".TABLE_REQ_CREDIT_CARD." LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID=".TABLE_REQ_CREDIT_CARD.".RequestID AND Req_Feedback_CC.BidderID= '".$_SESSION['BidderID']."' WHERE (( ".TABLE_REQ_CREDIT_CARD.".Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CC.Followup_Date Between '".($min_date)."' and '".($max_date)."') and ".TABLE_REQ_CREDIT_CARD.".source='SMS_Lead') ";
		$qry=$qry.$FeedbackClause." ".$mob_num_clause;
			}	
		}		
$srh_qry = $qry;
 ...
 ?>
	...
	?>
              ...
              <tr  bgcolor="<?php echo $colorvar;?>">			 
                <td class="bodyarial11"><a href="/sbiccleadlms-detail.php?postid=<? echo urlencode($row->RequestID); ?>&biddt=<? echo $_SESSION['BidderID'];?>" target="_blank"><? echo $row->Name; ?></a></td>
	   ...
	   ?>              
		<td class="bodyarial11">
                 <? echo getJumpMenu("cclms_sms_index.php",$row->RequestID,"4",$row->Feedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
     </td>
