...
$qryCheck = "SELECT BidderID FROM Bidders where leadidentifier = 'MIS_BL' and BidderID='".$_SESSION["BidderID"]."'";
$resCountCheck = $objAdmin->fun_get_num_rows($qryCheck);
$qryCheckResult = $obj->fun_db_query($qryCheck);
if($resCountCheck>0 && ($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="182.73.4.60" || $IP=="180.151.74.83" || $IP=="115.249.245.30" || $IP=="182.71.109.218"  || $IP=="185.93.231.12" || $IP == "113.193.239.185" || $IP=="122.176.54.210" ))
{
...
<html>
	...
	<title>MIS Dashboard - Loan Against Property</title>
...
<div align="right" style="padding-right:10px; padding-top:10px;"><a href="lms_mis_loanagainstproperty_index.php" style="color:blue; text-align:center; text-decoration:none; font-size:14px; font-weight:bold;">LAP</a> | <a href="lms_mis_businessloan_index.php" style="color:blue; text-align:center; text-decoration:none; font-size:14px; font-weight:bold;">Business Loan</a></div>
<div style="clear:both;"></div>
...
<div style="clear:both; height:15px;"></div>
	 ...
	 <!-- <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr>-->
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
                <form name="frmsearch" action="lms_mis_loanagainstproperty_index.php" method="get" onSubmit="return chkform();">
	  ...
	  <td width="58%"  valign="middle" class="bidderclass"><input type="text" name="refernce_no" id="refernce_no" value="<?php echo $refernce_no; ?>" >
                ...
                -->
                 <tr>
                  ...
                  <?php
	                $qryCheck = "SELECT BidderID, leadidentifier FROM Bidders where leadidentifier in ('CallerLAP') group by leadidentifier";//'CallerAccountCity',
					$resCountCheck = $objAdmin->fun_get_num_rows($qryCheck);
					$qryCheckResult = $obj->fun_db_query($qryCheck);
					while($row = $obj->fun_db_fetch_rs_object($qryCheckResult))
					{
					?>
                    <option value="<?php echo $row->leadidentifier; ?>" <? if($Campaign == $row->leadidentifier) { echo "selected"; } ?>><?php if($row->leadidentifier=="agent_othermetros") { echo "CFL"; } else { echo $row->leadidentifier; } ?></option>
					<?php								
					}
				  ?></select></td>
				  <td>Agents</td>
	...
	if(strlen(trim($RequestID))>0)
	{
		$strSQL="";
		$Msg="";
		$fbqry="select FeedbackID from Req_Feedback_LAP where AllRequestID=$RequestID and BidderID=".$_SESSION['BidderID']." AND Reply_Type=5";
...
//echo $strSQL;
	...
	{		
		...
		{
			$FeedbackClause=" AND (Req_Feedback_LAP.Feedback IS NULL OR Req_Feedback_LAP.Feedback='' OR Req_Feedback_LAP.Feedback='No Feedback') ";
		}
		...
		if(strlen($refernce_no)>3)
		{
			...
			$refernce_no_clause = " AND ".TABLE_REQ_LAP.".RequestID = '".$refernce_no_section."' ";
		}
	?>       <p class="bodyarial11">
              <?=$Msg?>
            </p>
		...
		if($ref_num>0)
		{
			$ref_num_clause = " AND ".TABLE_REQ_LAP.".Mobile_Number = '".$ref_num."' ";
		}
...
//			RequestID,Name,Net_Salary,Loan_Amount,City,Employment_Status,City_Other,Feedback, Followup_Date, Dated,Add_Comment
			$qry="SELECT RequestID,".$feedback_tble.".BidderID AS AgentID, Name, Mobile_Number, Net_Salary as NetProfit, Loan_Amount,City,City_Other,Total_Experience as Vintage, Feedback, Followup_Date, Dated,Add_Comment as Comment  FROM ".$feedback_tble.",".TABLE_REQ_LAP." LEFT OUTER JOIN Req_Feedback_LAP ON Req_Feedback_LAP.AllRequestID=".TABLE_REQ_LAP.".RequestID AND Req_Feedback_LAP.BidderID in (".$BidderIDstatic.") WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LAP.".RequestID and ".$feedback_tble.".BidderID in (".$BidderIDstatic.") and ( ".$feedback_tble.".Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_LAP.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
			$qry=$qry.$FeedbackClause." ".$ref_num_clause." ".$refernce_no_clause;
			$qry=$qry."group by ".TABLE_REQ_LAP.".Mobile_Number";
...
<td class="bodyarial11"><?php echo $uniqueid ="LAP".$row->RequestID."S".$row->AgentID; ?></td>
...
?>	
  ...
  <table  border="0" cellpadding="5" cellspacing="1" align="center">
              <tr>
                ...
                <form name="frmdownload" action="/mis_loanagainstproperty_download.php" method="post">
