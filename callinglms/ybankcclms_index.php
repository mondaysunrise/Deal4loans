...
<?php
if($_SESSION['leadidentifier']=='rblcallerlms_cc')
{
?>	 
...
<?php } 
else if($_SESSION['leadidentifier']=='rblcallerdigilms_cc' || $_SESSION['leadidentifier']=='ybankdigicallerlms_cc')
{
?>	 
...
<script type="text/javascript">
    ...
    function checkCall(RequestID,agent_user)
    {
	   $.ajax({
		   type: 'post',
		   url: '/dialerclick2callccmaintable.php',
		   data: {
			   RequestID:RequestID,
			   agent_user:agent_user,
			},
    ...
    <table width="98%" border="0">
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="ybankcclms_index.php" method="get" onSubmit="return chkform();">
        ...
        <tr>
		  ...
		  <option value="online" <?php if($lead_type=="online") {echo "selected";} ?>>Online</option><option value="data" <?php if($lead_type=="data") {echo "selected";} ?>>Data</option></select>--></td>
		  <td>&nbsp;</td>
...
$result = $obj->fun_db_query($qry);
 ?>
	  ...
	  <tr>
		...
		if($showsbibtnflag== 1){
		?>
		<td class="head1">Click for SBI</td>
		<?php 
		}
		?>
	...
	?>
              ...
              <tr  bgcolor="<?php echo $colorvar;?>">			 
				...
				<a href="/yesbankccleadlms-details.php?postid=<? echo urlencode($row->RequestID); ?>&biddt=<? echo $_SESSION['BidderID'];?>" target="_blank"><? if(strlen($full_name)>0) {echo $row->Name;  } else { echo "Customer"; } ?></a></td>
				<?php 
                if($showsbibtnflag== 1){
				?>
                <td class="bodyarial11"> <?php //echo ccMasking($row->Mobile_Number);  ?><a href="/sbiccleadlms-details.php?postid=<? echo urlencode($row->RequestID); ?>&biddt=<? echo $_SESSION['BidderID'];?>" target="_blank">Click for SBI</a></td>
                <?php 
                }
                ?>
	   ...
	   ?>              
		<td class="bodyarial11">
<? 
               echo getJumpMenu("ybankcclms_index.php",$row->RequestID,"4",$row->Feedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val); ?>
     </td>
