...
$qryCheck = "SELECT BidderID FROM Bidders where leadidentifier = 'blmainlms' and BidderID='".$_SESSION["BidderID"]."'";
...
//echo $strSQL;
	...
	{		
		...
		{	if($pro_code==1) {$appdtxt="PL";}elseif($pro_code==2){$appdtxt="HL";}elseif($pro_code==3){$appdtxt="CL";}elseif($pro_code==4){$appdtxt="CC";}elseif($pro_code==5){$appdtxt="LAP";}else{$appdtxt="";}
			...
			$refernce_no_clause = " AND `".$feedback_tble."`.FeedbackID = '".$refernce_no_section."' ";
		}
...
<td class="bodyarial11"><?php echo $uniqueid ="PL".$row->FeedbackID."S".$row->sentbidder; ?></td>
         	<td class="bodyarial11"><?php
			 $RequestID= $row->RequestID; ?>
		<a href="editbllead.php?id=<? echo urlencode($row->RequestID); ?>&Bid=<? echo $_SESSION['BidderID'];?>" target="_blank"><? echo $row->Name; ?></a></td>
