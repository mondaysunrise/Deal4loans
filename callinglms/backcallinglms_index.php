...
$qryCheck = "SELECT * FROM Bidders where leadidentifier in ('hdfcbackcalling','fullertonbackcalling','kotakbackcallingDelhi','kotakbackcallingJCK','kotakbackcallingOthers','kotakbackcalling', 'hdfcback_calling') and BidderID='".$_SESSION["BidderID"]."'";
...
if($resCountCheck>0)
{
	...
	$standard_fields = $rowqryCheck->standard_fields;
        $Bidder_Name = $rowqryCheck->Bidder_Name;
        //print_r($rowqryCheck);
}
else {
...
function ccMasking($number, $maskingCharacter = 'X') 
{
  ...
  if($_SESSION["BidderID"]==7219 || $_SESSION["BidderID"]==7220 || $_SESSION["BidderID"]==7221 || $_SESSION["BidderID"]==7222 || $_SESSION["BidderID"]==7223 || $_SESSION["BidderID"]==7224 || $_SESSION["BidderID"]==7225 || $_SESSION["BidderID"]==7226 || $_SESSION["BidderID"]==7237 || $_SESSION["BidderID"]==7239){
  $CallStatus = 1;
  }
// $CallStatus = 2;
  ...
  $val = "Req_Loan_Home";
	...
	if(isset($_REQUEST['Feedback']))
	{
       ...
       $QueryCity = "SELECT City,Dated,source FROM " . TABLE_REQ_PL_BACKCALLING . " WHERE source in ('pl_backcalling','Fullerton_pl_backcalling','Kotak_pl_backcalling')";
if ($_REQUEST['min_date'] != "" && $_REQUEST['max_date'] != "") {
    $QueryCity .= " AND Dated between '" . $_REQUEST['min_date'] . " 00:00:00' and '" . $_REQUEST['max_date'] . " 23:59:59'";
}
$QueryCity .= " AND City !='' GROUP BY City";
$resultCity = $obj->fun_db_query($QueryCity);
?>
<html>
		<head>     
...
<div class="lms-top-container"> 
    <div class="lms-text-wrapper"> Back Calling Process  LMS - <span style="font-size: initial; vertical-align: middle;"><? echo $Bidder_Name;?></span></div>
...
<div style="clear:both; height:15px;"></div>
    ...
    <table width="98%" border="0">
        ...
        <td align="center" width="100%"><div align="center">
            <form name="frmsearch" action="backcallinglms_index.php" method="get" onSubmit="return chkform();">
...
//echo $strSQL;
	...
	{		
		...
		if($BidderIDstatic!="")
		{
			...
			$qry="SELECT * FROM ".$feedback_tble.",".TABLE_REQ_PL_BACKCALLING." LEFT OUTER JOIN Req_Feedback_BCalling ON Req_Feedback_BCalling.AllRequestID=".TABLE_REQ_PL_BACKCALLING.".RequestID AND Req_Feedback_BCalling.BidderID IN (".$BidderIDstatic.") WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_PL_BACKCALLING.".RequestID and ".$feedback_tble.".BidderID IN (".$BidderIDstatic.") and ((".$feedback_tble.".Allocation_Date Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback_BCalling.Followup_Date Between '".($min_date)."' and '".($max_date)."')) ";
			if(($_REQUEST['City'] != "") && ($_REQUEST['City'] != "Please Select"))
                        {
                            $qry = $qry . " AND " . TABLE_REQ_PL_BACKCALLING . ".City ='" . $_REQUEST['City'] . "'";   
                        }
                        if($_REQUEST['MobileNum']!="")
                        {
                            $qry=  $qry." AND ".TABLE_REQ_PL_BACKCALLING.".Mobile_Number ='".$_REQUEST['MobileNum']."'";
                        }
                        $qry=$qry.$FeedbackClause." ".$mob_num_clause ." ".$referno_clause." group by ".TABLE_REQ_PL_BACKCALLING.".Mobile_Number ";
		}		
	$srh_qry = $qry;
	//echo $qry;
...
$getParameterVal = min($start+$limit,$resCount) % $limit;
 ...
 ?>
              ...
              <tr>
                  <td class="head1"><input type="hidden" value="<?php echo $srh_qry;?>">Reference No</td>
	...
	?>
			...
			<td class="bodyarial11"><a href="/edit-personal-details-back-calling.php?id=<? echo urlencode($row->RequestID); ?>&Bid=<? echo $BidderIDstatic;?>" target="_blank"><? echo $row->Name; ?></a></td>
...
<span id="clkNum<?php echo $color; ?>" onClick="getNumValue(<?php echo $color; ?>,<?php echo $row->RequestID; ?>,<?php echo $getParameterVal; ?>);" style="cursor:hand;"><? echo $row->Net_Salary; ?></span> <?php } else {  echo $row->Net_Salary; } ?>
		...
		<td class="bodyarial11">
                 <? echo getJumpMenu("backcallinglms_index.php",$row->RequestID,"2",$row->Feedback,$page,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
       </td>
	...
	?>
            ...
            <?php  if($_SESSION["BidderID"]==6976 || $_SESSION["BidderID"]==7237 || $_SESSION["BidderID"]==7239)
          {
          ?>
              <br>
  <table  border="0" cellpadding="5" cellspacing="1" align="center">
