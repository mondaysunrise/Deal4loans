	  ...
	  <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr>
        ...
        <td align="center" width="100%"><div align="center">
            <table width="700" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="plallocation_index.php" method="get" onSubmit="return chkform();">
...
$result = $obj->fun_db_query($qry);
 ?>
   ...
   for($i=0;$i<count($FinalBidder);$i++)
       ...
       -->
		<td class="bodyarial11">
                 <? echo getJumpMenu("plallocation_index.php",$row->RequestID,"1",$row->Feedback,$page,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
       <?php
	   $day = date("D");	
		if($time >=18 && $time<21 )
		{
...
?>
  ...
  <td class="bodyarial11">
  <?php
  if($_SESSION['BidderID']==64801111)
  {
  ?>
   <input type="submit" name="chkCall" value="Call" onClick="checkCall(<?php echo $row->RequestID ?>,<?php echo $_SESSION['BidderID'] ?>)" />
  <?php
  } else {
 ?>
           <input type="button" name="chkCall" value="Call" onClick="checkCall(<?php echo $row->RequestID ?>,<?php echo $_SESSION['BidderID'] ?>)" />   <?php } ?>     </td>
	               </tr>
                <?
		$color++;
		$i++;
...
{
//www.deal4loans.com/callinglms/plallocation_index.php?search=y&RequestID=2115107&type=1&pageno=&min_date=2016-05-01&max_date=2016-06-01&cmbfeedback=All&product=Req_Loan_Personal&Feedback=Not%20Applied 
	$strURL="";
