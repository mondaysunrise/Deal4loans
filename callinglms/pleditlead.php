...
$Tataaig_Home=  $obj->fun_get_mysql_result($viewlead,0,'Tataaig_Home');
$Tataaig_Health=  $obj->fun_get_mysql_result($viewlead,0,'Tataaig_Health');
$Tataaig_Auto=  $obj->fun_get_mysql_result($viewlead,0,'Tataaig_Auto');
$Accidental_Insurance = $obj->fun_get_mysql_result($viewlead,0,'Accidental_Insurance');
...
$SentEmail = $obj->fun_get_mysql_result($viewlead,0,'SentEmail');
...
$CC_Mailer = $obj->fun_get_mysql_result($viewlead,0,'CC_Mailer');
$Company_Name = $obj->fun_get_mysql_result($viewlead,0,'Company_Name');
$PL_EMI_Amt = $obj->fun_get_mysql_result($viewlead,0,'PL_EMI_Amt');
$Card_Limit = $obj->fun_get_mysql_result($viewlead,0,'Card_Limit');
$Salary_Drawn = $obj->fun_get_mysql_result($viewlead,0,'Salary_Drawn');
$Landline_Connection = $obj->fun_get_mysql_result($viewlead,0,'Landline_Connection');
$Mobile_Connection = $obj->fun_get_mysql_result($viewlead,0,'Mobile_Connection');
...
$Primary_Acc = $obj->fun_get_mysql_result($viewlead,0,'Primary_Acc');
$Referral_Flag = $obj->fun_get_mysql_result($viewlead,0,'Referral_Flag');
...
<tr>
	...
	<td><select  name="Primary_Acc" id="Primary_Acc"><option value="">Please Select</option> <? $bnknm=$obj->fun_db_query("select Bank_Name from Bank_Master group by Bank_Name "); 
	while($plbnk=$obj->fun_db_fetch_rs_array($bnknm))
	{ ?>
...
for($ij=0;$ij<count($finalBidderName_unique);$ij++)
{
	...
	<td width="11%" align="center"><b style="font-size:12px;"><? if(isset($stancinterestrate)) { echo $stancinterestrate; } else { echo $hdfcrate ;} ?></b></td>
...
elseif(($finalBidderName[$ij]=="IngVysya" || $finalBidderName[$ij]=="IngVysya" || $finalBidderName[$ij]=="ING Vysya") && ($grow["ingvyasya"]!='' || $Company_Type==4 || $Company_Type==5) && $Employment_Status ==1)
{
...
list($ingrate,$ingprepay_chrge,$ingpro_fee)=ingIR($monthsalary,$grow["ingvyasya"]);
	...
	<td width="11%" align="center"><b style="font-size:12px;"><? 
$ingrateexact=str_replace("%","",$interestrate);
$ingintr = trim($ingrateexact)/1200;
			echo $emicalc=round(100000 * $ingintr / (1 - (pow(1/(1 + $ingintr), ($term*12)))));
			?></b></td>
	...
	<td width="11%" align="center"><b style="font-size:12px;"><? echo $Processing_Fee; ?></b></td>
	</tr>	
<?
}
...
<td width="11%" align="center"><b style="font-size:12px;"><? echo $grow["kotak"]; ?></b></td>
	<td width="11%" align="center"><b style="font-size:12px;">4% - 5%</b></td>
