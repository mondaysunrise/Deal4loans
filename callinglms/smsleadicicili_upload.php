...
if ($_FILES[csv][size] > 0) {
    ...
    $IpruBidQuery = "SELECT BidderID FROM Bidders WHERE process IN('ICICIPRUEUREKA','ICICIPRURELIABLE')";
    $IpruBidQryResult = ExecQuery_bima($IpruBidQuery);
    $BidId = "";
    while ($RowsPru = bima_mysql_fetch_array($IpruBidQryResult)) {
        $BidId[] = $RowsPru['BidderID'];
    }
    $IpruBidId = implode(",", $BidId);
    ...
    do {
        if ($data[0] && strlen($data[2] > 5)) {
            ...
            if ($alreadyExist > 0) {
                ...
                $selbidqry = "Select BidderID From Bidders_List Where (BidderID in ($IpruBidId) and City like '%" . $data[3] . "%' and Restrict_Bidder=1)";
                $selbidqryresult = ExecQuery_bima($selbidqry);
                $bidrow = bima_mysql_fetch_array($selbidqryresult);
                $bidderid = $bidrow["BidderID"];
                if ($bidderid > 0) {
                    $dobarr = explode("-", $data[5]);
                    //print_r($dobarr);
                    //die;
                    $DOB = $dobarr[2] . "-" . $dobarr[1] . "-" . $dobarr[0];
                    $insertleadqry = ("INSERT INTO " . $tblname . " (Name, Phone, City, Email, DOB, Insurance_Plan, Allocated, Bidderid_Details, Net_Salary ,source, Dated, Updated_Date) VALUES 
				( 
					'" . $data[1] . "', 
					'" . $data[2] . "', 
					'" . $data[3] . "',
...
<html>
    ...
    <body style="margin:0px 0px 0px 0px; padding:0px 0px 0px 0px; background-color:#45B2D8;">
        ...
        <div align="center"><b>UPLOAD FOR Reliance LIFE INSURANCE</b></div>
        <div style="clear:both; height:15px;"></div>
        <table align="center">
            
            <tr> <td><?php if ($message == 1) {
    echo "<b style=\"color:#008000\">Your file has been imported.</b><br><br>";
} //generic success notice  ?> </td></tr>
            <tr><td>
