    ...
    <table width="98%" border="0">
	  <!---<tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=4" target="_blank">
        ...
        <td align="center" width="100%"><div align="center">
            <form name="frmsearch" action="cc_lms_bankwise_list.php" method="get" onSubmit="return chkform();"><table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
                ...
                <tr>
                  ...
                  <?php
                  $BankQuery = "SELECT cc_requestid,applied_bankname FROM credit_card_banks_apply WHERE applied_bankname!='' GROUP BY applied_bankname";            
                  $BankQryResult = $obj->fun_db_query($BankQuery);
                  ?>
                  <td width="29%"><select name="bankname" id="bankname">
                      <?php
                      while($BankRow = $obj->fun_db_fetch_rs_object($BankQryResult)){
                      ?>
                       <option value="<?php echo $BankRow->applied_bankname;?>" <?php if($_REQUEST['bankname']=="$BankRow->applied_bankname") { echo "Selected";}?>><?php echo $BankRow->applied_bankname;?></option>
                      <?php }?>
                      
                    </select></td>
                    <td width="12%"><strong>Products:</strong></td>
...
//echo "<br>".$strSQL;
	...
	?>       <p class="bodyarial11">
		...
		if($_REQUEST['products']==1)
                 ...
                 {
                   echo "SELECT * FROM Req_Credit_Card_Sms LEFT OUTER JOIN lead_allocate ON lead_allocate.AllRequestID=Req_Credit_Card_Sms.RequestID WHERE (( Req_Credit_Card_Sms.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Credit_Card_Sms.Followup_Date Between '".($min_date)."' and '".($max_date)."') and lead_allocate.BidderID  in (".$BidderIDstatic.") ) ";
                    echo $qry = "SELECT * from credit_card_banks_apply WHERE applied_bankname='".$_REQUEST['bankname']."'";
