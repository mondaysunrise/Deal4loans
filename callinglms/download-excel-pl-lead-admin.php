...
//RequestID	telecaller	leadrefnumber	Name	Email	Gender	Company_Name	City	Std_Code	Landline	Mobile_Number	Std_Code_O	Landline_O	Net_Salary	Pincode	IP_Address	DOB	Pancard	No_of_Banks	Residence_Address	Office_Address	Updated_Date	applied_card_name
    ...
    $Remarks = $row_result->Add_Comment; /* to download */
    
    $LANNo = $row_result->post_login_stat; /* to download */
    $BankFeedback = $row_result->axis_executive_name; /* to download */
    $BankRemarks = $row_result->comment_section; /* to download */
...
if($_REQUEST['GlobalID']=='7074'){
    $qry2 = "insert into temp (session_id, name, dob, email,  emp_status, c_name, city, city_other, car_make, car_model, car_type, loan_tenure, property_type, property_value) values ('" . $session_id . "','" . $ReferenceID . "', '" . $AgentID . "', '" . $Name . "', '" . $City . "', '" . $City_Other . "', '" . $DOB . "', '" . $Pancard . "', '" . $LANNo . "', '" . $Feedback . "', '" . $Remarks . "', '" . $BankFeedback . "', '" . $BankRemarks . "', '" . $Doe . "')";
}else{
    ...
    //// ".."
    //echo 	$qry1."<br>";	
    $obj->fun_db_query($qry2);
    // $fieldBidderStr = '';
    // $valuesBidderStr='';
}
if($_REQUEST['GlobalID']=='7074'){ 
   $qry = "select name as UniqueReferenceID, dob as AgentID, email as Name, emp_status as City, c_name as CityOther, city as DOB, city_other as PANCardNumber, car_make as LANNo, car_model as Feedback, car_type as Remarks, loan_tenure as BankFeedback, property_type as BankRemarks, property_value as DOE from temp where session_id='" . $session_id . "' order by doe DESC "; 
}else{
