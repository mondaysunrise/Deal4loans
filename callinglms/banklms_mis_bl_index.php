...
$qryCheck = "SELECT BidderID FROM Bidders where leadidentifier = 'HDFCMISBL' and BidderID='".$_SESSION["BidderID"]."'";
	 ...
	 <!-- <tr><td align="right"><a href="/commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr>-->
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="banklms_mis_bl_index.php" method="get" onSubmit="return chkform();">
	  ...
	  <td width="58%"  valign="middle" class="bidderclass"><input type="text" name="refernce_no" id="refernce_no" value="<?php echo $refernce_no; ?>" >
                 ...
                 <tr>
                  ...
                  <?php
	                $qryCheck = "SELECT BidderID, leadidentifier FROM Bidders where leadidentifier in ('hdfcbusinessloan') group by leadidentifier";//'CallerAccountCity',
					$resCountCheck = $objAdmin->fun_get_num_rows($qryCheck);
					$qryCheckResult = $obj->fun_db_query($qryCheck);
					while($row = $obj->fun_db_fetch_rs_object($qryCheckResult))
					{
						if($row->leadidentifie="hdfcbusinessloan") { $texttoview="Business loan";}
					?>
                    <option value="<?php echo $row->leadidentifier; ?>" <? if($Campaign == $row->leadidentifier) { echo "selected"; } ?>><?php echo $texttoview; ?></option>
					<?php								
					}
				  ?></select></td>
				  <td>Agents</td>
