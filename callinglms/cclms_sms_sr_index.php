...
<?php include "header_cc_lms.php"; ?>
    ...
    <table width="98%" border="0">
	   <tr>
        <td align="center" style="font-size:16px; font-weight:bold;">Sales Reject Leads</td>
      </tr>
              <tr>
        <td align="right"></td>
      </tr>
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="cclms_sms_sr_index.php" method="get" onSubmit="return chkform();">
...
//			$qry = "SELECT * FROM Req_Credit_Card LEFT OUTER JOIN lead_allocate ON lead_allocate.AllRequestID=Req_Credit_Card.RequestID WHERE (( Req_Credit_Card.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Credit_Card.Followup_Date Between '".($min_date)."' and '".($max_date)."') and Req_Credit_Card.source='SMS_LeadNew'  AND lead_allocate.BidderID= '".$_SESSION['BidderID']."') ";
			$qry = "SELECT * FROM Req_Credit_Card LEFT OUTER JOIN cards_curing_queue ON cards_curing_queue.RefRequestID=Req_Credit_Card.RequestID WHERE ((cards_curing_queue.dated Between '".($min_date)."' and '".($max_date)."') and cards_curing_queue.AgentID= '".$_SESSION['BidderID']."' and cards_curing_queue.identifier= 'sales reject') ";
...
$result = $obj->fun_db_query($qry);
 ?>
              ...
              <tr>
				...
				<td class="head1">Decline Code</td>
	...
	?>
              ...
              <tr  bgcolor="<?php echo $colorvar;?>">			 
                <td class="bodyarial11"><a href="/sbiccleadlms-details.php?postid=<? echo urlencode($row->RequestID); ?>&biddt=<? echo $_SESSION['BidderID'];?>&process=sr" target="_blank"><? if(strlen($full_name)>0) {echo $row->Name;  } else { echo "Customer"; } ?></td>
