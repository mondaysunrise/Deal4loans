...
if( $exactnet_salary>0)
	...
	//echo $gtcropcomp."<br>";
	$gtcropcompresult=mysql_query($gtcropcomp);
...
if(strlen($company_name)>3)
	...
	//echo $gtcropcomp."<br>";
	$gtcropcompresult=$obj->fun_db_query($gtcropcomp);
	$icicirow = $obj->fun_db_fetch_rs_array($gtcropcompresult);
	$crprecordcount = $obj->fun_db_get_num_rows($gtcropcompresult);
	...
	//echo $gtscocomp."<br>";
	//$gtscocompresult=$obj->fun_db_query($gtscocomp);
	$ingscorow=$obj->fun_db_fetch_rs_array($gtscocompresult);
	$ingscorecordcount = $obj->fun_db_get_num_rows($gtscocompresult);
...
//for SCO
	$gtscocomp="Select interest_rate_above50,interest_rate_less50,processing_fee From pl_company_ingvysya_sco Where (company_name like '%".$company_name."%' and interest_rate_above50>0 and status=1)";
	//echo $gtcropcomp."<br>";
	$gtscocompresult=mysql_query($gtscocomp);
...
if($category=="A+" || $category=="CAT A" || $category=="CAT B" || $category=="CAT G")
	...
	if($net_salary>=50000)
	{
		$finalloanamount_other=$net_salary*15;
	}
	else
	{
...
$induscmpqry=mysql_query("Select  company_name from pl_company_indusindspl Where (company_name like '%".$company."%')");
...
if(strlen($indusrow["company_name"])>2 && $getloanamout>=500000)
	...
	{
		if(($category=="A+" || $category=="CAT A" || $category=="CAT B" || $category=="CAT G" || $category=="C1000") && $getloanamout>=700000)
		{
			$interestrate = "13.49%";
			$intr=13.49/1200;
			$proc_fee="0.49%";
		...
		elseif(($category=="A+" || $category=="CAT A" || $category=="CAT B" || $category=="CAT G" || $category=="C1000") && $getloanamout>=1000000)
		{
			$interestrate = "12.99%";
			$intr=12.99/1200;
			$proc_fee="Rs.1999";
...
$kotakcmpqry=mysql_query("Select  * from pl_company_kotak Where (company_name like '%".$company."%')");
...
$perlacemi=round($princ * $intr / (1 - (pow(1/(1 + $intr), $term))));
		...
		elseif($category=="CAT D")
		{
			/*if($net_salary>=75000)
				...
				elseif($net_salary>=45000 && $net_salary<75000)
				{
					$interestrate = "16.50%";
					$intr=16.50/1200;
				}	
...
$princ=100000;
	...
	/*if(($category=="CAT A" || $category=="CAT C" || $category=="CAT B") && $getloanamout>1000000 && $net_salary>=85000)
	{
		if($finalloanamount_dbr>=2000000)
		{
			$interestrate = "12.99%";
		...
		elseif($finalloanamount_dbr>1500000 && $finalloanamount_dbr<2000000)
		{
			$interestrate = "13.25%";
			$intr=18/1200;
		}
		elseif($finalloanamount_dbr>1000000 && $finalloanamount_dbr<1500000)
		{
			$interestrate = "13.50%";
			$intr=18/1200;
		}
...
$interestrate = $icicirow["interest_rate"]." %";
	...
	{	
		if($category=="Super Cat A" || $category=="Super CAT A")
		{
			if($net_salary>25000)
			{
				$interestrate = "15.75%";
		...
		else if($category=="CAT B" || $category=="CAT C")
		{
			if($net_salary>33000)
			{
				$interestrate = "17.50%";
...
//MAx Loan Amount
if($category=="TATA Group" || $category=="Super Cat A" || $category=="Super CAT A" || $category=="CAT A")
	{
		if($loan_amt>=1500000)
		{
			$loan_amount=1500000;
