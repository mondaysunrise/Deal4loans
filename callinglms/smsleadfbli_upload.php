...
$currentdatetime = date('Y-m-d')." 23:59:59";
  ...
  $getdetails="select RequestID From ".$tblname." Where ( Phone not in (9811215138) and Phone='".$data[3]."' and Updated_Date between '".$days30datetime."' and '".$currentdatetime."') order by RequestID DESC";
...
$alreadyExist = mysql_num_rows($checkavailability);
			...
			{
				$dobarr=explode("/",$data[4]);
				$DOB = $dobarr[2]."-".$dobarr[1]."-".$dobarr[0];

			   $insertleadqry=("INSERT INTO ".$tblname." (Name, Email, Phone, DOB, Net_Salary, source, Dated, Updated_Date) VALUES 
				( 
					'".FixString($data[1])."', 
					'".FixString($data[2])."', 
					'".FixString($data[3])."',
