...
$DisplayNumber="";
if( $_SESSION['BidderID']==6684 || $_SESSION['BidderID']==7230 || $_SESSION['BidderID']==7347)
        {
...
<div style="clear:both; height:15px;"></div>
    ...
    <table width="98%" border="0">
       ...
       <tr><td align="right"><a href="/bidders_update_feedback_ref.php">Upload Feedback</a></td></tr>
        ...
        <td align="center" width="100%"><div align="center">
            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
              <form name="frmsearch" action="bajajlms_index.php" method="get" onSubmit="return chkform();">
				...
				<option value="OTP Done Not Ok" <? if($varCmbFeedback == "OTP Done Not Ok") { echo "selected"; }?> >OTP Done Not Ok</option>
					<option value="Not Interested On Rate or Offer" <? if($varCmbFeedback == "Not Interested On Rate or Offer") { echo "selected"; }?> >Not Interested On Rate or Offer</option>
					<option value="Direct Not Interested" <? if($varCmbFeedback == "Direct Not Interested") { echo "selected"; }?> >Direct Not Interested</option>
	  ...
	  <td width="58%"  valign="middle" class="bidderclass"><input type="text" name="mob_num" id="mob_num" value="<?php echo $mob_num; ?>" >
                 ...
                 <tr>
                  <td colspan="2"><strong>Company Category</strong>&nbsp;&nbsp;&nbsp;&nbsp;<select name="company_category" id="company_category">
          <option value="" <? if($company_category== "") { echo "selected"; }?>>All</option>
...
<? }?>
                ...
                <tr>
                    ...
                    <td></td>
                    <td></td>
                    <td><input name="Submit" type="submit" class="bluebutton" value="Search" border="0"></td></tr>
              </form>
            </table>
            <p>&nbsp;</p>
...
//echo $strSQL;
	...
	?>       <p class="bodyarial11">
		...
		if(strlen($company_category)>0)
		{
			$company_category_clause = " AND ".TABLE_REQ_LOAN_PERSONAL.".company_category = '".$company_category."' ";
		}
...
$qry="SELECT * FROM ".$feedback_tble.",".TABLE_REQ_LOAN_PERSONAL." LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID AND Req_Feedback_PL.BidderID= '".$_SESSION['BidderID']."' WHERE ".$feedback_tble.".AllRequestID=".TABLE_REQ_LOAN_PERSONAL.".RequestID and ".$feedback_tble.".BidderID = '".$_SESSION['BidderID']."' and ".$feedback_tble.".Reply_Type=1 and ( ".$feedback_tble.".Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_PL.Followup_Date Between '".($min_date)."' and '".($max_date)."') ";
if($_REQUEST['city'])
    {
        $qry.=  " AND City='".$_REQUEST['city']."'"; 
    }	
$qry=$qry.$FeedbackClause." ".$mob_num_clause. " ".$company_category_clause."  group by ".TABLE_REQ_LOAN_PERSONAL.".Mobile_Number  ";
 ...
 ?>
	...
	?>
		  ...
		  <tr  bgcolor="<?php echo $colorvar;?>">			 
			...
			?><a href="/plallocatedleadlms-detailsbajaj.php?postid=<? echo urlencode($row->RequestID); ?>&biddt=<? echo $_SESSION['BidderID'];?>" target="_blank"><? echo $row->Name; ?></a></td>
...
<span id="clkNum<?php echo $color; ?>" onClick="getNumValue(<?php echo $color; ?>,<?php echo $row->RequestID; ?>,<?php echo $getParameterVal; ?>);" style="cursor:hand;"><? echo $row->Net_Salary; ?></span> <?php } else {  echo $row->Net_Salary; } ?>
	   ...
	   ?>              
		<td class="bodyarial11">
<? echo getJumpMenu("bajajlms_index.php",$row->RequestID,"4",$row->Feedback,$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val); ?>
     </td>
 ...
 <form name="xlsdownload" action="/bidder_download_ref.php" method="post">
   <tr>
     <td align="center">
    ...
    <?php }?>  
<?	  if(($_SESSION['BidderID']==6116 || $_SESSION['BidderID']==6117) && $search=="y")
			{ ?>
	...
	<select name="type" id="type" onChange="MM_jumpMenu('parent',this,0)">
			  <option value="" <? if($varFeedback == "") { echo "selected"; }?>>No Feedback</option>
				...
				<option value="<? echo $strURL.'&Feedback=OTP Done Not Ok'?>" <? if($varFeedback == "OTP Done Not Ok") { echo "selected"; }?> >OTP Done Not Ok</option>
					<option value="<? echo $strURL.'&Feedback=Not Interested On Rate or Offer'?>" <? if($varFeedback == "Not Interested On Rate or Offer") { echo "selected"; }?> >Not Interested On Rate or Offer</option>
					<option value="<? echo $strURL.'&Feedback=Direct Not Interested'?>" <? if($varFeedback == "Direct Not Interested") { echo "selected"; }?> >Direct Not Interested</option>
