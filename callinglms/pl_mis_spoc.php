...
define("FDBCKPRELOGINREJECT", 'Pre-login Reject');
define("FDBCKAPPRESCH", 'Appointment - Rescheduled');
define("FDBCKAPPCANC", 'Appointment - Cancelled');
define("FDBCKASSPICKUP", 'Assigned For Pick-up');
define("FDBCKRINGING", 'Ringing');// For Ringing
define("FDBCKFOLLOWUP", 'Followup');
define("FDBCKCOMPPICKUP", 'Complete Pick-up');
define("FDBCKINCOMPPICKUP", 'Incomplete Pick-up');
define("FDBCKSENTLOGIN", 'Sent For Login');

define("FDBCKNOFEEDBACK", '');// NO Feedback
define("FDBCKLOGGEDIN", '5');// Logged In
define("FDBCKRTS", '4'); // RTS
define("FDBCKAPPROVED", '6');// Approved
define("FDBCKDISBURSED", '7');// Disbursed
define("FDBCKPOSTLOGINREJECT", '8'); // Post Login Reject
define("FDBCKCANCELLEDAUD", '9'); // Cancelled - AUD
        ...
        <table width="98%" border="0">
            ...
            <tr>
                <td align="center" width="100%">
                        <form name="frmsearch" action="pl_mis_spoc.php" method="get" onSubmit="return chkform();">
                            <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF">
                                ...
                                <tr>
                                    <td colspan="4" class="head1">Search (SPOC)</td>
                                </tr>
                                <tr>
                                    <td colspan="3">&nbsp;</td>
                                    <td>&nbsp;</td>
...
<tr>
                                <td class="head1" colspan="33">Personal Loan - Lead Assessment - Date range (<?php echo $min_date?> to <?php echo $max_date?>)</td>
                            </tr>
                            <tr>
                ...
                <td class="head1">%</td>-->
                <td class="head1">Logged In</td>
                <td class="head1">RTS</td>
                <td class="head1">Approved</td>
                <td class="head1">Disbursed</td>
                <td class="head1">Post Login Reject</td>
                <td class="head1">Cancelled - AUD</td>
...
if ($recordcnt > 0) {
    ...
    while ($rowAgent = $obj->fun_db_fetch_rs_object($qryCheckResultAgent)) {
       ...
       $LeadNoFeedbackCnt1 = $obj->fun_db_query($NoFeedbackVal1);
        ...
        ////No Feedback as Sent For Login
        $NoFeedbackDocSt = FDBCKNOFEEDBACK;
        $NoFeedbackVal = GetDocStatus($BidderID, $min_date, $max_date, $NoFeedbackDocSt,$City);
        $LeadNoFeedbackCnt = $obj->fun_db_query($NoFeedbackVal);
        $num_rowsNoFeedback = $obj->fun_db_get_num_rows($LeadNoFeedbackCnt);
        $percentageNoFeedback = $num_rowsNoFeedback * 100 / $num_rows;
        
        //Logged In
        $LoggedInDocSt = FDBCKLOGGEDIN;
        $LoggedInVal = GetDocStatus($BidderID, $min_date, $max_date, $LoggedInDocSt,$City);
        $LeadLoggedInCnt = $obj->fun_db_query($LoggedInVal);
        $num_rowsLoggedIn = $obj->fun_db_get_num_rows($LeadLoggedInCnt);
        $percentageLoggedIn = $num_rowsLoggedIn * 100 / $num_rows;
        
        //RTS
        $RTSfDocSt = FDBCKRTS;
        $RTSVal = GetDocStatus($BidderID, $min_date, $max_date, $RTSfDocSt,$City);
        $LeadRTSCnt = $obj->fun_db_query($RTSVal);
        $num_rowsRTS = $obj->fun_db_get_num_rows($LeadRTSCnt);
        $percentageRTS = $num_rowsRTS * 100 / $num_rows;
        
        //Approved
        $ApprovedDocSt = FDBCKAPPROVED;
        $ApprovedVal = GetDocStatus($BidderID, $min_date, $max_date, $ApprovedDocSt,$City);
        $LeadApprovedCnt = $obj->fun_db_query($ApprovedVal);
        $num_rowsApproved = $obj->fun_db_get_num_rows($LeadApprovedCnt);
        $percentageApproved = $num_rowsApproved * 100 / $num_rows;
        
        //Disbursed
        $DisbursedDocSt = FDBCKDISBURSED;
        $DisbursedVal = GetDocStatus($BidderID, $min_date, $max_date, $DisbursedDocSt,$City);
        $LeadDisbursedCnt = $obj->fun_db_query($DisbursedVal);
        $num_rowsDisbursed = $obj->fun_db_get_num_rows($LeadDisbursedCnt);
        $percentageDisbursed = $num_rowsDisbursed * 100 / $num_rows;
        
        //Post Login Reject
        $PostLoginRejectDocSt = FDBCKPOSTLOGINREJECT;
        $PostLoginRejectVal = GetDocStatus($BidderID, $min_date, $max_date, $PostLoginRejectDocSt,$City);
        $LeadPostLoginRejectCnt = $obj->fun_db_query($PostLoginRejectVal);
        $num_rowsPostLoginReject = $obj->fun_db_get_num_rows($LeadPostLoginRejectCnt);
        $percentagePostLoginReject = $num_rowsPostLoginReject * 100 / $num_rows;

        //Canclled AUD
        $CanclledAUDDocSt = FDBCKCANCELLEDAUD;
        $CanclledAUDVal = GetDocStatus($BidderID, $min_date, $max_date, $CanclledAUDDocSt,$City);
        $LeadCanclledAUDCnt = $obj->fun_db_query($CanclledAUDVal);
        $num_rowsCanclledAUD = $obj->fun_db_get_num_rows($LeadCanclledAUDCnt);
        $percentageCanclledAUD = $num_rowsCanclledAUD * 100 / $num_rows;
        ...
        $rowLeadSentLogin = $obj->fun_db_fetch_rs_object($LeadSentLoginCnt);
        
        $percentageSentLogin = $num_rowsSentLogin * 100 / $num_rows;
       
        //Sent for Login
        $SendforLogin = $num_rowsSentLogin-($num_rowsLoggedIn+$num_rowsRTS+$num_rowsApproved+$num_rowsDisbursed+$num_rowsPostLoginReject+$num_rowsCanclledAUD);
        $percentageSendforLogin = $SendforLogin * 100 / $num_rows;
        
        $UnPickedPercent = ($num_rowsNoFeedback1+$num_rowsPreLogRej+$num_rowsAppoResch+$num_rowsAppCanc+$num_rowsRinging)*100/$num_rows;
...
//Get Comment (Response) 
        ...
        ?>
            ...
            <tr  bgcolor="<?php echo $colorvar; ?>">                                        
                ...
                <!---<td class="bodyarial11"><? //echo $SendforLogin; ?></td>
                <td class="bodyarial11"><? //echo round($percentageSendforLogin,2); ?></td>-->
                
                
               
                <td class="bodyarial11"><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><? echo $num_rowsLoggedIn; ?></td>
                           </tr>
                           <tr>
                            <td><? echo round($percentageLoggedIn,2); ?>%</td>
                           </tr>
                ...
                <td class="bodyarial11"><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><? echo $num_rowsRTS; ?></td>
                           </tr>
                           <tr>
                            <td><? echo round($percentageRTS,2); ?>%</td>
                           </tr>
                ...
                <td class="bodyarial11"><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><? echo $num_rowsApproved; ?></td>
                           </tr>
                           <tr>
                            <td><? echo round($percentageApproved,2); ?>%</td>
                           </tr>
                ...
                <td class="bodyarial11"><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><? echo $num_rowsDisbursed; ?></td>
                           </tr>
                           <tr>
                            <td><? echo round($percentageDisbursed,2); ?>%</td>
                           </tr>
                ...
                <td class="bodyarial11"><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><? echo $num_rowsPostLoginReject; ?></td>
                           </tr>
                           <tr>
                            <td><? echo round($percentagePostLoginReject,2); ?>%</td>
                           </tr>
                ...
                <td class="bodyarial11"><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><? echo $num_rowsCanclledAUD; ?></td>
                           </tr>
                           <tr>
                            <td><? echo round($percentageCanclledAUD,2); ?>%</td>
                           </tr>
                ...
                $totalSendforLogin += $SendforLogin;
                $totalpercentageSendforLogin += $percentageSendforLogin;
                
                //$totalNoFeedback +=$num_rowsNoFeedback;
                //$totalpercentageNoFeedback += $percentageNoFeedback;
                $totalLoggedIn +=$num_rowsLoggedIn;
                $totalpercentageLoggedIn += $percentageLoggedIn;
                $totalRTS +=$num_rowsRTS;
                $totalpercentageRTS += $percentageRTS;
                $totalApproved +=$num_rowsApproved;
                $totalpercentageApproved += $percentageApproved;
                $totalDisbursed +=$num_rowsDisbursed;
                $totalpercentageDisbursed += $percentageDisbursed;
                $totalLoginReject +=$num_rowsLoginReject;
                $totalpercentageLoginReject += $percentageLoginReject;
                
                $totalCanclledAUD +=$num_rowsCanclledAUD;
                $totalpercentageCanclledAUD += $percentageCanclledAUD;
        ...
        ?>
            <tr>
             ...
             <td><table border="0" cellspacing="0" width="100%">
               ...
               <!-- <td><strong><?php //echo $totalSendforLogin;?></strong></td>
                <td><strong><?php //echo round($totalpercentageSendforLogin/$recordcnt,2);?></strong></td>-->
               
                <td><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><strong><?php echo $totalLoggedIn;?></strong></td>
                           </tr>
                           <tr>
                            <td><strong><?php echo round($totalpercentageLoggedIn/$recordcnt,2);?>%</strong></td>
                           </tr>
                ...
                <td><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><strong><?php echo $totalRTS;?></strong></td>
                           </tr>
                           <tr>
                            <td><strong><?php echo round($totalpercentageRTS/$recordcnt,2);?>%</strong></td>
                           </tr>
                ...
                <td><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><strong><?php echo $totalApproved;?></strong></td>
                           </tr>
                           <tr>
                            <td><strong><?php echo round($totalpercentageApproved/$recordcnt,2);?>%</strong></td>
                           </tr>
                ...
                <td><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><strong><?php echo $totalDisbursed;?></strong></td>
                           </tr>
                           <tr>
                            <td><strong><?php echo round($totalpercentageDisbursed/$recordcnt,2);?>%</strong></td>
                           </tr>
                ...
                <td><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><strong><?php echo $totalLoginReject;?></strong></td>
                           </tr>
                           <tr>
                            <td><strong><?php echo round($totalpercentageLoginReject/$recordcnt,2);?>%</strong></td>
                           </tr>
                ...
                <td><table border="0" cellspacing="0" width="100%">
                        <tr>
                            <td><strong><?php echo $totalCanclledAUD;?></strong></td>
                           </tr>
                           <tr>
                            <td><strong><?php echo round($totalpercentageCanclledAUD/$recordcnt,2);?>%</strong></td>
                           </tr>
