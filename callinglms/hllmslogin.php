...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	if(($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.161.193.191" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="122.176.54.194" || $IP=="182.71.109.218" || $IP=="122.177.21.156" || $IP=="113.193.239.185"  || $IP=="122.176.77.240" || $IP=="122.176.77.79" || $IP=="122.180.253.3" || $IP=="122.176.77.239" || $IP=="203.122.21.163" || $IP=="185.93.231.12" || $IP == "122.176.68.155" || $IP == "1.22.91.116" || ($getIPNum>0)))
	{
		...
		{
			redirectURL(SITE_URL."hllmslogin.php?msg=".urlencode("Please enter correct username!"));
		}
		if(!$objAdmin->fun_check_pwd_admin_existance($adminPass))
		{
			redirectURL(SITE_URL."hllmslogin.php?msg=".urlencode("Your password does not match with our record. Please enter correct password!"));
		}
...
//				$sqlTrackreult= $obj->fun_db_query($sqlTrackBidder);
				...
				if($_SESSION['leadidentifier']=="hlallocatelms" || $_SESSION['leadidentifier']=="hluncalledlms"  || $_SESSION['leadidentifier']=="hlallocatelmsCitywise")
				{
					header("Location:hlloanlms_index.php");
				}
				else if($_SESSION['leadidentifier']=="hlbtlms")
				{
					header("Location:hloanbtlms_index.php");
				}
				else
				{
					header("Location:hllmslogin.php");
				}
			}
		}
	}
        ...
        <tr>
          <td height="156" align="center" valign="middle" background="../images/login-form-login-bg.gif"><form method="post" action="" onSubmit="return validateMe(this);">
              <table width="250" border="0" cellpadding="4" cellspacing="0">
                ...
                <tr>
                  <td width="50%" class="style1">Username</td>
