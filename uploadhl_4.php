...
$filename = $RequestID."_".$filename;
  ...
  if(move_uploaded_file($tmpfile, "uploaded_files/".$filename)){
    	$status='<span style="color:#FF0000; font-size:10px;">Success</span>';
	
	$checkSql = "select DocID  from docs_uploaded_hl where RequestID='".$RequestID."' and Doc_Name='".$Doc_Name."'";
	...
	if($numRows>0)
	{
		...
		Mainupdatefunc ('docs_uploaded_hl', $DataArray, $wherecondition);
	}
	else
	{
		$data = array("RequestID"=>$RequestID , "Doc_Name"=>$Doc_Name , "Filename"=>$filename);
		$table = 'docs_uploaded_hl';
  ...
  echo returnStatus($status);
	...
	function returnStatus($status){
		return "<html><body><script type='text/javascript'>function init(){if(top.uploadComplete_4) top.uploadComplete_4('".$status."');}window.onload=init;
	</script></body></html>";
	}
