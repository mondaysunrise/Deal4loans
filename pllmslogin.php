...
if(($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="180.188.224.34" || $IP=="122.161.193.191" || $IP=="122.160.74.241" || $IP=="122.160.74.235" || $IP=="182.73.4.60" || $IP=="182.71.109.218" || $IP=="122.176.54.210" || $IP=="1.22.91.57" || $IP=="122.180.253.3" || $IP=="122.176.68.155"))
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		$stmt = $pdo->prepare('SELECT * FROM Bidders WHERE Email = :email and  PWD = :pwd and Reply_Type=1 and Global_Access_ID=1');
		...
		if($num_rows>0)
			...
			ExecQuery($sqlTrackBidder);
				...
				$strDir = dir_name();
					...
					if($BidderID==2748)
					{
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."pllms60_index.php");
						exit;
					
					}
					elseif($BidderID==65)
					{
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."pllms-incomingcall-view.php");
						exit;			
					}
					elseif($BidderID==3621)
					{
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."pllms615_index.php");
						exit;			
					}
					else
					{
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."pllms_index.php");
						exit;
					}				
				}				
		}
...
<div id="dvMainbanner" style="padding-top:40px;">
