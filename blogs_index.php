<?php
	...
	$FeedbackClause="";
	$OrderBy=" order by Req_Message.Dated desc";
...
{
	var gifName = form;
	document.frmsearch.action="blogs_index.php?search=y"+gifName;
...
function MM_jumpMenu(targ,selObj,restore){ //v3.0
 ...
 <form name="frmsearch" action="blogs_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//		echo $strSQL;
	...
	if($search=="y")
	{
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Message.Dated)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(Req_Message.Dated)<='".strtotime($max_date)."'";
		}
		else if($min_date!="" and $max_date=="")
		{
			$min_date=$min_date." 00:00:00";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Message.Dated)>='".strtotime($min_date)."'";
		}
		else if($min_date=="" and $max_date!="")
		{
			$max_date=$max_date." 23:59:59";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Message.Dated)<='".strtotime($max_date)."'";
		}
		else
		{
			$search_date=$search_date." and Req_Message.Dated!=''";
		}
 ...
 <?
		$search_qry="Select * from Req_Message where Is_Verified=0   ".$search_date ;
		$search_qry=$search_qry.$FeedbackClause;
		

		$qry="Select * from Req_Message where Is_Verified=0  ".$search_date  ;
   ...
   <tr>
    ...
    <td class="bodyarial11"><? echo $row["Subject"]; ?></td>
     <td class="bodyarial11"><? echo $row["Message"]; ?></td>
      ...
      <td class="bodyarial11"><a href="checkblog.php?id=<?php echo $row["PostID"]; ?>" target="_blank">View</a></font></td>
   </tr>
	<?	
					$i=$i+1;
		}
