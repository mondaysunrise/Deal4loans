<?php
  ...
  $val = "Req_Loan_Car";
	...
	$FeedbackClause="";
	//$OrderBy=" order by Req_Loan_Car.Dated desc";
...
{
	var gifName = form;
	document.frmsearch.action="cllms_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2010-02-02")
	{
		alert("Sorry!!!! Your minimum date is 2008-02-02.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function submitform()
{
 ...
 <br>
  <table width="712" border="0">
   <tr><td align="right"><a href="commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=3" target="_blank">today's Report</a></td></tr>
 <tr><td align="center" width="100%">
 <div align="center">
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <form name="frmsearch" action="cllms_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">From 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2010-02-01"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select FeedbackID from Req_Feedback_CL where AllRequestID=".$RequestID." and BidderID=".$_SESSION['BidderID']." AND Reply_Type=3");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update Req_Feedback_CL Set Feedback='".$Feedback."' ";
			$strSQL=$strSQL."Where FeedbackID=".$row["FeedbackID"];
		}
		else
		{
			$strSQL="Insert into Req_Feedback_CL(AllRequestID, BidderID, Reply_Type , Feedback) Values (";
			$strSQL=$strSQL.$RequestID.",".$_SESSION['BidderID'].",'3','".$Feedback."')";
		}
...
//		echo $strSQL;
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (Req_Feedback_CL.Feedback IS NULL OR Req_Feedback_CL.Feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND Req_Feedback_CL.Feedback='".$varCmbFeedback."' ";
		}
 ...
 <? 
	...
	if($_SESSION['BidderID']==1306)
	{
		/*$qry="SELECT Dated,RequestID,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID,Net_Salary from z2v_transactions,Req_Loan_Car LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Car.RequestID and Req_Feedback.BidderID='".$_SESSION['BidderID']."' WHERE 
(Req_Loan_Car.RequestID = LEFT(z2v_transactions.client_transaction_id, 6) and  
((Req_Loan_Car.Dated Between '".$min_date."' and '".$max_date."') or (Req_Feedback.Followup_Date Between '".$min_date."' and '".$max_date."') ) and z2v_transactions.viewable='viewed' and z2v_transactions.verified=0 and Req_Loan_Car.Allocated=0)"; */

$qry="SELECT RequestID,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID from Req_Loan_Car LEFT OUTER JOIN Req_Feedback_CL ON Req_Feedback_CL.AllRequestID= Req_Loan_Car.RequestID AND Req_Feedback_CL.BidderID= '".$_SESSION['BidderID']."' WHERE (Req_Loan_Car.Allocated=0 and (Req_Loan_Car.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and (DATE_SUB( NOW() , INTERVAL '00:07' HOUR_MINUTE ) >= Req_Loan_Car.Dated))";
...
if($_SESSION['BidderID']==1306)
	{
		 //$qry="SELECT RequestID,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID from Req_Loan_Car LEFT OUTER JOIN Req_Feedback_CL ON Req_Feedback_CL.AllRequestID= Req_Loan_Car.RequestID AND Req_Feedback_CL.BidderID= '".$_SESSION['BidderID']."' WHERE ( Req_Loan_Car.Is_Valid=0 and Req_Loan_Car.Allocated=0 and (Req_Loan_Car.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and (DATE_SUB( NOW() , INTERVAL '00:07' HOUR_MINUTE ) >= Req_Loan_Car.Dated))";
		 $qry="SELECT RequestID,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID from Req_Loan_Car LEFT OUTER JOIN Req_Feedback_CL ON Req_Feedback_CL.AllRequestID= Req_Loan_Car.RequestID AND Req_Feedback_CL.BidderID= '".$_SESSION['BidderID']."' WHERE ( Req_Loan_Car.Allocated=0 and (Req_Loan_Car.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback_CL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and (DATE_SUB( NOW() , INTERVAL '00:07' HOUR_MINUTE ) >= Req_Loan_Car.Dated))";
	}
...
<!--///////////////////////-->
<tr>
<td class="bodyarial11"><a href="cleditlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<td class="bodyarial11">
	<?php list($dated,$time) = split(' ', $row["Dated"]); ?><?php echo $dated;?></td>
<?
	    if($row["City"]=="Others")
        {
            $City= $row["City_Other"];
	...
	<td><form method="post" name="bidder_array" action=""><? list($FinalBidder,$finalBidderName)= getBiddersList("Req_Loan_Car",$row["RequestID"],$City);
   for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo getJumpMenu("cllms_index.php",$row["RequestID"],"3",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
...
onclick="window.open('http://www.deal4loans.com/get_complete_details.php?mob=<? echo urlencode($row['Mobile_Number']); ?>&pt=Req_Loan_Car')" >View</a></td>
</tr>
