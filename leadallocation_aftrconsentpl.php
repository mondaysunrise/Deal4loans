...
{
	//echo "hello";
	$conpprosuct=ExecQuery("select * from  Req_Compaign Where Bank_Name='PersonalLoan' and Compaign_ID=4037");
	...
	{
		$qry1= "select AllRequestID from Req_Feedback_Bidder_PL1 Where ((Consent=1 and final_allocate=0) and (DATE_SUB( NOW() , INTERVAL '00:15' HOUR_MINUTE ) >= Allocation_Date) and (Allocation_Date>='".$today_date."')) group by AllRequestID";
	}
	else
	{
		$qry1= "select AllRequestID from Req_Feedback_Bidder_PL1 Where ((Consent=1 and (DATE_SUB( NOW() , INTERVAL '00:15' HOUR_MINUTE ) >= Allocation_Date) and final_allocate=0) and (Allocation_Date>='".$today_date."' )) group by AllRequestID";
	}
...
if($recordcount>0)
{
	while($cust=mysql_fetch_array($result))
	{
		...
		//to get city
 	//"Select City, City_Other,source from Req_Loan_Personal Where RequestID=".$AllRequestID;
	 	$cust_city=ExecQuery("Select City, City_Other,source from Req_Loan_Personal Where RequestID=".$AllRequestID);
        ...
        else
        {
		 ...
		 $Customer_Source= $custrow["source"];
		//end to get city
echo $AllRequestID."--".$Customer_City;
		
		leadallocation($AllRequestID, $Customer_City,$Customer_Source);
	}
}
}
...
function leadallocation($LeadID, $Customer_City, $Customer_Source)
{
	echo $LeadID."----".$Customer_City;
	echo "<br>";
	echo $today = date('Y-m-d');

$qry2= "select * from Req_Feedback_Bidder_PL1 Where (Consent=1 and final_allocate=0 and AllRequestID='".$LeadID."') order by Allocation_Date DESC Limit 0,5";
	...
	while($cust2=mysql_fetch_array($result2))
		{
			...
			$final_allocation="INSERT into Req_Feedback_Bidder_PL (AllRequestID, BidderID, Reply_Type, Allocation_Date) VALUES ('".$AllRequestID."','".$BidderID."','".$ProductType."', Now())";
...
echo "<br><br>";
	...
	if($Bidderrecorcount>0)
	{
		...
		for($i=0;$i<$Bidderrecorcount;$i++)
		{
			...
			if(($ShowDate > $StartTime && $ShowDate < $EndTime)	|| $Customer_Source=="SMS_Lead_Appointment_Model")
			{
	...
	if(count($BankNameID)>0 && $BankNameID[0]>0)
	{
		if($ProductType ==2 || $ProductType ==1 || $ProductType ==5 || $ProductType ==4 || $ProductType ==3 )
		{
			...
			if(strlen($GetBidderID)>0)
			{
				...
				{
					getBidderContactDetailsToCustomersWishfin($ProductType,$GetBidderID,$LeadID);
				}else
				{
...
if($strbidderid=="3003" || $strbidderid=="3002" || $strbidderid=="5566" || $strbidderid=="3654"){ $cde="TC1";}
	if($strbidderid=="4407" || $strbidderid=="3654" || $strbidderid=="5203" || $strbidderid=="5889"){ $cde="TC2"; }
	if($strbidderid=="5920" || $strbidderid=="3801"){ $cde="TC3"; }
				$SMSMsgforkotak=$SMSMsgforkotak."(".$ctr.") ".$Name."-".$Phone.",Sal- ".$Net_Salary.",Co- ".$Company_Name.",Acc-".$Primary_Acc.",".$cde;
                                //Update 22-02-2017
			if($strbidderid=="5168")
                                { $callername="Vitthal Pawar";}
			elseif($strbidderid=="4090")
				{	$callername="Nikhil";}
                        elseif($strbidderid=="4091")
				{	$callername="Wasim Patel";}        
                        elseif($strbidderid=="4083")
				{	$callername="Kanchan";}        
                        elseif($strbidderid=="4087")
				{	$callername="Anand M";} 
                        elseif($strbidderid=="4088")
				{	$callername="Kalpana V";}        
                        elseif($strbidderid=="4089")
				{	$callername="Prashanti";}        
                        elseif($strbidderid=="6113")
				{	$callername="Lavanya";}        
                        elseif($strbidderid=="6645")
				{	$callername="Shristi";} 
                        elseif($strbidderid=="6646")
				{	$callername="Saurabh";} 
                        elseif($strbidderid=="6647")
				{	$callername="Sudha A G";} 
                        elseif($strbidderid=="6648")
				{	$callername="Rahul Kumar";} 
                        elseif($strbidderid=="6649")
				{	$callername="AJIT KUMAR BAL";} 
                        elseif($strbidderid=="6650")
				{	$callername="Gangadhar";} 
                        elseif($strbidderid=="6651")
				{	$callername="VANITHA K";} 
                        elseif($strbidderid=="6652")
				{	$callername="RADHEESH";} 
                        elseif($strbidderid=="6653")
				{	$callername="Preeti Singh";}        
                        elseif($strbidderid=="6654")
				{	$callername="Imran Khan";}
                        elseif($strbidderid=="6655")
				{	$callername="Priyanka Mishra";}
                        elseif($strbidderid=="6656")
				{	$callername="Ganesh Shinde";}
                        elseif($strbidderid=="5937")
				{	$callername="Sunil";}
				else
				{	$callername="";			}

				$SMSMsgforhdfc=$SMSMsgforhdfc."(".$ctr.") ".$Name."-".$Phone.",Sal- ".$Net_Salary.",Co- ".$Company_Name.",Acc-".$Primary_Acc.",".$callername;
...
$squnce1 = $total_no_agents - $last_allocated_to;
	...
	elseif($strbidderid=="5168" || $strbidderid=="4090" || $strbidderid=="5937")
	{	if(strlen(trim($strmobile_no)) > 0)
			{
			 SendSMSforLMS($message.$SMSMsgforhdfc, $strmobile_no);
			}
	}
        
        elseif($strbidderid=="4091" || $strbidderid=="4083" || $strbidderid=="4087" || $strbidderid=="4088" || $strbidderid=="4089" || $strbidderid=="6113" || $strbidderid=="6645" || $strbidderid=="6646" || $strbidderid=="6647" || $strbidderid=="6648" || $strbidderid=="6649" || $strbidderid=="6650" || $strbidderid=="6651" || $strbidderid=="6652" || $strbidderid=="6653" || $strbidderid=="6654" || $strbidderid=="6655" || $strbidderid=="6656")
	{	
        if(strlen(trim($strmobile_no)) > 0)
            {
             SendSMSforLMS($message.$SMSMsgforhdfc, $strmobile_no);
...
/***************************************************************/
//SMS to Wishfin Customers
/****************************************************************/
function getBidderContactDetailsToCustomersWishfin($strProduct,$strbidderid,$leadid)
{
	$table_NAme=getTableName($strProduct);
	$strmobileSQL = "SELECT Mobile_Number,Name,City FROM ".$table_NAme." WHERE (RequestID=".$leadid.")";
	//echo "bidder contact".$strmobileSQL."<br><br>";
	...
	$Phone=$Mobile_number;
	$GetBidderID = explode(',',$strbidderid);
	$SMSMessage="Dear Customer,Following are contact details of your chosen Banks @ Wishfin: ";
	$SMSMessageBidders="";
	$sms_4barclays="";
	$ctr=1;
	$mvarType = $strProduct;
	...
	if(strlen(trim($SMSMessageBidders))>0)
	{
		if(strlen(trim($Phone)) > 0)
		{
			SendSMSforWishfin($SMSMessage.$SMSMessageBidders, $Phone);  
		}
...
/***************************************************************/
//SMS to Wishfin Customers END here
/****************************************************************/

/*function sendleadintimate($bidderid,$Customer_City,$LeadID)
{
	$custdtls=ExecQuery("select City,City_Other,Name,Mobile_Number,Email,Company_Name,Annual_Turnover,Loan_Amount from Req_Loan_Personal Where (RequestID=".$LeadID.")");
	$snd= mysql_fetch_array($custdtls);

	$toemailid = "rohan.bhonsle@bajajfinserv.in";
	$ccemailid = "Balbir.singh@deal4loans.com, manav.seth@bajajfinserv.in";

if($snd["Annual_Turnover"]==1) { $annual_turnover="0-40 Lacs"; } 
else if($snd["Annual_Turnover"]==2) { $annual_turnover="1Cr - 3Crs"; } 
else if($snd["Annual_Turnover"]==3) { $annual_turnover="3Crs & above"; } 
else if($snd["Annual_Turnover"]==4) { $annual_turnover="40Lacs To 1 Cr"; } 
else { $annual_turnover="";  }

	if(strlen($toemailid)>1)
	{
	$msg_send='<p style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:13px;">Hi,<br />
  PFB the detail of Fresh Lead generated :-</p>
<p style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:13px;">Customer  Name : '.$snd["Name"].'<br />
  Mobile  No : '.$snd["Mobile_Number"].'<br />
  E-Mail  ID : '.$snd["Email"].'<br />
  City : '.$snd["City"].'<br />
  Other City : '.$snd["City_Other"].'<br />
   Company  Name : '.$snd["Company_Name"].'<br />
  Turnover : '.$annual_turnover.'<br />
  Loan Amount: '.$snd["Loan_Amount"].'</p>';
$headers  = 'From: Deal4loans <no-reply@deal4loans.com>' . "\r\n";
$headers .= "Return-Path: <no-reply@deal4loans.com>\r\n";  // Return path for errors
$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
...
mail($toemailid,'Business loan lead from deal4loans', $msg_send, $headers);
