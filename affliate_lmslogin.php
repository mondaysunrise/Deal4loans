...
$Msg = "";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		$PWD = $_POST["PWD"];
		$pdo = db_connect_PDO();
		$stmt = $pdo->prepare('SELECT * FROM Bidders WHERE Email = :email and  PWD = :pwd and BidderID=1624');
		...
		if($num_rows>0)
			...
			setSessionBidder($Email, $row);
					...
					if($BidderID==1624)
					{
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."affliate_updatelms_index.php");
						exit;					
					}
					else
					{
