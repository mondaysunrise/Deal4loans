...
if(strlen(trim($get_requestid))>0)
	{
	 ...
	 list($num_rows,$row)=MainselectfuncNew($result,$array = array());
		...
		$DataArray = array("comment_section"=>$get_comment_section, "axis_executive_name"=>$get_exec_name);
		...
		else
		{
			...
			$dataInsert = array("AllRequestID"=>$get_requestid, "BidderID"=>$get_bidderid, "Reply_Type"=>$get_product, "comment_section"=>$get_comment_section, "axis_executive_name"=>$get_exec_name);
$table = 'Req_Feedback';
