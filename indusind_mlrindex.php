<?php
	...
	if($_REQUEST['cityS']==4083)
	{
	$city = "'Delhi','Gaziabad','Noida','Gurgaon','Faridabad','Sahibabad'";
	}
	else if($_REQUEST['cityS']==4084)
	{
	$city = "'Kolkata'";
	}
	else if($_REQUEST['cityS']==4089)
	{
	$city = "'Hyderabad'";
	}
	else if($_REQUEST['cityS']==4085)
	{
	$city = "'Jaipur'";
	}
	else if($_REQUEST['cityS']==4086)
	{
	$city = "'Chandigarh'";
	}
	else if($_REQUEST['cityS']==4087)
	{
	$city = "'Bangalore'";
	}
	else if($_REQUEST['cityS']==4090)
	{
	$city = "'Mumbai','Navi mumbai','Thane'";
	}
	else if($_REQUEST['cityS']==4088)
	{
	$city = "'Chennai'";
	}
	else if($_REQUEST['cityS']==4091)
	{
	$city = "'Pune'";
	}
	else if($_REQUEST['cityS']==4092)
	{
	$city = "'Ahmedabad'";
	}
	else
...
{
	var gifName = form;
	document.frmsearch.action="indusind_mlrindex.php?search=y"+gifName;
...
if(document.frmsearch.min_date.value<"2014-08-22")
	{
		alert("Sorry!!!! Your minimum date is 2014-08-22.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function insertData(id)
		...
		var queryString = "?comment_section=" + get_comment_section + "&get_requestid=" + get_requestid + "&get_product=" + get_product + "&get_bidderid=" + get_bidderid;
				...
				//alert(queryString); 
				ajaxRequest.open("GET", "insert_comment_lmsindus.php" + queryString, true);
...
<tr>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#45B2D8">
		<tr><td style="padding-top:5px; font-size:13px;">&nbsp;&nbsp; <a  href="/bidders_consolidate.php" style="color:#FFFFFF;"><b>Main</b></a>
 </td></tr>
      <tr>
...
<tr><td>&nbsp;</td></tr>
 ...
 <form name="frmsearch" action="indusind_mlrindex.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="85%">
     ...
     <td width="30%" valign="middle" class="style1">City:</td>
		...
		<option value="4092" <?php if($cityS==4092) echo "selected"; ?>>Ahmedabad</option>
        <option value="4087" <?php if($cityS==4087) echo "selected"; ?>>Bangalore</option>
		<option value="4086" <?php if($cityS==4086) echo "selected"; ?>>Chandigarh</option>
        <option value="4088" <?php if($cityS==4088) echo "selected"; ?>>Chennai</option>
        <option value="4089" <?php if($cityS==4089) echo "selected"; ?>>Hyderabad</option>
        <option value="4083" <?php if($cityS==4083) echo "selected"; ?>>Delhi</option>
        <option value="4090" <?php if($cityS==4090) echo "selected"; ?>>Mumbai</option>
        <option value="4091" <?php if($cityS==4091) echo "selected"; ?>>Pune</option>
        <option value="4085" <?php if($cityS==4085) echo "selected"; ?>>Jaipur</option>
        <option value="4084" <?php if($cityS==4084) echo "selected"; ?>>Kolkata</option>
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select ingvyasyaplid from indusbank_exclusive_leads where indusbnkid=$RequestID");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update indusbank_exclusive_leads Set feedback='".$Feedback."'";
			$strSQL=$strSQL."Where indusbnkid=".$row["indusbnkid"];
		}
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (indusbank_exclusive_leads.feedback IS NULL OR indusbank_exclusive_leads.feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND indusbank_exclusive_leads.feedback='".$varCmbFeedback."' ";
		}
	?>
 ...
 if(strlen($city)>0)
		{
			$cityclause="and  indusbnk_city in (".$city.")";
		}
		else
		{
			$cityclause="";
		}
	$search_qry="SELECT * FROM indusbank_exclusive_leads where (indusbnk_status=1 ".$cityclause." and (indusbnk_dated  Between '".($min_date)."' and '".($max_date)."'))";
		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by indusbank_exclusive_leads.indusbnk_mobileno";
		$search_qry=$search_qry." order by indusbank_exclusive_leads.indusbnk_dated DESC";

		$qry="SELECT indusbnkid FROM indusbank_exclusive_leads where (indusbnk_status=1 ".$cityclause." and (indusbnk_dated  Between '".($min_date)."' and '".($max_date)."'))";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by indusbank_exclusive_leads.indusbnk_mobileno";
	//echo"hello -  ".$qry."<br>";
 ...
 <td width="71" align="center" bgcolor="#FFFFFF" class="style2">Feedback</td>
	...
	<?
		...
		$qry="SELECT * FROM indusbank_exclusive_leads where (indusbnk_status=1 ".$cityclause." and (indusbnk_dated  Between '".($min_date)."' and '".($max_date)."'))";				
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by indusbank_exclusive_leads.indusbnk_mobileno";
		$qry=$qry." order by indusbank_exclusive_leads.indusbnk_dated DESC";
...
//echo $qry;
		...
		{																												
	?>
	<input type="hidden" name="requestid_<? echo $i;?>" id="requestid_<? echo $i;?>" value="<? echo $row["indusbnkid"];?>">
 ...
 ?>
	 <? echo $row["indusbnk_name"]; ?></td>
      <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["indusbnk_mobileno"]; ?></td>

     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["indusbnk_monthlyincome"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["indusbnk_companyname"]; ?></td>
	   <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["indusbnk_loanamount"]; ?></td>	
     <td align="center" bgcolor="#DFF6FF" class="style3"><? if($row["indusbnk_occupation"]==0) { echo "Self Employed"; } else { echo "Salaried"; }?></td> 
   	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("indusind_mlrindex.php",$row["indusbnkid"],"1",$row["feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>		</td>
 ...
 <br>
<form name="frmdownload" action="downloadindusind.php" method="post">
