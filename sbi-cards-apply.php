...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	$source = "sbi_cards_apply";
	$section = isset($_POST["section_value"]) ? $_POST["section_value"] : '';
//	$section = $_REQUEST["section"];
	//$source = isset($_POST["source"]) ? $_POST["source"] : 'sbi_cards_apply';
	...
	if($_POST['request_from'] == 'saveUserData'){
		...
		if($alreadyExist>0){
			...
			if(empty($RequestID)){
				...
				$form_step=1;
				$InsertProductArr = array("Name"=>$first_name, "Email"=>$Email, "City"=>$City, "Net_Salary"=>$IncomeAmount, "Mobile_Number"=>$Phone, "Reference_Code"=>$OtpVal, "Dated"=>$Dated, "source"=>$source,"Updated_Date"=>$Dated, "DOB"=>$dob_default, "Employment_Status"=>$CurrentEmployment_deafult, "applied_card_name"=>'SBI SimplySave', "Section"=>$section, "form_step"=>$form_step);
				//print_r($InsertProductArr);
				...
				$UpdateProductArr = array("Name"=>$first_name, "Email"=>$Email, "City"=>$City, "Net_Salary"=>$IncomeAmount, "Mobile_Number"=>$Phone, "Reference_Code"=>$OtpVal);
				$UpdateProductWhereCond ="(RequestID='".$RequestID."')";
...
//						console.log(response);
				...
				$('#CurrentEmployment').on('keydown',function(){
