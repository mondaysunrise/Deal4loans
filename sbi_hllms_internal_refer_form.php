...
$bidid = isset($_SESSION['BidderID'])? $_SESSION['BidderID'] : '';
if($bidid != '6319'){
	echo "You are not authorised to access this";
	exit;
}
...
if ($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="61.246.3.127" || $IP=="122.160.30.168" || $IP=="122.160.74.241" || $IP=="182.71.109.218" || $IP=="113.193.239.185" || $IP=="122.180.253.3"  || $IP=="122.176.77.240" || $IP=="122.176.77.79" || $IP=="122.176.68.155")
{
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (strlen($hlname) > 0 && strlen($hlmobile) > 0) {
		...
		$getdetails="select RequestID From Req_Loan_Home Where (Mobile_Number not in (9971396361,9811215138,9811555306) and Mobile_Number='".$hlmobile."' and Updated_Date between '".$days30datetime."' and '".$currentdatetime."') order by RequestID DESC";
		$resultdt = d4l_ExecQuery($getdetails);
		//$hlrow = $obj->fun_db_fetch_rs_object($resultdt);
		$hlrow = d4l_mysql_fetch_array($resultdt);
		$alreadyExist= $hlrow['RequestID'];
		if($alreadyExist>0)
		{
		}
		else
		{
			$updatelead = "INSERT INTO Req_Loan_Home set CC_Bank='$which_khatha',Existing_Bank='$Existing_Bank',Existing_Loan='$Existing_Loan',Existing_ROI='$Existing_ROI',Property_Value='$hlProperty_Value',Co_Applicant_Name='$hlCo_Applicant_Name',Co_Applicant_DOB='$hlCo_Applicant_DOB',Co_Applicant_Income='$hlCo_Applicant_Income' ,Co_Applicant_Obligation='$hlCo_Applicant_Obligation',Total_Obligation='$hlTotal_Obligation',Tataaig_Home='$tataaig_home', Tataaig_Health='$tataaig_health',Tataaig_Auto='$tataaig_auto',Company_Name='$hlcompany_name', Name='$hlname',Email='$hlemail', Mobile_Number='$hlmobile',Employment_Status='$hlemployment_status', Std_Code='$hlstd_code',Landline='$hllandline',Std_Code_O='$hlstd_code_o',Landline_O='$hllandline_o',City='$hlcity',City_Other='$hlcity_other',Net_Salary='$hlnet_salary',Residence_Address='$hlresiaddress',Pincode='$hlpincode',Property_Identified='$hlproperty_identified',Loan_Time='$hlloantime',Loan_Amount='$hlloanamt',Budget='$hlbudget',Property_Loc='$hlproperty_loc',DOB='$hldob', Add_Comment='$hladd_comment', Dated=Now(),Updated_Date=Now(), source='$source',Referrer='$reference_from'";
			$result = d4l_ExecQuery($updatelead);
			$RequestID = d4l_mysql_insert_id();
			
			/* Code to avoid to send Reference Lead to Calling LMS Start*/
			$getBidderIDSql = "SELECT GROUP_CONCAT(BidderID) FROM Bidders_List WHERE BidderID IN (SELECT BidderID FROM `Bidders` WHERE `Global_Access_ID` = '6319') AND City LIKE '%".$hlcity."%' AND Restrict_Bidder = 1";
			$getBidderIDResult = d4l_ExecQuery($getBidderIDSql);
			$getBidderIDRow = d4l_mysql_fetch_array($getBidderIDResult);
			$Bidderid_Details = $getBidderIDRow[0];

			$updateBidderidDetailsSql ="Update Req_Loan_Home SET Bidderid_Details = '".$Bidderid_Details."', Allocated = 2 WHERE RequestID = '".$RequestID."'";
			$updateBidderidDetailsResult = d4l_ExecQuery($updateBidderidDetailsSql);
			/* Code to avoid to send Reference Lead to Calling LMS End*/
		}
	}
}
?>
...
<style>
	...
	<div class="content_right" style="background:#FFFFFF; ">
		...
		<form name="loan_form" method="post" action="<? echo $_SERVER['PHP_SELF']; ?>" onSubmit="return chkhomeloan(document.loan_form);">
            ...
            <table style='border:1px dotted #9C9A9C;'width="700" height="80%" align="center" >
                ...
                <tr>
                    <td ><b>City</b></td>
                    <td><select size="1" name="hlcity" > <?= plgetCityList($City) ?></select></td>
                    <td ><b>Pincode</b></td>
				...
				<tr>
					<td style="border:1px solid black;" colspan="4" bgcolor="#DAEAF9" align="center" class="fontstyle" ><b>Employment Details</b></td>
				</tr>
                <tr>
                ...
                <tr>
					<td><b>Company Name</b></td><td><input type="text" name="hlcompany_name" id="hlcompany_name"></td><td colspan="2"></td>
				</tr>
                <tr>
                    <td colspan="2">&nbsp;</td>
                ...
                <tr>
                    ...
                    <td ><input type="radio" name="hlproperty_identified" value="1">Yes<br><input type="radio" name="hlproperty_identified" value="0">No</td>
                ...
                <tr>
                    <td style="border:1px solid black;" colspan="4" bgcolor="#DAEAF9" align="center" class="fontstyle" ><b>Co applicant Details</b></td>
				</tr>
                <tr>
                ...
                <tr>
					<td style="border:1px solid black;" colspan="4" bgcolor="#DAEAF9" align="center" class="fontstyle" ><b>Add Comment</b></td>
				</tr>
                <tr>
                    ...
                    <td>
						<select name="source" id="source">
							<option value="HLReferralProgram">Referral Program</option>
							<option value="HLInternalReference">Internal Reference</option>
						</select>
					</td>
					-->
                </tr>
                ...
                <tr>
                    ...
                    <td>
						<select size="1" name="reference_from" >
							<option value="">Select</option>
							<option value="Arvind Kumar Rai">Arvind Kumar Rai</option>
							<option value="Desbasish Paul">Desbasish Paul</option>
							<option value="Manish Kumar Jha">Manish Kumar Jha</option>
							<option value="Jitendra Yadav">Jitendra Yadav</option>
							<option value="Mahendra Kumar">Mahendra Kumar</option>
							<option value="Satendra Kumar">Satendra Kumar</option>
							<option value="Neelam Pal">Neelam Pal</option>
							<option value="Ramanjeet Kaur">Ramanjeet Kaur</option>
							<option value="Saraswati Singh">Saraswati Singh</option>
							<option value="Sweety Soni">Sweety Soni</option>
							<option value="Harshita Jain">Harshita Jain</option>
							<option value="Kunal Kishor">Kunal Kishor</option>
							<option value="Reetika Pokhriyal">Reetika Pokhriyal</option>
