...
$filename = $RequestID."_".$filename;
  ...
  if(move_uploaded_file($tmpfile, "upload/".$filename)){
	...
	if($numRows>0)
	{
		$DataArray = array("Telephone_Bill"=>$filename);
  ...
  echo returnStatus($status);
	...
	function returnStatus($status){
		return "<html><body><script type='text/javascript'>function init(){if(top.uploadComplete_11) top.uploadComplete_11('".$status."');}window.onload=init;
	</script></body></html>";
	}
