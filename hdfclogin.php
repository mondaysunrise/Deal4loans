...
$Msg = "";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST')
	{
		...
		$PWD = $_POST["PWD"];
		$pdo = db_connect_PDO();
		$stmt = $pdo->prepare('SELECT * FROM Bidders WHERE Email = :email and  PWD = :pwd and BidderID in (1900,2663)');
		...
		if($num_rows>0)
			...
			setSessionBidder($Email, $row);
			 ...
			 /* Dump Resultset */
					...
					{
						$_SESSION['product']= 1;
						header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."bidders_consolidate12.php");
						exit;
					}	
					else
					{
