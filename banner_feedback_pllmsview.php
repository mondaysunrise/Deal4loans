...
{
	var gifName = form;
	document.frmsearch.action="banner_feedback_pllmsview.php?search=y"+gifName;
...
<tr>
 ...
 <form name="frmsearch" action="banner_feedback_pllmsview.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?  if(isset($joindate)) { $cmpdate=$joindate; } ?> 
 ...
 <? $Lmsbidder="";
 // fetch PL bidders
	$CheckSql = ExecQuery("select BidderID from Bidders where leadidentifier = 'plmainlms'");
		while($lmsbid=mysql_fetch_array($CheckSql))
		{
			$Lmsbidder[]=$lmsbid["BidderID"];
		}
		$Lmsbidderstr=implode(",",$Lmsbidder);
	 // fetch PL bidders end 
	if($comp_source=="AFL_MLR_OPTMED_PL" || $comp_source=="AFL_MLR_PROFILIAD_PL" || $comp_source=="AFL_MLR_AD2CLICK_PL" || $comp_source=="AFL_D4LMLR_LUCINI_PL")
		{
			$qry2="SELECT komli_uniqueid,Feedback,komli_date from komli_plcompaign LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= komli_plcompaign.komli_uniqueid AND Req_Feedback_PL.BidderID in (".$Lmsbidderstr.") WHERE ((komli_date Between '".$min_date."' and '".$max_date."') and vendor_name='".$comp_source."' and validated=1 and Req_Feedback_PL.Feedback not like '%Send now%')";
...
<td width="141" align="center" bgcolor="#FFFFFF" class="style2">Feedback</td>
  ...
  <?  
  	if($comp_source=="AFL_MLR_OPTMED_PL" || $comp_source=="AFL_MLR_PROFILIAD_PL" || $comp_source=="AFL_MLR_AD2CLICK_PL" || $comp_source=="AFL_D4LMLR_LUCINI_PL")
		{
	 $qry="SELECT komli_uniqueid,Feedback,komli_date from komli_plcompaign LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= komli_plcompaign.komli_uniqueid AND Req_Feedback_PL.BidderID in (".$Lmsbidderstr.") WHERE ((komli_date Between '".$min_date."' and '".$max_date."') and vendor_name='".$comp_source."' and validated=1 and Req_Feedback_PL.Feedback not like '%Send now%')";
...
if($recordcount>0)
		...
		{ ?>
 <tr>
 <td width="141" align="center" bgcolor="#DFF6FF" class="style3"><? echo $row2["komli_uniqueid"]; ?> </td>
 <td width="141"align="center" bgcolor="#DFF6FF" class="style3"><? if($row2["komli_uniqueid"]>0) { $plsql=ExecQuery("Select Name from Req_Loan_Personal Where (RequestID=".$row2["komli_uniqueid"].")"); $row=mysql_fetch_array($plsql);
 echo $row["Name"];  } ?></td>
<td width="141" align="center" bgcolor="#DFF6FF" class="style3"><? echo $row2["Feedback"]; ?></td>
  </tr>
   <? $i=$i+1; }  
		}?>
    </table>
