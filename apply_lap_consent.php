<?php
	...
	$lap_requestid = $_REQUEST['lap_requestid'];
	$lap_bank_name = $_REQUEST['lap_bank_name'];
	
if (strlen($lap_bank_name)>1 && $lap_requestid>1)
{
	$selqry="select Descr,Loan_Amount,Name,DOB,Company_Name,Net_Salary,City,Mobile_Number,Email,source from Req_Loan_Against_Property Where RequestID=".$lap_requestid;
	list($Numrows,$laprow)=MainselectfuncNew($selqry,$array = array());
	$countr = count($laprow)-1;
	$lap_banks=$laprow[$countr]['Descr'];
	$loan_amount = $laprow[$countr]["Loan_Amount"];
	$name = $laprow[$countr]["Name"];
	$DOB = $laprow[$countr]["DOB"];
	$company_name = $laprow[$countr]["Company_Name"];
	$salary = $laprow[$countr]["Net_Salary"]/12;
	$City = $laprow[$countr]["City"];
	$Mobile_Number = $laprow[$countr]["Mobile_Number"];
	$Email = $laprow[$countr]["Email"];
	$source = $laprow[$countr]["source"];

if(strlen($lap_banks)>1)
	{
		$newlap_banks= $lap_banks.",".$lap_bank_name;
		$wherecondition= "(Req_Loan_Against_Property.RequestID=".$lap_requestid.")";
		$dataarray=array("Descr"=>$newlap_banks);	
	}
	else
	{
		$dataarray=array("Descr"=>$lap_bank_name);
		$wherecondition= "(Req_Loan_Against_Property.RequestID=".$lap_requestid.")";
	}
	$rowcount=Mainupdatefunc("Req_Loan_Against_Property", $dataarray, $wherecondition);
}
?>
...
<div style="clear:both;"></div>
<div class="common-bread-crumb" style="margin:auto; width:74%; margin-top:70px; color:#0a8bd9;"><u><a href="index.php" class="text12" style="color:#0080d6;" >Home</a></u> > <a href="#"  class="text12" style="color:#0080d6;"><u>Loan Against Property</u></a> <span class="text12" style="color:#4c4c4c;">> Apply for Loan Against Property</span></div>
<div style="clear:both; height:15px;"></div>
<div style="width:995px;  margin:auto;">
<div id="content"><form name="cibil_form" method="post" action="apply_lap_consent_thank.php"  onSubmit="return ckhcreditcard(document.cibil_form); ">
<input type="hidden" name="requestid" id="requestid" value="<? echo $lap_requestid; ?>"/> 
...
<input type="hidden" name="lap_bank_name" id="lap_bank_name" value="<? echo $lap_bank_name; ?>"/>
<input type="hidden" name="Mobile_Number" id="Mobile_Number" value="<? echo $Mobile_Number; ?>"/>
<table align="center"  cellpadding="5" cellspacing="0"  width="100%">
<tr><td colspan="2"   bgcolor="#FFFFFF" class="heading_text" align="left">Fill in the below details to start your Loan Against Property application process</td></tr>
<tr>
  ...
  <td><input type="text" name="salary" id="salary" value="<? echo round($salary);?>" class="bajaj-fin_input"/></td></tr>
    ...
    <div class="widget">
      <table width="100%" align="center" cellpadding="0" cellspacing="0">
        <tr>
          <td width="58%" height="40" class="heading_text1" style="font-size:18px;"><span class="heading_text_b">Why <? echo $lap_bank_name; ?>?</span></td>
