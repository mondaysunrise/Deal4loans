...
{
	var gifName = form;
	document.frmsearch.action="bikelms_index.php?search=y"+gifName;
...
function submitform()
{
 ...
 <form name="frmsearch" action="bikelms_index.php?search=y" method="post" onSubmit="return chkform();">
   <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder"> <tr>
   ...
   <tr>
     ...
     <td width="80%">From 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="<?php echo date("Y-m-d"); ?>"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
	...
	</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	<strong>Net Salary</strong>&nbsp;&nbsp;&nbsp;		<select name="salaryrange" id="salaryrange">
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select FeedbackID from Req_Feedback where AllRequestID=".$RequestID." and BidderID=".$_SESSION['BidderID']." AND Reply_Type=3");		
		...
		else
		{
			...
			$strSQL=$strSQL.$RequestID.",".$_SESSION['BidderID'].",'10','".$Feedback."')";
		}
...
//	echo "in";
		...
		$qry="SELECT RequestID,Net_Salary,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID from Req_Loan_Bike LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Bike.RequestID AND Req_Feedback.BidderID= '".$_SESSION['BidderID']."' WHERE (Req_Loan_Bike.Allocated=0 and (Req_Loan_Bike.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."') and (DATE_SUB( NOW() , INTERVAL '00:07' HOUR_MINUTE ) >= Req_Loan_Bike.Dated) ".$salaryfilter." )";
...
if($_SESSION['BidderID']==3964)
	{
		 $qry="SELECT RequestID,Net_Salary,Mobile_Number,Name,City,City_Other,Dated,Feedback,Followup_Date,BidderID from Req_Loan_Bike LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Bike.RequestID AND Req_Feedback.BidderID= '".$_SESSION['BidderID']."' WHERE ( Req_Loan_Bike.Allocated=0 and (Req_Loan_Bike.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."') and (DATE_SUB( NOW() , INTERVAL '00:01' HOUR_MINUTE ) >= Req_Loan_Bike.Dated))";
	}
...
<!--///////////////////////-->
<tr>
<td class="bodyarial11"><a href="bikeeditlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
	...
	<form method="post" name="bidder_array" action=""><? list($FinalBidder,$finalBidderName)= getBiddersList("Req_Loan_Bike",$row["RequestID"],$City);
   for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo getJumpMenu("bikelms_index.php",$row["RequestID"],"10",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
