...
{
	var gifName = form;
	document.frmsearch.action="hdfcbt_view.php?search=y"+gifName;
...
function insertData(id)
		...
		var queryString = "?comment_section=" + get_comment_section + "&get_requestid=" + get_requestid + "&get_product=" + get_product + "&get_bidderid=" + get_bidderid;
				...
				//alert(queryString); 
				ajaxRequest.open("GET", "insert_comment_lmshdfcbt.php" + queryString, true);
...
<tr>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#45B2D8">
		<tr><td style="padding-top:5px; font-size:13px;">&nbsp;&nbsp; <a  href="/hdfc_index.php" style="color:#FFFFFF;"><b>Main</b></a>
 </td></tr>
      <tr>
...
<tr><td>&nbsp;</td></tr>
 ...
 <form name="frmsearch" action="hdfcbt_view.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td align="left" valign="middle" class="style1" width="17%"><input name="b122" type="button" class="buttonfordate" onClick="javascript:pedirFecha(max_date,'');" value="&lt; Insert"></td>
	   ...
	   <tr><td  align="center" height="30"><b>Feedback</b></td><td colspan="2"><select name="cmbfeedback" id="cmbfeedback" style="width:120px;">
   ...
   <tr>
	...
	{
		//echo "have enetered<br>";
		$strSQL="";
		$Msg="";

		$result = ExecQuery("select hdfcbtid from hdfc_balance_transfer_leads where hdfcbtid=$RequestID");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update hdfc_balance_transfer_leads Set Feedback='".$Feedback."'";
			$strSQL=$strSQL."Where hdfcbtid=".$row["hdfcbtid"];
		}
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (hdfc_balance_transfer_leads.Feedback IS NULL OR hdfc_balance_transfer_leads.Feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND hdfc_balance_transfer_leads.Feedback='".$varCmbFeedback."' ";
		}
 ...
 <? 
		...
		$search_qry="SELECT * FROM hdfc_balance_transfer_leads where (hdfcbt_city in ('Delhi','Gurgaon','Gaziabad','Faridabad','Noida','Jaipur','Chandigarh','Kolkata','Ahmedabad','Mumbai','Pune') and (hdfcbt_dated  Between '".($min_date)."' and '".($max_date)."'))";

		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by hdfc_balance_transfer_leads.hdfcbt_mobile";
		$search_qry=$search_qry." order by hdfc_balance_transfer_leads.hdfcbt_dated DESC";

		$qry="SELECT hdfcbtid FROM hdfc_balance_transfer_leads where (hdfcbt_city in ('Delhi','Gurgaon','Gaziabad','Faridabad','Noida','Jaipur','Chandigarh','Kolkata','Ahmedabad','Mumbai','Pune') and (hdfcbt_dated  Between '".($min_date)."' and '".($max_date)."'))";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by hdfc_balance_transfer_leads.hdfcbt_mobile";
...
//	echo "<br>";
 ...
 ?>
	  ...
	  <tr>
     <td colspan="6" style="border-bottom:1px solid #45B2D8;"><strong><? echo $startrow+1; ?> to <? echo min($startrow+$pagesize,$recordcount); ?> Out of <? echo $recordcount; ?> Records </strong>     </tr>
   ...
   <tr>
	...
	<?
		...
		$qry="SELECT * FROM hdfc_balance_transfer_leads where (hdfcbt_city in ('Delhi','Gurgaon','Gaziabad','Faridabad','Noida','Jaipur','Chandigarh','Kolkata','Ahmedabad','Mumbai','Pune') and (hdfcbt_dated  Between '".($min_date)."' and '".($max_date)."'))";
				
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by hdfc_balance_transfer_leads.hdfcbt_mobile";
		$qry=$qry." order by hdfc_balance_transfer_leads.hdfcbt_dated DESC";
...
//echo $qry;
		...
		{																												
	?>
	<input type="hidden" name="requestid_<? echo $i;?>" id="requestid_<? echo $i;?>" value="<? echo $row["hdfcbtid"];?>">
 ...
 ?>
	 <? echo $row["hdfcbt_name"]; ?></td>
      <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["hdfcbt_mobile"]; ?></td>

     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["hdfcbt_net_salary"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["hdfcbt_city"]; ?></td>
	  	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("hdfcbt_view.php",$row["hdfcbtid"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>		</td>
 ...
 <br>
<form name="frmdownload" action="downloadhdfcbt.php" method="post">
