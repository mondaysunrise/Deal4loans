...
{
	var gifName = form;
	document.frmsearch.action="icicipllms_adminview.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="icicipllms_adminview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select iciciallocateid from  icicipllms_allocation where cc_requestid=$RequestID  AND product=1");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update  icicipllms_allocation Set verifier_feedback='".$Feedback."' ";
   ...
   <tr>   
	...
	<?
		...
		else
		{
		 ...
		 $qry="Select *,ICICI_Allocated_Leads.icicirequestID AS icicireq From ICICI_Allocated_Leads  LEFT OUTER JOIN icicipllms_allocation ON ICICI_Allocated_Leads.icicirequestID= icicipllms_allocation.icicirequestID  Where (((ICICI_Allocated_Leads.Allocation_Date Between '".$min_date."' and '".$max_date."') or (icicipllms_allocation.icici_followupdate Between '".$min_date."' and '".$max_date."')))";
	...
	?>
<tr>
<td class="bodyarial11"><a href="icici_pladminlead.php?id=<? echo urlencode($row['icicireq']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
	   ...
	   ?>
<td><? if($turow["plcibilid"]>0) {echo "TU DONE";}	
		?></td>
	
    <td class="bodyarial11"><? echo $row["icici_feedback"];?>
	</td>
	<td>
<?
echo $row["verifier_feedback"];
	?>
</td>
	<td><? echo $row["icici_followupdate"];?></td>
</tr>
	<?						$i=$i+1;
		}		
		}
 ...
 <form name="frmdownload" action="icici_plleads_download.php" method="post">
   <tr>
     <td align="center">
