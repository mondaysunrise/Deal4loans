...
<div class="main-continer"><?  if($income>=150000 && $Experience>=2 && $validage==1) 
 { ?>
...
for($j=0; $j<=count($listofcars); $j++)
{
	...
	$resultvedio=ExecQuery($getvediodetails);
	$car_videocode = mysql_result($resultvedio,0,'car_videocode');
	$countJ = $j;
	...
	>
                ...
                <tr>
                  <td  style="background:url(images/newimages/<?php echo $model; ?>specification.jpg) repeat-x;"><table width="100%" border="0" cellpadding="4" cellspacing="2" height="200">
                      <tr>
                        <td colspan="2" class="text_car_body" ><iframe width="230" height="200" src="<?php echo $car_videocode; ?>" frameborder="0" allowfullscreen></iframe></td>
                      </tr>
                  </table></td>
                </tr>
              </table>
...
$emiPerLac=round($alac * $intr / (1 - (pow(1/(1 + $intr), $tenure))));
  $maxTenure=$tenure;
	...
	{
echo "We apologize to inform you that we are unable to find a suitable offer for you currently.";
	}
...
<div style=" float:left; " ><img src="images/newimages/<?php echo $model;?>select-rewards-box.jpg" /></div>
<div style="clear:both;"></div>
<!--<div id="scroll" class="flexcroll"> -->
<table width="960" border="0" cellpadding="0" cellspacing="0">
          <tr><?php
	$getGiftsSql = "SELECT * FROM hdfc_car_loan_gifts WHERE ".$lnAmt[0]." >= min_range AND ".$lnAmt[0]." < max_range AND status=1";	
//		$getGiftsSql = "SELECT * FROM hdfc_car_loan_gifts WHERE status=1";		
		...
		{
			$id = mysql_result($getGiftsQuery,$gS,'id');
			$Name = mysql_result($getGiftsQuery,$gS,'Name');
			$image = mysql_result($getGiftsQuery,$gS,'image');
			$specifications = mysql_result($getGiftsQuery,$gS,'specifications');
			echo '<td align="center" width="194">';
			echo '<table><tr><td align="center" valign="top" height="15"><input type="radio" name="reward_selected" id="reward_selected" value="'.$id.'" onClick="selectReward(this.value);"></td></tr>';	
			?>		
			<tr><td align='center' valign='top'> <!-- <a href="#" class="tooltip" style="text-decoration:none;"> --><img src='images/brochure/<?php echo $image; ?>' border=0 height=90 width=90 style="border:#333333 1px solid;" />
       <!--<span style="text-align:left;">
		...
		<!--
            ...
            <?php
		echo "<tr><td align='center' valign='top' height='40' style='font-size:10px;'>";
			echo $Name;
			echo '</td></tr>';
			echo '</table></td>';
		}
...
<input type="hidden" name="final_Referral_<? echo $j; ?>" id="final_Referral_<? echo $j; ?>" value="<? echo $Referral; ?>" >
                              ...
                              <input type="submit" name="submitMainForm" style="border: 0px none ; background-image: url(images/newimages/<?php echo $model; ?>slider-buttton.jpg); width: 223px; height: 50px; margin-bottom: 0px; font-size:1px;" value="<?php echo $countJ; ?>"/>
...
{
	echo "We apologize to inform you that we are unable to find a suitable offer for you currently.  Our representative will contact you soon.
