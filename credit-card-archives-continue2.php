...
function InsertTataAig($RequestID, $ProductName)
	...
	//	echo "select Dated, City, City_Other from ".$ProductName." where RequestID = $RequestID";
		$GetDateSql = ("select t_name,t_mobile,t_email,t_city,t_dated from ".$ProductName." where tataaigID = $RequestID");
...
$last_id = Maininsertfunc ($table, $dataInsert);
...
<!--												  <input type="checkbox" value="<? echo $row[$cntr]["ccndc_offerid"];?>_<? echo $row[$cntr]["bank_name"];?>" name="category_cc[]" id="category_cc" style="border:none; cursor:pointer;" onClick="showdetailsFaq(<? echo $r;?>,800)" id="imgfaq4"/>-->												  
...
while($i<count($myrow)-1)
	...
	if($recordcount>0)
	{
		while($cntr<count($row))
        {
			...
			?>
                                                <tr>
                                                  <td  width="25" align="left" valign="top"><!--<input type="checkbox" value="<? echo $row["ccndc_offerid"];?>_<? echo $row[$cntr]["bank_name"];?>" name="category_dc[]" id="category_dc" style="border:none; cursor:pointer;" onClick="showdetailsFaqDc(<? echo $r;?>,800)" id="imgfaqdc4"/>  -->  
												  
												  <input type="checkbox" value="<? echo $row[$cntr]["ccndc_offerid"];?>_<? echo $row[$cntr]["bank_name"];?>" name="category_dc[]" id="category_dc" style="border:none; cursor:pointer;" onClick="showdetailsFaqDc(<? echo $r;?>,800)"/>                                                         </td>
