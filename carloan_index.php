...
{
	var gifName = form;
	document.frmsearch.action="carloan_index.php?search=y"+gifName;
...
<div align="center">
 ...
 <form name="frmsearch" action="carloan_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if($search=="y")
	{
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Loan_Car.Updated_Date)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(Req_Loan_Car.Updated_Date)<='".strtotime($max_date)."'";
		}
		else if($min_date!="" and $max_date=="")
		{
			$min_date=$min_date." 00:00:00";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Loan_Car.Updated_Date)>='".strtotime($min_date)."'";
		}
		else if($min_date=="" and $max_date!="")
		{
			$max_date=$max_date." 23:59:59";
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Loan_Car.Updated_Date)<='".strtotime($max_date)."'";
		}
		else
		{
			$search_date=$search_date." and Req_Loan_Car.Updated_Date!=''";
		}
	?>
 <table width="858" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <?
			$search_qry="select Employment_Status,Car_Type,DOB,Email,Name,Company_Name,City,City_Other,Std_Code,Landline,Std_Code_O,Landline_O,Net_Salary,Car_Model,Loan_Amount,Pincode,Feedback,Updated_Date,source,Is_Valid,Bidder_Count,Add_Comment,RequestID from Req_Loan_Car LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Car.RequestID AND Req_Feedback.BidderID in (1306,1676) where (1=1 ".$search_date.") group by Req_Loan_Car.Mobile_Number order by Req_Loan_Car.Updated_Date desc" ;
		$qry="select Name,City,Net_Salary,DOB,Employment_Status,source,CL_Bank,Is_Valid,Feedback, Bidder_Count,RequestID from Req_Loan_Car LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Car.RequestID AND Req_Feedback.BidderID in (1306,1676) where (1=1 ".$search_date.") group by Req_Loan_Car.Mobile_Number order by Req_Loan_Car.Updated_Date desc";
		/*}
		else
		{
			echo "<script>alert('Please enter dates B/w  $checkdate to  $finaldate ')</script>";
 ...
 ?>
   ...
   <tr>
    ...
    <!--<td class="head1">Company Name </td>-->
     ...
     <td class="head1">Employment Status </td>
	  <td class="head1">source </td>
	   <td class="head1">Bank clicked</td>
       <td class="head1">Is Valid </td>
	  <td class="head1">Feedback </td>
	    <td class="head1">Bidder count </td>
		 <td class="head1">Sent to bidders </td>
         <td class="head1">Sent status </td>
	...
	<?
		...
		{*/
		$qry="select Allocated, IsModified, Name, City, Net_Salary, DOB, Employment_Status, source, CL_Bank, Is_Valid,Feedback, Bidder_Count,RequestID from Req_Loan_Car LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Car.RequestID AND Req_Feedback.BidderID in (1306,1676) where (1=1 ".$search_date.") group by Req_Loan_Car.Mobile_Number order by Req_Loan_Car.Updated_Date desc LIMIT $startrow, $pagesize";
		//}
		//echo $qry."<br>";
		$result=ExecQuery($qry);
   ...
   <tr>
    ...
    <!--<td class="bodyarial11"><? //echo $row["Company_Name"]; ?></td>-->
	 ...
	 <td class="bodyarial11"><? echo $row["source"]; ?></td>
	  <td class="bodyarial11"><? echo $row["CL_Bank"]; ?></td>
	  ...
	  <td class="bodyarial11"><? echo $row["Bidder_Count"]; ?></td>
	   <td class="bodyarial11"><?  $BidderID="";
	
	$BiddersChurn="SELECT Bidder_Name,Req_Feedback_Bidder1.BidderID As bid FROM Req_Feedback_Bidder1 LEFT OUTER JOIN Bidders_List ON Bidders_List.BidderID = Req_Feedback_Bidder1.BidderID and Bidders_List.Reply_Type =3 WHERE (AllRequestID = '".$row["RequestID"]."' AND Req_Feedback_Bidder1.Reply_Type =3)";
	//echo $BiddersChurn;
	...
	echo implode(',', $BidderID);
     ...
     <?php
		 if($row["Allocated"]==1 && $row["IsModified"]==2)
			{
				echo "LMS Sent";
			}
			else
	...
	/* $client_transaction_id = $row["RequestID"]."_CL";
	 $getSql = "select verified,viewable from z2v_transactions where client_transaction_id 	='".$client_transaction_id."'";
$getQuery = ExecQuery($getSql);
$verified  = mysql_result($getQuery,0,'verified');
$viewable  = mysql_result($getQuery,0,'viewable');
if($verified==1)
{
	echo "ValiUpdated_Date";
}*/
...
<!--<table width="500" border="0" cellspacing="1" cellpadding="4">
 <form name="frmdownload" action="carloan_download.php" method="post">
   <tr>
     <td align="center">
