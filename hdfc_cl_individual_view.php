...
function getReqValue1($pKey){
	$titles = array(
		...
		'1833' => 'Chennai',
		'1834' => 'Chandigarh',
		'1835' => 'Delhi,Gurgaon,Noida,Gaziabad,Faridabad',
		'1836' => 'Secunderabad,Hyderabad',
		'1837' => 'Jaipur',
		'1838' => 'Kolkata',
		'1839' => 'Mumbai,Navi Mumbai,Thane',
		'1840' => 'Pune',
		'1841' => 'Surat',
		'1842' => 'Amritsar',
		'1843' => 'Aurangabad',
...
$branch=getReqValue1($_SESSION['BidderID']);
//cho "hello".$branch."<br>";
...
{
	var gifName = form;
	document.frmsearch.action="hdfc_cl_individual_view.php?search=y"+gifName;
...
function chkform()
{
	...
	{
		alert("Sorry!!!! Your minimum date is 2010-01-10.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="hdfc_cl_individual_view.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="80%">
	...
	<option value="Loan Rejected" <? if($varCmbFeedback == "Loan Rejected") { echo "selected"; } ?>>Loan Rejected</option>
     ...
     <td width="30%"  valign="middle" class="bidderclass"></td>
 ...
 <? 
	if(strlen($branch)>0)
		...
		$search_qry="SELECT * FROM Req_Feedback_Bidder1,`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Loan_Car.RequestID AND Req_Feedback.BidderID =".$_SESSION['BidderID']."  WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID in (1825,1880,1881,2095,2096,2097,2106)  and `".$val."`.City in ".$propercity." and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date  Between '".($min_date)."' and '".($max_date)."' ) ";	
...
//echo $qry;
   ...
   <tr>
	 ...
	 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("stanc_cc_view.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
