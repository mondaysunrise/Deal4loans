...
{
	var gifName = form;
	document.frmsearch.action="citibankcc_leadsview.php?search=y"+gifName;
...
<? if (((!isset($val) && $viewtexttype==1) || ($val=="Req_Loan_Personal")) || ((!isset($val) && $viewtexttype==2) || ($val=="Req_Credit_Card")))
	{?>
	<tr>
 ...
 <form name="frmsearch" action="citibankcc_leadsview.php?search=y" method="post" onSubmit="return chkform();">
 <input type="hidden" name="BidderIDstatic" id="BidderIDstatic" value="<? echo $BidderIDstatic;?>">
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass"><?$current_date=date('Y-m-d');?> 
	     <input name="min_date" type="text" id="min_date" size="15" value="<? echo $_REQUEST['min_date']; ?>" autocomplete="off"></td>
     <td width="7%" align="left" valign="middle" class="bidderclass"></td>
      <td valign="middle" align="center" class="style1" width="8%">To</td>
     <td align="left" valign="middle" class="style1" width="24%"> <input name="max_date" type="text" id="max_date" size="15" value="<? echo $_REQUEST['max_date']; ?>" autocomplete="off"></td>
...
<tr><td width="29%" align="center"  valign="middle" class="bidderclass">Search with Reference No</td>
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		$strSQL="";
		$Msg="";
		$result = d4l_ExecQuery("select Feedback_ID from Req_Feedback where AllRequestID=$RequestID and BidderID=".$BidderIDstatic);		
		$num_rows = d4l_mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = d4l_mysql_fetch_array($result);
			$strSQL="Update Req_Feedback Set Feedback='".$Feedback."' ";
			$strSQL=$strSQL." Where leadid=".$row["Feedback_ID"];
		}
...
<table width="950" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" >
 <?  
		$qry="SELECT * ,".$feedback_tble.".BidderID AS sentbidder FROM Req_Feedback_Bidder_CC,citibank_credit_card_6250 LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID = citibank_credit_card_6250.RequestID AND Req_Feedback.BidderID= '".$BidderIDstatic."' WHERE (citibank_credit_card_6250.RequestID=`Req_Feedback_Bidder_CC`.AllRequestID and Req_Feedback_Bidder_CC.BidderID=6250 and (citibank_credit_card_6250.Dated Between '".($min_date)."' and '".($max_date)."' or Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."') and citibank_credit_card_6250.send_status=1)";
		$qry=$qry.$FeedbackClause." ".$refernce_no_clause;
		$qry=$qry."group by citibank_credit_card_6250.RequestID";
	//echo $qry."<br><br>";
 ...
 ?>
   ...
   <td width="149" align="center" bgcolor="#FFFFFF" class="style2">Reference No</td>
	...
	<?
		...
		$qry=$qry.$FeedbackClause." ".$refernce_no_clause;
		$qry=$qry."group by citibank_credit_card_6250.RequestID";
		$qry=$qry." order by citibank_credit_card_6250.Dated DESC";
...
//echo $qry."<br>";
		...
		{			
			$ccqry="Select Name,City,Net_Salary From Req_Credit_Card Where RequestID=".$row["RequestID"];
			$ccresult=d4l_ExecQuery($ccqry);
			$ccrow=d4l_mysql_fetch_array($ccresult);
	?>
   ...
   <tr>
   <td align="center" bgcolor="#DFF6FF" class="style3"><? echo "CC".$row["Feedback_ID"]."S6250"; ?></td>
      <td align="center" bgcolor="#DFF6FF" class="style3"><a href="/creditcardlead-details.php?postid=<? echo $row["RequestID"]; ?>&biddt=<? echo $BidderIDstatic; ?>" target="_blank">
		<? echo $ccrow["Name"]; ?></a>	</td>     
		<td align="center" bgcolor="#DFF6FF" class="style3"><? echo $ccrow["City"]; ?></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $ccrow["Net_Salary"]; ?></td>
	  <!--<td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["Loan_Amount"]; ?></td>-->
       <td align="center" bgcolor="#DFF6FF" class="style3"><? 		
	echo getJumpMenu("citibankcc_leadsview.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val,$BidderIDstatic) ?>
		</td>
