...
{
	var gifName = form;
	document.frmsearch.action="100nests_lms_hl.php?search=y"+gifName;
...
<tr>
 ...
 <form name="frmsearch" action="100nests_lms_hl.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
	...
	<? $geteligiblebidder=ExecQuery("select BidderID From Req_Feedback_Bidder1 where (Reply_Type=".$pro_code." and AllRequestID=".$row["RequestID"].")");
	 ...
	 while($get=mysql_fetch_array($geteligiblebidder))
		...
		$numberOfusers = count($arrgetBidderID);
			?>
	<td align="center" bgcolor="#DFF6FF" class="style3"><? //echo implode(",",$arrgetBidderID); 
	 if($numberOfusers>0 && $arrgetBidderID[0]>0) {
	echo $numberOfusers;
	}
