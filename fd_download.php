...
$dataInsert = array("session_id"=>$session_id, "name"=>$row_result[$cntr]["name"], "dob"=>$row_result[$cntr]["dob"], "email"=>$row_result[$cntr]["email"], "emp_status"=>$row_result[$cntr]["mobile_number"], "c_name"=>$row_result[$cntr]["investment_amount"], "city_other"=>$duration, "city"=>$row_result[$cntr]["city"], "doe"=>$row_result[$cntr]["dated"]);
$table = 'temp';
$insert = Maininsertfunc ($table, $dataInsert);
	 ...
	 $cntr=$cntr+1;
		...
		$qry="select name as Name, dob as DOB, email as Email, emp_status as MobileNumber, city as City, c_name as InvestmentAmount, city_other as Duration, doe as Dated   from temp where session_id='".$session_id."' order by doe DESC ";
