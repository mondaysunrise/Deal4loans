...
$filename = $RequestID."_".$filename;
  ...
  if(move_uploaded_file($tmpfile, "upload/".$filename)){
	...
	if($numRows>0)
	{
		$DataArray = array("Voterid"=>$filename);
  ...
  echo returnStatus($status);
	...
	function returnStatus($status){
		return "<html><body><script type='text/javascript'>function init(){if(top.uploadComplete_6) top.uploadComplete_6('".$status."');}window.onload=init;
	</script></body></html>";
	}
