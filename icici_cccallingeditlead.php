...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	{
		 $appointquery ="Update credit_card_cibil_check set appointment_date='".$appointment_date."',appointment_time='".$appointment_time."', appointment_place='".$appointment_place."', appointment_address= '".$appointment_address."',documents='".$completedocumentsstr."',other_documents='".$other_docs."' Where (RequestID=".$post." and RuleStatus='Approved')";
		$appointqueryresult = ExecQuery($appointquery);

	if(strlen($Final_Bid)>1 && $UserID==0 && $Allocated==2)
		{
	//insert in main table for allocation
	$InsertProductSql = "INSERT INTO Req_Credit_Card( Name, Email, Employment_Status, Company_Name, City, City_Other, Mobile_Number, Net_Salary, DOB,  Dated, source, CC_Holder,Card_Vintage,IP_Address, Updated_Date, Pincode, No_of_Banks, Company_HDFC_Cat, Salary_Account, Company_ICICI_Cat, Credit_Limit, Allocated, lead_cost, Bidderid_Details, Add_Comment, applied_card_name ) VALUES ('".$ccname."', '".$ccemail."', '".$ccemployment_status."', '".$cccompany_name."', '".$cccity."', '".$cccity_other."', '".$ccmobile."', '".$ccnet_salary."', '".$ccdob."', Now(), 'manual enter', '".$cccc_holder."','".$Card_Vintage."','$IP', Now(),'".$ccpincode."' ,'".$No_of_Banks."','".$Company_HDFC_Cat."',  '".$Salary_Account."','".$Company_ICICI_Cat."','".$Credit_Limit."', '".$Allocated."', '".$lead_cost."', '".$Final_Bid."', '".$ccadd_comment."', '".$final_cardselect."')";
	$InsertProductSqlresult=ExecQuery($InsertProductSql);
	$ProductValue = mysql_insert_id();
	//for Cibil entry
	$inserticiciqry="INSERT INTO icicilms_allocation (`cc_requestid`, `bidderid`, `product`, `allocation_date`, icici_sendnow_date, icici_bidders) VALUES ( '".$ProductValue."', '".$Bidder_Id."', '4', Now(), Now() ,'".$Final_Bid."')";
	$inserticiciqryresult = ExecQuery($inserticiciqry);

	if($ProductValue>0)
	{
		$apptquery ="Update credit_card_cibil_check set RequestID ='".$ProductValue."' Where (RequestID=".$post." and appointment_place!='' and appointment_address!='') order by cibilchkid DESC LIMIT 1";
		 $apptqueryresult = ExecQuery($apptquery);

		 ExecQuery($updatecclead="Update icici_cards_calling set UserID='".$ProductValue."' where RequestID=".$post);
	}
		}					
	}
...
<?php 
$viewqry="select * from icici_cards_calling LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID=icici_cards_calling.RequestID and Req_Feedback_CC.BidderID= '".$bidid."' where icici_cards_calling.RequestID=".$post." "; 
//echo "dd".$viewqry;
...
$UserID  = mysql_result($viewlead,0,'UserID');	
...
$getapptqry = "select other_documents,RuleStatus,Response,appointment_date, appointment_time,appointment_place,appointment_address,	documents from credit_card_cibil_check where (RequestID='".$post."' or RequestID='".$UserID."') order by cibilchkid DESC";
...
?>
<tr>
<td colspan="4" align="center" style="font-size:16px; font-weight:bold; " height="40" ><a href="/icici_cclms_tu1.php?reqdid=<? echo $post; ?>&teleid=<? echo $bidid; ?>" target="_blank" style="background-color:#FFCC00;">Transunion Check</a></td>
</tr>
<? } ?>
...
?>
<tr><td class="fontstyle"><b>Eligible for Bidders</b></td><? if(strlen($Bidderid_Details)>0){?><td class="fontstyle" colspan="2"> already send</td><? } else {?><td colspan="2"><div id="checkdiv" name="checkdiv"><? list($FinalBidder,$finalBidderName)= getBiddersList("icici_cards_calling",$post,$City);
...
?>
 <tr><td colspan="4" bgcolor="#DAEAF9" align="center" style="border:1px solid black;" ><b><input type="checkbox" name="ICICIDOC" onClick="eiewdocts();" id="ICICIDOC" style="<? echo $stylevaldc; ?>" value="Docsadded" <? if($ICICIDOC=="Docsadded") { echo "checked";} ?> >Documents </a></b></td></tr>
...
<input type="checkbox" name="documents[]" id="doc9" <? if((strlen(strpos($documents, "Latest 1 Month Card statement")) > 0)) echo "checked"; ?> value="Latest 1 Month Card statement">Latest 1 Month Card statement
   ...
   <tr><td colspan="4" bgcolor="#DAEAF9" align="center" style="border:1px solid black;" ><b>ICICI Appointment Details </b></td></tr>
