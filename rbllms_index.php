...
{
	var gifName = form;
	document.frmsearch.action="rbllms_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2014-11-01")
	{
		alert("Sorry!!!! Your minimum date is 2014-11-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}	
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="rbllms_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//		echo $strSQL;
 ...
 <?
 	$qry="Select * from Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID='98' Where (((((Req_Credit_Card.Employment_Status=1 and Req_Credit_Card.Net_Salary>=420000) or (Req_Credit_Card.Employment_Status=0 and Req_Credit_Card.No_of_Banks not like '%RBL%' and Req_Credit_Card.CC_Holder=1)) and (Req_Credit_Card.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 23 YEAR) >= Req_Credit_Card.DOB) and (Req_Credit_Card.Dated Between '".$min_date."' and '".$max_date."') or 
(Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and (Req_Credit_Card.City in ('Gurgaon','Hyderabad','Bengaluru','Bangalore','Chennai','Ahmedabad','Mumbai','Pune','Noida','Indore','Navi Mumbai','Delhi','Bhopal','Thane','Greater Noida','Baroda','Faridabad','Kolhapur','Mangalore','Vadodara') or Req_Credit_Card.City_Other in ('Gurgaon','Hyderabad','Bengaluru','Bangalore','Chennai','Ahmedabad','Mumbai','Pune','Noida','Indore','Navi Mumbai','Delhi','Bhopal','Thane','Greater Noida','Baroda','Faridabad','Kolhapur','Mangalore','Vadodara'))) and Req_Credit_Card.rbl_flag=0) and (Req_Credit_Card.applied_card_name like '%RBL Bank%'))";		
 ...
 $qry="		 
Select * from Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID='98' Where (((((Req_Credit_Card.Employment_Status=1 and Req_Credit_Card.Net_Salary>=420000) or (Req_Credit_Card.Employment_Status=0 and Req_Credit_Card.No_of_Banks not like '%RBL%' and Req_Credit_Card.CC_Holder=1)) and (Req_Credit_Card.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 23 YEAR) >= Req_Credit_Card.DOB) and (Req_Credit_Card.Dated Between '".$min_date."' and '".$max_date."') or 
(Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and (Req_Credit_Card.City in ('Gurgaon','Hyderabad','Bengaluru','Bangalore','Chennai','Ahmedabad','Mumbai','Pune','Noida','Indore','Navi Mumbai','Delhi','Bhopal','Thane','Greater Noida','Baroda','Faridabad','Kolhapur','Mangalore','Vadodara') or Req_Credit_Card.City_Other in ('Gurgaon','Hyderabad','Bengaluru','Bangalore','Chennai','Ahmedabad','Mumbai','Pune','Noida','Indore','Navi Mumbai','Delhi','Bhopal','Thane','Greater Noida','Baroda','Faridabad','Kolhapur','Mangalore','Vadodara'))) and Req_Credit_Card.rbl_flag=0) and (Req_Credit_Card.applied_card_name like '%RBL Bank%'))";
...
//echo $qry;
		...
		while($row=mysql_fetch_array($result))
		{
			...
			$FinalDay = $exptodayformat[0];	
				$tudetails="Select  autoflag from rbl_creditcard Where cc_requestID=".$row["RequestID"]; 
	$tudetailsQuery = ExecQuery($tudetails);
	$turow=mysql_fetch_array($tudetailsQuery);
	?>
<tr>
<td class="bodyarial11"><a href="rbl_cceditlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
	   ...
	   ?>
	<td><? if($turow["autoflag"]==1) { echo "Already send to RupeePower"; }	
		?></td>
	<td>
<? list($FinalBidder,$finalBidderName)= getBiddersList("Req_Credit_Card",$row['RequestID'],$City);
  for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo getJumpMenu("rbllms_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
