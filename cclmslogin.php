	...
	function formHandler(){
		...
		if($num_rows > 0){
			...
			mysql_free_result($result);
			 ...
			 /* Redirect browser */
			 $strDir = dir_name();
if($_SESSION['BidderID']==936)
			{
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."cclms_index.php");
			exit;
			}
			else if($_SESSION['BidderID']==935)
				{
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."cclms1_index.php");
			exit;
				}
				else{
			global $Msg;
