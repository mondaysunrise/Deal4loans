...
function getStateList($key){
	$strState;
	$strState = "";
	$strState = $strState.AmISelected("Please Select", $key, "Please Select");
	$strState = $strState.AmISelected("Andhra Pradesh", $key, "Andhra Pradesh");
	$strState = $strState.AmISelected("Arunachal Pradesh", $key, "Arunachal Pradesh");
	$strState = $strState.AmISelected("Assam", $key, "Assam");
	$strState = $strState.AmISelected("Bihar", $key, "Bihar");
	$strState = $strState.AmISelected("Chandigarh", $key, "Chandigarh");
	$strState = $strState.AmISelected("Chhattisgarh", $key, "Chhattisgarh");
	$strState = $strState.AmISelected("Delhi", $key, "Delhi");
	$strState = $strState.AmISelected("Goa", $key, "Goa");
	$strState = $strState.AmISelected("Gujarat", $key, "Gujarat");
	$strState = $strState.AmISelected("Haryana", $key, "Haryana");
	$strState = $strState.AmISelected("Himachal Pradesh", $key, "Himachal Pradesh");
	$strState = $strState.AmISelected("Jammu and Kashmir", $key, "Jammu and Kashmir");
	$strState = $strState.AmISelected("Jharkhand", $key, "Jharkhand");
	$strState = $strState.AmISelected("Karnataka", $key, "Karnataka");
	$strState = $strState.AmISelected("Kerala", $key, "Kerala");
	$strState = $strState.AmISelected("Madhya Pradesh", $key, "Madhya Pradesh");
	$strState = $strState.AmISelected("Maharashtra", $key, "Maharashtra");
	$strState = $strState.AmISelected("Manipur", $key, "Manipur");
	$strState = $strState.AmISelected("Meghalaya", $key, "Meghalaya");
	$strState = $strState.AmISelected("Mizoram", $key, "Mizoram");
	$strState = $strState.AmISelected("Nagaland", $key, "Nagaland");
	$strState = $strState.AmISelected("Odisha", $key, "Odisha");
	$strState = $strState.AmISelected("Puducherry", $key, "Puducherry");
	$strState = $strState.AmISelected("Punjab", $key, "Punjab");
	$strState = $strState.AmISelected("Rajasthan", $key, "Rajasthan");
	$strState = $strState.AmISelected("Sikkim", $key, "Sikkim");
	$strState = $strState.AmISelected("Tamil Nadu", $key, "Tamil Nadu");
	$strState = $strState.AmISelected("Telangana", $key, "Telangana");
	$strState = $strState.AmISelected("Tripura", $key, "Tripura");
	$strState = $strState.AmISelected("Uttarakhand", $key, "Uttarakhand");
	$strState = $strState.AmISelected("Uttar Pradesh", $key, "Uttar Pradesh");
	$strState = $strState.AmISelected("West Bengal", $key, "West Bengal");
	return $strState;
}
...
function setSessionBidderPDO($Login, $row){
	 /* Create Session Variables */
	$_SESSION['Login'] = $Login;
	$_SESSION['BidderID'] = $row->BidderID;
	$_SESSION['UName'] = $row->Bidder_Name;
...
function listbox_date($name, $default=0)
	{ 
    $result="<select name=".$name." id=".$name."  size=1><option value=\"\">DD</option>"; 
...
function listbox_month($name, $default=0)
	{ 
    $result="<select name=".$name." id=".$name." size=1> <option value=\"\">MM</option>"; 
    for ($m=1;$m<=12;$m++) { 
...
function listbox_year($name, $start, $end, $default=0)
	{ 
    $result="<select name=".$name." id=".$name." size=1><option value=\"\">YYYY</option>"; 
    for ($y=$end;$y>=$start;$y--) { 
...
function getFieldNames($sql,$array = array())
	{
		$pdo = db_connect_PDO();
...
function ProductSendSMStoRegis($strmobile_no)
	{
		$message = "Welcome to Deal4loans.com, India's largest loan aggregator with 6 million customers & 40 Bank relationships. Our services are absolutely FREE for all customers";
		SendSMSforLMS($message, $strmobile_no);
	}

function SendSMSforWishfin($SMSMessage, $PhoneNumber)
{
	//	Expiry date                         : 05-08-2017
	//http://httpapi.zone:7501/failsafe/HttpLink?aid=637348&pin=W!sh123&mnumber=XXXXXXX&message=test&signature=WISHFN
	$request = ""; //initialize the request variable
	$param["aid"]= "637348";
	$param["pin"]= "W!sh123";
	$param["mnumber"]="91".$PhoneNumber;
	$param["message"]=$SMSMessage;
	$param["signature"]="WISHFN";
...
//	echo $sql ."<br>";
	...
	if($num>0)
	{
		...
		$sql1 = "SELECT * FROM `xkyknzl5dwfyk4hg_whatsapp_callback` WHERE `mobile_number`= '91".$Mobile."' AND `mobile_number`!='' AND  (message_status NOT LIKE 'read' AND message_status NOT LIKE 'delivered') AND date_created>'".$initialDate."' ORDER BY date_created DESC";
//	echo $sql1 ."<br>";
		...
		if($num1>0)
		{
			$i=0;
			$message[]='yes';
			$message[]=wf_mysql_result($qry1,$i,'message_text');
			$message[]=wf_mysql_result($qry1,$i,'date_created');
		}
		else
		{
		 	$message[] = 'no';
