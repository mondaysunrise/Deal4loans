...
if( $exactnet_salary>0)
	...
	//echo $gtcropcomp."<br>";
	$gtcropcompresult=d4l_ExecQuery($gtcropcomp);
	$icicirow=d4l_mysql_fetch_array($gtcropcompresult);
	$crprecordcount = d4l_mysql_num_rows($gtcropcompresult);
		}
if($crprecordcount>0)
	{
...
if($getloanamout>=10000000)
		{
			$getloanamout=10000000;
		}
		else
		{
...
if(strlen($company_name)>3)
	...
	//echo $gtscocomp."<br>";
	//$gtscocompresult=d4l_ExecQuery($gtscocomp);
	$ingscorow=d4l_mysql_fetch_array($gtscocompresult);
	$ingscorecordcount = d4l_mysql_num_rows($gtscocompresult);
...
function indusindbank($strnet_salary,$company,$category,$DOB,$clubbed_emi,$city)
{
	//echo $category."<br>".$strnet_salary."<br>";
	 $net_salary = $strnet_salary;

	list($term,$print_term)=getdob($DOB);
	
	if($category=="A+" || $category=="CAT A" || $category=="CAT A+")
		{	
...
//Calculate Loan Amount
	...
	else
	{
		...
		if($finalloanamount>4000000)
			{
				if(strlen($category)>1)
				{
				$getloanamout=4000000;
				}
				else
				{
					$getloanamout=1500000;
...
$kotakcmpqry=d4l_ExecQuery("Select  * from pl_company_kotak Where (company_name like '%".$company."%')");
$kotakrow=d4l_mysql_fetch_array($kotakcmpqry);
...
if($term==12)
	...
	{ 
		if($category=="CAT A" || $category=="CAT B" || $category=="CAT C")
		{
				...
				//Clause till 18th august
				if($net_salary>75000)
				{
					$interestrate = 10.99;
					$intr=$interestrate/1200;
					}
				elseif($net_salary>20000 && $net_salary<=75000)
				{
					$interestrate = 11.39;
					$intr=$interestrate/1200;
				}
...
$getemicalc=round($loan_amount * $intr / (1 - (pow(1/(1 + $intr), $calcterm))));
	...
	//special Clause 
	if(($category=="TATA Group" || $category=="TATA GROUP" || $category=="Super Cat A" || $category=="Super CAT A") && $loan_amount>=500000)
	{
		$interestrate = 11.99;
		$intr=$interestrate/1200;
		$proc_Fee ="Rs.999";
	}
	elseif(($category=="CAT A" || $category=="CAT B") && $loan_amount>=500000)
	{
		$interestrate = 13.99;
		$intr=$interestrate/1200;
		$proc_Fee ="Rs.1999";
...
//Capital first
function capitalFirst($net_salary,$company,$category,$DOB,$Company_Type,$Employment_Status)
{	
	list($term,$print_term)=getdob($DOB);
	...
	{ //self employed
		...
		$intrcalc=18/1200;
		$princ=100000;
		$emicalc=round($princ * $intrcalc / (1 - (pow(1/(1 + $intrcalc), 36))));
		$loanPossible= $applicableFOIR /$emicalc;
		$viewLoanAmt = round($loanPossible * 100000);
...
//Incred
function incred($net_salary,$clubbed_emi,$company,$category,$DOB,$Company_Type)
{
	list($calterm, $calprint_term) = Calculate_Tenure($DOB, "4", "58");
	...
	$bflloansmt= round($exactnet_salary*10);

	 $bflintr1=11/1200;
	 $bflintr2=24/1200;
	 $bflintrte="11% - 24%";
	 $getemi1=round($bflloansmt * $bflintr1 / (1 - (pow(1/(1 + $bflintr1), $calterm)))); 
	 $getemi2=round($bflloansmt * $bflintr2 / (1 - (pow(1/(1 + $bflintr2), $calterm)))); 
	 $getemi = $getemi1." - ".$getemi2;

	$details[]=$bflintrte;
	$details[]=round($bflloansmt);
	$details[]=$getemi;
	$details[]=$calprint_term;
	$details[]="NA";
...
//RBL Bank function
function rblbankfunc($net_salary,$clubbed_emi,$company,$category,$DOB,$Company_Type)
{	
	if($net_salary>=35000)
	{
		list($calterm, $calprint_term) = Calculate_Tenure($DOB, "5", "59");
	}
	else
	{
		list($calterm, $calprint_term) = Calculate_Tenure($DOB, "4", "59");
	}
	...
	if($Company_Type==1)
	{
		...
		elseif($net_salary>=50000 && $net_salary<100000)//salary
		{
			if($calcterm>=49 && $calcterm<=60)
			{ 
				$loan_amt=$net_salary*15;
			...
			elseif($calcterm<=12)
			{	
				$loan_amt=$net_salary*4;
			}			
		}
		elseif($net_salary>=25000 && $net_salary<50000)//salary
		{
			if($calcterm>=49 && $calcterm<=60)
			{ 
				$loan_amt=$net_salary*13;
			...
			elseif($calcterm<=12)
			{	
				$loan_amt=$net_salary*3;
			}			
		}
		$finalloan_amount=$loan_amt;
	}
		else
		{
			if($net_salary>=100000)
			{
				$applicableFOIR = $net_salary * (.70);
			}
			elseif($net_salary>=50000 && $net_salary<100000)//salary
			{
				$applicableFOIR = $net_salary * (.65);
			}
			elseif($net_salary>35000 && $net_salary<50000)//salary
			{
				$applicableFOIR = $net_salary * (.60);
			}
			elseif($net_salary<=35000)//salary
			{
				$applicableFOIR = $net_salary * (.55);
			}

			$princ=100000;
			$emicalc=round($princ * $intr / (1 - (pow(1/(1 + $intr), $calterm))));
			$loanPossible= $applicableFOIR /$emicalc;
			$viewLoanAmt = round($loanPossible * 100000);
			$loan_amt_asfoir = $viewLoanAmt;
			$finalloan_amount=$loan_amt_asfoir;
		}


		//interest rate as per loan amount
	if($category=="CAT C" || $category=="CAT D" || $category=="")
		{
		if($finalloan_amount>300000)
			{
				if($category=="CAT C")
				{
					if($net_salary>75000)
		...
		$getemicalc=round($finalloan_amount * $intr / (1 - (pow(1/(1 + $intr), $calterm))));
		$details[]=$interestrate;

		if($finalloan_amount>100000)
		{
			$proc_fee="2%";
		}
		else
		{
			...
			$processing_fee=$calprint_term;
	$details[]=round($finalloan_amount);
	$details[]=$getemicalc;
	$details[]=$calprint_term;
	$details[]=$proc_fee;
	return($details);
