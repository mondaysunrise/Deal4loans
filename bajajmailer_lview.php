...
{
	var gifName = form;
	document.frmsearch.action="bajajmailer_lview.php?search=y"+gifName;
...
<tr>
 ...
 <table width="600" border="0" align="center" cellpadding="0" cellspacing="0">
   ...
   <tr>
     <td align="center" valign="middle" background="images/login-form-login-bg.gif"> <form name="frmsearch" action="bajajmailer_lview.php?search=y" method="post" onSubmit="return chkform();"><table width="95%" border="0"  cellpadding="1" cellspacing="0"> 
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="67%" align="left" valign="middle" class="bidderclass">
        ...
        <option value="OTP Not Done" <? if($varCmbFeedback == "OTP Not Done") { echo "selected"; } ?>>OTP Not Done</option>
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		$strSQL="";
		$Msg="";
		$result = ExecQuery("select bajaj_finservid from bajaj_finserv_mailer_leads where (bajaj_finservid=$RequestID)");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update bajaj_finserv_mailer_leads Set bajajf_feedback='".$Feedback."'";
			$strSQL=$strSQL."Where bajaj_finservid=".$row["bajaj_finservid"];
		}
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (bajaj_finserv_mailer_leads.bajajf_feedback IS NULL OR bajaj_finserv_mailer_leads.bajajf_feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND bajaj_finserv_mailer_leads.bajajf_feedback='".$varCmbFeedback."' ";
		}
 ...
 <? 
		$search_qry="SELECT * FROM bajaj_finserv_mailer_leads WHERE	(bajajf_sent=1 and bajajf_source not like '%bfsmailerhl%' and (bajajf_dated Between '".($min_date)."' and '".($max_date)."'))";	
		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by bajaj_finserv_mailer_leads.bajajf_mobile";
		$search_qry=$search_qry." order by bajaj_finserv_mailer_leads.bajajf_dated DESC";

		$qry="SELECT * FROM bajaj_finserv_mailer_leads WHERE (bajajf_sent=1 and bajajf_source not like '%bfsmailerhl%' and (bajajf_dated Between '".($min_date)."' and '".($max_date)."'))";
...
<?
		...
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by bajaj_finserv_mailer_leads.bajajf_mobile";
		$qry=$qry." order by bajaj_finserv_mailer_leads.bajajf_dated DESC";
...
//echo $qry;
	...
	?>	
   <tr>
       <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_name"]; ?></td> 
	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_mobile"]; ?></td> 
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_city"]; ?></td> 
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_net_salary"]; ?></td>
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["bajajf_loan_amount"]; ?></td>
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("bajajmailer_lview.php",$row["bajaj_finservid"],"1",$row["bajajf_feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback); ?>
		</td>
		 <td align="center" bgcolor="#DFF6FF" class="style3"><? $bajajrefno=ExecQuery("Select Referenceno from bajaj_cibildetails where bajajf_mobile='".$row["bajajf_mobile"]."' and bajajf_plrequestid=0 and Referenceno>0"); $bfsrow=mysql_fetch_array($bajajrefno); echo $bfsrow["Referenceno"]; ?></td>	
...
?>
	...
	<option value="<? echo $strURL.'&Feedback=OTP Not Done'?>" <? if($varFeedback == "OTP Not Done") { echo "selected"; } ?> >OTP Not Done</option>
