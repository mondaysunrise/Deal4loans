...
$age=$currentyear-$year;
	...
	$getpldetails=ExecQuery("select CC_Holder,City_Other,City,Company_Name,Name,Net_Salary,DOB,Reference_Code From Req_Loan_Personal Where (RequestID='".$leadid."')");
...
if($City=="Others")
	...
	if($leadid>0)
		...
		{
			if(($CC_Holder==1 || $is_permit==1 || strlen($Loan_A)>2))
			{
				$permited=1;
			}
				else
...
//echo $Final_Bid[$i];
		//
		$getdoc=ExecQuery("select document_proof,identification_proof,residence_proof,income_proof from bank_documents_required where bank_name like '%".$Final_Bid[$i]."%'");
		...
		while($myrow = mysql_fetch_array($get_Bankresult))
		 {?>
            <td valign="top" ><table width="97%" border="0" align="center" cellpadding="0" cellspacing="0" class="crdbg">
                ...
                <tr>
                  <td align="center" valign="bottom"><a href="<? if (strlen($myrow["cc_bank_url"])>0) {echo $myrow["cc_bank_url"];} else {echo "#";}?>" target="_blank">
