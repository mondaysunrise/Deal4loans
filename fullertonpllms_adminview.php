...
{
	var gifName = form;
	document.frmsearch.action="fullertonpllms_adminview.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2014-12-09")
	{
		alert("Sorry!!!! Your minimum date is 2014-12-09.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="fullertonpllms_adminview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">
		...
		<option value="Duplicate" <?if($varCmbFeedback == "Duplicate") { echo "selected"; }?>>Duplicate</option>
            ...
            <option value="Verified" <? if($varCmbFeedback == "Verified") { echo "selected"; } ?>>Verified</option>		
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select fullertonrequestID from Fullerton_Allocated_Leads where fullertonrequestID=$RequestID");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update Fullerton_Allocated_Leads Set verifier_feedback='".$Feedback."' ";
			$strSQL=$strSQL."Where fullertonrequestID=".$row["fullertonrequestID"];
		}
				echo $strSQL;
	...
	if($search=="y")
	{
		...
		elseif($varCmbFeedback=="All")
		{
			$FeedbackClause=" ";
		}
		else
		{
			//$FeedbackClause=" AND Fullerton_Allocated_Leads.verifier_feedback='".$varCmbFeedback."' ";
 ...
 <?
		$search_qry="Select * From Fullerton_Allocated_Leads Where ((Fullerton_Allocated_Leads.Allocation_Date Between '".$min_date."' and '".$max_date."'))";
		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by Fullerton_Allocated_Leads.RequestID";			
		...
		{
		$lead = mysql_result($changedresult,$i,'fullertonrequestID');
		$leadsid[] =$lead;
		}
 ?>
   <tr>
   ...
   <tr>   
	...
	<?
		...
		$qry="Select *  From Fullerton_Allocated_Leads Where ((Fullerton_Allocated_Leads.Allocation_Date Between '".$min_date."' and '".$max_date."'))";
...
//echo $qry;
	...
	?>
<tr>
<td class="bodyarial11"><a href="fullerton_plverifylead.php?id=<? echo urlencode($row['fullertonrequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
	   ...
	   ?>	
    <td class="bodyarial11"><? echo $row["fullerton_feedback"];?>
	</td>
	<td><? echo $row["fullerton_followupdate"];?></td>
</tr>
	<?						$i=$i+1;
		}		
		}
 ...
 <form name="frmdownload" action="fullerton_plleads_download.php" method="post">
   <tr>
     <td align="center">
