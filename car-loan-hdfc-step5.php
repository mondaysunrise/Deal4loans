...
//[last_inserted_id] => 741 [newj] => 4 [model] => maruti- [untouched_la_0] => 666370 [untouched_ten_0] => 60 [city] => Baroda [car_name_0] => 309 [comp_category] => CAT_C [rate_category_0] => C [krc_flag] => 0 [hdfc_carprice_0] => 666370 [text] => [nwLA_0] => 666370 [nwtenu_0] => 60 [roi_0] => 11.5 [final_Referral_0] => [final_total_interest_0] => [final_roi_0] => 11.5 % [final_emiPerLac_0] => 14655 [final_Loan_Amount_0] => 666370
//print_r($_REQUEST);
...
$checkRewardsSql = "select reward_selected from hdfc_car_loan_leads where RequestID='".$last_inserted_id."'";
list($checkRewardsNumRows,$checkRewardsSql)=MainselectfuncNew($checkRewardsSql,$array = array());
$checkreward_selected = $checkRewardsQuery[0]['reward_selected']; 
if($checkreward_selected>0)
{
}
else
{
	$getGiftsSql = "SELECT * FROM hdfc_car_loan_gifts WHERE ".$final_Loan_Amount." >= min_range AND ".$final_Loan_Amount." < max_range AND status=1 ORDER BY RAND() limit 0,1";	
	list($getGiftsQueryNumRows,$getGiftsQuery)=MainselectfuncNew($getGiftsSql,$array = array());

	$gift_id = $getGiftsQuery[0]['id'];
	
	$dataUpdate = array('reward_selected'=>$gift_id);
	$wherecondition = "(RequestID='".$last_inserted_id."')";
	Mainupdatefunc ('hdfc_car_loan_leads', $dataUpdate, $wherecondition);
}

$dataUpdate1 = array('Car_Model'=>$hdfc_car_name, 'Car_Price'=>$hdfc_car_price, 'intr_rate'=>$roi, 'Tenure'=>$Tenure, 'Loan_Amount'=>$final_Loan_Amount);
$wherecondition = "(RequestID='".$last_inserted_id."')";
Mainupdatefunc ('hdfc_car_loan_leads', $dataUpdate1, $wherecondition);
...
list($getfirstCarNR,$getfirstCarQuery)=MainselectfuncNew($getfirstCarSql,$array = array());
...
<form  method="POST" action="car-loan-hdfc-thanks.php" name="hdfc_calc"  onSubmit="return submitform(document.hdfc_calc);" >
...
<tr>  
<tr><td width="44">&nbsp;</td>   <td width="20" align="left"><img src="images/<?php echo $model; ?>-bullet.png" alt="" width="15" height="15" /></td> 
<td width="302" align="left" class="<?php echo $model; ?>-heading_body-text">Office Telephone</td>  <td width="24"><img src="images/<?php echo $model; ?>-bullet.png" alt="" width="15" height="15" /></td>   
<td width="278" align="left" class="<?php echo $model; ?>-heading_body-text">Office Address Line 2</td>  
</tr> 
<tr>  
<td>&nbsp;</td>  <td>&nbsp;</td>  <td align="left" class="<?php echo $model; ?>-heading_body-text"> 
...
if($numRewards>0 && strlen($reward_selected)>0)
{
?>
...
list($numGifts,$getGiftsQuery)=MainselectfuncNew($getGiftsSql,$array = array());
