<?php
	...
	$update_bidder = $_POST['update_bidder'];
	
	if($update_bidder==1)
	{
		$AgentID = $_POST['AgentID'];
		$sqlBidder = "select * from B_List_Property where BidderID='".$AgentID."'";
		$queryBidder = ExecQuery($sqlBidder);
		$numBidders = mysql_num_rows($queryBidder);
		$AgentName = mysql_result($queryBidder,0,'Bidder_Name');
		$Property_ID = mysql_result($queryBidder,0,'Property_ID');
	}
	$sql = "update property_details_hl set State = '".$State."' ,City = '".$City."' ,Price = '".$Price."' ,Title = '".$Title."' ,Rate = '".$Rate."' ,CoveredArea = '".$CoveredArea."' ,Facilities = '".$Facilities."' ,Description = '".$Description."' ,ApprovedBy = '".$ApprovedBy."' , BuilderName = '".$BuilderName."' , Status = '".$Status."' ,AgentName = '".$AgentName."' ,AgentEmail = '".$AgentEmail."' ,AgentMobile = '".$AgentMobile."' ,AgentID = '".$AgentID."' ,metadesc = '".$metadesc."', keyword = '".$keyword."' where PID='".$PID."'";
	$query = ExecQuery($sql);
	//echo "<br>".$sql;
	$report = "Updated";
//	$creadetPage = '<a href="http://www.deal4loans.com/personal-loan/'.$City.'" target="_blank">Updated Page</a>';

if(strlen($Property_ID)>0)
	{ $insertPropertyVal = $Property_ID.",".$PID; }
	else { $insertPropertyVal = $PID; }
	
	if($update_bidder==1)
	{
		$updateBidderSql = "update B_List_Property set Property_ID='".$insertPropertyVal."' where BidderID='".$AgentID."'";	
		$updateBidderQuery = ExecQuery($updateBidderSql);
	}
...
<tr>
    <td ><div align="center">Home Loan Eligibility Pages</div></td>
    </tr>
    <tr><td>
<?php
echo $report;
...
<a href="content_hleligibility_list.php">Go to List</a>
</td></tr></table>
</body>
</html>