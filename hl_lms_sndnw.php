...
{
	var gifName = form;
	document.frmsearch.action="hl_lms_sndnw.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2015-02-01")
	{
		alert("Sorry!!!! Your maximum date is 2015-02-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
	
	if(document.frmsearch.max_date.value>"<?php echo $Today; ?>")
	{
		alert("Sorry!!!! Your maximum date is <?php echo $Today; ?>.Please Select.");
...
function submitform()
{
 ...
 <form name="frmsearch" action="hl_lms_sndnw.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
    ...
    <tr><?php $currentdate=date('Y-m-d');?>
     ...
     <td width="80%">
		...
		<option value="Loan Taken" <? if($varCmbFeedback == "Loan Taken") { echo "selected"; } ?>>Loan Taken</option>
		<option value="Already Applied" <? if($varCmbFeedback == "Already Applied") { echo "selected"; } ?>>Already Applied</option>
	</select>
	 </td>
   </tr>
   <tr>
     <td width="20%"><strong>Search with Mobile No</strong></td>
     <td width="80%"><input type="text" name="mob_num" id="mob_num" value="<?php echo $mob_num; ?>" >
 ...
 <p>&nbsp;</p>
	...
	if($search=="y")
	{
		...
		if($mob_num>0)
		{
			$mob_num_clause = " AND Req_Loan_Home.Mobile_Number = '".$mob_num."' ";
		}
	?>
 <p class="bodyarial11"><?=$Msg?></p>
 <table width="870" border="0" cellpadding="4" cellspacing="1" class="blueborder" align="center">
 <?		if($_SESSION['BidderID']==5272 && $max_date<=$Today)
		{
			$srch_qry="SELECT Employment_Status,Property_Identified,Name,DOB,City,City_Other,Pincode,Net_Salary,Add_Comment,Loan_Amount,Feedback,Budget,Property_Loc,Loan_Time,comment_section,Property_Value from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (5272) WHERE (Req_Loan_Home.Allocated=1 and City in ('Mumbai','Navi Mumbai','Thane','Delhi','Gurgaon','Noida','Gaziabad','Faridabad','Jaipur','Chandigarh','Bangalore') and Req_Loan_Home.Loan_Amount>=2000000 and Req_Loan_Home.Employment_Status=1 and ((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."' ) or ((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and Req_Feedback_HL.BidderID in (5272))))";
			$srch_qry=$srch_qry.$mob_num_clause;
			$srch_qry=$srch_qry.$FeedbackClause;
			//$qry=$qry."group by Req_Loan_Home.Mobile_Number";
		}
		else
		{
			echo "<script>alert('correct date range ')</script>";
		}
	//echo"hello".$search_qry."<br><br>";
		$changedresult=ExecQuery($srch_qry);
 ...
 ?>
  ...
  <td class="head1"> Date</td>
	...
	<?
		...
		if($_SESSION['BidderID']==5272 && $max_date<=$Today)
		{	
			$qry="SELECT * from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (5272) WHERE (Req_Loan_Home.Allocated=1 and City in ('Mumbai','Navi Mumbai','Thane','Delhi','Gurgaon','Noida','Gaziabad','Faridabad','Jaipur','Chandigarh','Bangalore') and Req_Loan_Home.Loan_Amount>=2000000 and Req_Loan_Home.Employment_Status=1 and ((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."' ) or ((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and Req_Feedback_HL.BidderID in (5272))))";
	...
	?>
   <tr>
   <td class="bodyarial11"><?//echo $row["RequestID"]; ?><a href="hleditlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
	...
	<?
	 ...
	 <td><?
		$GoneToBidders_Arr = "";
		$ReqID = $row["RequestID"];
	  $BiddersChurn="SELECT Bidder_Name,Req_Feedback_Bidder_HL.BidderID As bid FROM Req_Feedback_Bidder_HL LEFT OUTER JOIN Bidders_List ON Bidders_List.BidderID = Req_Feedback_Bidder_HL.BidderID and Bidders_List.Reply_Type =2 WHERE (AllRequestID = '".$ReqID."' AND Req_Feedback_Bidder_HL.Reply_Type =2)";
	//echo $BiddersChurn;
	...
	while($newrow=mysql_fetch_array($BiddersChurnSql))
				{
			$GoneToBidders_Arr[]=$newrow["Bidder_Name"]."(".$newrow["bid"].")";
				}
					echo implode(',', $GoneToBidders_Arr);
		?>
		</td>
  ...
  <td class="bodyarial11"><?php echo $dated;?></td>
    ...
    <?php
	 echo getJumpMenu("hl_lms_sndnw.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
	<?php			
...
?>
	...
	<select name="type" id="type" onChange="MM_jumpMenu('parent',this,0)">
		...
		<option value="<? echo $strURL.'&Feedback=Loan Taken'?>" <? if($varFeedback == "Loan Taken") { echo "selected"; } ?>>Loan Taken</option>
		<option value="<? echo $strURL.'&Feedback=Already Applied'?>" <? if($varFeedback == "Already Applied") { echo "selected"; } ?>>Already Applied</option>
	</select>	
<?
}
?>
