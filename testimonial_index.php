<?php
	...
	$FeedbackClause="";
	$OrderBy=" order by Testimonial.Dated desc";
...
{
	var gifName = form;
	document.frmsearch.action="testimonial_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2007-10-01")
	{
		alert("Sorry!!!! Your minimum date is 2007-10-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function MM_jumpMenu(targ,selObj,restore){ //v3.0
 ...
 <form name="frmsearch" action="testimonial_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//		echo $strSQL;
	...
	if($search=="y")
	{
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(Testimonial.Dated)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(Testimonial.Dated)<='".strtotime($max_date)."'";
		}
		else if($min_date!="" and $max_date=="")
		{
			$min_date=$min_date." 00:00:00";
			$search_date=$search_date." and UNIX_TIMESTAMP(Testimonial.Dated)>='".strtotime($min_date)."'";
		}
		else if($min_date=="" and $max_date!="")
		{
			$max_date=$max_date." 23:59:59";
			$search_date=$search_date." and UNIX_TIMESTAMP(Testimonial.Dated)<='".strtotime($max_date)."'";
		}
		else
		{
			$search_date=$search_date." and Testimonial.Dated!=''";
		}
 ...
 <?
		$search_qry="Select * from Testimonial where Is_Verified=0 and  Name!=''  ".$search_date ;
		$search_qry=$search_qry.$FeedbackClause;
		

		$qry="Select * from Testimonial where Is_Verified=0 and Name!='' ".$search_date  ;
   ...
   <tr>
    ...
    <td class="bodyarial11"><? echo $row["Subject"]; ?></td>
     <td class="bodyarial11"><? echo $row["Message"]; ?></td>
      ...
      <td class="bodyarial11"><a href="checktestimonial.php?id=<?php echo $row["TestID"]; ?>" target="_blank">View</a></font></td>
   </tr>
	<?	
					$i=$i+1;
		}
