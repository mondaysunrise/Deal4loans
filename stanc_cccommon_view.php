...
function getReqValue1($pKey){
	$titles = array(
		...
		'2' => 'Delhi,Gaziabad,Gurgaon,Noida',
		'3' => 'Bangalore',
		'4' => 'Hyderabad',
		'5' => 'Kolkata',
		'6' => 'Pune',
...
{
	var gifName = form;
	document.frmsearch.action="stanc_cccommon_view.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2010-12-29")
	{
		alert("Sorry!!!! Your minimum date is 2010-12-29.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="stanc_cccommon_view.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?$current_date=date('Y-m-d');?> 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2011-01-12"<? } else { ?>value="<? echo $min_date; ?>" <? }?>></td>
	   <td>
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="80%">
	...
	<option value="Loan Rejected" <? if($varCmbFeedback == "Loan Rejected") { echo "selected"; } ?>>Loan Rejected</option>
	 ...
	 <option value="9" <?if($city==9) { echo "selected";} ?>>All</option>
	 <option value="1" <?if($city==1) { echo "selected";} ?>>Mumbai</option>
	<option value="2" <?if($city==2) { echo "selected";} ?>>Delhi</option>
	<option value="3" <?if($city==3) { echo "selected";} ?>>Bangalore</option>
	<option value="4" <?if($city==4) { echo "selected";} ?>>Hyderabad</option>
	<option value="5" <?if($city==5) { echo "selected";} ?>>Kolkata</option>
	<option value="6" <?if($city==6) { echo "selected";} ?>>Pune</option>
	<option value="7" <?if($city==7) { echo "selected";} ?>>Chennai</option>
	...
	if(strlen($branch)>0)
		{
			...
			$citybidder_id=getReqbiddid($city);
			$fedbck_biddid="(".$citybidder_id.",1794)";
		}
		
		if($city<9)
		{
			$search_qry="SELECT * FROM Req_Feedback_Bidder1,`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Credit_Card.RequestID AND Req_Feedback.BidderID in ".$fedbck_biddid." WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID =1794  and `".$val."`.City in ".$propercity." and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date  Between '".($min_date)."' and '".($max_date)."' ) ";	
		...
		{
			$search_qry="SELECT * FROM Req_Feedback_Bidder1,`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Credit_Card.RequestID AND Req_Feedback.BidderID in (1796,1797,1798,1799,1800,1801,1802,1803,1794)  WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID =1794 and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date  Between '".($min_date)."' and '".($max_date)."' ) ";	
			$search_qry=$search_qry.$FeedbackClause;
			$search_qry=$search_qry."group by ".$val.".Mobile_Number";
			$search_qry=$search_qry." order by ".$val.".Dated DESC";
...
//echo $qry;
   ...
   <tr>
	 ...
	 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("stanc_cccommon_view.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val,$city) ?></td>
