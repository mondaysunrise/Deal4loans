...
{
	var gifName = form;
	document.frmsearch.action="hdfc_cl_index.php?search=y"+gifName;
...
<tr><td>&nbsp;</td></tr>
 ...
 <form name="frmsearch" action="hdfc_cl_index.php?search=y" method="post" onSubmit="return chkform();">
 ...
 <? 
		$search_qry="SELECT * FROM hdfccarloan_leads where ( (Is_Valid=1 or captcha_valid=1) and  (Dated  Between '".($min_date)."' and '".($max_date)."'))";

		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by hdfccarloan_leads.Mobile_Number";
		$search_qry=$search_qry." order by hdfccarloan_leads.Dated DESC";

		$qry="SELECT RequestID FROM hdfccarloan_leads where  ( (Is_Valid=1 or captcha_valid=1) and  (Dated  Between '".($min_date)."' and '".($max_date)."'))";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by hdfccarloan_leads.Mobile_Number";
 ...
 ?>
  <tr>
   ...
   <tr>
	 ...
	 <td width="71" align="center" bgcolor="#FFFFFF" class="style2">Car Model</td>
	 <td width="71" align="center" bgcolor="#FFFFFF" class="style2">Download</td>
   </tr>
	<?
		...
		$qry="SELECT * FROM hdfccarloan_leads where ( (Is_Valid=1 or captcha_valid=1) and (Dated  Between '".($min_date)."' and '".($max_date)."'))";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by hdfccarloan_leads.Mobile_Number";
		$qry=$qry." order by hdfccarloan_leads.Dated DESC";
   ...
   <tr>
	 ...
	 <? echo $row["Salutation"]; ?>. <? echo $row["FName"]; ?> <? echo $row["MName"]; ?> <? echo $row["LName"]; ?></td>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3"><? if($row["Employment_Status"]==0) { echo "Self Employed"; } else { echo "Salaried"; }?></td> 
   	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["Car_Model"]; ?>		</td>
  <td align="center" bgcolor="#DFF6FF" class="style3"><a href="https://www.deal4loans.com/pdf/<?php echo $row["AppID"]; ?>.pdf" target="_blank">Download</a></td>
   </tr>
		<?		
		$i=$i+1;		
	}
?>
 ...
 <br>
<form name="frmdownload" action="downloadhdfc.php" method="post">
