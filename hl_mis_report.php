...
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" style="border:#0033CC 1px solid;">
 ...
 <form name="frmsearch" action="hl_mis_report.php?search=y" method="post" >
...
$BidderID = array(460,732,812,207);
//$BidderID = array(9);
$seq = array(0,1,2,3);
$table ='';
...
for($j=0;$j<count($BidderID);$j++)
{
	...
	$bidderDetailsQuery = ExecQuery($bidderDetailsSql);
	...
	$totalLeadsSql = "SELECT RequestID from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (".$BidderID[$j].") WHERE (((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."' ) and MOD(Req_Loan_Home.RequestID,4)='".$seq[$j]."' ) or ((Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."') and Req_Feedback_HL.BidderID in (".$BidderID[$j].")))";
	$totalLeadsQuery = ExecQuery($totalLeadsSql);
	...
	$totalNumLeads = mysql_num_rows($totalLeadsQuery);
		...
		/*$table .= "<tr><td colspan='2' class='data_text_c'><b>";
		$sqlPL = "select * from Req_Feedback_HL left join Req_Loan_Home on Req_Loan_Home.RequestID = Req_Feedback_HL.AllRequestID where (Req_Loan_Home.Dated between '".$start_time."' and '".$end_time."') and  Req_Feedback_HL.BidderID='".$BidderID[$j]."'";
		$sqlQuery = ExecQuery($sqlPL);
		$countLeads = mysql_num_rows($sqlQuery);
		//echo $sqlPL."<br>" ;
		$table .= "Total Leads - ".$countLeads;
		$table .= "</b></td></tr>";
		*/
		$sql_PL = "select Req_Feedback_HL.Feedback as Feedback, count(*) as CountLeads from Req_Feedback_HL left join Req_Loan_Home on Req_Loan_Home.RequestID = Req_Feedback_HL.AllRequestID where (Req_Loan_Home.Dated between '".$start_time."' and '".$end_time."') and  Req_Feedback_HL.BidderID='".$BidderID[$j]."' group by Req_Feedback_HL.Feedback";
