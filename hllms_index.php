...
{
	var gifName = form;
	document.frmsearch.action="hllms_index.php?search=y"+gifName;
...
function chkform()
{
	...
	/*if(document.frmsearch.max_date.value>"2008-05-21")
	{
		alert("Sorry!!!! Please Enter Maximum date 2008-05-21 .");
		document.frmsearch.max_date.value="";
		document.frmsearch.max_date.focus();
		return false;
	}*/
...
function submitform()
{
 ...
 <form name="frmsearch" action="hllms_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">From 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2008-01-08"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
...
//		echo $strSQL;
 ...
 <? 
		//if (($min_date>="2008-01-08 00:00:00") && ($max_date<="2008-05-21 23:59:59"))
			//{
				$qry="SELECT RequestID,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID from Req_Loan_Home LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID= Req_Loan_Home.RequestID AND Req_Feedback.BidderID= '".$_SESSION['BidderID']."'  WHERE (Req_Loan_Home.Bidderid_Details IS NULL or Req_Loan_Home.Bidderid_Details='' and (Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."'))";
 ...
 ?>
   ...
   <td class="bodyarial11"><?//echo $row["RequestID"]; ?><a href="editlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
	...
	<?
	   ...
	   ?>
	<td><form method="post" name="bidder_array" action=""><? list($FinalBidder,$finalBidderName)= geteligibleBiddersList("Req_Loan_Home",$row["RequestID"],$City);
...
<td class="bodyarial11"><?php echo$row["Updated_Date"];?></td>
    <td class="bodyarial11"><? echo getJumpMenu("hllms_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
