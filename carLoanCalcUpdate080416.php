...
$db = mysql_select_db("deal4loans_primary", $conn) or die(mysql_error());

$print_term = $_REQUEST['hdfc_print_term'];
$term = $print_term * 12;
...
list($hdfc_tenure,$hdfc_print_term,$hdfc_inter,$hdfc_Loan_Amount,$EMICalc) =  hdfccl_listdcompChange($income,$age,$car_category,$car_segment,$company_cat,$hdfc_car_price, $hdfc_clid, $hdfc_ratecat, $hdfc_city, $hdfc_car_name, $emp_Stat , $Experience, $salary_account, $resi_stable,$krc_flag, $Car_Type=1,$print_term, $term);

echo $hdfc_tenure.", ".$hdfc_print_term.", ".$hdfc_inter.", ".$hdfc_Loan_Amount.", ".$EMICalc;

function hdfccl_listdcompChange($income,$age,$car_category,$car_segment,$company_cat,$hdfc_car_price, $hdfc_clid, $hdfc_ratecat, $hdfc_city, $hdfc_car_name, $emp_Stat , $Experience, $salary_account, $resi_stable,$krc_flag, $Car_Type=1, $print_term=0, $term=0)
{

	//$term = 29;
	if($print_term>0 && $term>0)
	{}
	else
	{
...
//rate with LTV
$getltv=mysql_query("select ltv_36months,ltv_60months,ltv_84months From hdfc_car_list_category Where (hdfc_clid='".$hdfc_clid."')");

$rowltv = mysql_fetch_array($getltv);
...
else
{
	...
	$alac=$final_loanamt;
	$intr=$hdfcinterate/1200;
	$emiPerLac=round($alac * $intr / (1 - (pow(1/(1 + $intr), $nwterm))));
	//echo "EMI Per Lac - ". $emiPerLac;
	$details[]= $nwterm;
	$details[]= $print_term;
