...
session_start();
	...
	require 'getlistofeligible_fulbidders.php';
...
if(($strCity=="Mumbai" || $strCity=="Thane" || $strCity=="Navi Mumbai"))
	...
	if(($strCity=="Mumbai" || $strCity=="Thane" || $strCity=="Navi Mumbai") && $IncomeAmount>=600000 && count($arrFinal_Bid)>1)
		...
		else
		{
			...
			if(strlen($arrFinal_Bid[0])>0)
			{
				$resultarray=array("1015");
			}	
		}
		}		
		if(($strCity=="Delhi" || $strCity=="Noida" || $strCity=="Gurgaon" || $strCity=="Faridabad" || $strCity=="Gaziabad" || $strCity=="Greater Noida"))
		{
...
$smscamp="Select Mobile_no From Req_Compaign Where (Sms_Flag=1 and BidderID=".$resultarray[$r].")";
list($smscampRows,$myrow)=MainselectfuncNew($smscamp,$array = array());

for($j=0;$j<$smscampRows;$j++)
				{
$strmobile_no = $myrow[$j]["Mobile_no"];
...
if(strlen(trim($strmobile_no)) > 0)
			{
			 ...
			 $strmobile_norc=9811215138;
			 //SendSMSforLMS($message.$SMSMessage, $strmobile_norc);
			}
				}
	}
		}
