...
//SendMailToCustomers("2005,1880","111685","3");
SendMailToCustomers("1000","492426","1");
//2005,1880
//1956
...
function SendMailToCustomers($GetBankID,$CustomerID,$Product)
{
	...
	for($bid=0;$bid<count($GetBidderID);$bid++)	
	{
		...
		if($GetBankCount>0)
		{
			...
			list($GetCount,$Arrrow)=MainselectfuncNew($GetBank_Sql,$array = array());
		$i=0;
			$BidderName = $Arrrow[$i]['Bank_Name'];
			$ExpBidderName[] = $BidderName;
			$ExpBidderContact[] = $BankerContact;
			$bdrBidderID = $Arrrow[$i]['biddrbid'];
	...
	list($GetCount,$Myrow)=MainselectfuncNew($GetCustIDSql,$array = array());
		$r=0;
	
	$GetCustIDQuery = ExecQuery($GetCustIDSql);
	$full_name = $Myrow[$r]['Name'];
	$email  = $Myrow[$r]['Email'];
	$city  = $Myrow[$r]['City'];
	$Net_Salary  = $Myrow[$r]['Net_Salary'];
		 
	if($city == "Others")
	{
		$city  = $Myrow[$r]['City_Other'];
	}
	$mobile_no  = $Myrow[$r]['Mobile_Number'];

	if($Product==1)
	{
		$Account_No  = $Myrow[$r]['PL_Tenure'];
	}
	else if($Product==3)
	{
		$Account_No  = $Myrow[$r]['Account_No'];
	}
	...
	if(((strlen($email)) > 0) && (count($ExpBidderName)>0) ) 
	{
		if($Product==1)
		{
			if($Net_Salary>=240000 && $Account_No=='' && (count($ExpBidderName)>1))
			{
...
for($m=0; $m <count($ExpBidderName);$m++)
	...
	if($defrow[$m]['Define_PrePost'] == "PostPaid")
			{
	if($ExpBidderName[$m]=="HDFC" || $ExpBidderName[$m]=="Standard Chartered" || $ExpBidderName[$m]=="Citibank")
				{
					$Message.="<tr>
...
2) They may be able to give you discount based on your account number.<br> 3)They may have special pre-approved offer for you based on your account</a></td>
		...
		//$Message.="<b>".$ExpBidderName[$m]." - ".$ExpBidderContact[$m]."</b> <a href='http://www.deal4loans.com/apply-car-loans-continue.php?requestid=".$CustomerID."&salary_acc=".$ExpBidderName[$m]."' target='_blank'>Please share your account number.</a><br>";
				}
				else
				{
					$Message.="<tr>
