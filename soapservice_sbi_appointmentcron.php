...
sbisecond_webservice($db_table,$primary_key_table,$product_RequestID);
//soft approval 
...
//sbisecond_webservice($db_table1,$primary_key_table1,$product_RequestID1);
  echo "<br>**********************************************************sbi_credit_card_5633_log*************************************<br>";

 function sbisecond_webservice($db_table,$primary_key_table,$product_RequestID)
 {
...
//  echo $sbiccqry="Select `ApplicationNumber`,".$primary_key_table." From ".$db_table." where (`request2_xml`='' and `ApplicationNumber`>0 and `Messages` IS NULL and (`ProcessingStatus`=1) and first_dated>'".$min_date."')  group by ".$product_RequestID." order by ".$primary_key_table." DESC  limit 0,1";
  ...
  echo $sbiccqry="Select `ApplicationNumber`,".$primary_key_table." From ".$db_table." where (`request2_xml`='' and `ApplicationNumber`>0 and (`ProcessingStatus`=1) and first_dated>'".$min_date."')  group by ".$product_RequestID." order by ".$primary_key_table." DESC";
