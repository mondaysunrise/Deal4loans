...
$getBidderIDQry = "SELECT BidderID FROM Bidders WHERE Profile = '".$BidID."' AND leadidentifier = 'ybankdigicallerlms_cc'";
...
$updateqry= "Update lead_allocate set BidderID='".$getBidderID."' Where AllRequestID = '".$ReqID."' and BidderID=7231";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if($yesapirun==1)
	{
		...
		$source = 'CallerDigitechVerifierYesBank';
		$lead_allocation_logic = 190;
...
<tr>
	<td colspan="4">
	<?php
	$checkSubVerifierQry = "SELECT * FROM `lead_allocate` WHERE BidderID IN (SELECT BidderID FROM `Bidders` WHERE `leadidentifier` IN ('CallerDigitechVerifierYesBank','CallerVerifierYesBank')) AND AllRequestID = '".$requestid."'";
	$checkSubVerifierResult = d4l_ExecQuery($checkSubVerifierQry);
	$checkSubVerifierNumRows = d4l_mysql_num_rows($checkSubVerifierResult);
	if($checkSubVerifierNumRows == 0)
	{
	?>
