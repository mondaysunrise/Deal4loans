...
Personal Loan [Not Called]</td></tr>
  <tr>
    <td align="center">
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" style="border:#0033CC 1px solid;">
 ...
 <form name="frmsearch" action="plmisreportnc.php?search=y" method="post" onSubmit="return chkform();">
...
if($lms_details=="All")
{
	$biddIDS = array(9,10,846,847,854,63,67,68);
	$seq = array(3,4,0,1,2,5,6,7);
}
else 
{
	$biddIDS = array($lms_details);
	if($lms_details==9)	{ $seq = array(3);} else if($lms_details==10) { $seq = array(4);} else if($lms_details==846) { $seq = array(0);} else if($lms_details==847) { $seq = array(1);} else if($lms_details==854) { $seq = array(2);}
}
//echo count($biddIDS);
	for($j=0;$j<count($biddIDS);$j++)
	{	
		$sql_PL = "SELECT count(RequestID) as countLeads, Feedback from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='".$biddIDS[$j]."' WHERE (((MOD(Req_Loan_Personal.RequestID,5)='".$seq[$j]."' and Req_Loan_Personal.Direct_Allocation=0) or (MOD(Req_Loan_Personal.RequestID,5)='".$seq[$j]."' and (DATE_SUB( NOW() , INTERVAL '00:07' HOUR_MINUTE ) >= Req_Loan_Personal.Updated_Date) and Req_Loan_Personal.Is_Valid=0 and Req_Loan_Personal.Direct_Allocation=1)) and ((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) or ((Req_Feedback_PL.Followup_Date Between '".$min_date."' and '".$max_date."' ) and Req_Feedback_PL.BidderID='".$biddIDS[$j]."')) and (Req_Loan_Personal.source!='hdfc_plmlr' and Req_Loan_Personal.source!='PlUploadibibo' and Req_Loan_Personal.source!='postpaid-mailer' and Req_Loan_Personal.ex_source!='below60k' and Req_Loan_Personal.source!='HDFC Bank PL NEW' and Req_Loan_Personal.source!='HDFC Bank PL' and Req_Loan_Personal.source!='bajajfinserv_testing') and Req_Loan_Personal.Allocated=0) group by Req_Feedback_PL.Feedback";
//	echo $sql_PL;
	//echo "<br><br>";
 ...
 <table width="950" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" >
  ...
  <tr>
    ...
    <?php
	for($kk=0;$kk<$count_Leads;$kk++)
	{
		$countLeads = mysql_result($sql_Query,$kk,'countLeads');
		$Feedback = mysql_result($sql_Query,$kk,'Feedback');
		if(strlen($Feedback)>0)
		{ } else {
		?>
		<tr>
             <td align="center" bgcolor="#DFF6FF" class="style3">Not Called</td>
