...
{
	var gifName = form;
	document.frmsearch.action="icicilms_index.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2012-12-01")
	{
		alert("Sorry!!!! Your minimum date is 2012-12-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="icicilms_index.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select FeedbackID from Req_Feedback_CC where AllRequestID=$RequestID and BidderID=".$_SESSION['BidderID']." AND Reply_Type=4");		
...
//		echo $strSQL;
 ...
 <?
 if($min_date>='2014-10-31 23:59:59')
		{
		$qry="Select RequestID From Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID='935' Where (((Req_Credit_Card.Dated Between '".$min_date."' and '".$max_date."') or ((Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and Req_Feedback_CC.BidderID='935')) and Req_Credit_Card.Bidderid_Details='' and (Req_Credit_Card.applied_card_name like '%ICICI%') and Req_Credit_Card.cards_flag =0)";

		}
		else
		{
		$qry="Select RequestID From Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID='935' Where (((Req_Credit_Card.Employment_Status=1 and 
...
(Req_Credit_Card.CC_Holder=1 and Req_Credit_Card.Card_Vintage>=4 and Req_Credit_Card.Credit_Limit>=2))) or (Req_Credit_Card.Employment_Status=0 and Req_Credit_Card.Net_Salary>=250000 and ((Req_Credit_Card.Salary_Account like '%ICICI%') or (Req_Credit_Card.No_of_Banks like '%ICICI%')))) and (City in ('Surat','Nashik','Nasik','Mumbai','Thane','Navi Mumbai','Dombivali','Kalyan','Badlapur','Naigaon','Pune','Bangalore','Mysore','Coimbatore','Chennai','Hyderabad','Secunderabad','Vizag','Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Greater Noida','Kolkata','Hooghly','Howrah','Ahmedabad','Gandhinagar','Jamnagar','Rajkot','Valsad','Vapi','Bharuch','Ankleshwar','Vadoadara','Baroda','Vadodara','Chandigarh','Mohali','Panchkula','Bhubaneswar','Bhubneshwar','Indore','Bhopal','Kochi','Jaipur','Jodhpur','Udaipur','Jammu') or  City_Other in ('Surat','Nashik','Nasik','Mumbai','Thane','Navi Mumbai','Dombivali','Kalyan','Badlapur','Naigaon','Pune','Bangalore','Mysore','Coimbatore','Chennai','Hyderabad','Secunderabad','Vizag','Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Greater Noida','Kolkata','Hooghly','Howrah','Ahmedabad','Gandhinagar','Jamnagar','Rajkot','Valsad','Vapi','Bharuch','Ankleshwar','Vadoadara','Baroda','Vadodara','Chandigarh','Mohali','Panchkula','Bhubaneswar','Bhubneshwar','Indore','Bhopal','Kochi','Jaipur','Jodhpur','Udaipur','Jammu')) and ((Req_Credit_Card.Dated Between '".$min_date."' and '".$max_date."') or 
((Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and Req_Feedback_CC.BidderID='935')) and Req_Credit_Card.Bidderid_Details='' and Req_Credit_Card.cards_flag =0)";
		}
   ...
   <td class="head1">Eligible Bidders</td>
	...
	<?
	 ...
	 if($min_date>='2014-10-31 23:59:59')
		{
		 $qry="		 
Select * From Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID='935' Where (((Req_Credit_Card.Dated Between '".$min_date."' and '".$max_date."') or ((Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and Req_Feedback_CC.BidderID='935')) and Req_Credit_Card.Bidderid_Details='' and (Req_Credit_Card.applied_card_name like '%ICICI%') and Req_Credit_Card.cards_flag =0)";
		}
		else
		{
			 $qry="		 
Select Name,RequestID,Mobile_Number,Net_Salary,City,City_Other,Feedback,Followup_Date From Req_Credit_Card LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= Req_Credit_Card.RequestID and Req_Feedback_CC.BidderID='935' Where (((Req_Credit_Card.Employment_Status=1 and 
(((Req_Credit_Card.Net_Salary>=210000 and Req_Credit_Card.Net_Salary < 300000) and (Req_Credit_Card.Salary_Account like '%ICICI%')) or (Req_Credit_Card.Net_Salary>=300000) or 
(Req_Credit_Card.CC_Holder=1 and Req_Credit_Card.Card_Vintage>=4 and Req_Credit_Card.Credit_Limit>=2))) or (Req_Credit_Card.Employment_Status=0 and Req_Credit_Card.Net_Salary>=250000 and ((Req_Credit_Card.Salary_Account like '%ICICI%') or (Req_Credit_Card.No_of_Banks like '%ICICI%')))) and (City in ('Surat','Nashik','Nasik','Mumbai','Thane','Navi Mumbai','Dombivali','Kalyan','Badlapur','Naigaon','Pune','Bangalore','Mysore','Coimbatore','Chennai','Hyderabad','Secunderabad','Vizag','Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Greater Noida','Kolkata','Hooghly','Howrah','Ahmedabad','Gandhinagar','Jamnagar','Rajkot','Valsad','Vapi','Bharuch','Ankleshwar','Vadoadara','Baroda','Vadodara','Chandigarh','Mohali','Panchkula','Bhubaneswar','Bhubneshwar','Indore','Bhopal','Kochi','Jaipur','Jodhpur','Udaipur','Jammu') or  City_Other in ('Surat','Nashik','Nasik','Mumbai','Thane','Navi Mumbai','Dombivali','Kalyan','Badlapur','Naigaon','Pune','Bangalore','Mysore','Coimbatore','Chennai','Hyderabad','Secunderabad','Vizag','Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Greater Noida','Kolkata','Hooghly','Howrah','Ahmedabad','Gandhinagar','Jamnagar','Rajkot','Valsad','Vapi','Bharuch','Ankleshwar','Vadoadara','Baroda','Vadodara','Chandigarh','Mohali','Panchkula','Bhubaneswar','Bhubneshwar','Indore','Bhopal','Kochi','Jaipur','Jodhpur','Udaipur','Jammu')) and ((Req_Credit_Card.Dated Between '".$min_date."' and '".$max_date."') or 
((Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and Req_Feedback_CC.BidderID='935')) and Req_Credit_Card.Bidderid_Details=''and Req_Credit_Card.cards_flag =0)";
		}
	...
	$rblapptqry = "select rblccid from rbl_creditcard where (cc_requestID ='".$post."') order by rblccid DESC";
	$rblapptqryQuery = ExecQuery($rblapptqry);
	$rblrow=mysql_fetch_array($rblapptqryQuery);
	?>
<tr>
<td class="bodyarial11"><a href="icici_cceditlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
	   ...
	   ?>
	<td><? if($turow["cibilchkid"]>0) {echo "TU DONE";}	
		?></td>
		<td><? if($turow["rblccid"]>0) {echo "RBL DONE";}	
		?></td>
	<td>
<? list($FinalBidder,$finalBidderName)= getBiddersList("Req_Credit_Card",$row['RequestID'],$City);
  for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo getJumpMenu("icicilms_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
