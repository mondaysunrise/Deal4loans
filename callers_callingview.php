...
{
	var gifName = form;
	document.frmsearch.action="callers_callingview.php?search=y"+gifName;
...
<? if (((!isset($val) && $viewtexttype==1) || ($val=="Req_Loan_Personal")) || ((!isset($val) && $viewtexttype==2) || ($val=="".$val."")))
  ...
  <tr>
	...
	<? } ?>
	    <tr><td>
		<form name="frmsearch" action="callers_callingview.php?search=y" method="post" onSubmit="return chkform();">
...
//echo $qry."<br>";
   ...
   <td align="center" bgcolor="#DFF6FF" class="style3"><? echo "PL".$row["Feedback_ID"]."S".$BidderIDstatic; ?></td>
	...
	echo getJumpMenu("callers_callingview.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val,$row["BidderID"]) ?>
		</td>
