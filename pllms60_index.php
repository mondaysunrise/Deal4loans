...
<table width="914" border="0"  align="center">
  ...
  <tr><td align="left"><img src="images/logo.gif" alt="Deal4Loans" onClick="javascript:location.href='/index.php'"/></td><td align="right"> <h1>
	 <?php 	
echo "<font style='Font-size:12px;'>Welcome ".ucwords($_SESSION['UserType'])." <b>".$_SESSION['UName']." ( <a href=Logout.php>Logout</a> )</b>";
	?>	  
</h1><br><br><a href="commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a></td></tr></table>
   </td></tr>
 <tr><td align="center">
  <table width="912" border="0" align="center">
   <tr><td align="center" width="100%">
  <form name="frmsearch" action="pllms60_index.php?search=y" method="post" onSubmit="return chkform();">
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
   ...
   <tr>
     ...
     <td width="80%">From 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="<?php echo date("Y-m")."-01"; ?>"<? } else { ?>value="<? echo $min_date; ?>" <? }?>>
     ...
     <td width="80%">
		...
		<option value="Duplicate" <? if($varCmbFeedback == "Duplicate") { echo "selected"; }?>>Duplicate</option>			<option value="Send Now" <? if($varCmbFeedback == "Send Now") { echo "selected"; } ?>>Send Now</option>			
 ...
 <tr><td align="center" style="padding-top:2px;">
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = d4l_ExecQuery("select FeedbackID from Req_Feedback_PL where AllRequestID=$RequestID and BidderID=".$_SESSION['BidderID']." AND Reply_Type=1");		
...
//		echo $strSQL;
 ...
 <table width="900" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#CCCCCC">
 <? 
	$qryExcludeSourceSql = "SELECT * FROM manual_user_details where 1=1";
	$qryExcludeSourceQuery = d4l_ExecQuery($qryExcludeSourceSql);
	$qryExcludeSourceNum = d4l_mysql_num_rows($qryExcludeSourceQuery);
	$excludeSource = '';
	for($eS=0;$eS<$qryExcludeSourceNum;$eS++)
	{
		$source_eS = d4l_mysql_result($qryExcludeSourceQuery,$eS,'source');
		$excludeSource .= " and Req_Loan_Personal.source!='".$source_eS."' ";
	}
			$salaryfilter="AND (Net_Salary>='0' and Net_Salary<'60001')";
		
		/*	$search_qry="SELECT RequestID, City, Name, Mobile_Number, Net_Salary, Company_Name, Loan_Amount, Employment_Status, City_Other, DOB, Pincode, Residential_Status, CC_Holder, EMI_Paid, Card_Vintage, Company_Type from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='2748'  WHERE (((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) or ((Req_Feedback_PL.Followup_Date Between '".$min_date."' and '".$max_date."' ) and Req_Feedback_PL.BidderID='2748')) and Req_Loan_Personal.source!='hdfc_plmlr' and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Employment_Status=1 ".$salaryfilter." )";
		$search_qry=$search_qry.$FeedbackClause;*/

			$qry="SELECT RequestID from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='2748'  WHERE (
((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) or
((Req_Feedback_PL.Followup_Date Between '".$min_date."' and '".$max_date."' ) and Req_Feedback_PL.BidderID='2748')) and (Req_Loan_Personal.source!='hdfc_plmlr' and Req_Loan_Personal.source!='PlUploadibibo' and Req_Loan_Personal.source!='postpaid-mailer' and Req_Loan_Personal.ex_source!='below60k' and Req_Loan_Personal.ex_source!='SMS_APP_MODEL' and Req_Loan_Personal.source!='HDFC Bank PL NEW' and Req_Loan_Personal.source!='HDFC Bank PL' and Req_Loan_Personal.source!='incoming_call' and Req_Loan_Personal.source!='fullertonmlr' and Req_Loan_Personal.source!='bajajfinserv_testing' and Req_Loan_Personal.source!='Businessloan RBl test' and Req_Loan_Personal.source!='SMS_Lead_Appointment_Model' and Req_Loan_Personal.source!='mywishbankPL' and Req_Loan_Personal.source!='mywishbankchat' and Req_Loan_Personal.source!='wishfinPL' and Req_Loan_Personal.source!='wf_appt_leads' and Req_Loan_Personal.source!='wishfinchat' ".$excludeSource.") and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Employment_Status=1 ".$salaryfilter." )"; //
 ...
 ?>
   <tr>
     <td colspan="11" bgcolor="#FFFFFF"><strong><? echo $startrow+1; ?> to <? echo min($startrow+$pagesize,$recordcount); ?> Out of <? echo $recordcount; ?> Records </strong></td>
     </tr>
   <tr>
   <td width="39" class="head1">Name</td>
	<td width="45" class="head1">Mobile</td>
	<td width="40" class="head1">Salary</td>
	<td width="40" class="head1">City</td>
	<!--<td class="head1">stat</td>-->
   <td width="118" class="head1">Eligible Bidders</td>
     <td width="103" class="head1">Feedback</td>
     <?
	if((($varFeedback=="FollowUp")|| ($varFeedback=="Callback Later")) || ($varCmbFeedback == "FollowUp")) { ?> <td width="104" class="head1">FollowUp date</td>
     <? } ?>
	 <td width="119" class="head1">Feedback Date</td>
	 <td width="123" class="head1">View history</td>
 ...
 $qry="SELECT RequestID, source,Referral_Flag,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID,Net_Salary,  	  updated_flag from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID='2748'  WHERE (
((Req_Loan_Personal.Updated_Date Between '".$min_date."' and '".$max_date."' ) or
((Req_Feedback_PL.Followup_Date Between '".$min_date."' and '".$max_date."' ) and Req_Feedback_PL.BidderID='2748')) and (Req_Loan_Personal.source!='hdfc_plmlr' and Req_Loan_Personal.source!='PlUploadibibo' and Req_Loan_Personal.source!='postpaid-mailer' and Req_Loan_Personal.ex_source!='below60k' and Req_Loan_Personal.ex_source!='SMS_APP_MODEL' and Req_Loan_Personal.source!='HDFC Bank PL NEW' and Req_Loan_Personal.source!='HDFC Bank PL' and Req_Loan_Personal.source!='incoming_call' and Req_Loan_Personal.source!='fullertonmlr' and Req_Loan_Personal.source!='bajajfinserv_testing' and Req_Loan_Personal.source!='Businessloan RBl test' and Req_Loan_Personal.source!='SMS_Lead_Appointment_Model' and Req_Loan_Personal.source!='mywishbankPL' and Req_Loan_Personal.source!='mywishbankchat' and Req_Loan_Personal.source!='wishfinPL' and Req_Loan_Personal.source!='wf_appt_leads' and Req_Loan_Personal.source!='wishfinchat') and Req_Loan_Personal.Allocated=0 and Req_Loan_Personal.Employment_Status=1 ".$salaryfilter." )";
		$qry=$qry.$FeedbackClause;
...
<!--///////////////////////-->
<tr>
<td bgcolor="#FFFFFF" class="bodyarial11"><a href="pleditlead60.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=2748&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
<td bgcolor="#FFFFFF" class="bodyarial11"><? echo $row["Mobile_Number"]; ?></td>
<td bgcolor="#FFFFFF" class="bodyarial11"><? echo $row["Net_Salary"]; ?></td>
<td bgcolor="#FFFFFF" class="bodyarial11"><? echo $row["City"]; ?></td>
...
<?
	   ...
	   ?>
	<td bgcolor="#FFFFFF"><? list($FinalBidder,$finalBidderName)= getBiddersList("Req_Loan_Personal",$row["RequestID"],$City,$row["Referral_Flag"],$row["source"]);
   for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td bgcolor="#FFFFFF" class="bodyarial11"><? echo getJumpMenu("pllms60_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>
    <?php
	$feedsSql = "SELECT Feedback,BidderID from Req_Feedback_PL where BidderID in (846,847,854,9,10) and Reply_Type=1 and AllRequestID='".$row["RequestID"]."'";

	$feedsQuery = d4l_ExecQuery($feedsSql);

	$numFeeds = d4l_mysql_num_rows($feedsQuery);
	
	$feeds = d4l_mysql_result($feedsQuery,0,'Feedback');
	$BiddsID = d4l_mysql_result($feedsQuery,0,'BidderID');
	
	$BidsSql = "SELECT Bidder_Name from Bidders where BidderID ='".$BiddsID."'";
	$BidsQuery = d4l_ExecQuery($BidsSql);
	$BiddName = d4l_mysql_result($BidsQuery,0,'Bidder_Name');
	if($numFeeds)
	{
	...
	<?php if(($Followup_Date>$FinalMinDate || $Followup_Date<$FinalMaxDate) && ($Followup_Date !='0000-00-00 00:00:00') &&  ($TodayFormat==$FinalDay) )  { if($row['updated_flag']==1)
	 ...
	 <td class="bodyarial11" bgcolor="#FFFFFF">
	 <?php } ?>
...
<td width="39" bgcolor="#FFFFFF"><a HREF="javascript:void(0)"
...
<div align="center">
 ...
 <!--<form name="frmdownload" action="downloadless2.4.php" method="post">
...
{
	var gifName = form;
	document.frmsearch.action="pllms60_index.php?search=y"+gifName;
