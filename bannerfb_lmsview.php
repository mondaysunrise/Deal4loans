...
{
	var gifName = form;
	document.frmsearch.action="bannerfb_lmsview.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2015-12-01")
	{
		alert("Sorry!!!! Your minimum date is 2015-12-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="bannerfb_lmsview.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?  $cmpdate=Date('Y-m-d');?> 
 ...
 <? 
	if(isset($comp_source) || strlen($_SESSION["Comp_Source"])>0 )
		{
			$qry2="SELECT komli_uniqueid from  komli_plcompaign LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= komli_plcompaign.komli_uniqueid AND Req_Feedback_PL.BidderID in (846,847,854,9,10,2748,11,2783,3621,63,67,68) WHERE ((komli_date Between '".$min_date."' and '".$max_date."') and  vendor_name='".$comp_source."' and validated=1 and Req_Feedback_PL.Feedback not in ('FollowUp','Callback Later','Send Now'))";
...
<td width="141" align="center" bgcolor="#FFFFFF" class="style2">Feedback</td>
  ...
  <?  
	 $qry="SELECT komli_uniqueid,Feedback from  komli_plcompaign LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID= komli_plcompaign.komli_uniqueid AND Req_Feedback_PL.BidderID in (846,847,854,9,10,2748,11,2783,3621,63,67,68) WHERE ((komli_date Between '".$min_date."' and '".$max_date."') and  vendor_name='".$comp_source."' and validated=1 and Req_Feedback_PL.Feedback not in ('FollowUp','Callback Later','Send Now'))";
