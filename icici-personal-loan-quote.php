...
//print_r($_POST);
//echo "<br>";
//Array ( [City] => Delhi [day] => 12 [month] => 12 [year] => 1980 [Pincode] => 423344 [Employment_Status] => 1 [company_name] => efdsfds [Company_Type] => 2 [Years_In_Company] => 5 [Total_Experience] => 8 [Net_Salary1] => 45000 [Residential_Status] => 1 [CC_Holder] => 1 [card_obligation] => 65443 [existing_relationship] => 1 [relatonship] => 1 [LoanAny] => 1 [Loan_Any] => Array ( [0] => hl [1] => cl [2] => other ) [EMI_Paid] => 3 [] => 45354 [image_x] => 42 [image_y] => 13 [image] => Submit )  
//(($Employment_Status==1) && ($age>=$checkAge && $age<59) && (($Total_Experience>2)) &&  ($Net_Salary1>$minimumSalary) && ($eligiblewrtLoanAmt == "Yes") && ($Company_Type!=6) && ($Residential_Status==2 || $Residential_Status==1))
...
if($Employment_Status==1)
{
	...
	$Years_In_Company = $_POST['Years_Company'];
...
if($category=="Elite")	{	$maxLoanAmount = 1500000;	}
else if($category=="Super Prime" && $existing_relationship==1 && ($relationship==1 || $relationship==2) )	{	$maxLoanAmount = 1500000;	}
else if($category=="Super Prime" && $existing_relationship==0 && ($relationship==1 || $relationship==2) ) {	$maxLoanAmount = 1000000;	}
else if($existing_relationship==1 && ($relationship==1 || $relationship==2))	{	$maxLoanAmount = 1000000;	}
else {	$maxLoanAmount = 700000;	}
...
{
		//$other_emi
	$eligibleEMIAmt = $Net_Salary1 * (35/100);
...
else
{
	...
	else if($Net_Salary1>=50000 && $Net_Salary1<200000)
	{
		//$other_emi
		$eligibleEMIAmt = $Net_Salary1 * (60/100);
		if($other_emi>$eligibleEMIAmt)	{ 	$eligiblewrtLoanAmt = "No";	}
		else { 	$eligiblewrtLoanAmt = "Yes";  }
	}	
	else
...
?>
  ...
  <form name="loan_form" method="post" action="icici-personal-loan-continue.php" onSubmit="return submitform(document.loan_form);" >
    ...
    <input type="hidden" name="Months_Company" value="<?php echo $Months_Company ; ?>" />
    <input type="hidden" name="Total_Exp_Month" value="<?php echo $Total_Exp_Month ; ?>" />
