...
<div id="payment_container-dash_page">
  ...
  <div class="content_right">
    <h1>Leads assign to Field Executive</h1>
    <br />
    <?php 
	if($_REQUEST['msg']!="")
			{
   ...
   <table width="700" border="0" cellpadding="4" cellspacing="1" class="blueborder" bgcolor="#FFFFFF" style="border:#666 1px solid;">
              <form name="frmsearch" action="bl-assign-to1.php" method="get" onSubmit="return chkform();">
...
$w = 1;
$color = 1;
$Query = "select Req_Loan_Personal.RequestID as AllRequestID, Name,City, Mobile_Number, docpickerid, id from zexternal_appointment_docs LEFT OUTER JOIN Req_Loan_Personal ON Req_Loan_Personal.RequestID = zexternal_appointment_docs.RequestID where 1=1 and zexternal_appointment_docs.Reply_Type=6 and zexternal_appointment_docs.viewstatus=1 ";
...
{		
	...
	if(strlen($refernce_no)>3)
	{
		...
		$getRefIDSearchSql = "select AllRequestID from Req_Feedback_PL where FeedbackID='".$refernce_no_section."'";
...
while($rowsCon = $obj->fun_db_fetch_rs_object($result))
 ...
 ?>
        <tr bgcolor="<?php echo $colorvar;?>">
          <td><?php echo $w;?></td>
          <td><?php
		  	$getRefIDSql = "select FeedbackID,BidderID from Req_Feedback_PL where AllRequestID='".$rowsCon->AllRequestID."'";
			$getRefIDResult = $obj->fun_db_query($getRefIDSql);
			$rowsRefID = $obj->fun_db_fetch_rs_object($getRefIDResult);
			
		   echo $uniqueid = "PL".$rowsRefID->FeedbackID."S".$rowsRefID->BidderID; ?></td>
          <td><a href="doc-pick-user-details-bl.php?id=<?php echo $rowsCon->id;?>" target="_blank"><?php echo $rowsCon->Name;?></a></td>
          <td><?php echo $rowsCon->Mobile_Number;?></td>
          <td><?php echo $rowsCon->City;?></td>
          <td><select name="pickup_person" id="pickup_person"  onchange="loadDoc(<?php echo $rowsCon->id;?>,this.value,'<?php echo $_SESSION['UID'];?>')"><option value="">Please Select</option>
