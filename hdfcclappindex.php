<?php
	...
	if(isset($_REQUEST['Feedback']))
	{
   	...
   	$checkCity="";
	if(isset($_REQUEST['checkCity']))
	{
		$checkCity=$_REQUEST['checkCity'];
	}
	//Paging
	$pagesize=25;
	$startrow=0;
...
{
	var gifName = form;
	document.frmsearch.action="hdfcclappindex.php?search=y"+gifName;
...
function chkform()
{
	...
	<? } 
		...
		if(document.frmsearch.min_date.value<"2012-10-04")
	{
		alert("Sorry!!!! Your minimum date is 2012-10-04.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="hdfcclappindex.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">
    <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     <td width="20%" valign="middle" class="style1">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; City</td>
     <td width="24%" align="left" valign="middle" class="bidderclass" >  <?php
     $qryCity = ExecQuery("SELECT * FROM Bidders_List WHERE Reply_Type='3' and BidderID=1825");
	 $strCity = mysql_result($qryCity,0,'City');
 	 $cityList = explode(",", $strCity);
	 ?>
     <select name="checkCity">
	 ...
	 {
	 	$selected = "";
	 	if($checkCity==$cityList[$i])
		{	
			$selected = "selected";
		}
	 	echo ' <option value="'.$cityList[$i].'" '.$selected.'>'.$cityList[$i].'</option>';
	 }
	 ?>
	 </select></td>
	   <td>&nbsp;
...
// print_r($_POST);	
	...
	//$appendSql = " and (((DOB!='' and DATE_SUB(CURDATE(),INTERVAL 21 YEAR) >= DOB) and Employment_Status=1) or ((DOB!='' and DATE_SUB(CURDATE(),INTERVAL 25 YEAR) >= DOB) and Employment_Status=0)) and Net_Salary>=200000";
		...
		$search_qry = "SELECT * from hdfc_car_loan_leads where `Source` in ('car-loan-hdfc','hdfc_car_loan') and Loan_Amount>0 and Name!='' and (Dated Between '".($min_date)."' and '".($max_date)."') and City in ('".$cityList."')  ";
		$search_qry=$search_qry."group by Mobile_Number";
		...
		$qry=$qry."group by Mobile_Number";
...
//echo $qry;
 ?>
	  <tr>
     <td colspan="7" style="border-bottom:1px solid #45B2D8;"><strong><? echo $startrow+1; ?> to <? echo min($startrow+$pagesize,$recordcount); ?> Out of <? echo $recordcount; ?> Records </strong>
     </tr>
   <tr>
     <td width="179" align="center" bgcolor="#FFFFFF" class="style2">Name</td>
	 <td width="106" align="center" bgcolor="#FFFFFF" class="style2">Mobile</td>
	 <td width="156" align="center" bgcolor="#FFFFFF" class="style2">City</td>
     <td width="144" align="center" bgcolor="#FFFFFF" class="style2">Employment Status </td>
          <td width="149" align="center" bgcolor="#FFFFFF" class="style2">Salary</td>
     <td width="113" align="center" bgcolor="#FFFFFF" class="style2">Loan Amount</td>
    <td width="81" align="center" bgcolor="#FFFFFF" class="style2">SMS Details</td>
 <!--	<td width="180" align="center" bgcolor="#FFFFFF" class="style2">Download</td> -->
...
//echo "<br>".$qry;
   ...
   <tr>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3" height="25" >
	 <? echo $row["Name"]; ?></td>
	  ...
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["City"]; ?></td>
		...
		<?php if(strlen($row["sms_to"])>0) { ?>
		<? echo $row["sms_to"]; ?>, <? echo $row["sms_number"];  } ?></td>
           	   <!-- <td align="center" bgcolor="#DFF6FF" class="style3">	 -->
				<?php 
				$income_proof = $row["income_proof"];
				$address_proof = $row["address_proof"];
				$identity_proof = $row["identity_proof"];
				$bank_statement = $row["bank_statement"];
				$income_proof_doc= $row["income_proof_doc"];
				$address_proof_doc= $row["address_proof_doc"];
				$identify_proof_doc = $row["identify_proof_doc"];
				$bank_statement_doc= $row["bank_statement_doc"];
				
				if(strlen($income_proof_doc)>0)
				{
				
				?>
              <!--  <a href="http://www.deal4loans.com/upload_hdfcCL/<?php echo $income_proof_doc; ?>" target="_blank"><?php echo $income_proof; ?></a><br> -->
                <?php
				}
				if(strlen($address_proof_doc)>0)
				{
				
				?>
                <!--<a href="http://www.deal4loans.com/upload_hdfcCL/<?php echo $address_proof_doc; ?>" target="_blank"><?php echo $address_proof; ?></a><br> -->
                <?php				
				}
				if(strlen($identity_proof_doc)>0)
				{
				
				?>
               <!-- <a href="http://www.deal4loans.com/upload_hdfcCL/<?php echo $identify_proof_doc; ?>" target="_blank"><?php echo $identify_proof; ?></a><br> -->
                <?php
				}
				if(strlen($bank_statement_doc)>0)
				{
				
				?>
             <!--   <a href="http://www.deal4loans.com/upload_hdfcCL/<?php echo $bank_statement_doc; ?>" target="_blank"><?php echo $bank_statement; ?></a><br> -->
                <?php				
				}
				
				?>	
                <!-- </td>
	<td align="center" bgcolor="#DFF6FF" class="bodyarial11" ><a href="#" onClick="return popitup('hdfcgeneratePDF.php?vid=<?php echo $row["RequestID"]; ?>')">Download</a></td> -->
   </tr>
	<?
		$i=$i+1;
		}
 ...
 <form name="frmdownload" action="hdfcapp_download.php" method="post">
   <tr>
     <td align="center">
