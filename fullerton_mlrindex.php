...
{
	var gifName = form;
	document.frmsearch.action="fullerton_mlrindex.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.min_date.value<"2014-02-11")
	{
		alert("Sorry!!!! Your minimum date is 2014-02-11.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
	...
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#45B2D8">
		<tr><td style="padding-top:5px; font-size:13px;">&nbsp;&nbsp; </a>
 </td></tr>
      <tr>
...
<tr><td>&nbsp;</td></tr>
 ...
 <form name="frmsearch" action="fullerton_mlrindex.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="85%">
     ...
     <td width="10%"  valign="middle" class="bidderclass">&nbsp;</td>
       <td width="30%" valign="middle" class="style1">Feedback</td>
		 <td width="50%" align="left" valign="middle" class="bidderclass"><select name="cmbfeedback" id="cmbfeedback">
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select RequestID from fullerton_exclusivecamp where RequestID=$RequestID");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update fullerton_exclusivecamp Set Feedback='".$Feedback."'";
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (fullerton_exclusivecamp.Feedback IS NULL OR fullerton_exclusivecamp.Feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND fullerton_exclusivecamp.Feedback='".$varCmbFeedback."' ";
		}
	?>
 <p class="bodyarial11"><?=$Msg?></p>
 <table width="950" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" >
 <? 
	$search_qry="SELECT *  FROM fullerton_exclusivecamp where (Dated  Between '".($min_date)."' and '".($max_date)."')";

		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by fullerton_exclusivecamp.Mobile_Number";
		$search_qry=$search_qry." order by fullerton_exclusivecamp.Dated DESC";

		$qry="SELECT RequestID FROM fullerton_exclusivecamp where (Dated Between '".($min_date)."' and '".($max_date)."')";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by fullerton_exclusivecamp.Mobile_Number";
		//echo $qry."<br><br>";
 ...
 ?>
  <tr>
	 ...
	 <td width="180" align="center" bgcolor="#FFFFFF" class="style2">Bidders</td>
 <td width="71" align="center" bgcolor="#FFFFFF" class="style2">Feedback</td>
	...
	<?
		...
		$qry="SELECT * FROM fullerton_exclusivecamp where (Dated Between '".($min_date)."' and '".($max_date)."')";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by fullerton_exclusivecamp.Mobile_Number";
		$qry=$qry." order by fullerton_exclusivecamp.Dated DESC";
...
//echo $qry."<br>";
   ...
   <tr>  
	 ...
	 <td align="center" bgcolor="#DFF6FF" class="style3"><? 
	 $biddtls=ExecQuery("select Bidder_Name from Bidders_List Where (BidderID='".$row["Bidderid_Details"]."')");
	 $newrow=mysql_fetch_array($biddtls);
	 echo "".$newrow["Bidder_Name"]."(".$row["Bidderid_Details"].")";
	 ?>
	 </td>
   	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("fullerton_mlrindex.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?>		</td>
	<td align="center" bgcolor="#DFF6FF" class="bodyarial11" ><table width="100%"><tr><td><textarea  name="comment_section_<? echo $i;?>" id="comment_section_<? echo $i;?>" cols="15" rows="2"><? echo $row["Add_Comment"]; ?></textarea></td><td><a onClick="insertData(<? echo $i;?>);" style="cursor:pointer; color:blue;" class="style3">Save</a></td></tr></table></td>
 ...
 <br>
<form name="frmdownload" action="fuldownloading.php" method="post">
