...
$BidderID = '';
	...
	$finalBidders = implode(',', $BidderID);
		
		//if($row_result["Email"]=="")
		//{		$dob=$row_result["dob"];} else { $dob=$row_result["DOB"];}
		$qry1="insert into temp (session_id, name, dob, email, emp_status, c_name, city, city_other, mobile_number, std_code, landline, std_code_o, landline_o , net_salary, descr, vehicle_owned, cc_holder, is_processed, pancard, no_of_banks, pincode, card_vintage, doe, source, count_replies, request_id, address_apt ) values ('".$session_id."', '".$row_result["Name"]."', '".$row_result["DOB"]."', '".$row_result["Email"]."', '".$emp_status."', '".$row_result["Company_Name"]."', '".$row_result["City"]."', '".$row_result["City_Other"]."', '".$row_result["Mobile_Number"]."', '".$finalBidders."', '".$row_result["Landline"]."', '".$row_result["Std_Code_O"]."', '".$row_result["Landline_O"]."', '".$row_result["Net_Salary"]."', '".$row_result["Descr"]."', '".$vehicle_owned."', '".$cc_holder."', '".$strcrd."', '".$row_result["Pancard"]."',  '".$row_result["applied_card_name"]."', '".$row_result["Pincode"]."', '".$card_vintage."', '".$row_result["Dated"]."', '".$row_result["source"]."', '".$row_result["Bidder_Count"]."', '".$row_result["RequestID"]."', '".$row_result["No_of_Banks"]."')";
	...
	//echo "3::";
	$qry="select name, dob,mobile_number, email, emp_status, c_name, city, city_other, net_salary, descr, vehicle_owned, cc_holder,card_vintage , pancard, is_processed As OptionGiven, Feedback,  no_of_banks AS CardOpted, address_apt as ExistingCard, doe, source, pincode , is_valid, count_replies As BidderCount, std_code as Bidders from temp where session_id='".$session_id."'";
