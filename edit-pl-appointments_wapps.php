...
//print_r ($Loan_Any)."<br>";
		...
		$office_address = $_REQUEST['office_address'];
                $purpose_of_loan = $_REQUEST['LoanPurpose'];
                $Contact_Time = $_REQUEST["LoanTime"];
...
//		print_r($serializecc_obligation);
		...
		$cibil_reference_id = FixString($_POST['cibil_reference_id']);
                ...
                $NumEMIPaid = FixString($_POST["NumEMIPaid"]);
                $CurrentTenure = FixString($_POST["CurrentTenure"]);
                $CurrentROI = FixString($_POST["CurrentROI"]);
	...
	//	echo "<br>query".$updatelead."<br>";
	 ...
	 if($plfeedback=="Send Now" && $Final_Bidder>100)
	 {
		...
		$echoQuery = '';
	 	...
	 	$updateAllocation = "Update Req_Loan_Personal set Allocated='1'  where RequestID=".$post;
...
//	 	$echoQuery .= $InsertFeedBackSqlPL."<br>";
	...
	//	$echoQuery .= $final_allocation."<br>";
		...
		$insertExpertMailerSql = "INSERT INTO wishfin_mailer_send (RequestID, BidderID, AgentID, Dated) VALUES ('".$post."', '".$Final_Bidder."', '".$Bidder_Id."', Now())";
...
<?php 
$viewqry="select Pancard,company_category, CC_Age,Annual_Turnover, Company_Type,PL_Bank,PL_Tenure,Name,Tataaig_Home,Tataaig_Health,Tataaig_Auto,Accidental_Insurance,Add_Comment, Mobile_Number,Landline,Landline_O,Std_Code, Std_Code_O,Net_Salary,Residential_Status,City,City_Other, Is_Valid,Dated,Employment_Status,Loan_Amount,Email,Contactable, source,Loan_Any,Pincode,SentEmail,Emi_Paid, CC_Holder,Card_Vintage,Followup_Date,Feedback,CC_Mailer,Company_Name, PL_EMI_Amt,Card_Limit,Salary_Drawn,Landline_Connection,Mobile_Connection, Total_Experience,Years_In_Company,DOB,Bidderid_Details,checked_bidders,Primary_Acc,Referral_Flag,Creative,Existing_Bank,Existing_Loan,Existing_ROI,Contact_Time from Req_Loan_Personal LEFT OUTER JOIN Req_Feedback_PL ON Req_Feedback_PL.AllRequestID=Req_Loan_Personal.RequestID and Req_Feedback_PL.BidderID= '".$bidid."' where Req_Loan_Personal.RequestID=".$post." "; 
//echo "dd".$viewqry;
...
$company_category = mysql_result($viewlead,0,'company_category');
$LoanTime = mysql_result($viewlead,0,'Contact_Time');
...
$sqlExtraFields = "select * from Req_Loan_Personal_Extra_Fields where RequestID='".$post."'";
	...
	if($numRowsExtraFields>0)
	{
		...
		$card_outstanding = mysql_result($queryExtraFields,0,'card_outstanding');
                ...
                $LoanPurpose = mysql_result($queryExtraFields,0,'purpose_of_loan');
		...
		$obligation_details = mysql_result($queryExtraFields,0,'obligation_details');
                ...
                $current_tenure = mysql_result($queryExtraFields,0,'current_tenure');
                $current_roi = mysql_result($queryExtraFields,0,'current_roi');
                $no_of_emipaid = mysql_result($queryExtraFields,0,'no_of_emipaid');
...
<table style='border:1px dotted #9C9A9C;' cellspacing="2" cellpadding="5" width="1000" border="0" >
<tr>
	<td style="border:1px solid black;" colspan="4" bgcolor="#DAEAF9" align="center" class="fontstyle" ><b>Personal Details</b></td></tr>
		...
		<td class="fontstyle"><b>DOB (As per Pancard)</b></td>
			<td class="fontstyle"><input type="text" name="pldob" id="pldob" value="<? echo $DOB;?>"></td>
			<td class="fontstyle"><b>Mobile</b></td>
			<td class="fontstyle">+91<input type="hidden" name="plmobile" value="<? echo $Mobile;?>"><strong><?php echo ccMasking($Mobile); ?><? //echo $Mobile;?></strong></td>
			</tr>
		<tr>
...
<tr>
	...
	<td><input type="text" name="plcompany_name" id="plcompany_name" onKeyUp="ajax_showOptions(this,'getCountriesByLetters',event)" value="<? echo str_replace("'", '',$Company_Name);?>"></td><td colspan="2" ><span id='formatedIncome' style='font-size:11px; font-weight:bold;color:666699;font-Family:Verdana;'></span><span id='wordIncome' style='font-size:11px;
font-weight:bold;color:666699;font-Family:Verdana;text-transform: capitalize;'></span></td>
        </tr>
        <tr>
         <td class="fontstyle"><b>Company Category</b></td>
         <td><select name="company_category" id='company_category'>
                 <option value="">Select Company Category</option>
                 <option value="Listed" <?php if($company_category=='Listed') { echo "Selected";}?>>Listed</option>
                 <option value="Unlisted" <?php if($company_category=='Unlisted') { echo "Selected";}?>>Unlisted</option>
             </select></td>
        </tr>
	<tr>
...
<td><input type="text" name="incorporation_date" id="incorporation_date" value="<? echo $incorporation_date; ?>"></td>
	...
	<tr>
     ...
     <input type="radio" value="0" name="pf_deduction"  id="pf_deduction2" class="NoBrdr" <? if($pf_deduction==0){ echo "checked";}?>>No</td>
        ...
        <tr>
		<td class="fontstyle"><b>No. of EMIs Paid (Months)</b></td>
		<td class="fontstyle"><select name="NumEMIPaid" id="NumEMIPaid" style="width: 190px;">
		  <option value="">Please Select</option>
                  <?php for($m = 1; $m<=60;$m++) {?>      
                  <option value="<?php echo $m;?>" <? if($no_of_emipaid==$m) {echo "selected";} ?>><?php echo $m;?></option>
                  <?php }?>
                    </select>
                </td>
		<td class="fontstyle"><b>Current Tenure (Years)</b></td>
		<td class="fontstyle"><select name="CurrentTenure" id="CurrentTenure" style="width: 190px;">
		  <option value="">Please Select</option>
                  <?php for($ct = 1; $ct<=5;$ct++) {?>      
                  <option value="<?php echo $ct;?>" <? if($current_tenure==$ct) {echo "selected";} ?>><?php echo $ct;?></option>
                  <?php }?>
                    </select></td>
	</tr>
        
        <tr>
		<td class="fontstyle"><b>Current ROI</b></td>
		<td class="fontstyle"><select name="CurrentROI" id="CurrentROI" style="width: 190px;">
		  <option value="">Please Select</option>
                      
                  <option value="10.00" <? if($current_roi=='10.00') {echo "selected";} ?>>10.00%-10.50%</option>
                   <option value="10.50" <? if($current_roi=='10.50') {echo "selected";} ?>>10.50% - 11.00%</option>
                    <option value="11.00" <? if($current_roi=='11.00') {echo "selected";} ?>>11.00% - 11.50%</option>
                     <option value="11.50" <? if($current_roi=='11.50') {echo "selected";} ?>>11.50%-12.00%</option>
                      <option value="12.00" <? if($current_roi=='12.00') {echo "selected";} ?>>12.00%-12.50%</option>
                      
                       <option value="12.50" <? if($current_roi=='12.50') {echo "selected";} ?>>12.50%-13.00%</option>
                        <option value="13.00" <? if($current_roi=='13.00') {echo "selected";} ?>>13.00%-13.50%</option>
                        <option value="13.50" <? if($current_roi=='13.50') {echo "selected";} ?>>13.50%-14.00%</option>
                        <option value="14.00" <? if($current_roi=='14.00') {echo "selected";} ?>>14.00%-14.50%</option>
                        <option value="14.50" <? if($current_roi=='14.50') {echo "selected";} ?>>14.50%-15.00%</option>
                        <option value="15.00" <? if($current_roi=='15.00') {echo "selected";} ?>>15.00%-15.50%</option>
                        <option value="15.50" <? if($current_roi=='15.50') {echo "selected";} ?>>15.50%-16.00%</option>
                        <option value="16.00" <? if($current_roi=='16.00') {echo "selected";} ?>>16.00%-16.50%</option>
                        <option value="16.50" <? if($current_roi=='16.50') {echo "selected";} ?>>16.50%-17.00%</option>
                        <option value="17.00" <? if($current_roi=='17.00') {echo "selected";} ?>>17.00%-17.50%</option>
                        <option value="17.50" <? if($current_roi=='17.50') {echo "selected";} ?>>17.50%-18.00%</option>
                        
                        <option value="18.00" <? if($current_roi=='18.00') {echo "selected";} ?>>18.00%-18.50%</option>
                        <option value="18.50" <? if($current_roi=='18.50') {echo "selected";} ?>>18.50%-19.00%</option>
                        <option value="19.00" <? if($current_roi=='19.00') {echo "selected";} ?>>19.00%-19.50%</option>
                        <option value="19.50" <? if($current_roi=='19.50') {echo "selected";} ?>>19.50%-20.00%</option>
                        <option value="20.00" <? if($current_roi=='20.00') {echo "selected";} ?>>20.00%-20.50%</option>
                        <option value="20.50" <? if($current_roi=='20.50') {echo "selected";} ?>>20.50%-21.00%</option>
                        
                        
                        <option value="21.00" <? if($current_roi=='21.00') {echo "selected";} ?>>21.00%-21.50%</option>
                        <option value="21.50" <? if($current_roi=='21.50') {echo "selected";} ?>>21.50%-22.00%</option>
                        <option value="22.00" <? if($current_roi=='22.00') {echo "selected";} ?>>22.00%-22.50%</option>
                        <option value="22.50" <? if($current_roi=='22.50') {echo "selected";} ?>>22.50%-23.00%</option>
                        <option value="23.00" <? if($current_roi=='23.00') {echo "selected";} ?>>23.00%-23.50%</option>
                        <option value="23.50" <? if($current_roi=='23.50') {echo "selected";} ?>>23.50%-24.00%</option>
                        <option value="24.00" <? if($current_roi=='24.00') {echo "selected";} ?>>24.00%-24.50%</option>
                        <option value="24.50" <? if($current_roi=='24.50') {echo "selected";} ?>>24.50%-25.00%</option>
                        <option value="25.00" <? if($current_roi=='25.00') {echo "selected";} ?>>25.00%-25.50%</option>
                        <option value="25.50" <? if($current_roi=='25.50') {echo "selected";} ?>>25.50%-26.00%</option>
                        <option value="26.00" <? if($current_roi=='26.00') {echo "selected";} ?>>26.00%-26.50%</option>
                        <option value="26.50" <? if($current_roi=='26.50') {echo "selected";} ?>>26.50%-27.00%</option>
                       
                        <option value="27.00" <? if($current_roi=='27.00') {echo "selected";} ?>>27.00%-27.50%</option>
                        <option value="27.50" <? if($current_roi=='27.50') {echo "selected";} ?>>27.50%-28.00%</option>
                        <option value="28.00" <? if($current_roi=='28.00') {echo "selected";} ?>>28.00%-28.50%</option>
                        <option value="28.50" <? if($current_roi=='28.50') {echo "selected";} ?>>28.50%-29.00%</option>
                        <option value="29.00" <? if($current_roi=='29.00') {echo "selected";} ?>>29.00%-29.50%</option>
                        <option value="29.50" <? if($current_roi=='29.50') {echo "selected";} ?>>29.50%-30.00%</option>
                        <option value="30.00" <? if($current_roi=='30.00') {echo "selected";} ?>>30.00%</option>
...
<tr><td class="fontstyle"><b>Loan Time</b></td>
    <td class="fontstyle"><select name="LoanTime">
            <option value="">Select Loan Time</option>
            <option value="1" <?php if($LoanTime==1) { echo "Selected";}?>>Urgent</option>
            <option value="7" <?php if($LoanTime==7) { echo "Selected";}?>>1 Week</option>
            <option value="14"<?php if($LoanTime==14) { echo "Selected";}?>>2 Weeks</option>
            <option value="30"<?php if($LoanTime==30) { echo "Selected";}?>>1 Month</option>
            <option value="32"<?php if($LoanTime==32) { echo "Selected";}?>>More Than 1 Month</option>
         </select></td>
<td class="fontstyle"><b>End Use of Loan</b></td>
<td class="fontstyle"><select name="LoanPurpose">
            <option value="">Select End Use of Loan</option>
            <option value="1"<?php if($LoanPurpose==1) { echo "Selected";}?>>Debt Consolidation</option>
            <option value="2"<?php if($LoanPurpose==2) { echo "Selected";}?>>Medical Emergency</option>
            <option value="3"<?php if($LoanPurpose==3) { echo "Selected";}?>>Marriage In Family</option>
            <option value="4"<?php if($LoanPurpose==4) { echo "Selected";}?>>Travel</option>
            <option value="5"<?php if($LoanPurpose==5) { echo "Selected";}?>>Investment/Property Purchase</option>
            <option value="6"<?php if($LoanPurpose==6) { echo "Selected";}?>>Personal Use</option>
         </select></td>
</tr>
<tr>
...
//echo $City;?>
<tr><td class="fontstyle" valign="top"><b>Eligible for Bidders</b></td><? if(strlen($Bidderid_Details)>0){ ?><td class="fontstyle" colspan="2"> already send</td><? } else {?><td colspan="2"><div id="checkdiv"><? 
...
<td><input name="company" id="company" type="text" onKeyUp="ajax_showOptions(this,'getCountriesByLetters',event)" value="<? echo str_replace("'", '', $Company_Name);?>"/>                                   
...
if($allocatedCount>0)
{
 ...
 <tr><td colspan="4">
     <form action="editpersonaldetailspl_whatsapp.php" name="whatsapp_frm" method="post">
				...
				<input type="hidden" name="requestid" id="requestid" value="<? echo $post;?>" />
 ...
 ?>
               ...
               <tr><td colspan="4"><hr /></td></tr>
            	...
            	$sql = "SELECT message,xkyknzl5dwfyk4hg_wishfin_whatsapp.date_created FROM `xkyknzl5dwfyk4hg_wishfin_whatsapp` LEFT OUTER JOIN xkyknzl5dwfyk4hg_tms_whatsapp on xkyknzl5dwfyk4hg_tms_whatsapp.whatsapp_id=xkyknzl5dwfyk4hg_wishfin_whatsapp.id WHERE `mobile_number` = '".$Mobile."' AND `mobile_number`!='' AND  process_name='deal4loan_experts_message' ORDER BY `xkyknzl5dwfyk4hg_wishfin_whatsapp`.`date_created` ASC ";
				$qry = d4l_ExecQuery($sql);
				$num =d4l_mysql_num_rows($qry);
				$message = '';
				$commonDate = '';
				$commonHeader='';
				if($num>0)
				{
					$messageCounter =1;
					$initialDate=d4l_mysql_result($qry,0,'date_created');
					for($i=0;$i<$num;$i++)
					{
						$message[$messageCounter]=d4l_mysql_result($qry,$i,'message');
						$commonDate[$messageCounter]=d4l_mysql_result($qry,$i,'date_created');
				...
				$qry1 = d4l_ExecQuery($sql1);
				$num1 =d4l_mysql_num_rows($qry1);
				if($num1>0)
				{
					for($i=0;$i<$num1;$i++)
					{
						$message[$messageCounter]=d4l_mysql_result($qry1,$i,'message_text');
						$commonDate[$messageCounter]=d4l_mysql_result($qry1,$i,'date_created');
