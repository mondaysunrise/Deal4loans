...
{
	var gifName = form;
	document.frmsearch.action="banner-comp-hllmsview.php?search=y"+gifName;
...
function chkform()
{
	...
	if(ss.length<10 || ss.length>10)
	{
		...
		if(document.frmsearch.min_date.value<"2014-03-26")
	{
		alert("Sorry!!!! Your minimum date is 2014-03-26.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="banner-comp-hllmsview.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?$current_date=date('Y-m-d');?> 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2014-03-26"<? } else { ?>value="<? echo $min_date; ?>" <? }?>></td>
	   <td>
 ...
 <? 
		if($comp_source!='netcorehl')
		{
		$qry2="SELECT RequestID from  Req_Loan_Home WHERE ((Req_Loan_Home.Updated_Date Between '".$min_date."' and '".$max_date."') and source='".$comp_source."' and City in ('Ahmedabad','Kolkata','Jaipur','Chandigarh','Lucknow','Jalandhar','Cochin','Nagpur ','Delhi','Noida','Gurgaon','Mumbai','Thane','Bangalore','Chennai','Hyderabad','Pune','Surat','Coimbatore','Gaziabad') and Net_Salary >='360000' and (Req_Loan_Home.DOB!='' and DATE_SUB(CURDATE(),INTERVAL 23 YEAR) >= Req_Loan_Home.DOB))";
		$qry2=$qry2."group by Mobile_Number";
		}
		elseif($comp_source=="netcorehl")
				{
				$qry2="SELECT komli_uniqueid from  komli_plcompaign WHERE ((komli_date Between '".$min_date."' and '".$max_date."') and  komli_city in ('Chennai','Bangalore','Hyderabad','Mumbai','Navi Mumbai','Thane','Delhi','Kolkata','Ahmedabad','Pune','Vizag','Vishakapatanam','Surat','Indore','Cochin','Chandigarh','Jaipur','Baroda','Coimbatore','Noida','Gurgaon','Faridabad','Gaziabad') and  vendor_name='".$comp_source."' and komli_netsalary  >=360000 and validated=1)";
...
list($recordcount2,$row2)=Mainselectfunc($qry2,$array = array());
...
<td width="141" align="center" bgcolor="#FFFFFF" class="style2">Count </td>
    ...
    <tr>
       <td align="center" bgcolor="#DFF6FF" class="style3">Valid Leads</td>
<td align="center" bgcolor="#DFF6FF" class="style3"><? echo $recordcount2; ?></td>
      </tr>
      
 <?  if($comp_source=="clove_network" || $comp_source=="tyroo" || $comp_source=="ibibohl")
   { 
   ?>
