...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if($last=="")
	{
		...
		$InsertProductSql= array("Name"=>$full_name, "Gender" => $Gender, "Pancard" => $panno, "Residence_Address" => $Residence_Address, "Std_Code_O"=>$Std_Code_O, "Landline_O"=>$Landline_O, "Pincode"=>$pincode);
		...
		{
		$duplicatepush=1;
		$DataArray = array("qualification" => $Qualification , "designation" => $Designation, "cc_requestid"=>$requestID,"relation_with_bank"=>$relation_with_bank, "lead_source" => "Direct", "applied_bankname"=> "Standard Chartered");
		...
		{
			$duplicatepush=0;
			$InsertProductSql= array("qualification" => $Qualification , "designation" => $Designation, "lead_source" => "Direct", "applied_bankname"=> "Standard Chartered");
			$ProductValue = Maininsertfunc("credit_card_banks_apply", $InsertProductSql);
		}

		if($card_id==13) { $card_type=13;} elseif($card_id==19){ $card_type=9;} elseif($card_id==21){ $card_type=10;} elseif($card_id==67){ $card_type=7;}
...
//		list($AnnIncome,$extrapt) = explode(".",$Net_Salary);
	...
	$DataArray = array("request_data" =>$request_data, "response_data" =>$response_data, "cc_requestid"=>$requestID);
