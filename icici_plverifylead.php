...
//print_r ($Loan_Any)."<br>";
		...
		if(strlen($alternatecuurentadd_proof)>1 && $cuurentadd_prooforg=="")
		{
...
if($appointment_date!='0000-00-00' && $appointment_date!='' && strlen($appointment_time)>0 && strlen($appointment_place)>1)
	{
		 $appointquery ="Update icici_pl_cibili_check set appointment_date='".$appointment_date."',appointment_time='".$appointment_time."', appointment_place='".$appointment_place."', appointment_address= '".$appointment_address."',documents='".$completedocumentsstr."' Where  icicirequestID=".$post;
			...
			$updatelead="Update  ICICI_Allocated_Leads set Mobile_Number='$plmobile',Email='$plemail', City='$plcity',Name='$plname',cuurentadd_proof='$cuurentadd_proof',Existing_Noofemi='$NoofEMIpaid',identification_proof = '$identification_proof', salary_proof='$salary_proof',experience_proof='$experience_proof', bankstat_proof='$bankstat_proof',Annual_Turnover='$Annual_Turnover',Company_Type='$plCompany_Type',PL_Tenure= '$acc_no',PL_EMI_Amt='$emi_amt',Company_Name='$plcompany_name', DOB='$pldob', Residential_Status='$plresidential_status',Card_Limit='$plcard_limit', Net_Salary='$plnet_salary', Loan_Amount='$plloan_amount', Employment_Status='$plemployment_status', CC_Holder='$plcc_holder', Card_Vintage='$plcard_vintage', Total_Experience='$pltotal_experience', Years_In_company='$plyears_in_company', Loan_Any='$Loan_A', Emi_Paid='$plemi_paid', Add_Comment='$pladd_comment',Dated=Now(), Primary_Acc='$Primary_Acc', Existing_Bank='$pl_Existing_Bank',Existing_Loan='$pl_Existing_Loan',Existing_ROI='$pl_Existing_ROI' where icicirequestID=".$post;			
			$updateleadresult = ExecQuery($updatelead);
	}

 if(strlen($plfeedback)>0)
	{
		  if($plfeedback=="Verified")
		{
			echo $upccquerysn ="Update icicipllms_allocation set icici_sendnow_date=Now(),icici_sent=1 Where (icicirequestID=".$post.")";
