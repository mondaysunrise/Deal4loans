...
$getAgentSql = "select BidderID from Bidders Where leadidentifier IN ('diallercallerccpredictive','diallerleadccsmsnew') AND BidderID='".$_SESSION["BidderID"]."'";
...
//Get Bidder LeadIdentifier to get process_type
$getBidderSql = "select leadidentifier from Bidders Where BidderID='".$_SESSION["BidderID"]."'";
$getBidderResult = d4l_ExecQuery($getBidderSql);
$getBidderRow = d4l_mysql_fetch_assoc($getBidderResult);
$getBidderLeadIdentifier = $getBidderRow['leadidentifier'];
if($getBidderLeadIdentifier == 'CCTRANSFER2CALLER')
{
	$process_type = 'HL_LMS';
}
elseif(($getBidderLeadIdentifier == 'rblcallerlms_cc') || ($getBidderLeadIdentifier == 'rblcallerinternallms_cc'))
{
	$process_type = 'RBL_LMS';
}
elseif(($getBidderLeadIdentifier == 'CibilCallingLmsF') || ($getBidderLeadIdentifier == 'CibilCallingLms'))
{
	$process_type = 'ZBL_LMS';
}
else {
	$process_type = 'LMS';
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$productflag= $_REQUEST["productflag"];
	...
	else
	{
		...
		$ccupqry="INSERT IGNORE INTO sbi_credit_card_5633 set RequestID='".$RequestID."',CompanyName='".$Company_Name."',ResidenceAddress1='".$ResidenceAddress1."',ResidenceAddress2='".$ResidenceAddress2."',ResidenceAddress3='".$ResidenceAddress3."',OfficeAddress1='".$OfficeAddress1."',OfficeAddress2='".$OfficeAddress2."',OfficeAddress3='".$OfficeAddress3."',OfficePin='".$OfficePin."',OfficeCity='".$OfficeCity."',LandlineNo='".$Phone_Number."',OfficeState='".$OfficeState."',Qualification='".$Qualification."',CardName='".$card_name."',Designation='".$Designation."',sbi_cc_holder='".$sbicardholder."',applied_in_6_months='".$sbicardapplied."',relationship_type='".$sbirelation."',account_number='".$account_number."',Dated='".$Dated."',pushflag='".$pushflag."', productflag='4', table_name='Req_Credit_Card', agent_id='".$bidderid."', process_track='1', MobileNumber='".$ccMobile_Number."' ".$insert_pancard."";
	}
	//echo $ccupqry;
	...
	$upcctblenw="Update Req_Credit_Card set Name='".$Name."',DOB='".$DOB."',Company_Name='".$Company_Name."',Std_Code='".$resiSTD."',Landline='".$resi_landline."', Email='".$Email."',Net_Salary='".$Net_Salary."',Gender='".$Gender."',Residence_Address ='".$ResidenceAddress."',State='".$State."', Office_Address='".$OfficeAddress."', Employment_Status='".$Employment_Status."', Pancard='".$panno."', CC_Holder='".$CC_Holder."',Pincode='".$pincode."' ".$cityclause." ".$othercityclause." Where (RequestID=".$RequestID.")";
...
$cc_alldetailsSql = "select * from sbi_credit_card_5633 Where (RequestID=".$requestid." AND productflag='4' ".$checkfinalsbiccid.") order by sbiccid DESC LIMIT 0,1";
...
$OfficeCity = trim($ccrowal["OfficeCity"]);
...
$ccfb_alldetails = d4l_ExecQuery("select * from Req_Feedback_CC Where (BidderID=".$bidderid." and AllRequestID=".$requestid.")");
...
<?php } ?>
<input type="hidden" name="biddtnw" readonly value="<? echo $bidderid;?>" />
<input type="hidden" name="RequestID" readonly value="<? echo $requestid;?>" />
<input type="hidden" name="sbiccid" readonly="readonly" value="<? echo $sbiccid;?>" />
...
<tr>
	<td colspan="4" bgcolor="#DAEAF9" align="center" style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold; border:1px solid black;"><b style="color:red; font-size:16px;">Customer Applied on Wishfin</b></td></tr>
<tr>
<?php } ?>

<tr>
	<td colspan="4" align="center" style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold; height:25px;">Credit Card customer details</td>
</tr>
<tr>
...
<tr>
	...
	<td>
		<input type="text" class="d4l-input" name="panno" id="panno" value="<? echo $ccrow["Pancard"] ;?>"  maxlength="10"/>
		<span id="panvalidation">
		<input type="button" id="validatepanbtn" onclick="getPanValidate();" class="validate_btn" value="Validate"></input>
		</span>
	</td>
...
<tr>
	...
	<td>
		<table cellpadding="2" cellspacing="2">
			<tr>
				<td>
					<div id="txtHint2" style="float:left; width:50px;"><input type="text" class="std"  name="STD"  onfocus="(this.value == 'STD') && (this.value = '')" onblur="(this.value == '') && (this.value = 'STD')" id="STD" maxlength="7" style="width:50px;">
				</div>
				</td>
				<td>
...
<?php 
$process_pause = "pause"; // Told by BHupendra 29-05-17
if($process=="cq_pause")
{ // for old leadrefno ?>
<form method="POST" action="/soapservice_sbi5633_1.php" name="sendform">
<input type="hidden" name="custid"id="custid"  value="<?php echo $requestid; ?>" />
<input type="hidden" name="process_type" id="process_type"  value="<?php echo $process_type; ?>" />
...
<input type="Submit" id="submitbtn" name="Submitd" value="SBI with old leadref" style="color:#FFF; background-color:#C00;" /> 

</form>
<?php
}
elseif($process=="sr_pause")
{ ?>
<form method="POST" action="/soapservice_sbi5633_1.php" name="sendform">
<input type="hidden" name="process_type" id="process_type"  value="<?php echo $process_type; ?>" />
<input type="hidden" name="process" id="process"  value="<?php echo $process; ?>" />
<input type="hidden" name="agent_id" id="agent_id"  value="<?php echo $bidderid; ?>" />
<input type="hidden" name="leadref" id="leadref"  value="generate new"  />
<input type="hidden" name="custid"id="custid"  value="<?php echo $requestid; ?>" /><input type="Submit" id="submitbtn" name="Submitd" value="SBI with New leadref" style="color:#FFF; background-color:#C00;"></form>
<? 
}
else {
	...
	*/
	...
	//	$checkSMSMobileandPanSql .= " ) AND ((scc.first_dated != '0000-00-00 00:00:00' OR sccl.first_dated != '0000-00-00 00:00:00') OR (scc.response_xml != '' AND sccl.response_xml != '')) ";
	...
	if($checkSMSMobileandPanNumRows>0)
	{
		...
		else 
		{
			...
			else
			{
				...
				$checkPANInRequestSql = "SELECT * FROM `sbi_credit_card_5633` as scc WHERE request_xml LIKE '%<PAN>".$ccrow["Pancard"]."</PAN>%' AND (scc.first_dated > DATE_SUB(NOW(), INTERVAL 180 DAY)) AND (scc.ProcessingStatus IN (1,2,4,5,6,7) OR (scc.StatusCode = 156 AND RequestID != '".$requestid."'))";
					...
					else 
					{
						...
						else 
						{
							...
							if($checkSccPANMobileLogNumRows>0 && strlen($ccrow["Mobile_Number"]) > 0 && strlen($ccrow["Pancard"]) > 0) 
							{
								...
								if($checkSccPANMobileLogNum156Rows>0 && strlen($ccrow["Mobile_Number"]) > 0 && strlen($ccrow["Pancard"]) > 0 && ($checkRequestID!=$requestid )) 
								{
									//Insert sbi check error log
									...
									<form method="POST" action="/soapservice_sbi5633_1.php" name="sendform">
										...
										<input type="hidden" name="agent_id" id="agent_id"  value="<?php echo $bidderid; ?>" />
										<input type="Submit" id="submitbtn" name="Submitd1" value="SBI Send now" style="color:#FFF; background-color:#C00;" />
									</form>
									<?php
								}
							}
	...
	if($_SESSION['leadidentifier']== "amexcallerlms_cc" || $_SESSION['leadidentifier']== "amercianexpresscallerlms_cc"  || $_SESSION['leadidentifier']== "rblcallerlms_cc" || $_SESSION['leadidentifier']== "rblcallerinternallms_cc" || $_SESSION['leadidentifier']== "amercianexpressinternalcallerlms_cc" || $_SESSION['leadidentifier']== "CCTRANSFER2CALLER")
	{
	}
	else {
	?>
		...
		?>
		<!--
		| <a href="scb_agent_form1.php?cust_id=<?php echo $requestid; ?>" style="color:#FFF; background-color:#C00;" target="_blank">Scb Send now</a>-->
		<?php 
			} 
		} 
		?>
		<?php 
		$getrblSql = "select Query,City from Bidders_List where (BidderID = 4905 and City like '%".$Citystr."%')";
		$getrblQuery = d4l_ExecQuery($getrblSql);
		$getrblNumRows = d4l_mysql_num_rows($getrblQuery);
		if($getrblNumRows>0 && (($getBidderLeadIdentifier != 'CibilCallingLmsF') && ($getBidderLeadIdentifier!= 'CibilCallingLms')) )
		{
			...
			{
		?>
		 | <a href="rblcc_agent_form1.php?cust_id=<?php echo $requestid; ?>" style="color:#FFF; background-color:#C00;" target="_blank">RBL Send now</a>
		<?php 
			} 
		}
		?>
		...
		if($getAmexNumRows>0)
		{
			...
			{
		?>
		 | <a href="amex_agent_form1.php?cust_id=<?php echo $requestid; ?>" style="color:#FFF; background-color:#C00;" target="_blank">Amex Send now</a>
		<?php 
			}
		}
		?>
		...
		if($getICICINumRows>0)
		{
			...
			{
		?>
		<!-- | <a href="icici-credit-card-continue_lms_direct.php?cust_id=<?php echo $requestid; ?>" style="color:#FFF; background-color:#C00;" target="_blank">ICICI Send now</a>-->
		<?php 
			}
		}
	}
