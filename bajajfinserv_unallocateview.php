...
{
	var gifName = form;
	document.frmsearch.action="bajajfinserv_unallocateview.php?search=y"+gifName;
...
function MM_jumpMenu(targ,selObj,restore){ //v3.0
 ...
 <form name="frmsearch" action="bajajfinserv_unallocateview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if($search=="y")
	{
		$min_dateval=$min_date;
		$max_dateval=$max_date;
...
if($mob_num>0)
 ...
 <? 
if(strlen($min_dateval)>2 && strlen($max_dateval)>2)
		{
 $search_qry="select * from bajaj_finserv_mailer_leads LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=bajaj_finserv_mailer_leads.bajaj_finservid AND Req_Feedback.BidderID=".$_SESSION['BidderID']." where (((bajajf_dated Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."')) and bajajf_sent=0 and (bajajf_source='bfsmailer' or bajajf_source='bfsmailerhl' or bajajf_source='bfsmailerhlTJ' or bajajf_source='bfsmailerTJ') and bajajf_employment_status=1 and (bajajf_city 	 in ('Chennai','Bangalore','Hyderabad','Mumbai','Navi Mumbai','Thane
Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Sahibabad','Kolkata','Ahmedabad','Pune','Vizag','Vishakapatanam','Surat','Bharuch','Ankleshwar','Vapi','navsari','valsad','silvassa','daman','Indore','Aurangabad','Cochin','Chandigarh','Jaipur','Baroda','Nasik','Coimbatore','Goa','Vasco','Ponda','Panjim','Masa','Nagpur','Nasik','Bhubaneswar','Cuttack','Mysore','Madurai') or  	bajajf_city_other in ('Chennai','Bangalore','Hyderabad','Mumbai','Navi Mumbai','Thane
Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Sahibabad','Kolkata','Ahmedabad','Pune','Vizag','Vishakapatanam','Surat','Bharuch','Ankleshwar','Vapi','navsari','valsad','silvassa','daman','Indore','Aurangabad','Cochin','Chandigarh','Jaipur','Baroda','Nasik','Coimbatore','Goa','Vasco','Ponda','Panjim','Masa','Nagpur','Nasik','Bhubaneswar','Cuttack','Mysore','Madurai')))";
	$search_qry=$search_qry.$mob_num_clause;
		$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry."group by bajaj_finserv_mailer_leads.bajajf_mobile";
		//echo $search_qry."<br>";
		$qry="select * from bajaj_finserv_mailer_leads LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=bajaj_finserv_mailer_leads.bajaj_finservid AND Req_Feedback.BidderID=".$_SESSION['BidderID']." where (((bajajf_dated Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."')) and bajajf_sent=0  and bajajf_employment_status=1 and (bajajf_source='bfsmailer' or bajajf_source='bfsmailerhl' or bajajf_source='bfsmailerhlTJ' or bajajf_source='bfsmailerTJ') and (bajajf_city  in ('Chennai','Bangalore','Hyderabad','Mumbai','Navi Mumbai','Thane
Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Sahibabad','Kolkata','Ahmedabad','Pune','Vizag','Vishakapatanam','Surat','Bharuch','Ankleshwar','Vapi','navsari','valsad','silvassa','daman','Indore','Aurangabad','Cochin','Chandigarh','Jaipur','Baroda','Nasik','Coimbatore','Goa','Vasco','Ponda','Panjim','Masa','Nagpur','Nasik','Bhubaneswar','Cuttack','Mysore','Madurai') or  	bajajf_city_other in ('Chennai','Bangalore','Hyderabad','Mumbai','Navi Mumbai','Thane
Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Sahibabad','Kolkata','Ahmedabad','Pune','Vizag','Vishakapatanam','Surat','Bharuch','Ankleshwar','Vapi','navsari','valsad','silvassa','daman','Indore','Aurangabad','Cochin','Chandigarh','Jaipur','Baroda','Nasik','Coimbatore','Goa','Vasco','Ponda','Panjim','Masa','Nagpur','Nasik','Bhubaneswar','Cuttack','Mysore','Madurai')))";
$qry=$qry.$mob_num_clause;
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by bajaj_finserv_mailer_leads.bajajf_mobile";
		$qry=$qry.$OrderBY;
		//echo"hello".$qry."<br>";
	...
	{
		echo "Please select date range again";
	}
 ?>
   <tr>
...
$qry=$qry.$mob_num_clause;
	...
	//echo "hrllo".$qry;
		...
		{
	?>
	<input type="hidden" name="requestid_<? echo $i;?>" id="requestid_<? echo $i;?>" value="<? echo $row["bajaj_finservid"];?>">
   ...
   <tr>
        <td class="bodyarial11"><a href="/bajajfmailer_editlead.php?postid=<? echo $row["bajaj_finservid"]; ?>&biddt=<? echo $_SESSION['BidderID']; ?>" target="_blank"><? echo $row["bajajf_name"]; ?></a></td>
    <td class="bodyarial11"><? echo $row["bajajf_city"]; ?></td>
	    <td class="bodyarial11"><? echo $row["bajajf_mobile"]; ?></td>
     <td class="bodyarial11"><? echo $row["bajajf_company_name"]; ?></td>
     <td class="bodyarial11"><? echo $row["bajajf_net_salary"]; ?></td>
     <td class="bodyarial11"><? echo $row["bajajf_loan_amount"]; ?></td>
     <td class="bodyarial11"><? echo getJumpMenu("bajajfinserv_unallocateview.php",$row["bajaj_finservid"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback) ?></td>
