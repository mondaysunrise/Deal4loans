<?php
	...
	function formHandler(){
		foreach($_POST as $a=>$b)
			$$a=$b;
//	echo $sql = "select *  from Bidders where Email='$Email' and PWD='$PWD' and Is_Verified=11";
		$result = "select *  from Bidders where Email='$Email' and PWD='$PWD' and Is_Verified=100";
		...
		if($num_rows > 0){
			...
			setSessionBidder($Email, $row);
			 ...
			 /* Redirect browser */
			 $strDir = dir_name();
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."newallproducts_index.php");
			exit;
		}
		else{
			global $Msg;
