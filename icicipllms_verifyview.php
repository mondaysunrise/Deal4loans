...
{
	var gifName = form;
	document.frmsearch.action="icicipllms_verifyview.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="icicipllms_verifyview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <?
		$qry="Select RequestID From ICICI_Allocated_Leads LEFT OUTER JOIN icicipllms_allocation ON ICICI_Allocated_Leads.icicirequestID= icicipllms_allocation.icicirequestID   Where (((ICICI_Allocated_Leads.Dated Between '".$min_date."' and '".$max_date."')) and ICICI_Allocated_Leads.approved=1 and icicipllms_allocation.icici_sent=0)";
 ...
 ?>
   ...
   <tr>   
	...
	<?
		...
		$qry="Select *  From ICICI_Allocated_Leads LEFT OUTER JOIN icicipllms_allocation ON ICICI_Allocated_Leads.icicirequestID= icicipllms_allocation.icicirequestID   Where (((ICICI_Allocated_Leads.Dated Between '".$min_date."' and '".$max_date."')) and ICICI_Allocated_Leads.approved=1 and icicipllms_allocation.icici_sent=0)";
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by ICICI_Allocated_Leads.RequestID";
...
//echo $qry;
	...
	?>
<tr>
<td class="bodyarial11"><a href="icici_plverifylead.php?id=<? echo urlencode($row['icicirequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
