<?php
	...
	if($ccuserid>0)
	{
		...
		if(strlen($row[$cntr]['Descr'])>0)
		{
			$strcrd_nme=$row[$cntr]['Descr'].",".$Descr;
		}
		else
		{
			$strcrd_nme=$Descr;
		...
		$ccupdate= "Update Req_Credit_Card set Descr='".$strcrd_nme."',Dated=Now() Where (Req_Credit_Card.RequestID=".$ccuserid.")";

	}
	$chekValid = ("select Reference_Code,Mobile_Number from Req_Credit_Card Where (RequestID='".$ccuserid."')");
	 list($recordcount,$ArrRows)=MainselectfuncNew($chekValid,$array = array());
		$i=0;
	
	$chekValidValue = $ArrRows[$i]['Reference_Code'];
	$Mobile_Number = $ArrRows[$i]['Mobile_Number'];
	if($chekValidValue>0)
	{
...
<?php
	...
	$sql = "SELECT * FROM `Req_Credit_Card` where RequestID='".$lastInserted."'";
	 list($recordscount,$Myrow)=MainselectfuncNew($sql,$array = array());
		$j=0;
	$Name = $Myrow[$j]['Name'];
	...
	$Net_Salary = $Myrow[$j]['Net_Salary'];
	$Company_Name = $Myrow[$j]['Company_Name'];
	$Phone = $Mobile_Number;
?>
...
<tr><td width="100">&nbsp;</td><td class="formfield">City <font size="1" color="#FF0000">*</font></td><td><select  name="City" class="w175" id="City" style="width:142px; height:18px;  "  >
      ...
      <td class="formfield">Pincode <font size="1" color="#FF0000">*</font></td><td><input name="pincode" class="w175" id="pincode" maxlength="6"  onChange="intOnly(this);"  onKeyUp="intOnly(this);" onKeyPress="intOnly(this); " /></td></tr>
</table>
</td></tr>
<tr><td colspan="2" align="left" style="color:#FFFFFF; font-family:Arial, Helvetica, sans-serif;">
...
<tr><td width="97">&nbsp;</td>
<td class="formfield" colspan="2" align="right"><input name="panCardLater" id="panCardLater" type="checkbox"> I would like to submit my PAN later</td><td width="75">&nbsp;</td>
...
<td width="162" class="formfield"><input name="existing_customer" id="existing_customer" checked="checked" value="no" type="radio">
&nbsp;No&nbsp;&nbsp;&nbsp;&nbsp;
<input name="existing_customer" id="existing_customer" value="yes" type="radio">&nbsp;Yes</td><td width="317">&nbsp;</td><td width="14" class="formfield">&nbsp;</td>
<td width="63">&nbsp;</td>
</tr>
