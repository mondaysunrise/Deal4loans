...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	$getdetails="select RequestID From ".$Type_Loan."  Where (Mobile_Number not in (9971396361,9811215138,9999570210) and Mobile_Number='".$mobile."' and Updated_Date between '".$days30datetime."' and '".$currentdatetime."') order by RequestID DESC";
	//echo $getdetails."<br>";
	//exit();
...
$table = $Type_Loan;
				...
				{
				$dataInsert = array("UserID"=>$UserID, "Name"=>$fullname, "Email"=>$email_id, "Mobile_Number"=>$mobile, "Residence_City"=>$city, "Dated"=>$Dated, "source"=>$source, "IP_Address"=>$IP, "Updated_Date"=>$Dated, "Net_Salary"=>$net_salary, "Employment_Status"=>'1', "IsPublic"=>$accept, "Privacy"=>$accept);
$table = $Type_Loan;
	...
	NewAir2webSendSMS($SMSprMessage, $mobile, $product_code , $last_inserted_id);
		...
		$wu=0;
			if($CheckNumRows>0)
			{
				...
				{
				$dataInsert = array("UserID"=>$UserID, "Name"=>$fullname, "Email"=>$email_id, "Mobile_Number"=>$mobile, "Residence_City"=>$city, "Dated"=>$Dated, "source"=>$source, "IP_Address"=>$IP, "Updated_Date"=>$Dated, "Net_Salary"=>$net_salary, "Employment_Status"=>'1', "DOB"=>'1987-06-03', "Privacy"=>$accept);
$table = $Type_Loan;
				}
				else
				{
...
if($rowscount >0)
{
  $message.="<tr><td style='font-family:Verdana, Arial, Helvetica, sans-serif; font-size:11px; color:#061c33; line-height:18px; '> <table  border='1' cellspacing='0' cellpadding='0'>
		...
		<td  width='85' height='40'bgcolor='#197ad6' class='txt-hd' style='color: #FFFFFF; text-align:center;font-weight:bold; border-bottom:1px #FFFFFF; font-size:11px;'  >&nbsp;</td>
