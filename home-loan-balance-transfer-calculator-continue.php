...
if(isset($_POST['submit']) || $_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	$Existing_Bank= FixString($_POST['Existing_Bank']);
	$Existing_Loan= FixString($_POST['Loan_Amount']);
	$Existing_ROI= FixString($_POST['roi']);
	$hdfclife = FixString($_POST["hdfclife"]);
	$Dated = ExactServerdate();
	if($_REQUEST['City_Other'])
	{
	...
	if(($validMobile==1) && ($Name!="") && strlen($City)>0 && $validname==1)
	{
		...
		$getdetails="select RequestID From ".$Type_Loan."  Where ((Mobile_Number='".$Phone."' and Mobile_Number not in ('9811555306','9811215138','9971396361','9810395952','9555060388')) and Updated_Date between '".$days30datetime."' and '".$currentdatetime."') order by RequestID DESC";
		list($alreadyExist,$myrow)=MainselectfuncNew($getdetails,$array = array());
			...
			if($CheckNumRows>0)
			{
				...
				$data = array("UserID"=>$UserID, "Name"=>$Name, "Email"=>$Email, "City"=>$City, "Mobile_Number"=>$Phone, "Net_Salary"=>$Net_Salary, "Loan_Amount"=>$Existing_Loan, "Dated"=>$Dated, "source"=>$source, "IP_Address"=>$IP, "Updated_Date"=>$Dated,'Employment_Status'=>$Employment_Status, 'DOB'=>$dateofbirth, 'Existing_ROI'=>$Existing_ROI, 'Existing_Loan'=>$Existing_Loan, 'Existing_Bank'=>$Existing_Bank);
			}
			else
			{
...
<div class="text12" style="margin:auto; width:100%; height:11px; margin-top:70px; color:#0a8bd9;"><u><a href="index.php" class="text12" style="color:#0080d6;" >Home</a></u> > <span class="text12" style="color:#4c4c4c;">Home Loan Balance Transfer Calculator</span></u></div>
...
<div class="box_two"><span class="frmbldtxt" style="padding-top:3px;">
                     ...
                     <div style="clear:both;"></div>
                     <!--
                     <div class="box_one"><span class="frmbldtxt" style="padding-top:3px;"><span class="formtext"><b>Pre Payment Charges</b></span> :</span></div><div class=" box_two"><span class="frmbldtxt" style="padding-top:3px; "><?php echo $pre_payment_charges; ?> % </span></div>
                     -->
