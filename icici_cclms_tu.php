...
$telecallerid = $_REQUEST["teleid"];

$tudetails="Select * from credit_card_cibil_check Where RequestID=".$reqdid; 
list($turowcount,$turow)=MainselectfuncNew($tudetails,$array = array());
$turowcontr=count($turow)-1;
$City = $turow[$turowcontr]["City"];
...
<tr><td>
<form name="icici_cc" action="icici_card_chktransunion_lms.php" method="POST" onSubmit="return ckhcreditcard(document.icici_cc); "><input type="hidden" name="RequestID" value="<? echo $reqdid;?>">
<input type="hidden" name="telecallerid" value="<? echo $telecallerid;?>">				
			<table width="90%">		       
         <tr>
		...
		<td width="48%" align="left" class="frmbldtxt">
         <input type="text" name="Pancard" id="Pancard" class="input-bx12"  maxlength="10" tabindex="1" autocomplete="off" value="<? echo $turow["PanNo"]; ?>"  />
			...
			<option value="Salary" <? if($turow["existing_relation"]=="Salary") { echo "Selected";} ?>>Salary Account</option>	   
			<option value="Saving" <? if($turow["existing_relation"]=="Saving") { echo "Selected";} ?>>Saving Account</option>	   
			<option value="Current" <? if($turow["existing_relation"]=="Current") { echo "Selected";} ?>>Current Account</option>	   </select></td>     </tr>
    <tr>
     ...
     <tr>
		...
		<td width="48%" align="left" class="frmbldtxt">
         <select name="Address_State" id="Address_State">
		   ...
		   //$stateArr = array('Rajasthan', 'Madhya Pradesh', 'Maharashtra', 'Andhra Pradesh', 'Uttar Pradesh', 'Jammu and Kashmir', 'Gujarat', 'Karnataka', 'Orissa', 'Chhattisgarh', 'Tamil Nadu', 'Bihar', 'West Bengal', 'Arunachal Pradesh', 'Jharkhand', 'Assam', 'Himachal Pradesh', 'Uttarakhand', 'Punjab', 'Haryana', 'Kerala', 'Meghalaya', 'Manipur', 'Mizoram', 'Nagaland', 'Tripura', 'Andaman & Nicobar Islands', 'Sikkim', 'Goa', 'Delhi', 'Puducherry', 'Dadra and Nagar Haveli', 'Chandigarh', 'Daman and Diu', 'Lakshadweep');
