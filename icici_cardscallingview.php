...
{
	var gifName = form;
	document.frmsearch.action="icici_cardscallingview.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="icici_cardscallingview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//		echo $strSQL;
 ...
 <?
		$qry="Select RequestID From icici_cards_calling LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= icici_cards_calling.RequestID and Req_Feedback_CC.BidderID='".$_SESSION['BidderID']."' Where (((icici_cards_calling.Dated Between '".$min_date."' and '".$max_date."') or ((Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and Req_Feedback_CC.BidderID='".$_SESSION['BidderID']."')) and icici_cards_calling.Bidderid_Details='' and (icici_cards_calling.TelecallerID='".$_SESSION['BidderID']."') and icici_cards_calling.Privacy =1)";
 ...
 $qry="		 
Select RequestID, Followup_Date, Mobile_Number, Net_Salary, City, City_Other, Name, Feedback From icici_cards_calling LEFT OUTER JOIN Req_Feedback_CC ON Req_Feedback_CC.AllRequestID= icici_cards_calling.RequestID and Req_Feedback_CC.BidderID='".$_SESSION['BidderID']."' Where (((icici_cards_calling.Dated Between '".$min_date."' and '".$max_date."') or ((Req_Feedback_CC.Followup_Date Between '".$min_date."' and '".$max_date."') and Req_Feedback_CC.BidderID='".$_SESSION['BidderID']."')) and icici_cards_calling.Bidderid_Details='' and (icici_cards_calling.TelecallerID='".$_SESSION['BidderID']."') and icici_cards_calling.Privacy =1)";		
		$qry=$qry.$FeedbackClause;
		$qry=$qry."group by icici_cards_calling.Mobile_Number";
		...
		while($row=mysql_fetch_array($result))
		{
			...
			$tudetails="Select cibilchkid from credit_card_cibil_check Where (RequestID=".$row["RequestID"]." and TelecallerID=".$_SESSION['BidderID']; 
	$tudetailsQuery = ExecQuery($tudetails);
	$turow=mysql_fetch_array($tudetailsQuery);
	?>
<tr>
<td class="bodyarial11"><a href="icici_cccallingeditlead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<? list($FinalBidder,$finalBidderName)= getBiddersList("icici_cards_calling",$row['RequestID'],$City);
  for($i=0;$i<count($FinalBidder);$i++)
    ...
    <td class="bodyarial11"><? echo getJumpMenu("icici_cardscallingview.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
