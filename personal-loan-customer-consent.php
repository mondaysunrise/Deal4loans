...
$checkSql = "select AllRequestID from Req_Feedback_Bidder_PL1 where (AllRequestID='".$RequestID."' and (DATE_SUB( NOW() , INTERVAL '00:15' HOUR_MINUTE ) >= Allocation_Date) and BidderID='".$Nobanks."')";
...
else
{
	...
	//	echo "Select AllRequestID from Req_Feedback_Bidder_PL1 where (Consent=0 and AllRequestID='".$RequestID."' and BidderID='".$Nobanks."')"; 
		// echo "<br>";
		$qry_rmbid= "Select AllRequestID from Req_Feedback_Bidder_PL1 where (Consent=0 and AllRequestID='".$RequestID."' and BidderID='".$Nobanks."')";
	list($qryrmbidcount,$qry_rmbidQuery)=MainselectfuncNew($qry_rmbid,$array = array());
		...
		else
		{
			...
			Mainupdatefunc ('Req_Feedback_Bidder_PL1', $dataUpdate, $wherecondition);
			//echo $upconsent."<br>";
			//echo "<br>";
			//change bookkeeping
			$BK_Year = date('Y');
...
<title>Thank you Personal Loan</title>
...
<div class="text12" style="margin:auto;  height:11px; margin-top:70px; color:#0a8bd9;"><u><a href="index.php" class="text12" style="color:#0080d6;" >Home</a></u> > <a href="personal-loans.php"  class="text12" style="color:#0080d6;"><u>Personal Loan</u></a> </div>
