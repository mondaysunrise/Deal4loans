...
<table width="600" border="0" align="center" cellpadding="0" cellspacing="0" style="border:#0033CC 1px solid;">
 ...
 <form name="frmsearch" action="hlmisreport.php?search=y" method="post" onSubmit="return chkform();">
   ...
   <td colspan="3" align="center"><table border="0" cellpadding="0" cellspacing="0" width="85%">
	 ...
	 <option value="72" <? if($lms_details == "72") { echo "selected"; }?>>HL LMS 6</option>
        <option value="3635" <? if($lms_details == "3635") { echo "selected"; }?>>HL Evening LMS</option>
     </select>
		</td>
...
if($lms_details=="All")
{
	$biddIDS = array(732,812,460,207,64,72,3635);
}
else 
{
	...
	{	
	$sql_PL = "select Req_Feedback_HL.Feedback as Feedback, count(*) as CountLeads from Req_Feedback_HL left join Req_Loan_Home on Req_Loan_Home.RequestID = Req_Feedback_HL.AllRequestID where (Req_Loan_Home.Dated between '".$min_date."' and '".$max_date."') and  Req_Feedback_HL.BidderID='".$biddIDS[$j]."' group by Req_Feedback_HL.Feedback";
 ...
 <table width="950" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" >
  <tr>
	 ...
	 else if($biddIDS[$j]==460)
	 {
	  ...
	  else if($biddIDS[$j]==64)
	 {
	 	echo "HL LMS 5 [64]";
	 }
	  else if($biddIDS[$j]==72)
	 {
	 	echo "HL LMS 6 [72]";
	 }
 	 else if($biddIDS[$j]==3635)
	 {
	 	echo "PL Evening LMS [3635]";
	 }
