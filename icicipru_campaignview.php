...
{
	var gifName = form;
	document.frmsearch.action="icicipru_campaignview.php?search=y"+gifName;
...
function chkform()
{
	...
	<? } 
		...
		if(document.frmsearch.min_date.value<"2013-04-15")
	{
		alert("Sorry!!!! Your minimum date is 2013-04-15.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
<tr>
 ...
 <form name="frmsearch" action="icicipru_campaignview.php?search=y" method="post" onSubmit="return chkform();">
 ...
 <? 
		$search_qry="SELECT * from  client_campaign_leads WHERE ((client_campaign_leads.clientld_date  Between '".($min_date)."' and '".($max_date)."' ) and client_name='".$client_name."' and clientld_city in ('Ahmedabad','Bangalore','Baroda','Chennai','Delhi','Hyderabad','Kolkata','Mumbai','Pune','Surat','Vizag') )";	
		//$search_qry=$search_qry.$FeedbackClause;
		$search_qry=$search_qry." group by  client_campaign_leads.clientld_mobile";
		$search_qry=$search_qry." order by client_campaign_leads.clientld_date DESC";

		$qry="SELECT * from client_campaign_leads WHERE ((client_campaign_leads.clientld_date  Between '".($min_date)."' and '".($max_date)."' ) and client_name='".$client_name."' and clientld_city in ('Ahmedabad','Bangalore','Baroda','Chennai','Delhi','Hyderabad','Kolkata','Mumbai','Pune','Surat','Vizag')) ";
		//$qry=$qry.$FeedbackClause;
		$qry=$qry."group by client_campaign_leads.clientld_mobile";

		list($recordcount,$result2)=Mainselectfunc($qry,$array = array());	
 ?>
   <tr>
   ...
   <tr>
	 ...
	 <td width="140" align="center" bgcolor="#FFFFFF" class="style2">Email</td>
	...
	<?
		...
		$qry="SELECT * from client_campaign_leads WHERE ((client_campaign_leads.clientld_date  Between '".($min_date)."' and '".($max_date)."' ) and client_name='".$client_name."' and client_splcondition='".$property_type."' )";
		//$qry=$qry.$FeedbackClause;
		$qry=$qry."group by client_campaign_leads.clientld_mobile";
		$qry=$qry." order by client_campaign_leads.clientld_date DESC";
	...
	?>
	   <tr>
       <td align="center" bgcolor="#DFF6FF" class="style3" ><? echo $row[$ii]["clientld_name"]; ?></td>
	   <td align="center" bgcolor="#DFF6FF" class="style3" ><? echo $row[$ii]["clientld_email"]; ?></td>
	  <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$ii]["clientld_city"]; ?></td>
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row[$ii]["clientld_mobile"]; ?></td>
        </tr>
	<?
					$i=$i+1;
		}
 ...
 <br>
<form name="frmdownload" action="client_campdownload.php" method="post">
