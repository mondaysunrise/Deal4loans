...
{
	var gifName = form;
	document.frmsearch.action="cc_sbi_report.php?search=y"+gifName;
...
echo "<font style='Font-size:12px;'>Welcome ".ucwords($_SESSION['UserType'])." <b>".$_SESSION['UName']." ( <a href=/Logout.php>Logout</a> )</b>";
 ...
 ?>
 <form name="frmsearch" action="cc_sbi_report.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr>
   ...
   <tr>
     <td colspan="3" class="head1" align="center">Processing Status for SBI</td>
     </tr>
   <tr>
     <td colspan="3">&nbsp;</td>
   </tr>
 ...
 <td class="head1">Count of Leads</td>
	...
	<?php
		...
		$qry="SELECT scc.`ProcessingStatus` as Process, COUNT(*) as countLeads,rcc.source, DATE(first_dated) FROM `sbi_credit_card_5633` as scc JOIN `Req_Credit_Card` as rcc ON (rcc.`RequestID`=scc.`RequestID`) WHERE  first_dated  BETWEEN  '".$min_date."' AND  '".$max_date."'  and process_type!='TWL_LMS' and productflag!=10 GROUP BY scc.`ProcessingStatus` order by ProcessingStatus asc LIMIT $startrow, $pagesize";
