<?php
	...
	while($row_result=mysql_fetch_array($search_result))
	{
		...
		if($row_result["Card_Vintage"]=="0") { $card_vintage="Please select"; }
	...
	$BiddersChurn="SELECT Bidder_Name,Req_Feedback_Bidder1.BidderID As bid FROM Req_Feedback_Bidder1 LEFT OUTER JOIN Bidders_List ON Bidders_List.BidderID = Req_Feedback_Bidder1.BidderID and Bidders_List.Reply_Type =1 WHERE (AllRequestID = '".$row_result["RequestID"]."' AND Req_Feedback_Bidder1.Reply_Type =1)";
	//echo $BiddersChurn;
	...
	$descr=implode(',', $BidderID);
		...
		$smstatChurn="SELECT a2wstatus FROM sms_acknowledgement WHERE (air2web_requestid = '".$row_result["RequestID"]."' AND product_type=1)";
	//echo $BiddersChurn;
	...
	$FeedbackLMS = mysql_result($queryFeeds,0,'Feedback');
		...
		$qry1="insert into temp (session_id, name, dob, emp_status, c_name, city, city_other, year_in_comp, total_exp, net_salary, residential_status, loan_any, emi_paid, cc_holder, loan_amount, pincode, doe, current_age, source, count_replies, employer,Feedback,count_views,is_valid,is_processed,mobile_number,card_vintage,add_comment,descr, is_modified, plan_interested,address_apt,email  ) values ('".$session_id."', '".$row_result["Name"]."', '".$row_result["DOB"]."', '".$emp_status."', '".$row_result["Company_Name"]."', '".$row_result["City"]."', '".$row_result["City_Other"]."', '".$row_result["Years_In_Company"]."', '".$row_result["Total_Experience"]."', '".$row_result["Net_Salary"]."', '".$residential_status."', '".$row_result["Loan_Any"]."', '".$emi_paid."', '".$cc_holder."', '".$row_result["Loan_Amount"]."', '".$row_result["Pincode"]."','".$row_result["Updated_Date"]."', '".$dobofbirth."', '".$row_result["source"]."', '".$row_result["Bidder_Count"]."', '".$strCountBidders."','".$feedbck."','".$Salary_Drawn."','".$row_result["Is_Valid"]."','".$row_result["IsProcessed"]."','".$row_result["Mobile_Number"]."','".$card_vintage."','".$row_result["Add_Comment"]."','".$descr."','".$statsms."', '".$FeedbackLMS."','".$row_result["Referrer"]."','".$row_result["Email"]."')";
	...
	// mobile_number,
	$qry="select name, dob, mobile_number as MobileNumber,email, emp_status, c_name, city, city_other, year_in_comp, total_exp, net_salary, residential_status, loan_any, emi_paid, cc_holder, card_vintage, loan_amount, pincode, doe, current_age, source, count_replies as SentToTotalBidders, employer as CountofBidders,Feedback,count_views AS SalaryDrawn,is_valid AS Valid,is_processed AS DirectAllocate, add_comment AS Comments,descr AS BidderGone, is_modified AS SmsStat, plan_interested as LMSFeedback, address_apt AS Referrer from temp where session_id='".$session_id."'";
