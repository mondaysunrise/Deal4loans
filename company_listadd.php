...
if(($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="122.176.54.210"  || $IP=="1.22.91.57"))
{
 ...
 if ($_SERVER['REQUEST_METHOD'] == 'POST'){ 
		...
		$fullerton = $_POST["fullerton"];
		$citibank = $_POST["citibank"];

		$standard_chartered = $_POST["standard_chartered"];

		$tatacapital = $_POST["tatacapital"];
		$bajajfinserv = $_POST["bajajfinserv"];
		$icici_bank = $_POST["icici_bank"];
		$kotak = $_POST["kotak"];
		$Indusind = $_POST["Indusind"];
		$capitalfirst = $_POST["capitalfirst"];
		$adityabirla = $_POST["adityabirla"];
		$rblbank = FixString($_POST["rblbank"]);
		$iifl = FixString($_POST["iifl"]);
		

		$Dated = ExactServerdate();
		$data = array('company_name'=>$company_name, 'hdfc_bank'=>$hdfc_bank, 'fullerton'=>$fullerton, 'citibank'=>$citibank,'standard_chartered'=>$standard_chartered, 'tatacapital'=>$tatacapital, 'bajajfinserv'=>$bajajfinserv, 'icici_bank'=>$icici_bank, 'kotak'=>$kotak, 'Indusind'=>$Indusind, "capitalfirst"=>$capitalfirst, "adityabirla"=>$adityabirla , "rblbank"=>$rblbank, "iifl"=>$iifl);
		$table = 'pl_company_list';
 ...
 if($ProductValue>0)
 {
$getcompany='select * from pl_company_list where (plcompanyid='.$ProductValue.')';
	...
	<table width="100%" cellpadding="5" cellspacing="0" border="1">
        ...
        <tr>
            ...
            <td align="center">
                ...
                <option value="CAT C"  <? if( $row["hdfc_bank"]=="CAT C") echo "Selected"; ?>>CAT C</option>
               ...
               <option value="CSA A"  <? if( $row["hdfc_bank"]=="CSA A") echo "Selected"; ?>>CSA A</option>
                ...
                <option value="Super A"  <? if( $row["hdfc_bank"]=="Super A") echo "Selected"; ?>>Super A</option>
			 ...
			 <option value="CAT A" <? if( $row["fullerton"]=="CAT A") echo "Selected"; ?>>CAT A</option>
			  <option value="CAT B"  <? if( $row["fullerton"]=="CAT B") echo "Selected"; ?>>CAT B</option>
			   <option value="CAT C"  <? if( $row["fullerton"]=="CAT C") echo "Selected"; ?>>CAT C</option>
				...
				<option value="Super A"  <? if( $row["fullerton"]=="Super A") echo "Selected"; ?>>Super A</option>
				</select>  
				</td>
            <td align="center">
			...
			<option value="CAT B"  <? if( $row["Indusind"]=="CAT B") echo "Selected"; ?>>CAT B</option>
