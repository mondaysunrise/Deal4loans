...
{
	var gifName = form;
	document.frmsearch.action="bajajpl_neview.php?search=y"+gifName;
...
function chkform()
{
	...
	<? } 
		...
		if(document.frmsearch.min_date.value<"2012-05-01")
	{
		alert("Sorry!!!! Your minimum date is 2011-05-01.Please Select.");
		document.frmsearch.min_date.value="";
		document.frmsearch.min_date.focus();
		return false;
	}
...
function insertData(id)
		...
		var get_requestid= document.getElementById('feedbackid_'+ id).value;
		
				
		var queryString = "?comment_section=" + get_comment_section + "&get_feedbackid=" + get_requestid;
				
				
				ajaxRequest.open("GET", "insert_comment_lmsbajajne.php" + queryString, true);
...
<b>2.</b> Tell the loan seeker how much tax savings will this home loan get him/her.
	...
	<tr><td align="right"><a href="/bajajpl_neviewsrch.php" target="_blank">Search</a></td></tr>
 <tr><td align="center">
 ...
 <form name="frmsearch" action="bajajpl_neview.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <table border="0" width="90%" cellpadding="0" cellspacing="0"><tr>
     ...
     <td width="24%" align="left" valign="middle" class="bidderclass" ><?$current_date=date('Y-m-d');?> 
	   <input name="min_date" type="text" id="min_date" size="15" <? if($min_date=="") { ?>value="2011-03-16"<? } else { ?>value="<? echo $min_date; ?>" <? }?>></td>
	   <td>
...
<tr><td align="right"><b>Feedback</b> &nbsp;&nbsp;&nbsp;&nbsp;</td> <td colspan="2" width="50%" align="left" valign="middle" class="bidderclass">
  ...
  <tr><td colspan="3">&nbsp;</td></tr>
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ExecQuery("select FeedbackID from Req_Feedback_HL where FeedbackID =".$RequestID." and BidderID in (732,812,460,207)");		
		$num_rows = mysql_num_rows($result);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($result);
			$strSQL="Update Req_Feedback_HL Set axis_executive_name='".$Feedback."' ";
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (Req_Feedback_HL.axis_executive_name IS NULL OR Req_Feedback_HL.axis_executive_name='') ";
		}
		...
		{
			$FeedbackClause=" AND Req_Feedback_HL.axis_executive_name='".$varCmbFeedback."' ";
		}
 ...
 <? 
	$qry="SELECT * FROM Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID=Req_Loan_Home.RequestID AND Req_Feedback_HL.BidderID in (732,812,460,207,2823)  WHERE (
((Req_Loan_Home.City in ('Chennai','Bangalore','Hyderabad','Mumbai','Navi Mumbai','Thane','Delhi','Kolkata') and Req_Loan_Home.Net_Salary>=900000) or (Req_Loan_Home.City in ('Ahmedabad','Pune') and Req_Loan_Home.Net_Salary>=600000) or (Req_Loan_Home.City in ('Vijaywada','Vizag','Madurai','Surat','Rajkot','Indore','Nagpur','Aurangabad','Cochin','Chandigarh','Ludhiana','Jalandhar','Jaipur','Jodhpur','Baroda','Nasik','Mysore','Coimbatore','Kolhapur','Raipur','Bhopal','Jamshedpur','Panipat','Karnal','Yamunanagar') and Req_Loan_Home.Net_Salary>=480000))
and ((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."')) and Req_Feedback_HL.Feedback='Not Eligible' )";
 ...
 ?>
   ...
   <tr>
	 ...
	 <td width="70" align="center" bgcolor="#FFFFFF" class="style2">Company Name</td>
     <td width="91" align="center" bgcolor="#FFFFFF" class="style2">Net Salary </td>
      ...
      <td width="90" align="center" bgcolor="#FFFFFF" class="style2">comment LMS</td>
...
and ((Req_Loan_Home.Updated_Date Between '".($min_date)."' and '".($max_date)."') or (Req_Feedback_HL.Followup_Date Between '".($min_date)."' and '".($max_date)."')) and Req_Feedback_HL.Feedback='Not Eligible' )";
	...
	<input type="hidden" name="feedbackid_<? echo $i;?>" id="feedbackid_<? echo $i;?>" value="<? echo $row["FeedbackID"];?>">
   ...
   <tr>
     ...
     <td align="center" bgcolor="#DFF6FF" class="style3"><? echo $row["Add_Comment"]; ?></td>
	    <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("bajajpl_neview.php",$row["FeedbackID"],"1",$row["axis_executive_name"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback);
	 		 
	// echo $row["FeedbackID"].",".$row["axis_executive_name"].",".$pageno.",".$varmin_date.",".$varmax_date.",".$varCmbFeedback."<br>";
	 ?>
	 </td>
