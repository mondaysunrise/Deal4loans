...
{
	var gifName = form;
	document.frmsearch.action="creditcard_sourcewiseindex.php?search=y"+gifName;
...
<div align="center">
 ...
 <form name="frmsearch" action="creditcard_sourcewiseindex.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
...
//	echo "<br>".$getendtime=date("H:i:s");
		if($min_date!="" and $max_date!="")
		{
			...
			$search_date=$search_date." and UNIX_TIMESTAMP(Req_Credit_Card.Updated_Date)>='".strtotime($min_date)."' and UNIX_TIMESTAMP(Req_Credit_Card.Updated_Date)<='".strtotime($max_date)."'";
		}
		else
		{
 ...
 <?
		$qry="select RequestID from Req_Credit_Card where (1=1 ".$search_date.") group by source ";
 ...
 ?>
   ...
   <tr>
	...
	<?
		...
		$qry="select count(RequestID) AS count, sum(Bidder_Count) AS totalbidders, source from Req_Credit_Card where (1=1 ".$search_date.") group by source order by count(RequestID) DESC LIMIT $startrow, $pagesize";
