...
//Check If Bidder is of 'hlallocatelms', 'hluncalledlms' GROUP
function getGroupBidders($bidid){
	$checkGroupQry = "SELECT * FROM Bidders WHERE BidderID = '".$bidid."' AND leadidentifier IN ('hlallocatelms', 'hluncalledlms')";
	$checkGroupResult = d4l_ExecQuery($checkGroupQry);
	$checkGroupCount = d4l_mysql_num_rows($checkGroupResult);
	return $checkGroupCount;
}

//Check If Bidder is of 'hlallocatelmsCitywise' GROUP
function getCityWiseGroupBidders($bidid){
	$checkGroupQry = "SELECT * FROM Bidders WHERE BidderID = '".$bidid."' AND leadidentifier IN ('hlallocatelmsCitywise')";
	$checkGroupResult = d4l_ExecQuery($checkGroupQry);
	$checkGroupCount = d4l_mysql_num_rows($checkGroupResult);
	return $checkGroupCount;
}
...
$checkCityWiseGroupBidders = getCityWiseGroupBidders($bidid);

if($checkGroupBidders){
	$qryCheck = "SELECT * FROM Bidders where (leadidentifier in ('hlallocatelms', 'hluncalledlms'))";
	$qryCheckResult = d4l_ExecQuery($qryCheck);
	$hlmgrid="";
	$hlbidderid="";
	$hlmgrname="";
	while($row_result=d4l_mysql_fetch_array($qryCheckResult))
	{
...
elseif($checkCityWiseGroupBidders){
...
//Get leadidentifier of Logged in Bidder
$getLeadIdentifierSql = "SELECT leadidentifier FROM Bidders WHERE BidderID = '".$bidid."'";
$getLeadIdentifierResult = d4l_ExecQuery($getLeadIdentifierSql);
$getLeadIdentifierResponse = d4l_mysql_fetch_assoc($getLeadIdentifierResult);
$leadidentifier = $getLeadIdentifierResponse['leadidentifier'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if(strlen($hlfeedback)>0 || strlen($comment_section)>0)
	{
		...
		{
			$result = d4l_ExecQuery("Update Req_Loan_Home set checked_bidders='".$SelectedBank."' where (RequestID=".$post.")");
		}
		$strSQL="";
		$Msg="";
		if($checkGroupBidders || $checkCityWiseGroupBidders){
			$result = d4l_ExecQuery("select FeedbackID from Req_Feedback_HL where AllRequestID=".$post." and BidderID=".$bidid." AND Reply_Type=2");
			...
			$result = d4l_ExecQuery("select FeedbackID from Req_Feedback where AllRequestID=".$post." and BidderID=".$bidid." AND Reply_Type=2");
			$num_rows = d4l_mysql_num_rows($result);
			if($num_rows > 0)
			{
				$row = d4l_mysql_fetch_array($result);
		...
		$resultfb = d4l_ExecQuery("select feedbkid from feedback_bookkeeping where (AllRequestID=".$post." and BidderID=".$bidid." AND Reply_Type=2 and Feedback='".$hlfeedback."' and Comments='".$comment_section."' and Followup_Date='".$FollowupDate."')");	
		$fbnum_rows = mysql_num_rows($resultfb);
		if($fbnum_rows > 0)
		{
			$fbrow = d4l_mysql_fetch_array($resultfb);
			$strfbSQL="Update feedback_bookkeeping Set Feedback='".$hlfeedback."' ,Comments='".$comment_section."', Followup_Date='".$FollowupDate."', leadidentifier='".$leadidentifier."', Dated=Now()";
		...
		$fbresult = d4l_ExecQuery($strfbSQL);
		if ($fbresult == 1)
		{}
		else
		{
	...
	if (!(in_array($bidid, $hlmgrid)))
	{
		...
		if($managerid>0)
		{
			...
			if ($resultfb == 1)
			{
				...
				// lead lifecycle
				$fbqry="select feedbkid,BidderID from feedback_bookkeeping where (AllRequestID=$post and BidderID=$bidid and Feedback='".$lyfcycleclause."' and leadidentifier='".$leadidentifier."' and Comments='".$managerid."')";
				$resultfb = d4l_ExecQuery($fbqry);	
					
				$num_rowsfb = d4l_mysql_fetch_array($resultfb);
				if($num_rowsfb>0) {}
				else
				{
					$strfbSQL="Insert into feedback_bookkeeping(AllRequestID, BidderID, Reply_Type , Feedback, Comments, leadidentifier, Dated) Values (";
					$strfbSQL=$strfbSQL.$post.",'".$bidid."', 2, '".$lyfcycleclause."','".$managerid."','".$leadidentifier."', Now())";

					//echo "<br><br>".$fbqry;
					$resultfb = d4l_ExecQuery($strfbSQL);
				}
			}
			else
			{
	...
	if($bidid == 6717){
		if($select_aro != '' && $select_aro != $allocated_bidder){
			$aroresult = d4l_ExecQuery("select * from hlcallinglms_allocation where AllRequestID=".$post);
			$aro_num_rows = d4l_mysql_num_rows($aroresult);
			if($aro_num_rows > 0)
			{
				$row = d4l_mysql_fetch_array($aroresult);
			...
			$insertaroresult = d4l_ExecQuery($strSQL);
			$lyfcycleclause="Agent Allocate";

			// lead lifecycle
			$fbqry="select feedbkid,BidderID from feedback_bookkeeping where (AllRequestID=$post and BidderID=$bidid and Feedback='".$lyfcycleclause."' and leadidentifier='".$leadidentifier."' and Comments='".$select_aro."')";
			$resultfb = d4l_ExecQuery($fbqry);	
			$num_rowsfb = d4l_mysql_fetch_array($resultfb);
			if($num_rowsfb>0) {}
			else
			{
				$strfbSQL="Insert into feedback_bookkeeping(AllRequestID, BidderID, Reply_Type , Feedback, Comments, leadidentifier, Dated) Values (";
				$strfbSQL=$strfbSQL.$post.",'".$bidid."', 2, '".$lyfcycleclause."','".$select_aro."','".$leadidentifier."', Now())";
				$resultfb = d4l_ExecQuery($strfbSQL);
			}
		}
	}
...
<?php 
	...
	$Referrer = d4l_mysql_result($viewlead,0,'Referrer');
	...
	$comment_section = d4l_mysql_result($viewlead,0,'comment_section');
	if($Referral_Flag==0)
	{
	...
	$doe = d4l_mysql_result($getCheckQuery,0,'DOE');
	$allocated_bidder = d4l_mysql_result($getCheckQuery,0,'BidderID');
?>
</head>
<body>
...
<tr>
	...
	<td>
		...
		$mgridmappresult = d4l_ExecQuery($mgridmapp); 
		$mgrrow=d4l_mysql_fetch_array($mgridmappresult);
		?>
...
<tr>
	...
	<td>
		<select name="select_aro" id="select_aro">
			...
			$hlbiddersResult = d4l_ExecQuery($hlbiddersqry);
			$hlbidders=array();
			while($row_result=d4l_mysql_fetch_assoc($hlbiddersResult)){
...
<tr>
	...
	<?php 
	if($leadidentifier == 'hlallocatelms' || $leadidentifier == 'hluncalledlms'){
		$sqllifecycle="Select BidderID,Dated,Feedback,Comments,Followup_Date, smstext, emailtext From feedback_bookkeeping Where (AllRequestID='".$post."' and Reply_Type=2 AND BidderID IN (SELECT BidderID FROM Bidders WHERE leadidentifier IN ('hlallocatelms', 'hluncalledlms'))) ORDER BY Dated ASC";
	}
	else{
		$sqllifecycle="Select BidderID,Dated,Feedback,Comments,Followup_Date, smstext, emailtext From feedback_bookkeeping Where (AllRequestID='".$post."' and Reply_Type=2 AND BidderID IN (SELECT BidderID FROM Bidders WHERE leadidentifier IN ('".$leadidentifier."'))) ORDER BY Dated ASC";
	}
	$sqllifecycleresult = d4l_ExecQuery($sqllifecycle); 
	$lcrecordcount = d4l_mysql_num_rows($sqllifecycleresult);
	if($lcrecordcount>0)
	{
	?>
		...
		<?php 
		while($lcrow=d4l_mysql_fetch_array($sqllifecycleresult))
		{
			...
			$sqllifecyclebid=d4l_ExecQuery("Select Bidder_Name From Bidders Where BidderID='".$lcrow["BidderID"]."'");
			$bidrow=d4l_mysql_fetch_array($sqllifecyclebid);
		?>
			<tr>
...
<tr><td colspan="8">
     <form action="hllmsallocate_editlead_whatsapp.php" name="whatsapp_frm" method="post">
				...
				<input type="hidden" name="whatsapp_process_name" id="whatsapp_process_name" value="<? echo 'deal4loan_homeloan7057_message'; ?>" />
 ...
 ?>
               ...
               <tr><td colspan="4"><hr /></td></tr>
            	...
            	$sql = "SELECT message,xkyknzl5dwfyk4hg_wishfin_whatsapp.date_created FROM `xkyknzl5dwfyk4hg_wishfin_whatsapp` LEFT OUTER JOIN xkyknzl5dwfyk4hg_tms_whatsapp on xkyknzl5dwfyk4hg_tms_whatsapp.whatsapp_id=xkyknzl5dwfyk4hg_wishfin_whatsapp.id WHERE `mobile_number` = '".$Mobile."' AND `mobile_number`!='' AND process_name='deal4loan_homeloan7057_message' ORDER BY `xkyknzl5dwfyk4hg_wishfin_whatsapp`.`date_created` ASC ";
