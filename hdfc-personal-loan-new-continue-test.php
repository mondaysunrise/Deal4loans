...
$checkDupSql = "select * from hdfc_pl_calc_leads where mobile_number = '".$mobile_number."' and mobile_number not in (9971396361,9811555306,9811215138) and (Dated between '".$days30ago."' and '".$todayDate."')";
...
//Insert DATA For HDFC PL
	...
	$dataInsert = array('name'=>$full_name, 'mobile_number'=>$mobile_number, 'email_id'=>$email_id, 'city'=>$city, 'net_salary'=>$salary, 'Dated'=>$Dated, 'AppID'=>$app_code, 'Status'=>$approvel, 'other_city'=>$city_other, 'source'=>'HDFC PL Page');
...
function chkpersonalloan(Form)
{
	...
	if(Form.clubbed_emi.value=="")
	{
		alert("Please fill zero if no loan running or clubbed EMI of all loans.");
		Form.clubbed_emi.focus();
		return false;
	}

if(Form.reqd_loan_amt.value=="")
	{
		alert("Please fill Required Loan Amount.");
		Form.reqd_loan_amt.focus();
		return false;
	}
  ...
  <tr>
    <td><table width="275"  border="0" align="right" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td height="30" align="left" class="blutxt"  >Rate of Interest ranges between<br>13.99% - 20% </td>
        </tr>
      <tr>
...
<td valign="top">
			<form name="hdfc_calc" method="POST" action="hdfc-personal-loan-new-thanks-test.php" onSubmit="return chkpersonalloan(document.hdfc_calc);">
              ...
              <tr>
                <td align="right" valign="top"><table width="100%"  border="0" align="right" cellpadding="0" cellspacing="0">
                  <tr>
					...
					<input name="Phone" id="Phone" type="hidden" value="<?php echo $mobile_number; ?>"/>
					<input name="email_id" id="email_id" type="hidden" value="<?php echo $email_id; ?>"/>
					<input name="net_salary" id="net_salary" type="hidden" value="<?php echo $salary; ?>"/>
					<input name="City" id="City" type="hidden" value="<?php echo $city; ?>"/>
					<input name="city_other" id="city_other" type="hidden" value="<?php echo $city_other; ?>"/>
				...
				<option value="8">Hostel</option>
					...
					<td align="left" class="boldtxt">Required Loan Amount</td>
				     <td align="left"><input type="text" name="reqd_loan_amt" id="reqd_loan_amt"  style="width: 144px;" tabindex="14" onkeyup="intOnly(this);" onkeypress="intOnly(this);" /></td>
                  </tr>
				   <tr>
				   <td>&nbsp;</td>
