...
if(strlen($plfeedback)>0 && $plfeedback=="Verified")
	{
		$Is_Verified=1;

	if($plcity=="Delhi" || $plcity=="Noida" || $plcity=="Gurgaon" || $plcity=="Gaziabad" || $plcity=="Faridabad" || $plcity=="Sahibabad" || $plcity=="Greater Noida")
		{
			$agent_name="Manasi";
			$agent_number="8527329494";
		}
		elseif($plcity=="Chennai")
		{
			$agent_name="Om Prabhakar";
			$agent_number="9500032840";
		}
//Delhi – Manasi- 8527329494 - manasi.clement@fullertonindia.com
//Chennai – Om Prabhakar – 9500032840 - b.omprabhakar@fullertonindia.com

		$SMSMessage = "Appointment Fixed: with fullerton at $appointment_time on $appointment_date, .".$agent_name.",".$agent_number.",will visit on the appointed day & doc Reqd ".$document_proof_doc.",".$other_documents;
		if(strlen(trim($plmobile)) > 0)
		{
			SendSMSforLMS($SMSMessage, $plmobile);
		}

if(strlen(trim($agent_number))>0)
		{
		$AgentSMSMessage="Appointment Fixed: $plname, $plmobile,sal $plnet_salary, $plloan_amount,$plcompany_name,add- $appointment_address, $appointment_date,$appointment_time";

		if(strlen(trim($agent_number)) > 0)
		{
			SendSMSforLMS($AgentSMSMessage, $agent_number);
		}
	...
	{
	$updatelead="Update  Fullerton_Allocated_Leads set Is_Verified='".$Is_Verified."',fullerton_feedback='".$plfeedback."',fullerton_followupdate ='".$FollowupDate."', appointment_date='".$appointment_date."',appointment_time='".$appointment_time."', appointment_place='".$appointment_place."', appointment_address= '".$appointment_address."', cuurentadd_proof='$cuurentadd_proof',Existing_Noofemi='$NoofEMIpaid',identification_proof = '$identification_proof', salary_proof='$salary_proof',experience_proof='$experience_proof', bankstat_proof='$bankstat_proof',Annual_Turnover='$Annual_Turnover',Company_Type='$plCompany_Type',PL_Tenure= '$acc_no',Name='$plname',PL_EMI_Amt='$emi_amt',Company_Name='$plcompany_name', DOB='$pldob', Residential_Status='$plresidential_status',Email='$plemail', City='$plcity', Card_Limit='$plcard_limit', City_Other='$plcity_other', Mobile_Number='$plmobile', Std_Code='$plstd_code', Landline='$pllandline',  Net_Salary='$plnet_salary', Loan_Amount='$plloan_amount', Pincode='$plpincode', Employment_Status='$plemployment_status', CC_Holder='$plcc_holder', Card_Vintage='$plcard_vintage', Total_Experience='$pltotal_experience', Years_In_company='$plyears_in_company', Loan_Any='$Loan_A', Emi_Paid='$plemi_paid', Add_Comment='$pladd_comment',Dated=Now(), Primary_Acc='$Primary_Acc', Existing_Bank='$pl_Existing_Bank',Existing_Loan='$pl_Existing_Loan',Existing_ROI='$pl_Existing_ROI' where fullertonrequestID=".$post;
	$msg="Changes Saved";
	}
	//echo $updatelead."<br>";
	 $updateleadresult=ExecQuery($updatelead);

	 $calllog=ExecQuery("INSERT INTO icicipl_callLOG (fullertonrequestID, TelecallerID, fullerton_feedback, calldated) VALUES ('".$plrequestid."', '".$bidid."', '".$plfeedback."', Now())");
}
?>
<html>
<head>
...
<?php 
$viewqry="select cuurentadd_proof,Existing_Noofemi,  	appointment_date,appointment_address,appointment_time,appointment_place,bankstat_proof,salary_proof,experience_proof,identification_proof,Name,CC_Age,Add_Comment,Mobile_Number,Landline,Std_Code,Net_Salary,Residential_Status,City,City_Other,Dated,Employment_Status,Loan_Amount,Email,Loan_Any,Pincode,Emi_Paid,CC_Holder,Card_Vintage,fullerton_feedback,Company_Name,PL_EMI_Amt,Card_Limit,Salary_Drawn,Total_Experience,Years_In_Company,DOB,PL_Tenure,Primary_Acc,Company_Type,Annual_Turnover,PL_Bank,Existing_Bank,Existing_ROI,Existing_Loan,fullerton_followupdate from Fullerton_Allocated_Leads where Fullerton_Allocated_Leads.fullertonrequestID=".$post." "; 
...
$Feedback = mysql_result($viewlead,0,'fullerton_feedback');
...
$Followup_Date = @mysql_result($viewlead,0,'fullerton_followupdate');
$identification_proof = @mysql_result($viewlead,0,'identification_proof');
$monthsalary = $Net_Salary/12;
$getDOB =DetermineAgeGETDOB($DOB);
$getcompany='select fullerton from pl_company_list where ((company_name="'.$Company_Name.'"))';
$getcompanyresult = ExecQuery($getcompany);
$grow=mysql_fetch_array($getcompanyresult);
$fullertoncategory= $grow["fullerton"];
$appointment_date = @mysql_result($viewlead,0,'appointment_date');
$appointment_address = @mysql_result($viewlead,0,'appointment_address');
$appointment_place = @mysql_result($viewlead,0,'appointment_place');
$appointment_time = @mysql_result($viewlead,0,'appointment_time');
?>
<style>
.fontstyle
	{
...
{
	$fullertonrateexact=str_replace("%","",$fullertoninterestrate);
$fullertonintr = trim($fullertonrateexact)/1200;
	?>	
...
<tr> <td width="11%" height="25" align="center" valign="middle"><b style="font-size:12px;"><? echo "Fullerton"; ?></b></td>
...
<td width="11%" align="center"><b style="font-size:12px;"><? echo $fullertongetemicalc=round($fullertongetloanamout * $fullertonintr / (1 - (pow(1/(1 + $fullertonintr), ($fullertonterm*12))))); ?></b></td>
<td width="11%" align="center"><b style="font-size:12px;"><? 
	echo $emicalc=round(100000 * $fullertonintr / (1 - (pow(1/(1 + $fullertonintr), ($fullertonterm*12)))));
			?></b></td>
<td width="11%" align="center"><b style="font-size:12px;"><? echo $fullertoncategory; ?></b></td>
	<td width="11%" align="center"><b style="font-size:12px;">4%</b></td>
	<td width="11%" align="center"><b style="font-size:12px;">2%</b></td>
</tr>
<? 
}
?>
...
<td>Alternate Proof for Identity <input type="text" name="alternateidentification_proof" id="alternateidentification_proof" value="<? echo $identification_proof; ?>"></td>
 ...
 <tr><td colspan="4"><input type="radio" name="bankstat_proof" id="bankstat_proof" value="3 Months Bank Statement" <? if($bankstat_proof=="3 Months Bank Statement") {echo "checked";}?>>6 Months Bank Statement</td></tr> 
 <tr><td colspan="4" bgcolor="#DAEAF9" align="center" style="border:1px solid black;" ><b>Appointment Details </b></td></tr>
<tr><td>Appointment Date</td><td><input type="Text"  name="appointment_date" id="appointment_date" maxlength="25" size="15" <?php if($appointment_date !='0000-00-00') { ?>value="<?php  echo $appointment_date; ?>" <?php } ?>><a href="javascript:NewCal('appointment_date','yyyymmdd',false,24)"><img src="images/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td><td>Appointment Time</td><td><select name="appointment_time" id="appointment_time" tabindex="14">
