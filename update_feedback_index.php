...
{
	var gifName = form;
	document.frmsearch.action="update_feedback_index.php?search=y"+gifName;
...
function insertData(id)
		...
		var get_bidderid= document.getElementById('bidderid_'+ id).value;
...
<tr>
 ...
 <form name="frmsearch" action="update_feedback_index.php?search=y" method="post" onSubmit="return chkform();">
  ...
  <tr><td class="style1">Feedback</td><td colspan="2"><select name="cmbfeedback" id="cmbfeedback" style="width:120px;">
   ...
   <tr>
	...
	if(strlen(trim($RequestID))>0)
	{
		$strSQL="";
		$Msg="";
		//echo "select FeedbackID from Req_Feedback where AllRequestID=$RequestID and BidderID=".$Bid;
		//echo "<br>";
		//print_r($_REQUEST);
		$result = ExecQuery("select FeedbackID from Req_Feedback where AllRequestID=$RequestID and BidderID=".$Bid);		
		...
		else
		{
			...
			$strSQL=$strSQL.$RequestID.",".$Bid.",".$pro_code.",'".$Feedback."')";
		}
...
//		echo $strSQL;
 ...
 <? 
	if($city>0)
		...
		$search_qry="SELECT * , Req_Feedback_Bidder1.BidderID as BidID FROM Req_Feedback_Bidder1,`".$val."` LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=".$val.".RequestID AND Req_Feedback.BidderID in (".$checkbidderid.") WHERE Req_Feedback_Bidder1.AllRequestID=`".$val."`.RequestID and Req_Feedback_Bidder1.BidderID in (".$checkbidderid.") and Req_Feedback_Bidder1.Reply_Type=".$pro_code." and (Req_Feedback_Bidder1.Allocation_Date  Between '".($min_date)."' and '".($max_date)."' ) ";	
...
//echo "<br>";
	...
	<input type="hidden" name="requestid_<? echo $i;?>" id="requestid_<? echo $i;?>" value="<? echo $row["RequestID"];?>">
		<input type="hidden" name="product_<? echo $i;?>" id="product_<? echo $i;?>" value="<? echo $pro_code;?>">
			<input type="hidden" name="bidderid_<? echo $i;?>" id="bidderid_<? echo $i;?>" value="<? echo $row['BidderID'];?>">
   <tr>
	 ...
	 <td align="center" bgcolor="#DFF6FF" class="style3"><? echo getJumpMenu("update_feedback_index.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val,$row['BidderID'],$checkbidderid) ?></td>
 ...
 <br>
 <form name="frmdownload" action="update_feedback_download.php" method="post">
...
<?
function getJumpMenu($varPHPPage, $varRequestID, $varType, $varFeedback, $varpageon, $varmindate, $varmaxdate,$cmbfeedback,$varVal,$Bid,$checkbidderid)
{
	$strURL="";
$strURL=$varPHPPage."?search=y&RequestID=".$varRequestID."&type=".$varType."&pageno=".$varpageon."&min_date=".urlencode($varmindate)."&max_date=".urlencode($varmaxdate)."&cmbfeedback=".urlencode($cmbfeedback)."&product=".$varVal."&city=".$checkbidderid."&Bid=".$Bid;
?>
