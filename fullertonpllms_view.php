...
{
	var gifName = form;
	document.frmsearch.action="fullertonpllms_view.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <? $current_date=date('Y-m-d');?>
  <table width="712" border="0">
   <tr><td align="right"><a href="commonlms_plreport.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=1" target="_blank">today's Report</a> &nbsp;</td></tr>   
 <tr><td align="center" width="100%">
 <div align="center">
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <form name="frmsearch" action="fullertonpllms_view.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	{		
		...
		$resultst = ExecQuery("select fullertonrequestID from Fullerton_Allocated_Leads where (fullertonrequestID=$RequestID and BidderID=".$_SESSION['BidderID'].")");		
		$num_rows = mysql_num_rows($resultst);
		if($num_rows > 0)
		{
			$row = mysql_fetch_array($resultst);
			$strSQLst="Update Fullerton_Allocated_Leads Set fullerton_feedback='".$Feedback."' ";
			$strSQLst=$strSQLst."Where fullertonrequestID=".$row["fullertonrequestID"];
		echo $strSQLst;
		}
	...
	if($search=="y")
	{
		...
		{
			$FeedbackClause=" AND (Fullerton_Allocated_Leads.fullerton_feedback IS NULL OR Fullerton_Allocated_Leads.fullerton_feedback='') ";
		}
		...
		{
			$FeedbackClause=" AND Fullerton_Allocated_Leads.fullerton_feedback='".$varCmbFeedback."' ";
		}
	?>
 <p class="bodyarial11"><?=$Msg?></p>
 <table width="758" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <?
		$qry="Select RequestID From Fullerton_Allocated_Leads Where (((Fullerton_Allocated_Leads.Allocation_Date Between '".$min_date."' and '".$max_date."') or (Fullerton_Allocated_Leads.fullerton_followupdate Between '".$min_date."' and '".$max_date."')) and Fullerton_Allocated_Leads.approved=0)";
			$qry=$qry.$FeedbackClause;
			$qry=$qry."group by Fullerton_Allocated_Leads.RequestID";				
	//echo $qry."<br>";
 ...
 ?>
   ...
   <tr>   
	...
	$qry="		 
Select * From Fullerton_Allocated_Leads Where (((Fullerton_Allocated_Leads.Allocation_Date Between '".$min_date."' and '".$max_date."') or (Fullerton_Allocated_Leads.fullerton_followupdate Between '".$min_date."' and '".$max_date."'))  and Fullerton_Allocated_Leads.approved=0)";
$qry=$qry.$FeedbackClause;
		$qry=$qry."group by Fullerton_Allocated_Leads.RequestID";
...
//echo $qry;
		...
		{
			$Followup_Date = $row['fullerton_followupdate'];				
			...
			?>
<tr>
<td class="bodyarial11"><a href="fullerton_pleditlead.php?id=<? echo urlencode($row['fullertonrequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
...
<?
	   ...
	   ?>	
    <td class="bodyarial11"><? echo getJumpMenu("fullertonpllms_view.php",$row["fullertonrequestID"],"1",$row["fullerton_feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
	<?php if(($Followup_Date>$FinalMinDate || $Followup_Date<$FinalMaxDate) && ($Followup_Date !='0000-00-00 00:00:00') &&  ($TodayFormat==$FinalDay) )  { ?><td bgcolor="#FF0000"> <?php } else { ?>	
	 ...
	 <?php } ?>	 
	 <input type="Text"  name="FollowupDate-<?php echo $row["fullertonrequestID"]; ?>" maxlength="25" size="15" <?php if($Followup_Date !='0000-00-00 00:00:00') { ?>value="<?php  echo $Followup_Date; ?>" <?php } ?>>
	 <a href="javascript:NewCal('FollowupDate-<?php echo $row["fullertonrequestID"]; ?>','yyyymmdd',true,24)">	
	 </a>
	</td>	
</tr>
	<?						$i=$i+1;
