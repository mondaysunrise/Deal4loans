...
{   
	...
	if($strreferral==1)
	{
	 $qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and Referral_Flag=1 and BankID!=66 and BankID!=80)";
	}
	...
	else if($strreferral==3)
	{
		...
		$getIncludeBiddersQry = ExecQuery($getIncludeBiddersSql);
		$recordCountIncludeBidders = mysql_num_rows($getIncludeBiddersQry);
		for($ii=0;$ii<$recordCountIncludeBidders;$ii++)
		{
			$BidderIDInclude = mysql_result($getIncludeBiddersQry,$ii,'BidderID');
			$BidderIDIncludeArr[$ii] = $BidderIDInclude;
		}
		$BidderIDIncludeStr = implode(',', $BidderIDIncludeArr);
		 $qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and  BidderID in (".$BidderIDIncludeStr."))";
	}
	else
	{
