...
$filename = $RequestID."_".$filename;
  ...
  if(move_uploaded_file($tmpfile, "upload/".$filename)){
	...
	{
		//update
		$DataArray = array("CC_Photocopy"=>$filename);
  ...
  echo returnStatus($status);
	...
	function returnStatus($status){
		return "<html><body><script type='text/javascript'>function init(){if(top.uploadComplete_14) top.uploadComplete_14('".$status."');}window.onload=init;
	</script></body></html>";
	}
?>