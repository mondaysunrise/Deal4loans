...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$AppointmentAddress = $_POST["AppointmentAddress"];
	...
	$ccupqry="UPDATE sbi_credit_card_5633 SET appointment_datetime='".$AppointmentDate."', appointment_address='".$AppointmentAddress."' Where sbiccid='".$sbiccid."'";
	$ccupqryresult=d4l_ExecQuery($ccupqry);
}


$ccdetails = "SELECT * FROM Req_Credit_Card WHERE RequestID='".$requestid."'";
...
$cc_alldetailsSql = "SELECT * FROM sbi_credit_card_5633 WHERE RequestID= '".$requestid."' AND productflag != 10 AND ProcessingStatus = 1 AND ApplicationNumber != '' ORDER BY sbiccid DESC LIMIT 0,1";
...
$AppointmentAddress = $ccrowal["appointment_address"];
...
//Feedback Details
$FeedbackDetailSql = "SELECT * FROM Req_Feedback_CC WHERE BidderID='".$bidderid."' AND AllRequestID='".$requestid."' AND Reply_Type = 4";
$FeedbackDetailResult = d4l_ExecQuery($FeedbackDetailSql);
$FeedbackDetailResponse = d4l_mysql_fetch_assoc($FeedbackDetailResult);
$Feedback = $FeedbackDetailResponse["Feedback"];
$followup_date = $FeedbackDetailResponse["Followup_Date"];
$comment_section = $FeedbackDetailResponse["comment_section"];
...
<tr>
	<td colspan="4" bgcolor="#DAEAF9" align="center" style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold; border:1px solid black;"><b style="color:red; font-size:16px;">Customer Applied on Wishfin</b></td>
</tr>
<?php 
}
?>
<tr>
	<td colspan="4" align="center" style="font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px; font-weight:bold; height:25px;">Credit Card customer details [<strong>Selected Card - </strong><?php if(!empty($applied_card_name)){echo $applied_card_name;}else{ echo $CardName;} ?>]</td>
</tr>
<tr>
	<td width="180"><span class="style2"><strong>Customer Name: </strong> </span></td>
...
<tr>
	<td><span class="style2"> <strong>Mobile No: </strong> </span></td>
	<td><span class="style21"><? echo ccMasking($ccrow["Mobile_Number"]); ?></span></td>
	<td><span class="style2"> Email: </span></td>
	<td><span class="style21">
		<input type="text" value="<? echo $ccrow["Email"]; ?>" name="Email" id="Email" readonly /></span>
	</td>
</tr>
<tr>
	<td><span class="style2"> <strong>DOB: </strong> </span></td>
	<td><span class="style21">
...
<tr>
  	<td><span class="style2"><strong>PanCard No</strong></span></td>
    <td>
		<input type="text" class="d4l-input" name="panno" id="panno" value="<? echo $ccrow["Pancard"] ;?>"  maxlength="10" readonly />
	</td>
...
<tr>
    ...
    <td>
		<span class="style21">
			<input type="text" value="<? echo $City; ?>" readonly />
		</span>
	</td>
<td><span class="style2"> <strong>Post-paid mobile number: </strong> </span></td>
	<td>
				+91<input type="text" class="stdnumber" id="postpaid_mobile" name="postpaid_mobile" value="<? echo $Landline; ?>" maxlength="10" size="15" readonly />
...
<tr>
   ...
   <td><span class="style21">
       <input type="text" value="<? echo $ccrow["Pincode"]; ?>" readonly />
       </span>
	</td>
...
<tr>
	...
	<td><span class="style21">
		<input type="text" value="<? if($ccrow["Employment_Status"]==1) {echo "Salaried";}elseif($ccrow["Employment_Status"]==0) {echo "Self Employment";}?>" readonly />
		</span>
	</td>
	<td><span class="style2"> <strong>Company Name: </strong> </span></td>
	<td><span class="style21"><input name="Company_Name" id="Company_Name" type="text" class="d4l-input" onkeyup="ajax_showOptions(this,'getCountriesByLetters',event)"   style="width:200px;" maxlength="30" value="<? echo $CompanyName; ?>" readonly /></span>
	</td>
</tr>   
<tr>
	...
	<td><span class="style21"><input type="text"  name="Net_Salary" id="Net_Salary" value="<? echo $ccrow["Net_Salary"]; ?>" readonly /></span></td>
</tr>
<tr>
...
<tr>
	<td class="fontstyle"><b>Appointment Date</b></td>
	<td class="fontstyle">
	...
	<td width="180"><span class="style2"><strong>Appointment Address</strong></span></td>
	<td width="392"><span class="style21">
		<input type="radio" name="AppointmentAddress" id="radio-one" value="1" class="css-checkbox" <?php if($AppointmentAddress==1) { echo "checked";} ?> />
		<label for="radio-one" class="css-label radGroup2" >ResidenceAddress</label>
		<input type="radio" name="AppointmentAddress" id="radio-two" value="2" class="css-checkbox" <?php if($AppointmentAddress==2) { echo "checked";} ?>/>
		<label for="radio-two" class="css-label radGroup2">OfficeAddress</label>
	</span></td>
</tr>
<tr>
	...
	<tr><td colspan="4" align="right">
		...
		?>
			<!--<form method="POST" action="/soapservice_sbi5633_verifier.php" name="sendform">
				...
				<input type="hidden" name="sbiccid"id="sbiccid"  value="<?php echo $sbiccid; ?>" />
