...
if($City=="Others")
	...
	if($leadid>0)
				{														
					//$qry="Update Req_Loan_Personal SET IP_Address='$IP',Company_Type='$Company_Type',PL_EMI_Amt='$PL_EMI_Amt',Primary_Acc='$Primary_Acc', Residential_Status='$Residential_Status' ,Card_Limit= '$Credit_Limit', Years_In_Company='$Years_In_Company', Total_Experience='$Total_Experience',EMI_Paid='$EMI_Paid', Loan_Any='$Loan_A',identification_proof='$Document_proof_doc',Is_Valid='$Is_Valid',Bidderid_Details='$strFinal_Bid',Allocated='$Allocated',Salary_Drawn='$Salary_Drawn',Direct_Allocation=1,Pincode ='$Pincode', Loan_Amount = '$Loan_Amount', Employment_Status = '$Employment_Status' , CC_Holder= '$CC_Holder', Card_Vintage = '$Card_Vintage', Company_Name ='$Company_Name' Where RequestID=".$leadid;
...
$Reference_Code = $plrow[$m]['Reference_Code'];
...
Rate</td>
            ...
            <td width="89" class="fontbld10">Tenure (in Yrs)</td>
			
            <td width="361" class="fontbld10">Apply</td>
...
if($recordcount>0)
				{
		$identification_prf=$myrow[$m]["identification_proof"];
	$residence_prf=$myrow[$m]["residence_proof"];
	$income_prf=$myrow[$m]["income_proof"];
	$document_prf=$myrow[$m]["document_proof"];
...
<? echo $getbankid[$i];?></td>
	...
	<?
		...
		{?>
  <td width="300" height="45" colspan="4"   bgcolor="#FFFFFF" class="fontbld10">&nbsp;</td>
	<?	}
		//echo "<a href='/hdfc-personal-loan-eligibility.php' target='_blank'>Know More</a>";	
	}
	elseif((strncmp ("Fullerton", $getbankid[$i],9))==0 || ($getbankid[$i]=="Fullerton"))
	{
  ...
  <td width="300" height="45" colspan="4"   bgcolor="#FFFFFF" class="fontbld10">&nbsp;</td>
	...
	?>
	 ...
	 <td  width="300" bgcolor="#FFFFFF" height="51" colspan="6"><b>&nbsp;</b></td>
 ...
 <td width="300" height="45" colspan="4"   bgcolor="#FFFFFF" class="fontbld10">&nbsp;</td>
   ...
   <td   bgcolor="#FFFFFF" height="53" colspan="5"><b>&nbsp;</b></td>
		<? 
...
?>
	...
	<!--</td>-->
	<? //}?>
	<td ><form action="apply_pl_consent.php" method="POST" target="_blank" >
        ...
        {?>
				<td valign="top" >
					<table width="97%" border="0" align="center" cellpadding="0" cellspacing="0" class="crdbg">
						<tr>
							<td height="30" class="crdbhdng"><a href="<? if (strlen($myrow[$p]["cc_bank_url"])>0) {echo $myrow[$p]["cc_bank_url"];} else {echo "#";}?>" target="_blank"><? echo $myrow["cc_bank_name"];?></a></td>
						</tr>
						<tr>
							<td height="255" align="center" valign="bottom"><a href="<? if (strlen($myrow[$p]["cc_bank_url"])>0) {echo $myrow[$p]["cc_bank_url"];} else {echo "#";}?>" target="_blank"><img src="<? echo $myrow[$p]["card_image"];?>"  width="150" height="244" /></a></td>
						</tr>
						<tr>
							<td height="22" valign="bottom" class="crdbold">Features</td>
						</tr>
						<tr>
							<td height="270" valign="top" class="crdtext"><? echo $myrow[$p]["cc_bank_features"];?></td>
						</tr>
						<tr>
							<td align="center" valign="bottom"><a href="<? if (strlen($myrow[$p]["cc_bank_url"])>0) {echo $myrow[$p]["cc_bank_url"];} else {echo "#";}?>" target="_blank"><input type="image" style="background-image:url(new-images/crds-apply.gif); background-repeat:no-repeat; width:141px; height:65px; border:none;" src="new-images/crds-apply.gif" /></a></td>
						</tr>
					</table>
				</td>
				<? $p = $p +1;}?>
