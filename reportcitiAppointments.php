...
$endDate = date("Y-m-d", $mkDate)." 23:59:59";
		...
		list($numR,$getBidders_Query)=MainselectfuncNew($getBidders_Sql,$array = array());
		$Bid = "";
		for($j=0;$j<$numR;$j++)
		{
			$BidderID = $getBidders_Query[$j]['BidderID'];
			$Bid[] = $BidderID;
		}
	...
	$getDocSql = "select * from citi_appointments left join Req_Feedback_Bidder1 on Req_Feedback_Bidder1.AllRequestID = citi_appointments.RequestID where (Req_Feedback_Bidder1.Allocation_Date between '".$startDate."' and '".$endDate."') and Req_Feedback_Bidder1.BidderID in (".$bidderStr.") group by citi_appointments.RequestID";
	...
	$Content .='<tr><td colspan="6" bgcolor="pink" align="center"><b> Appointments Citibank </b></td></tr>';
	$Content .='<tr>';
...
//	$Content .='<td class="head1" align="center"><b>Docs</b></td>';	
	...
	for($jj=0;$jj<$numgetDoc;$jj++)
	{
		...
		list($numgetBidders,$getBiddersQuery)=MainselectfuncNew($getBiddersSql,$array = array());
		...
		list($numgetBidder,$getBidderQuery)=MainselectfuncNew($getBidderSql,$array = array());
...
$Email="balbir.singh@deal4loans.com";
...
//mail($Email,'Appointments Leads Citibank - Deal4loans', $Content, $headers);

//reportAppointmentsfil.php
///usr/bin/php /var/www/vhosts/deal4loans.com/httpdocs/reportcitiAppointments.php
?>