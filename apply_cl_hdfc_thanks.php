...
//echo $cl_requestid = $_REQUEST['cl_requestid'];
//$cl_bank_name = $_REQUEST['cl_bank_name'];
//echo $leadid = $_REQUEST['cl_requestid'];



if (strlen($cl_bank_name)>1 && $cl_requestid>1)
{
	echo $selqry="select CL_Bank from Req_Loan_Car Where RequestID=".$cl_requestid;
...
<table cellpadding="0" cellspacing="2" width="840" align="center" style="border:1px #dbf2ff solid; background-color:#dbf2ff;">
		...
		<? for($i=0;$i<count($Final_Bid);$i++)
             ...
             if(trim($Final_Bid[$i])=="HDFC")
			{
				...
				if(!empty($_REQUEST['TenureVal']))
					{
						$hdfc_tenure = $_REQUEST['TenureVal']*12;
					}
				else{
						$hdfc_tenure = $hdfc_tenure;
					}
				
				if(!empty($_REQUEST['TenureVal']))
					{
						$hdfc_print_term = $_REQUEST['TenureVal'];
					}
				else{
						$hdfc_print_term = $hdfc_print_term;
					}
