...
//echo $sql = "select * from Bidders where Email='$Email' and PWD='$PWD' and Is_Verified>1";
		$result = ("select * from Bidders where Email='$Email' and PWD='$PWD' and BidderID=2353");
		...
		if($num_rows > 0){
			...
			$Global_Access_ID = $row[$cntr]['Global_Access_ID'];
			$_SESSION['DefinePrePost'] = $row[$cntr]['Define_PrePost'];
			$_SESSION['product']= $row[$cntr]["Reply_Type"];
	...
	//	echo $sql = "Select Reply_Type,Dated from Bidders_List where BidderID='".$_SESSION['BidderID']."'";
			...
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."appt_citi_index.php");
			exit;
