...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	$resiaddress3 = $_POST["resiaddress3"];
...
if(strlen($middle_name)>0 && $middle_name!="Middle Name" && $last_name=="")
 ...
 $InsertProductSql= array("RequestID" => $requestID, "OfficeAddress1" => $OfficeAddress1 , "OfficeAddress2" => $OfficeAddress2 , "OfficePin" => $OfficePin, "TypeOfLandline" => $Land_linenumber, "OfficeCity" => $OfficeCity, "LandlineNo" => $Phone_Number, "OfficeState"=> $OfficeState, "Qualification"=> $Qualification, "CardName" => $card_name, "Designation" => $Designation, "Dated" =>$Dated);
...
if($Land_linenumber=="Residence")
	...
	else
	{
		...
		if($strcity=="Ghaziabad")
		{
			$resislct="select std from sbi_cc_city_state_list Where (city like'%Gaziabad%') group by city Limit 0,1";
		}
		else
		{
...
if(($sbicompcat=='A' || $sbicompcat=='B') || (($sbicompcat!='A' && $sbicompcat!='B') && $CC_Holder==1 && $Net_Salary>=360000))
{	
	$validlead=1;
}
