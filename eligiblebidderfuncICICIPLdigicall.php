...
{   
	...
	$mvarType = 1;
	 ...
	 $qry = "SELECT * FROM Bidders_List WHERE (Reply_Type='".$mvarType."' and City LIKE '%".$mvarCity."%' and  Restrict_Bidder=1 and Referral_Flag=1 and BankID!=66 and BidderID in (4900,5115))";
	...
	list($firstcountD,$row_result_D)=MainselectfuncNew($CheckDateSql,$array = array());
	...
	list($firstcountW,$row_result_W)=MainselectfuncNew($CheckWeekSql,$array = array());
	...
	list($firstcountM,$row_result_M)=MainselectfuncNew($CheckMonthSql,$array = array());
	...
	list($firstcountLT,$row_result_LT)=MainselectfuncNew($CheckLifeTimeSql,$array = array());
	$Total4LifeTime = $row_result_LT[0]['SumLifeTime'];
		...
		$propercity="('".$newcity."')";
		$query = str_replace("Req_Loan_Personal", "ICICI_Allocated_Leads", $query);
		$query = str_replace("RequestID", "icicirequestID", $query);
		
		$qry2 = $query." and (City in ".$propercity." or City_Other in ".$propercity." )and ICICI_Allocated_Leads.icicirequestID ='".$RequestID."'";
		echo $qry2."<br>";
