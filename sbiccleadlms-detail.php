...
$upcctblenw="Update Req_Credit_Card set Name='".$Name."',DOB='".$DOB."',Company_Name='".$Company_Name."',Std_Code='".$resiSTD."',Landline='".$resi_landline."', Email='".$Email."',Net_Salary='".$Net_Salary."',Gender='".$Gender."',Residence_Address ='".$Residence_Address."',State='".$State."', Office_Address='".$Office_Address."', Pancard='".$panno."',Pincode='".$pincode."' ".$cityclause." ".$othercityclause." Where (RequestID=".$RequestID.")";
...
for($cN=0;$cN<$numRowsCarName;$cN++)
{
	...
	?>
   	 <option value="<?php echo $resicity; ?>" <? if($resicity==ucwords(strtolower($City)) || $cityalias==ucwords(strtolower($City))) {echo "Selected";} ?> ><?php echo ucwords(strtolower($resicity)); ?></option>
                  <?php
				 }
?>	</select></span></td>
...
?>	
  ...
  <? if($send_status==0 && $ccrow["Net_Salary"]>=300000 && ($City=="Mumbai"|| $City=="Thane" || $City=="Navi Mumbai" || $City=="Delhi" || $City=="Noida" || $City=="Gurgaon" || $City=="Gaziabad" || $City=="Faridabad" || $City=="Kolkata" || $City=="Chennai" || $City=="Pune" || $City=="Bangalore" || $City=="Hyderabad" || $City=="Ahmedabad")) { ?>
...
<td><span class="style2">Citi Designation </span></td><td><select name="citiDesignation" id="citiDesignation" >
   ...
   <tr><td colspan="4" align="center"><input type="Submit" name="Submit" value="Submit"></td></tr>
    <? if($send_status!=1) { ?><tr><td colspan="4" align="left"><input type="Submit" name="Submitciti" value="Citibank Form"></td></tr><? } ?>
  </table>
</form>
</td></tr>
<tr><td>
<table style='border:1px dotted #9C9A9C;' cellspacing="0" cellpadding="5" width="700" height="80%" align="center" border="1" >
<tr><td colspan="4" align="right"><form method="POST" action="/soapservice_sbi5633_1.php" name="sendform" target="_blank"><input type="hidden" name="custid"id="custid"  value="<?php echo $requestid; ?>"><input type="Submit" name="Submitd" value="SBI Send now" style="color:#FFF; background-color:#C00;"></form> | <?php 
...
if($getStancNumRows>0)
{
	...
	{
?>
<a href="scb_agent_form1.php?cust_id=<?php echo $requestid; ?>" style="color:#FFF; background-color:#C00;" target="_blank">Standard Chartered Send now</a>
      <?php } } ?>  
</td></tr>
