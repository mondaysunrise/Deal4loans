...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && ($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134" || $IP=="122.161.196.68" || $IP=="122.161.193.191" || $IP=="61.246.3.127" || $IP=="122.160.30.168")) {
		...
		$pl_IncomeProof = $pl_PanCard.",".$pl_SalSlip.",".$pl_BankStmnt .",".$pl_PassSizePhoto;
...
<tr>
	<td style="border:1px solid black;" colspan="4" bgcolor="#DAEAF9" align="center" class="fontstyle" ><b>Personal Details</b></td></tr>
		...
		<td class="fontstyle"><b>DOB</b></td>
			...
			<td class="fontstyle">+91 <?php echo ccMasking($Mobile); ?><input type="hidden" name="plmobile" size="15" value="<? echo $Mobile;?>"></td>
			</tr>
		<tr>
...
<td><input name="company" id="company"   type="text" onKeyUp="ajax_showOptions(this,'getCountriesByLetters',event)" value="<? echo $Company_Name;?>" size=45/> </td><td colspan="2"></td></tr>
<tr>
...
//echo $City;?>
<tr><td class="fontstyle" colspan="4"><b>Eligible for Bidders</b></td></tr>
<tr><? if(strlen($Bidderid_Details)>0){?><td class="fontstyle" colspan="4"> already send</td><? } else { ?><td colspan="4"><div id="checkdiv" name="checkdiv"><form method="POST" action="personalloan_smsleaddetails_continue.php" name="sendform" target="_blank"></form><? 
...
{	$compforbajaj="bajaj";}else	{$compforbajaj="";	}
	$leadsource="SMS_Lead_Appointment_Model";
list($FinalBidder,$finalBidderName)= getBiddersList("Req_Loan_Personal",$post,$City,$Referral_Flag,$leadsource);
//print_r($FinalBidder);
   for($i=0;$i<count($FinalBidder);$i++)
			{	   
	  if((($FinalBidder[$i]==2490 || $FinalBidder[$i]==4018 || $FinalBidder[$i]==2496 || $FinalBidder[$i]==4019 || $FinalBidder[$i]==2497 || $FinalBidder[$i]==4020 || $FinalBidder[$i]==2498 || $FinalBidder[$i]==2499 || $FinalBidder[$i]==4342 || $FinalBidder[$i]==2500 || $FinalBidder[$i]==2813 || $FinalBidder[$i]==2887 || $FinalBidder[$i]==3894 || $FinalBidder[$i]==4302 || $FinalBidder[$i]==4309 || $FinalBidder[$i]==4529 || $FinalBidder[$i]==4528 || $FinalBidder[$i]==4663 || $FinalBidder[$i]==4914 || $FinalBidder[$i]==4915))  && ($ingvyasyacategory=="" && $Primary_Acc!="IngVysya" && $Company_Type!=4 && $Company_Type!=5))
	   {			  
		}
		...
		elseif(($FinalBidder[$i]==4083 || $FinalBidder[$i]==4084 || $FinalBidder[$i]==4085 || $FinalBidder[$i]==4086 || $FinalBidder[$i]==4087 || $FinalBidder[$i]==4088 || $FinalBidder[$i]==4089 || $FinalBidder[$i]==4090 || $FinalBidder[$i]==4091 || $FinalBidder[$i]==4092  || $FinalBidder[$i]==5168 || $FinalBidder[$i]==5409 || $FinalBidder[$i]==5410 || $FinalBidder[$i]==5411 || $FinalBidder[$i]==5412 || $FinalBidder[$i]==5413 || $FinalBidder[$i]==5414 || $FinalBidder[$i]==5415) && (($PL_EMI_Paid<=9 && $Existing_Loan>1)))
				{
			//echo "indus entere1<br>".$PL_EMI_Paid."".$pl_Existing_Loan;
				}
...
elseif(($FinalBidder[$i]==2721 || $FinalBidder[$i]==3359 || $FinalBidder[$i]==4494 || $FinalBidder[$i]==3722 || $FinalBidder[$i]==2722 || $FinalBidder[$i]==3390 || $FinalBidder[$i]==3579 || $FinalBidder[$i]==2809 || $FinalBidder[$i]==2723 || $FinalBidder[$i]==2830 || $FinalBidder[$i]==3376 || $FinalBidder[$i]==2937 || $FinalBidder[$i]==5164) && (($citicategory=="") && ($Company_Type<4)))
	...
	elseif((($kotakcategory=='' && $Net_Salary<480000))  && ($FinalBidder[$i]==2999 ||  $FinalBidder[$i]==3001 || $FinalBidder[$i]==5566 || $FinalBidder[$i]==3004 || $FinalBidder[$i]==3008 || $FinalBidder[$i]==3009 || $FinalBidder[$i]==5226 || $FinalBidder[$i]==3002 || $FinalBidder[$i]==3003 || $FinalBidder[$i]==3005 || $FinalBidder[$i]==3006 || $FinalBidder[$i]==3007 || $FinalBidder[$i]==3010 || $FinalBidder[$i]==3011 || $FinalBidder[$i]==3012 || $FinalBidder[$i]==3014 || $FinalBidder[$i]==3015 || $FinalBidder[$i]==3654 || $FinalBidder[$i]==3890 || $FinalBidder[$i]==3889 || $FinalBidder[$i]==4407 || $FinalBidder[$i]==4837 || $FinalBidder[$i]==4846 || $FinalBidder[$i]==5203 || $FinalBidder[$i]==4592 || $FinalBidder[$i]==4595 || $FinalBidder[$i]==5344 || $FinalBidder[$i]==5345 || $FinalBidder[$i]==5346 || $FinalBidder[$i]==5347 || $FinalBidder[$i]==5348 || $FinalBidder[$i]==5349 || $FinalBidder[$i]==5350 || $FinalBidder[$i]==5351 || $FinalBidder[$i]==5352 || $FinalBidder[$i]==5353 || $FinalBidder[$i]==5354 || $FinalBidder[$i]==5355 || $FinalBidder[$i]==3801 || $FinalBidder[$i]==5325 || $FinalBidder[$i]==5378 || $FinalBidder[$i]==5386 || $FinalBidder[$i]==5677 || $FinalBidder[$i]==5676 || $FinalBidder[$i]==5916 || $FinalBidder[$i]==5920))
				{
				}
	elseif((($kotakcategory=='' && $Net_Salary<480000)) && ($FinalBidder[$i]==3000 || $FinalBidder[$i]==2998))
				{
				}	
	...
	elseif($compforbajaj=="bajaj" && ($finalBidderName[$i]=="Bajaj Finserv" || $finalBidderName[$i]=="Bajaj finserv"))
		...
		elseif(($FinalBidder[$i]==5648 || $FinalBidder[$i]==5649 || $FinalBidder[$i]==5650) && $Primary_Acc!="HDFC" && $Net_Salary<216000)
				...
				{ 	
					$finalbidderarr[]=$FinalBidder[$i];
					$getsmspllog ="Select leadlogid From smsapp_leadallocation_log Where (BidderID='".$FinalBidder[$i]."' and RequestID=".$post." and ProductID=1)"; 
					$getsmspllogresult = ExecQuery($getsmspllog);
					$plsmsldlg=mysql_fetch_array($getsmspllogresult); 
					
					$finalbidderarr[]=$FinalBidder[$i];
					//appt_date, appt_time, special_remarks
					$appdetailsqry=ExecQuery("select appt_date, appt_time, special_remarks from smspl_status_details where (BidderID= ".$FinalBidder[$i]." and `AllRequestID`=".$post." and ProductID=1)");
					$apppl=mysql_fetch_array($appdetailsqry); 
					$appointment_date = $apppl["appt_date"];
					$appointment_time = $apppl["appt_time"];
					$special_remarks = $apppl["special_remarks"];
					//for appointment details
$getValidBiddersForSmsCityWise=ExecQuery("Select City_Wise from req_plcompaign_smscontact Where Sms_Flag=1 and Reply_Type=1 and BidderID='".$FinalBidder[$i]."'");
...
if(strlen($strcitywise)>0)
	...
	$getsmspl="select Mobile_no,Bank_Name from req_plcompaign_smscontact Where (Sms_Flag=1 and Reply_Type=1 and BidderID='".$FinalBidder[$i]."'and City_Wise like '%".$City."%') order by Compaign_ID ASC LIMIT 0,1";
			}
else
			{
	//echo "I M INSIDE CITY WISE IN ELSE BLOCK";
 $getsmspl='select Mobile_no,Bank_Name from req_plcompaign_smscontact where ((BidderID="'.$FinalBidder[$i].'" and Sms_Flag=1 and Reply_Type=1)) order by Compaign_ID ASC LIMIT 0,1';
			}
				...
				<br><form method="POST" action="personalloan_smsleaddetails_continue.php" name="sendform_<? echo  $FinalBidder[$i];?>" target="_blank">
				...
				<input type='hidden' value='<?php echo $FinalBidder[$i]; ?>' name='Final_Bidder' id='Final_Bidder'>
				<input type='hidden' value='<?php echo $plsmsld["Mobile_no"]; ?>' name='Bidder_Number' id='Bidder_Number'>
				<table border=1 width="100%" cellpadding="6" cellspacing="0"> <tr><td width="100">
				<?php echo $finalBidderName[$i]."(".$FinalBidder[$i].")"; ?></td>
				<td>
				<?php echo $plsmsld["Bank_Name"]; ?></td>
				<? if(strlen($plsmsld["Mobile_no"])>0) {?>
	<td  align="center"><?php if(strlen($plsmsld["Mobile_no"])>0) { echo $plsmsld["Mobile_no"];} else { echo "&nbsp;";}?></td>
	<td align="center"><? if($plsmsldlg>0) { echo "<b>Already Sent</b>";} elseif(strlen($plsmsld["Mobile_no"])>0) {?> <input type="submit" name="submit" value="Send Now" style="background-color:#036; color:#fff;"><? } else { echo "&nbsp;"; } ?></td>
	<td><textarea rows="1" cols="10" name="special_remarks" id="special_remarks_<? echo $i; ?>" onChange="NosplcharComment(this);"><? echo $special_remarks; ?></textarea></td>
	<td><input type="Text"  name="appointment_date" id="appointment_date_<? echo $i; ?>" maxlength="25" size="15" <?php if($appointment_date !='0000-00-00 00:00:00') { ?>value="<?php  echo $appointment_date; ?>" <?php } ?>><a href="javascript:NewCal('appointment_date_<? echo $i; ?>','yyyymmdd',true,24)"><img src="images/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td>
	<td class="fontstyle"> <select name="appointment_time" id="appointment_time" class="fontstyle">
		    <option value="please select">Time slab</option>
		    <option <? if($appointment_time=="8(am)-9(am)") echo "selected"; ?> value="8(am)-9(am)">8(am)-9(am)</option>
		    <option <? if($appointment_time=="9(am)-10(am)") echo "selected"; ?> value="9(am)-10(am)">9(am)-10(am)</option>
		    <option <? if($appointment_time=="10(am)-11(am)") echo "selected"; ?> value="10(am)-11(am)">10(am)-11(am)</option>
		    <option <? if($appointment_time=="11(am)-12(am)") echo "selected"; ?> value="11(am)-12(am)">11(am)-12(am)</option>
		    <option value="12(am)-1(pm)" <? if($appointment_time=="12(am)-1(pm)") echo "selected"; ?>>12(am)-1(pm)</option>
		    <option <? if($appointment_time=="1(pm)-2(pm)") echo "selected"; ?> value="1(pm)-2(pm)">1(pm)-2(pm)</option>
		    <option value="2(pm)-3(pm)" <? if($appointment_time=="2(pm)-3(pm)") echo "selected"; ?>>2(pm)-3(pm)</option>
		    <option <? if($appointment_time=="3(pm)-4(pm)") echo "selected"; ?> value="3(pm)-4(pm)">3(pm)-4(pm)</option>
		    <option value="4(pm)-5(pm)" <? if($appointment_time=="4(pm)-5(pm)") echo "selected"; ?>>4(pm)-5(pm)</option>
		    <option <? if($appointment_time=="5(pm)-6(pm)") echo "selected"; ?> value="5(pm)-6(pm)">5(pm)-6(pm)</option>
		    <option <? if($appointment_time=="6(pm)-7(pm)") echo "selected"; ?> value="6(pm)-7(pm)">6(pm)-7(pm)</option>
		    <option <? if($appointment_time=="7(pm)-8(pm)") echo "selected"; ?> value="7(pm)-8(pm)">7(pm)-8(pm)</option>
	    </select></td>
		<? if($plsmsldlg["leadlogid"] >0) {?>
		<td><a href="personalloan_smsleaddetails_escalation.php?bidderid=<?php echo $FinalBidder[$i]; ?>&custid=<? echo $post;?>&estype=1" target="_blank">Escalate 1</a><br><br><a href="personalloan_smsleaddetails_escalation.php?bidderid=<?php echo $FinalBidder[$i]; ?>&custid=<? echo $post;?>&estype=2" target="_blank">Escalate 2</a></td><td><a href="personalloan_smsleaddetails_escalation.php?bidderid=<?php echo $FinalBidder[$i]; ?>&custid=<? echo $post;?>&estype=3" target="_blank">Escalate 3</a></td><? } ?>
		<td><input type="submit" name="submit_appdt" value="Save" style="background-color:#036; color:#fff;"></td>
		<? } else
		{ ?>
			<td colspan="6">&nbsp;</td>
	<? } ?>
	...
	// already sent
	$plsmsldlg="";
	$finalbidderstr=implode("','",$finalbidderarr);
	$r=0;
	$getsmspllog ="Select BidderID From smsapp_leadallocation_log Where (BidderID not in ('".$finalbidderstr."') and RequestID=".$post." and ProductID=1)"; 
	$getsmspllogresult = ExecQuery($getsmspllog);
	while($plsmsldlg=mysql_fetch_array($getsmspllogresult))
	{
		$FinalBidderid=$plsmsldlg["BidderID"];
			//appt_date, appt_time, special_remarks
		$appdetailsqry=ExecQuery("select appt_date, appt_time, special_remarks from smspl_status_details where (BidderID= ".$FinalBidderid." and `AllRequestID`=".$post." and ProductID=1)");
		$apppl=mysql_fetch_array($appdetailsqry); 
		$appointment_date = $apppl["appt_date"];
		$appointment_time = $apppl["appt_time"];
		$special_remarks = $apppl["special_remarks"];
		//for appointment details
$getValidBiddersForSmsCityWise=ExecQuery("Select City_Wise from req_plcompaign_smscontact Where Sms_Flag=1 and Reply_Type=1 and BidderID='".$FinalBidderid."'");
$citwisesms=mysql_fetch_array($getValidBiddersForSmsCityWise);
$strcitywise= $citwisesms['City_Wise'];
if(strlen($strcitywise)>0)
			{
	//echo "I M INSIDE CITY WISE IF BLOCK";
	$getsmspl="select Mobile_no,Bank_Name from req_plcompaign_smscontact Where (Sms_Flag=1 and Reply_Type=1 and BidderID='".$FinalBidderid."'and City_Wise like '%".$City."%') order by Compaign_ID ASC LIMIT 0,1";
			}
else
			{
	//echo "I M INSIDE CITY WISE IN ELSE BLOCK";
 $getsmspl='select Mobile_no,Bank_Name from req_plcompaign_smscontact where ((BidderID="'.$FinalBidderid.'" and Sms_Flag=1 and Reply_Type=1)) order by Compaign_ID ASC LIMIT 0,1';
			}
				$getsmsplresult = ExecQuery($getsmspl);
				$plsmsld=mysql_fetch_array($getsmsplresult); 
				if($getCheckNum>1)
					...
					?>
					<table width="100%" cellpadding="0" cellspacing="0"><tr><td width="100%">
				<br><form method="POST" action="personalloan_smsleaddetails_continue.php" name="sendform_<? echo  $FinalBidderid;?>" target="_blank">
				<input type="hidden" name="callerid" id="callerid" value="<? echo $bidid;?>">
				<input type="hidden" name="reqcity" id="reqcity" value="<? echo $City;?>">
				<input type="hidden" name="plrequestid" id="plrequestid" value="<? echo $post;?>">
				<input type='hidden' value='<?php echo $FinalBidderid; ?>' name='Final_Bidder' id='Final_Bidder'>
				<input type='hidden' value='<?php echo $plsmsld["Mobile_no"]; ?>' name='Bidder_Number' id='Bidder_Number'>
				<table border=1 width="100%" cellpadding="6" cellspacing="0"> <tr><td width="100">
				<?php echo $finalBidderName[$i]."(".$FinalBidderid.")"; ?></td>
				<td>
				<?php echo $plsmsld["Bank_Name"]; ?></td>
				<? if(strlen($plsmsld["Mobile_no"])>0) {?>
	<td  align="center"><?php if(strlen($plsmsld["Mobile_no"])>0) { echo $plsmsld["Mobile_no"];} else { echo "&nbsp;";}?></td>
	<td align="center"><? if($plsmsldlg>0) { echo "Already Sent";} elseif(strlen($plsmsld["Mobile_no"])>0) {?> <input type="submit" name="submit" value="Send Now" style="background-color:#036; color:#fff;"><? } else { echo "&nbsp;"; } ?></td>
	<td><textarea rows="1" cols="10" name="special_remarks" id="special_remarks" id="special_remarks_<? echo $r; ?>" onChange="NosplcharComment(this);"><? echo $special_remarks; ?></textarea></td>
	<td><input type="Text"  name="appointment_date" id="appointment_daten_<? echo $r; ?>" maxlength="25" size="15" <?php if($appointment_date !='0000-00-00 00:00:00') { ?>value="<?php  echo $appointment_date; ?>" <?php } ?>><a href="javascript:NewCal('appointment_daten_<? echo $r; ?>','yyyymmdd',true,24)"><img src="images/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td>
	<td class="fontstyle"> <select name="appointment_time" id="appointment_time" class="fontstyle">
		    <option value="please select">Time slab</option>
		    <option <? if($appointment_time=="8(am)-9(am)") echo "selected"; ?> value="8(am)-9(am)">8(am)-9(am)</option>
		    <option <? if($appointment_time=="9(am)-10(am)") echo "selected"; ?> value="9(am)-10(am)">9(am)-10(am)</option>
		...
		<td><a href="personalloan_smsleaddetails_escalation.php?bidderid=<?php echo $FinalBidderid; ?>&custid=<? echo $post;?>&estype=1" target="_blank">Escalate 1</a><br><br><a href="personalloan_smsleaddetails_escalation.php?bidderid=<?php echo $FinalBidderid; ?>&custid=<? echo $post;?>&estype=2" target="_blank">Escalate 2</a></td><td><a href="personalloan_smsleaddetails_escalation.php?bidderid=<?php echo $FinalBidderid; ?>&custid=<? echo $post;?>&estype=3" target="_blank">Escalate 3</a></td>
		<td><input type="submit" name="submit_appdt" value="Save" style="background-color:#036; color:#fff;"></td>
		<? } else
		{ ?>
			<td colspan="6">&nbsp;</td>
...
<tr>
	<td class="fontstyle"><b>ID Proof</b></td>
	<td class="fontstyle">   
    <select name="IDProof" id="IDProof">
		...
		<option value="Voter_IDCard" <?php if(trim($identification_proof)=="Voter_IDCard") {echo "Selected";} ?>>Voter ID Card</option>
		<option value="Election_IDCard" <?php if(trim($identification_proof)=="Election_IDCard") {echo "Selected";} ?>>Election ID Card</option>
		<option value="Others" <?php if(trim($identification_proof)=="Others") {echo "Selected";} ?>>Others</option>		
	</select>
	</td>
    ...
    <option value="">Please Select</option>
		...
		<option value="Bank_Statement" <?php if(trim($residence_proof)=="Bank_Statement") {echo "Selected";} ?>>Bank Statement</option>
		<option value="Utility_Bill" <?php if(trim($residence_proof)=="Utility_Bill") {echo "Selected";} ?>>Utility Bill</option>
		<option value="Gas_Receipt" <?php if(trim($residence_proof)=="Gas_Receipt") {echo "Selected";} ?>>Gas Receipt</option>
		<option value="Rent_Agreement" <?php if(trim($residence_proof)=="Rent_Agreement") {echo "Selected";} ?>>Rent Agreement</option>
        <option value="Others" <?php if(trim($residence_proof)=="Others") {echo "Selected";} ?>>Others</option>
	</select></td>
</tr>
<tr>
	...
	<td class="fontstyle">
     <input type="radio" name="SalSlip" id="SalSlip" value="3Month_SalSlip" <?php if((strlen(strpos($income_proof, "3Month_SalSlip")) > 0)) echo "checked"; ?>> Yes &nbsp;&nbsp;<input type="radio" name="SalSlip" id="SalSlip2" value="">No
   </td>
</tr>
<tr>
	<td class="fontstyle"><b>6 Month Bank Statement</b></td>
	<td>
    <input type="radio" name="BankStmnt" id="BankStmnt" value="6Month_BankStatement" <?php if((strlen(strpos($income_proof, "6Month_BankStatement")) > 0)) echo "checked"; ?>> Yes &nbsp;&nbsp;<input type="radio" name="BankStmnt" id="BankStmnt2" value="">No    
	</td>
	<td class="fontstyle"><b>2 Passport Size Photo</b></td>
	<td class="fontstyle">
      <input type="radio" name="PassSizePhoto" id="PassSizePhoto" value="2Passport_SizePhoto" <?php if((strlen(strpos($income_proof, "2Passport_SizePhoto")) > 0)) echo "checked"; ?>> Yes &nbsp;&nbsp;<input type="radio" name="PassSizePhoto" id="PassSizePhoto2" value="">No
    </td>
</tr>
...
<tr>
    ...
    <select name="plfeedback" id="feedback">
		...
		<option value="Not Eligible CNL" <?if($Feedback == "Not Eligible CNL") { echo "selected"; }?>>Not Eligible (City not listed)</option>
		...
		<option value="Closed" <? if($Feedback == "Closed" || $Feedback == "Disbursed") { echo "selected"; } ?>>Cibil Reject</option>
		<option value="CFL Approved" <? if($Feedback == "CFL Approved") { echo "selected"; } ?>>CFL Approvedk</option>
		<option value="CFL Reject" <? if($Feedback == "CFL Reject") { echo "selected"; } ?>>CFL Reject</option>
 ...
 <tr>
   ...
   <tr><td colspan="4" ><br><br><br><br><br><br><br><br><br><?php echo $_SERVER['REQUEST_METHOD']." - ".$IP."<br>"; echo "query - ".$updatelead."<br>"; ?></td></tr>
</table>
</form>
</body>
</html>
