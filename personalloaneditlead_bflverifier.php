...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $verifierDateTime = $_REQUEST['verifierDateTime'];
...
//echo "hello".$Final_Bid."<br>";
    ...
    $resBankFeedback = ExecQuery("select FeedbackID,not_contactable_counter,last_update_dated from Req_Feedback_PL where AllRequestID=" . $post . " and BidderID=" . $Bidder_Id . " AND Reply_Type=1");
    $rowBankFeedback = mysql_fetch_array($resBankFeedback);

    $verFollowUpDate = $rowBankFeedback['last_update_dated'];

    if (isset($_REQUEST['BankFeedback']) && $plfeedback == "") {


        if ($BankFdbck == 'Appointment' || $BankFdbck == 'Documents Picked' || $BankFdbck == 'Document Pending' || $BankFdbck == 'Follow Up') {


            if ($verifierDateTime <= $verFollowUpDate) {
                $flag = 0;
            } else if ($verifierDateTime <= $curDate) {
                $flag = 0;
            } else {
        ...
        /* echo "Cur Date".$curDate;
          echo $BankFdbck;
          echo "<br>";
          echo "verDate".$verifierDateTime;
          echo "<br>";
          echo "verFollowUp".$verFollowUpDate;
          echo "<br>";
          echo $flag;
          //die; */
...
//if($BankFdbck=='Appointment' || $BankFdbck=='Follow Up' || $BankFdbck=='Document Pending' || $BankFdbck=='No Response')
        // {
        $strSQL = $strSQL . ", last_update_dated='" . $verifierDateTime . "'";
        //}
...
<html>
    <head>
        ...
        <script>
            $(function () {
                $("#verifierDateTime").datepicker({
                    dateFormat: 'yy-mm-dd',
                    minDate: 0, maxDate: "+200",
...
//echo "dd".$viewqry;
        ...
        $VerifierDateTime = mysql_result($viewlead, 0, 'VerifierDateTime');
...
//echo $citicategory."<br>";
        ...
        ?>
        <form name="loan_form" method="post" action="<? echo $_SERVER['PHP_SELF'] ?>?id=<? echo $post; ?>&bid=<? echo $bidid; ?>&to=<? echo $min_date ?>&from=<? echo $max_date; ?>" onsubmit="return ValidateFolowupDate()">
			...
			<td class="fontstyle" ><b>Other City</b></td>
                        <td class="fontstyle"><input type="text" name="plcity_other" id="plcity_other" value="<?echo $City_Other;?>"></td>
		</tr>
                <tr>
                ...
                <tr>
                    <td>Company Type</td><td><select name="plCompany_Type" id="plCompany_Type" style="width: 190px;">
                            ...
                            ?>>Listed With Bajaj</option>
                            <option value="2" <?
                            if ($Company_Type == 2) {
                                echo "selected";
                            }
                            ?>>Listed With MCA</option>
                            <option value="3" <?
                            if ($Company_Type == 3) {
                                echo "selected";
                            }
                ...
                <tr><td colspan="4" bgcolor="#DAEAF9" align="center" style="border:1px solid black;" ><b>Verifier Feedback</b></td></tr>
                <tr>
                    ...
                    <td class="fontstyle">
                        ...
                        if ($Feedback == 'OTP Done - OK' || $Feedback == 'OTP Done - Not Ok') {
                            ...
                            <select name="plfeedback" id="feedback" onchange="return fdbkconfirm(this.value)">
                                ...
                                if ($Feedback == "No Response") {
                                    echo "selected";
                                }
                                ?>>No Response</option>
                                <option value="Not Connected" <?
                                if ($Feedback == "Not Connected") {
                                    echo "selected";
                                }
                    ...
                    <td><textarea rows="2" cols="20" readonly="readonly" name="pladd_comment" id="pladd_comment" ><? echo $Add_Comment; ?></textarea></td>

                </tr>
                <tr>
                    <td class="fontstyle"><b>Agent Follow Up Date</b></td>
                    <td class="fontstyle"><?php echo $followup_date; ?></td>	
                </tr>
                <tr>
                    <td class="fontstyle"><b>Verifier Feedback</b></td>
                    <td class="fontstyle">
                        <select name="BankFeedback" id="BankFeedback" onchange="showDatetimeDiv(this.value);">
                            <option value="">Select Bank feedback</option>
                            <option value="Not Eligible" <?
                            if ($BankFeedabck == "Not Eligible") {
                                echo "selected";
                            }
                            ?>>Not Eligible</option>
                            <option value="Not Interested - Direct" <?
                            if ($BankFeedabck == "Not Interested - Direct") {
                                echo "selected";
                            }
                            ?>>Not Interested - Direct</option>
                            <option value="Not Interested - Offer" <?
                            if ($BankFeedabck == "Not Interested - Offer") {
                                echo "selected";
                            }
                            ?>>Not Interested - Offer</option>
                            <option value="Duplicate" <?
                            if ($BankFeedabck == "Duplicate") {
                                echo "selected";
                            }
                            ?>>Duplicate</option>
                            <option value="Not Contactable" <?
                            if ($BankFeedabck == "Not Contactable") {
                                echo "selected";
                            }
                            ?>>Not Contactable</option>
                            <option value="No Response" <?
                            if ($BankFeedabck == "No Response") {
                                echo "selected";
                            }
                            ?>>No Response</option>
                            <option value="Appointment" <?
                            if ($BankFeedabck == "Appointment") {
                                echo "selected";
                            }
                            ?>>Appointment</option>
                            <option value="Documents Picked" <?
                            if ($BankFeedabck == "Documents Picked") {
                                echo "selected";
                            }
                            ?>>Documents Picked</option>
                            <option value="Document Pending" <?
                            if ($BankFeedabck == "Document Pending") {
                                echo "selected";
                            }
                            ?>>Document Pending</option>
                            <option value="Follow Up" <?
                            if ($BankFeedabck == "Follow Up") {
                                echo "selected";
                            }
                            ?>>Follow Up</option>
                            <option value="Approved" <?
                            if ($BankFeedabck == "Approved") {
                                echo "selected";
                            }
                            ?>>Approved</option>
                            <option value="Dispatched" <?
                            if ($BankFeedabck == "Dispatched") {
                                echo "selected";
                            }
                            ?>>Dispatched</option>
                            <option value="Post Login Reject" <?
                            if ($BankFeedabck == "Post Login Reject") {
                                echo "selected";
                            }
                            ?>>Post Login Reject</option>
                        </select>
                    </td>
                    <td class="fontstyle"><b>Verifier Remarks</b></td>
                    <td class="fontstyle"><textarea rows="2" cols="20" name="BankRemarks" id="BankRemarks" ><? echo $BankRemarks; ?></textarea></td>
                </tr>
                <?php
                if ($BankFeedabck == 'Appointment' || $BankFeedabck == 'Follow Up' || $BankFeedabck == 'Document Pending' || $BankFeedabck == 'No Response') {
                    $style = 'style="display: block;"';
                }
                ?>
                <tr>
                    <td></td>
                    <td class="fontstyle" colspan="2" <?php echo $style; ?>  id="DateTimeDiv">
                        <table>
                            <tr>
                                <td class="fontstyle"><b>Verifier Follow Up Date</b></td>
                                <td class="fontstyle"><input type="Text"  name="verifierDateTime" id="verifierDateTime" value="<?php echo $VerifierDateTime; ?>" /></td>
        ...
        <script>
            ...
            function showDatetimeDiv(val) {
                if (val == 'Appointment' || val == 'Follow Up' || val == 'Document Pending' || val == 'No Response') {
            ...
            function ValidateFolowupDate()
            {
                var frm = document.loan_form;
                var BankFeedback = frm.BankFeedback.value;
                var verifierDateTime = frm.verifierDateTime.value;
                if ((BankFeedback == 'Appointment') || (BankFeedback == 'Documents Picked') || (BankFeedback == 'Document Pending') || (BankFeedback == 'Follow Up')) {

                    if ((verifierDateTime == '0000-00-00 00:00:00') || (verifierDateTime == ''))
                    {
                        alert('Please select Verifier Follow Up Date');
                        verifierDateTime.focus;
                        return false;
                    }else{
                        return true;
                    }
