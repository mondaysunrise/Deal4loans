...
{
	var gifName = form;
	document.frmsearch.action="creditcard_lms.php?search=y"+gifName;
...
function submitform()
{
 ...
 <? $current_date=date('Y-m-d');?>
  <table width="712" border="0">
   <tr><td align="right"><a href="kotak_creditcard_lms.php?bidderid=935&product=4" target="_blank">today's Report</a></td></tr>
 <tr><td align="center" width="100%">
 <div align="center">
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <form name="frmsearch" action="creditcard_lms.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <p>&nbsp;</p>
	...
	if(strlen(trim($RequestID))>0)
	{
		...
		$result = ("select FeedbackID from Req_Feedback where AllRequestID=".$RequestID." and BidderID=935 AND Reply_Type=4");		
		...
		else
		{
			...
			$strSQL=$strSQL.$RequestID.",'935','4','".$Feedback."')";
		}
 ...
 <?
		...
		$qry="SELECT * FROM Req_Feedback_Bidder1,Req_Credit_Card LEFT OUTER JOIN Req_Feedback ON Req_Feedback.AllRequestID=Req_Credit_Card.RequestID AND Req_Feedback.BidderID=935  WHERE Req_Feedback_Bidder1.AllRequestID=Req_Credit_Card.RequestID and Req_Feedback_Bidder1.BidderID =943 and Req_Feedback_Bidder1.Reply_Type=4 and MOD(Req_Credit_Card.RequestID,2)=0  and  ( Req_Feedback_Bidder1.Allocation_Date Between '".($min_date)."' and '".($max_date)."' or Req_Feedback.Followup_Date Between '".($min_date)."' and '".($max_date)."')";
...
<!--///////////////////////-->
<tr>
<td class="bodyarial11"><a href="creditcardedit.php?id=<? echo urlencode($row[$i]['RequestID']); ?>&Bid=935&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row[$i]["Name"]; ?></a></td>
...
<?
    ...
    <td class="bodyarial11"><? echo getJumpMenu("creditcard_lms.php",$row[$i]["RequestID"],"1",$row[$i]["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
