...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if(strlen(trim($requestid))>0)
	{
		...
		if($num_rows > 0)
		{
			...
			$strSQL="Update smspl_status_details Set leadlogid='".$leadlogid."',special_remarks='".$special_remarks."',final_team_feedback='".$postfeedback."', stat_dated=Now()";
			$strSQL=$strSQL."Where statid=".$row["statid"];
		}
		else
		{
			$strSQL="Insert into smspl_status_details(AllRequestID, caller_id, ProductID , special_remarks, stat_dated, leadlogid, final_team_feedback) Values (";
			$strSQL=$strSQL.$requestid.",'".$caller_id."','1', '".$special_remarks."',Now(),'".$leadlogid."','".$postfeedback."')";
		}
		//echo $strSQL;
...
$varpostfeedback = $plrowal["final_team_feedback"];
...
<tr>
    ...
    <td><span class="style21"><? echo $plrow["City"]; ?></span></td>
	 ...
	 <tr>
	  ...
	  <tr>
        <td><span class="style2">Post Feedback</span></td>
        <td><span class="style21"><select name="postfeedback" id="postfeedback" style="width:120px;">
		<option value="" <? if($varpostfeedback == "") { echo "selected"; } ?>>No Feedback</option>
		<option value="Documents Pending" <? if($varpostfeedback == "Documents Pending") { echo "selected"; } ?>>Documents Pending</option>
		<option value="Documents Picked" <? if($varpostfeedback == "Documents Picked") { echo "selected"; } ?>>Documents Picked</option>
		<option value="Login" <? if($varpostfeedback == "Login") { echo "selected"; } ?>>Login/WIP</option>
		<option value="Pre-Login Reject" <? if($varpostfeedback == "Pre-Login Reject") { echo "selected"; } ?>>Pre-Login Reject</option>
		<option value="Approved" <? if($varpostfeedback == "Approved") { echo "selected"; } ?>>Approved</option>
		<option value="Disbursed" <? if($varpostfeedback == "Disbursed") { echo "selected"; } ?>>Disbursed</option>
		<option value="Post Login Reject" <? if($varpostfeedback == "Post Login Reject") { echo "selected"; } ?>>Post Login Reject</option>	
		<option value="Rescheduled" <? if($varpostfeedback == "Rescheduled") { echo "selected"; } ?>>Rescheduled</option>
		<option value="Ringing" <? if($varpostfeedback == "Ringing") { echo "selected"; } ?>>Ringing</option>
		<option value="cust_noncoperative" <? if($varpostfeedback == "cust_noncoperative") { echo "selected"; } ?>>Cust Not Co-operating</option>
			</select></span></td>
     </tr>
