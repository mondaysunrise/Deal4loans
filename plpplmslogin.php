<?php
	...
	function formHandler(){
		...
		$result = ExecQuery("select *  from Bidders where (Email='$Email' and PWD='$PWD' and BidderID=11)");
		
		$num_rows = mysql_num_rows($result);
$IP = getenv("REMOTE_ADDR");
if($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.100.29"  || $IP=="122.177.167.80" || $IP=="122.177.152.42" || $IP=="122.177.217.1" || $IP=="122.176.122.134")
			{
		if($num_rows > 0){
			 /* Get Resultset */
			...
			mysql_free_result($result);
			 ...
			 /* Redirect browser */
			 $strDir = dir_name();
			//if($row['BidderID']==846 || $row['BidderID']==847)
			//{
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."plpplms_view.php");
			exit;
			//}
			//else
			//{

			//header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."pllms_indexdnc.php");
			//exit;
			//}
		}
		else{
