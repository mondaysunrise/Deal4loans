...
{
	var gifName = form;
	document.frmsearch.action="hllms_3.php?search=y"+gifName;
...
function chkform()
{
	...
	if(document.frmsearch.Mobile.value=="")
	{
		alert("Sorry!!!! Please Enter Mobile Number.");
		document.frmsearch.Mobile.value="";
		document.frmsearch.Mobile.focus();
		return false;
	}
...
function insertData(id)
 ...
 <br>
  ...
  <tr><td align="right"><a href="hllms_2.php">Total Search</a> | <a href="commonlms_report.php?bidderid=<?php echo $_SESSION['BidderID'];?>&product=2" target="_blank">today's Report</a></td></tr>
 <tr><td align="center" width="100%">
 <div align="center">
 <table width="600" border="0" cellpadding="4" cellspacing="1" class="blueborder">
 <form name="frmsearch" action="hllms_3.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
   ...
   <tr>
     ...
     <td width="80%">
		<input type="text" name="Mobile" id="Mobile" value="<?php echo $Mobile; ?>"> </td>
   </tr>
 ...
 <table width="870" border="0" cellpadding="4" cellspacing="1" class="blueborder" align="center">
   <?
		if($_SESSION['BidderID']==2776)
		{
			...
			if(strlen($Mobile)>0)
			{
				$qry="SELECT RequestID from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (2776) WHERE ((Req_Loan_Home.Mobile_Number='".$Mobile."') and Req_Loan_Home.Net_Salary>=900000 and Req_Loan_Home.City in ('Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Sahibabad') and Req_Loan_Home.Employment_Status=1 and Req_Loan_Home.Updated_Date>'2012-04-15 00:00:00')";
			}
 ...
 ?>
   ...
   <?
		...
		{	
			...
			if(strlen($Mobile)>0)
			{
//			(Req_Loan_Home.Mobile_Number='".$Mobile."'  )
				$qry="SELECT Company_Name,comment_section,Net_Salary,Loan_Amount,Referral_Flag ,RequestID,Updated_Date,Mobile_Number,Name,City,City_Other,Feedback,Followup_Date,BidderID,updated_flag from Req_Loan_Home LEFT OUTER JOIN Req_Feedback_HL ON Req_Feedback_HL.AllRequestID= Req_Loan_Home.RequestID and Req_Feedback_HL.BidderID in (2776) WHERE ((Req_Loan_Home.Mobile_Number='".$Mobile."') and Req_Loan_Home.Net_Salary>=900000 and Req_Loan_Home.City in ('Delhi','Noida','Gurgaon','Gaziabad','Faridabad','Sahibabad') and Req_Loan_Home.Employment_Status=1 and Req_Loan_Home.Updated_Date>'2012-04-15 00:00:00')";
			}
...
//echo $qry;
   ...
   <tr>
     ...
     <td class="bodyarial11"><? echo getJumpMenu("hllms_3.php",$row["RequestID"],"1",$row["Feedback"],$pageno,$varmin_date,$varmax_date,$varCmbFeedback, $val) ?></td>
   
     <td  class="bodyarial11" bgcolor="<? echo $bgcolor; ?>">
         <?php
	
 $sqlFeeds = "SELECT * FROM `Req_Feedback` where Reply_Type=2 and AllRequestID='".$row["RequestID"]."' and	`BidderID` IN ( 460, 732, 812, 207 )";	
