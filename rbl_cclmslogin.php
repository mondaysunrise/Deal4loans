...
function formHandler(){
	...
	$result = ("select *  from Bidders where (Email='$Email' and PWD='$PWD' and (BidderID=98 or BidderID=4905))");		
	...
	if($IP_Remote=='192.99.32.74') { $IP= $_SERVER['HTTP_X_REAL_IP']; }
		...
		if(($row["BidderID"]==98) && ($IP=="122.176.100.27" || $IP=="122.176.100.28" || $IP=="122.176.122.134"))
		{
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."rbllms_index.php");
			exit;
		}
		elseif($row["BidderID"]==4905)
		{
			header("Location: http://".$_SERVER['HTTP_HOST'].$strDir."rbl_ccaprvdview.php");
			exit;
		}
		else{
			global $Msg;
