...
{
	var gifName = form;
	document.frmsearch.action="icicicardslms_verifyview.php?search=y"+gifName;
...
if (restore) selObj.selectedIndex=0;
 ...
 <form name="frmsearch" action="icicicardslms_verifyview.php?search=y" method="post" onSubmit="return chkform();">
   <tr>
 ...
 <?
		$qry="Select RequestID From icicilms_allocation LEFT OUTER JOIN Req_Credit_Card ON Req_Credit_Card.RequestID= icicilms_allocation.cc_requestid Where ((icicilms_allocation.icici_sendnow_date Between '".$min_date."' and '".$max_date."') and  icici_bidders!='') ";
   ...
   <td class="head1">Eligible Bidders</td>
	...
	<?
		...
		else
		{
		 ...
		 $qry="		 
Select RequestID,icici_followupdate,Name,Mobile_Number,Net_Salary,City,City_Other,icici_feedback From icicilms_allocation  LEFT OUTER JOIN Req_Credit_Card ON Req_Credit_Card.RequestID= icicilms_allocation.cc_requestid  Where ((icicilms_allocation.icici_sendnow_date Between '".$min_date."' and '".$max_date."') and  icici_bidders!='') ";
$qry=$qry.$FeedbackClause;
			$qry=$qry."group by icicilms_allocation.cc_requestid";
	...
	?>
<tr>
<td class="bodyarial11"><a href="icici_cardeverifylead.php?id=<? echo urlencode($row['RequestID']); ?>&Bid=<? echo $_SESSION['BidderID'];?>&to=<? echo $min_date;?>&from=<? echo $max_date;?>" target="_blank"><? echo $row["Name"]; ?></a></td>
