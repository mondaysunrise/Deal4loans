<?php     
   require 'scripts/session_check_online.php';
	...
	//print_r($_POST);
		...
		$getReqID = $_POST['getReqID'];
		if($check_address>0)
		{
			if($check_address==1) { $add_field = "office_address"; } else if($check_address==2) { $add_field = "residence_address"; }
			$sqlAddress = "select ".$add_field." as Address from Req_Loan_Personal_Extra_Fields where RequestID='".$getReqID."'";
			$qryAddress = ExecQuery($sqlAddress);
	       	$Address= mysql_result($qryAddress,0,'Address');
		}
		$AgentFeedback = 1; 
    ...
    $getReqID = $rowApptDetails->RequestID;
?>
<script language="javascript" type="text/javascript" src="scripts/datetime.js"></script>	 
      <form method="POST" action="" name="sendform">
				<input type="hidden" name="id" id="id" value="<? echo $_REQUEST['id']; ?>">
				<input type="hidden" name="getReqID" id="getReqID" value="<? echo $getReqID; ?>">    
        <table width="730" border="0" cellspacing="1" cellpadding="0" style="border:#333 0px solid;" align="center" >
  <?php 
	if($msg!="")
	{
	?>
...
<?php } ?>
        ...
        <tr>  <td bgcolor="#FFFFFF" colspan="2"><textarea rows="2" cols="55" name="special_remarks" id="special_remarks_3" onchange="NosplcharComment(this);"><?php echo $special_remarks ?></textarea></td>
         ...
         <tr><td colspan="1" ><b>Address -  </b></td><td colspan="3" ><input type="radio" name="check_address" value="1" > Office Address <input type="radio" name="check_address" value="2" > Residence Address
         <br>
        <b> Current Address(for Pick Up)</b>  - <?php echo $Address; ?>
       ...
       //  echo "select residence_address, office_address from Req_Loan_Personal_Extra_Fields where RequestID='".$getReqID."'";
      	$qryAddress = ExecQuery("select residence_address, office_address from Req_Loan_Personal_Extra_Fields where RequestID='".$getReqID."'");
       $residence_address= mysql_result($qryAddress,0,'residence_address');
       $office_address= mysql_result($qryAddress,0,'office_address');
       if(strlen($office_address)>10) { echo "<b>Office Address</b> - ".$office_address."<br>"; }
       if(strlen($residence_address)>10) { echo "<b>Residence Address</b> - ".$residence_address."<br>"; }
        ?>
