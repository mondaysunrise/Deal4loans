...
define ('BAAGRID_NO_RECORDS', '<FONT color="red"><p>No matching records were found</p></font>');
...
$mstr_m_d_o;
$mstr_m_d_t;
$mstr_m_d_o = Trim($_REQUEST['m_d_o']);
$mstr_m_d_t = Trim($_REQUEST['m_d_t']);
class baaGrid {
...
//echo "bye::";
      ...
      function setAlign($colno,$aval) {
               switch ($aval) {
               case 'c':
               case 'C': $this->align[$colno] = 'center'; break;
               case 'l':
               case 'L': $this->align[$colno] = 'left'; break;
               case 'r':
               case 'R': $this->align[$colno] = 'right'; break;
               default: $this->align[$colno] = 'left'; break;
               }
               return;
      ...
      function setXTab($rows, $cols, $totCol, $aggType='T') {
               ...
               switch ($aggType) {
               case 'A':
               case 'a': $this->xAggType = 'A'; break;
               case 'C':
               case 'c': $this->xAggType = 'C'; break;
               default:  $this->xAggType = 'T'; break;
               }
               return;
      }
      ...
      ******************************************/
      ...
      function _query() {
               ...
               case DB_MSSQL :
                    ...
                    if (!$res) {
                         $this->_handleError(mssql_get_last_message());
                         return false;
                    }
                    break;
               case DB_POSTGRESQL:
                    ...
                    if (!$res) {
                         $this->_handleError(pg_last_error());
                         return false;
                    }
                    break;
               case DB_ODBC:
                    $res = @odbc_exec($this->conn,$this->query);
                    if (!$res) {
                         $this->_handleError(odbc_errormsg());
                         return false;
                    }
                    break;
               }
      ...
      function _fetch_row($rc) {
               switch ($this->db) {
               case DB_MYSQL:
                    return mysql_fetch_row($this->cursor); break;
               case DB_MSSQL:
                    return mssql_fetch_row($this->cursor); break;
               case DB_POSTGRESQL:
                    return pg_fetch_row($this->cursor,$rc); break;
               case DB_ODBC:
                    return (odbc_fetch_into($this->cursor,$row)) ? $row : false;
               }
      }
      function _getFieldNames(&$fa, &$fn) {
               ...
               case DB_MSSQL:
                    $fn = mssql_num_fields($this->cursor);
                    for ($i=0; $i<$fn; $i++)  {
                         $fa[] = mssql_field_name($this->cursor,$i);
                         if (strpos(strtolower(mssql_field_type($this->cursor,$i)),'date')!==false)
                             $this->datefield[$i] = 1;
                    }
                    break;

               case DB_POSTGRESQL:
                    $fn = pg_num_fields($this->cursor);
                    for ($i=0; $i<$fn; $i++)  {
                         $fa[] = pg_field_name($this->cursor,$i);
                         if (strpos(strtolower(pg_field_type($this->cursor,$i)),'date')!==false)
                             $this->datefield[$i] = 1;
                    }
                    break;

               case DB_ODBC:
                    $fn = odbc_num_fields($this->cursor);
                    for ($i=1; $i<=$fn; $i++)  {
                         $fa[] = odbc_field_name($this->cursor,$i);
                         if (strpos(strtolower(odbc_field_type($this->cursor,$i)),'date')!==false)
                             $this->datefield[$i-1] = 1;
                    }
                    break;
               }
               return;
	...
	global $mstr_m_d_t;
               ...
               echo "<tr>\n<th class='{$this->headClass[0]}' rowspan='2'>{$this->heads[$this->xRow]}</th> <th colspan='$numCols'  class='{$this->headClass[2]}'>{$this->heads[$this->xCol]}</th>              <th class='{$this->headClass[0]}' rowspan='2'>Totals</th></tr>\n";
               ...
               foreach ($this->xColHeads as $title=>$v) {
				  ...
				  // echo $title;
                      // echo "<th class='{$this->headClass[1]}' ><a href='displaylead.php?Code=$title&m_d_o=$mstr_m_d_o&m_d_t=$mstr_m_d_t' target='_blank'>$title</a></th>\n";
					  echo "<th class='{$this->headClass[1]}' >";
					  if($title=="")
				   {
						   $title="No Feedback";
					}
					 elseif($title=="Send Now")
				   {
						$title="In Process";	
				   }
				  elseif($title=="Duplicate" || $title=="Other Product" || $title=="Not Interested" || $title=="Callback Later" || $title=="Wrong Number" || $title=="Not Eligible" || $title=="Ringing")
				   {
					   $title="Rejected";
				   }
				   else
      ...
      function _printXData() {
				...
				echo $rowhead = date($this->dateFormat, strtotime($rowhead));
				                else
                        ...
                        foreach ($this->xColHeads as $colhead => $v) {
                                 ...
                                 case 'T': $cell = $rowdata[$colhead]['sum']; break;
                                 case 'C': $cell = $rowdata[$colhead]['count']; break;
                                 case 'A': $cell = $rowdata[$colhead]['count']==0?0:$rowdata[$colhead]['sum']/$rowdata[$colhead]['count']; break;
                                 }
                                 ...
                                 echo "<td class='{$this->colClass[1]}' style='text-align:center'>$cell</td>\n";
                        }

                         switch ($this->xAggType) {
                         case 'T': $cell = $rowtotal['sum']; break;
                         case 'C': $cell = $rowtotal['count']; break;
                         case 'A': $cell = $rowtotal['count']==0?0:$rowtotal['sum']/$rowtotal['count']; break;
                         }
                        if (isset($this->decpl[$this->xTot])) {
                         $cell = number_format($cell, $this->decpl[$this->xTot]);
                        }
                        echo "<td class='{$this->colClass[2]}' style='text-align:center'>$cell</td>\n";
                        echo "</tr>\n";
               }
               return;
      }
      function _printXTotals() {
               ...
               foreach ($this->xColHeads as $colhead => $v) {
                         switch ($this->xAggType) {
                         case 'T': $cell = $this->totals[$colhead]['sum']; break;
                         case 'C': $cell = $this->totals[$colhead]['count']; break;
                         case 'A': $cell = $this->totals[$colhead]['count']==0?0:$this->totals[$colhead]['sum']/$this->totals[$colhead]['count']; break;
                         }
                        ...
                        echo "<th class='{$this->headClass[1]}' style='text-align:center'>$cell</th>\n";
               }
                 switch ($this->xAggType) {
                 case 'T': $cell = $rowtotal['sum']; break;
                 case 'C': $cell = $rowtotal['count']; break;
                ...
                echo "<th class='{$this->headClass[0]}' style='text-align:center'>$cell</th>\n";
  ...
  $numCols = count($this->xColHeads) ;
               ksort($this->xColHeads);
               echo "<tr>\n<th class='{$this->headClass[0]}' rowspan='2'></th> <th colspan='$numCols'  class='{$this->headClass[2]}'></th>
              <th class='{$this->headClass[0]}' rowspan='2'></th></tr>\n";
               echo "<tr class='{$this->headClass[0]}'>\n";
               foreach ($this->xColHeads as $title=>$v) {
                        ...
                        if (isset($this->datefield[$this->xCol]))
                            ...
                            $title = stripSlashes($title);
                        echo "<th class='{$this->headClass[1]}' ><a href=displaylead.php?Code=$title&m_d_o=$mstr_m_d_o&m_d_t=$mstr_m_d_t'>$title</a></th>\n";
               }
               return;
