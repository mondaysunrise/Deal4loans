...
function emailMasking($email, $maskingCharacter = '*')
{
	$em = explode("@",$email);
	$name = $em[0];
	$len = strlen($name);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
	...
	if((strlen($cust_mobileno)>0) && (strlen($cust_emailid)>0))
			{
		$CheckSql = "select Add_Comment,Name,RequestID, Email, Net_Salary, Mobile_Number, City, DOB, Updated_Date,Bidder_Count from ".$product_type." where  (RequestID=$cust_mobileno and Email='".$cust_emailid."' and Updated_Date between '".$days30datetime."' and '".$currentdatetime."')"; 
		
					}
		elseif (strlen($cust_mobileno)>0)
			{
		$CheckSql = "select Add_Comment, RequestID, Name, Email, Net_Salary, Mobile_Number, City, DOB, Updated_Date,Bidder_Count from ".$product_type." where  (RequestID=$cust_mobileno and Updated_Date between '".$days30datetime."' and '".$currentdatetime."')"; 
		    }
...
else
{
	...
	$mob = $_REQUEST['mob'];
	$checkSqlMob = "select Mobile_Number from ".$product_type." where (RequestID='".$mob."' and Updated_Date between '".$days30datetime."' and '".$currentdatetime."')";
	$result1=ExecQuery($checkSqlMob);
	$cust_mobileno = mysql_result($result1,0,'Mobile_Number');


	$CheckSql = "select Name, Email,RequestID,Add_Comment, Mobile_Number, Net_Salary, City, DOB, Updated_Date, Dated,Bidder_Count  from ".$product_type." where (Mobile_Number='".$cust_mobileno."' and Updated_Date between '".$days30datetime."' and '".$currentdatetime."')";
//	echo "<br>ok:".$CheckSql."<br>";
	$result=ExecQuery($CheckSql);
	$msg = "Value found";
...
<FORM NAME="search_entry" action="<? echo $_SERVER['PHP_SELF'] ?>" method="post"  >
			...
			<tr>
				...
				<tr>
				<tr>
				<td> Email id &nbsp;<input type="text" name="cust_emailid" size="30" value="<?php echo $cust_emailid; ?>"><b> <!--OR </b> &nbsp;Mobile &nbsp;<input type="text" name="cust_mobileno" size="10">-->
				</td>
				<tr>
	...
	<?php
		...
		<td class="bodyarial11"><?php echo $nrow["RequestID"]; ?></td>
			...
			<td class="bodyarial11"><?php echo emailMasking($nrow["Email"]); ?></td>
			<td class="bodyarial11"><?php echo ccMasking($nrow["Mobile_Number"]);?></td>
...
?>
	...
	$nrowFeed = mysql_fetch_array($GETFeedbackQuery);
