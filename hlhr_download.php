...
$dob_loan=$row_result["dob"];
		...
		$qry1="insert into temp (session_id, name, dob, emp_status, c_name, city, city_other, total_exp, net_salary,  property_type, property_value, loan_amount, pincode, source, count_views, is_modified, is_processed, residence_address, budget, property_identified, property_loc, doe, count_replies,bidderid,Feedback,descr,mobile_number,email) values ('".$session_id."', '".$row_result["Name"]."', '".$dob."', '".$emp_status."', '".$row_result["Company_Name"]."', '".$row_result["City"]."', '".$row_result["City_Other"]."', '".$row_result["Total_Experience"]."', '".$row_result["Net_Salary"]."', '".$property_type."', '".$row_result["Property_Value"]."', '".$row_result["Loan_Amount"]."', '".$row_result["Pincode"]."', '".$row_result["source"]."', '".$row_result["Count_Views"]."','".$row_result["IsModified"]."', '".$row_result["IsProcessed"]."', '".$row_result["Residence_Address"]."', '".$row_result["Budget"]."', '".$property_identified."', '".$row_result["Property_Loc"]."', '".$row_result["Updated_Date"]."', '".$row_result["Bidder_Count"]."','".$row_result["Bidderid_Details"]."','".$row_result["Feedback"]."','".$descr."','".$row_result["Mobile_Number"]."' ,'".$row_result["Email"]."')";
			$result1=ExecQuery($qry1);
		
		
	}
	
	$qry="select name, dob,mobile_number,email, emp_status, c_name, city, city_other, net_salary, descr, property_type, property_value, loan_amount, pincode, source, residence_address, budget, property_identified, property_loc, doe, count_replies AS BidderCount,bidderid,Feedback,descr AS BiddersGone from temp where session_id='".$session_id."'";
